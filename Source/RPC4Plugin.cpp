...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char *uniqueID,
...
int RPC4::LocalSlotObjectComp(const LocalSlotObject &key, const LocalSlotObject &data) {
   if (key.callPriority > data.callPriority) return -1;
   if (key.callPriority == data.callPriority) {
      if (key.registrationCount < data.registrationCount) return -1;
      if (key.registrationCount == data.registrationCount) return 0;
      return 1;
   }
...
int RPC4::LocalCallbackComp(const MessageID &key, RPC4::LocalCallback *const &data) {
   if (key < data->messageId) return -1;
   if (key > data->messageId) return 1;
   return 0;
}
...
bool RPC4::RegisterFunction(const char *uniqueID,
   ...
   if (skhi.IsInvalid() == false) return false;

   registeredNonblockingFunctions.Push(uniqueID, functionPointer, _FILE_AND_LINE_);
   return true;
}

void RPC4::RegisterSlot(const char *sharedIdentifier,
                        void (*functionPointer)(RakNet::BitStream *userData, Packet *packet), int callPriority) {
...
bool RPC4::RegisterBlockingFunction(const char *uniqueID,
...
void RPC4::RegisterLocalCallback(const char *uniqueID, MessageID messageId) {
   ...
   if (objectExists) {
      ...
      if (objectExists == false) lc->functions.InsertAtIndex(str, index, _FILE_AND_LINE_);
   } else {
...
void RPC4::CallLoopback(const char *uniqueID, RakNet::BitStream *bitStream) {
   ...
   if (skhi.IsInvalid() == true) {
      if (rakPeerInterface)
         ...
         p = tcpInterface->AllocatePacket(sizeof(MessageID) + sizeof(unsigned char) + (unsigned int)strlen(uniqueID) +
                                          1);
#endif

      if (rakPeerInterface) p->guid = rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
      else
   ...
   if (rakPeerInterface) p = AllocatePacketUnified(out.GetNumberOfBytesUsed());
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
   else
...
void RPC4::Call(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority,
...
bool RPC4::CallBlocking(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority,
                        PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
   ...
   while (gotBlockingReturnValue == false) {
      // TODO - block, filter until gotBlockingReturnValue==true or ID_CONNECTION_LOST or
      // ID_DISCONNECTION_NOTIFICXATION or ID_RPC_REMOTE_ERROR/RPC_ERROR_FUNCTION_NOT_REGISTERED
      RakSleep(30);
         ...
         if ((packet->data[0] == ID_CONNECTION_LOST || packet->data[0] == ID_DISCONNECTION_NOTIFICATION) &&
             ((systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID && packet->guid == systemIdentifier.rakNetGuid) ||
              (systemIdentifier.systemAddress != UNASSIGNED_SYSTEM_ADDRESS &&
               packet->systemAddress == systemIdentifier.systemAddress))) {
            // Push back to head in reverse order
            rakPeerInterface->PushBackPacket(packet, true);
            while (packetQueue.Size()) rakPeerInterface->PushBackPacket(packetQueue.Pop(), true);
            return false;
...
void RPC4::Signal(const char *sharedIdentifier, RakNet::BitStream *bitStream, PacketPriority priority,
                  PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
   ...
   if (invokeLocal) {
      ...
      // TimeUS t2 = GetTimeUS();
      if (functionIndex.IsInvalid()) return;
...
void RPC4::InvokeSignal(DataStructures::HashIndex functionIndex, RakNet::BitStream *serializedParameters,
                        Packet *packet) {
   if (functionIndex.IsInvalid()) return;
...
void RPC4::InterruptSignal(void) { interruptSignal = true; }
   ...
   for (i = 0; i < globalRegistrationIndex; i++) {
      if (globalRegistrationBuffer[i].registerFunctionPointer) {
         if (globalRegistrationBuffer[i].callPriority == (int)0xFFFFFFFF)
            RegisterFunction(globalRegistrationBuffer[i].functionName,
                             globalRegistrationBuffer[i].registerFunctionPointer);
         else
            RegisterSlot(globalRegistrationBuffer[i].functionName, globalRegistrationBuffer[i].registerFunctionPointer,
                         globalRegistrationBuffer[i].callPriority);
      } else if (globalRegistrationBuffer[i].registerBlockingFunctionPointer)
         RegisterBlockingFunction(globalRegistrationBuffer[i].functionName,
                                  globalRegistrationBuffer[i].registerBlockingFunctionPointer);
      else
