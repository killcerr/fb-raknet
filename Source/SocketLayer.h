...
public:
        ...
        /// Called when RecvFrom would otherwise occur. Return number of bytes read. Write data into
dataOut
        // Return -1 to use RakNet's normal recvfrom, 0 to abort RakNet's normal recvfrom, and
positive to return data virtual int RakNetRecvFrom( char dataOut[ MAXIMUM_MTU_SIZE ], SystemAddress
*senderOut, bool calledFromMainThread )=0;
};
*/
...
class RAK_DLL_EXPORT SocketLayer {
  public:
   ...
   static RakNetSocket* CreateBoundSocket( RakPeer *peer, unsigned short port, bool blockingSocket,
 const char *forceHostAddress, unsigned int sleepOn10048, unsigned int extraSocketOptions, unsigned
 short socketFamily, _PP_Instance_ chromeInstance ); #if defined(WINDOWS_STORE_RT) static
 RakNetSocket* CreateWindowsStore8Socket( RakPeer *peer, unsigned short port, bool blockingSocket,
 const char *forceHostAddress, unsigned int sleepOn10048, unsigned int extraSocketOptions,
 _PP_Instance_ chromeInstance ); #endif static RakNetSocket* CreateBoundSocket_IPV4( RakPeer *peer,
 unsigned short port, bool blockingSocket, const char *forceHostAddress, unsigned int sleepOn10048,
 unsigned int extraSocketOptions, _PP_Instance_ chromeInstance ); #if RAKNET_SUPPORT_IPV6==1 static
 RakNetSocket* CreateBoundSocket_SupportIPV4And6( RakPeer *peer, unsigned short port, bool
 blockingSocket, const char *forceHostAddress, unsigned int sleepOn10048, unsigned int
 extraSocketOptions, unsigned short socketFamily, _PP_Instance_ chromeInstance ); #endif static
 RakNetSocket* CreateBoundSocket_PS3Lobby( unsigned short port, bool blockingSocket, const char
 *forceHostAddress, unsigned short socketFamily ); static RakNetSocket* CreateBoundSocket_PSP2(
 unsigned short port, bool blockingSocket, const char *forceHostAddress, unsigned short socketFamily
 );
   */
   ...
   //static bool IsPortInUse(unsigned short port, const char *hostAddress, unsigned short
 socketFamily ); static bool IsSocketFamilySupported(const char *hostAddress, unsigned short
 socketFamily); #endif
   */
...
//	static void RecvFromBlocking_IPV4( RakNetSocket *s, RakPeer *rakPeer, char *dataOut, int
//*bytesReadOut, SystemAddress *systemAddressOut, RakNet::TimeUS *timeRead );
// 	#if RAKNET_SUPPORT_IPV6==1
// 		static void RecvFromBlockingIPV4And6( RakNetSocket *s, RakPeer *rakPeer, char
// *dataOut, int *bytesReadOut, SystemAddress *systemAddressOut, RakNet::TimeUS *timeRead );
// #endif
//	static void RecvFromBlocking( RakNetSocket *s, RakPeer *rakPeer, char *dataOut, int
//*bytesReadOut, SystemAddress *systemAddressOut, RakNet::TimeUS *timeRead );
#if defined(WINDOWS_STORE_RT)
//	static void RecvFromBlocking_WindowsStore8( RakNetSocket *s, RakPeer *rakPeer, char
//*dataOut, int *bytesReadOut, SystemAddress *systemAddressOut, RakNet::TimeUS *timeRead );
#endif
   ...
   /// \return 0 on success, nonzero on failure.
   //	static int SendToTTL( RakNetSocket *s, const char *data, int length, SystemAddress
   //&systemAddress, int ttl );
   ...
   // 	static int SendTo_PS3Lobby( RakNetSocket *s, const char *data, int length, const
   // SystemAddress &systemAddress ); 	static int SendTo_PSP2( RakNetSocket *s, const char *data,
   // int length, const SystemAddress &systemAddress ); 	static int SendTo_360( RakNetSocket
   // *s, const char *data, int length, const char *voiceData, int voiceLength, const SystemAddress
