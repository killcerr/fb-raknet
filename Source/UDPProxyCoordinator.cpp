...
// bool operator<( const DataStructures::MLKeyRef<unsigned short> &inputKey, const
// UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() < cls.ping;} bool operator>(
// const DataStructures::MLKeyRef<unsigned short> &inputKey, const
// UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() > cls.ping;} bool operator==(
// const DataStructures::MLKeyRef<unsigned short> &inputKey, const
// UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() == cls.ping;}

int UDPProxyCoordinator::ServerWithPingComp(const unsigned short &key,
                                            const UDPProxyCoordinator::ServerWithPing &data) {
   if (key < data.ping) return -1;
   if (key > data.ping) return 1;
   return 0;
}

int UDPProxyCoordinator::ForwardingRequestComp(const SenderAndTargetAddress &key, ForwardingRequest *const &data) {
   if (key.senderClientAddress < data->sata.senderClientAddress) return -1;
   if (key.senderClientAddress > data->sata.senderClientAddress) return 1;
   if (key.targetClientAddress < data->sata.targetClientAddress) return -1;
   if (key.targetClientAddress > data->sata.targetClientAddress) return 1;
   return 0;
}
//
// bool operator<( const DataStructures::MLKeyRef<UDPProxyCoordinator::SenderAndTargetAddress>
...
UDPProxyCoordinator::~UDPProxyCoordinator() { Clear(); }

void UDPProxyCoordinator::SetRemoteLoginPassword(RakNet::RakString password) { remoteLoginPassword = password; }
...
void UDPProxyCoordinator::Update(void) {
   ...
   while (idx < forwardingRequestList.Size()) {
      ...
      if (fw->timeRequestedPings != 0 &&
         ...
         // Forwarding request succeeded, we waited a bit to prevent duplicates. Can forget about
         // the entry now.
...
void UDPProxyCoordinator::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void UDPProxyCoordinator::OnForwardingRequestFromClientToCoordinator(Packet *packet) {
   ...
   if (sourceAddress == UNASSIGNED_SYSTEM_ADDRESS) sourceAddress = packet->systemAddress;
   ...
   if (hasServerSelectionBitstream) incomingBs.Read(&(fw->serverSelectionBitstream));
   ...
   if (serverList.Size() > 1) {
      ...
      unsigned int idx;
      for (idx = 0; idx < serverList.Size(); idx++) outgoingBs.Write(serverList[idx]);
...
void UDPProxyCoordinator::SendForwardingRequest(SystemAddress sourceAddress, SystemAddress targetAddress,
...
void UDPProxyCoordinator::OnForwardingReplyFromServerToCoordinator(Packet *packet) {
   ...
   if (success == UDPFORWARDER_SUCCESS) {
      ...
      // 05/18/09 Keep the entry around for some time after success, so duplicates are reported if
      // attempting forwarding from the target system before notification of success
...
void UDPProxyCoordinator::OnPingServersReplyFromClientToCoordinator(Packet *packet) {
   ...
   ServerWithPing swp;
   ForwardingRequest *fw = forwardingRequestList[index];
   if (fw->timeRequestedPings == 0) return;
   ...
   if (packet->systemAddress == sata.senderClientAddress) {
      for (idx = 0; idx < serversToPingSize; idx++) {
         ...
         for (index2 = 0; index2 < fw->sourceServerPings.Size(); index2++) {
            if (fw->sourceServerPings[index2].ping >= swp.ping) break;
         }
      ...
      for (idx = 0; idx < serversToPingSize; idx++) {
         ...
         for (index2 = 0; index2 < fw->targetServerPings.Size(); index2++) {
            if (fw->targetServerPings[index2].ping >= swp.ping) break;
         }
         fw->sourceServerPings.Insert(swp, index2, _FILE_AND_LINE_);
      }
   }
   ...
   if (fw->sourceServerPings.Size() > 0 && fw->targetServerPings.Size() > 0) {
      fw->OrderRemainingServersToTry();
      fw->timeRequestedPings = 0;
      TryNextServer(fw->sata, fw);
   }
...
void UDPProxyCoordinator::TryNextServer(SenderAndTargetAddress sata, ForwardingRequest *fw) {
   ...
   SendForwardingRequest(sata.senderClientAddress, sata.targetClientAddress, fw->currentlyAttemptedServerAddress,
                         fw->timeoutOnNoDataMS);
}

void UDPProxyCoordinator::SendAllBusy(SystemAddress senderClientAddress, SystemAddress targetClientAddress,
                                      RakNetGUID targetClientGuid, SystemAddress requestingAddress) {
   RakNet::BitStream outgoingBs;
   outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
...
void UDPProxyCoordinator::ForwardingRequest::OrderRemainingServersToTry(void) {
   // DataStructures::Multilist<ML_ORDERED_LIST,UDPProxyCoordinator::ServerWithPing,unsigned short>
   // swpList;
   ...
   if (sourceServerPings.Size() == 0 && targetServerPings.Size() == 0) return;
