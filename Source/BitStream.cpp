...
// Use this if you pass a pointer copy to the constructor (_copyData==false) and want to
// overallocate to prevent reallocation
...
BitStream::~BitStream() {
   if (copyData && numberOfBitsAllocated > (BITSTREAM_STACK_ALLOCATION_SIZE << 3))
      rakFree_Ex(data, _FILE_AND_LINE_);  // Use realloc and free so we are more efficient than
                                          // delete and new for resizing
}
...
void BitStream::Write(BitStream &bitStream, BitSize_t numberOfBits) { Write(&bitStream, numberOfBits); }

void BitStream::Write(BitStream &bitStream) { Write(&bitStream); }

bool BitStream::Read(BitStream *bitStream, BitSize_t numberOfBits) {
   if (GetNumberOfUnreadBits() < numberOfBits) return false;
...
bool BitStream::Read(char *outByteArray, const unsigned int numberOfBytes) {
   // Optimization:
   if ((readOffset & 7) == 0) {
      if (readOffset + (numberOfBytes << 3) > numberOfBitsUsed) return false;
...
void BitStream::ResetWritePointer(void) { numberOfBitsUsed = 0; }
...
void BitStream::Write0(void) {
   ...
   if ((numberOfBitsUsed & 7) == 0) data[numberOfBitsUsed >> 3] = 0;
...
void BitStream::WriteAlignedBytesSafe(const char *inByteArray, const unsigned int inputLength,
...
// sequence must be the same as was used with WriteBits. This will be a problem with packet
...
bool BitStream::ReadAlignedBytes(unsigned char *inOutByteArray, const unsigned int numberOfBytesToRead) {
   ...
   if (readOffset + (numberOfBytesToRead << 3) > numberOfBitsUsed) return false;
...
bool BitStream::ReadAlignedBytesSafe(char *inOutByteArray, unsigned int &inputLength,
                                     const unsigned int maxBytesToRead) {
   if (ReadCompressed(inputLength) == false) return false;
   if (inputLength > maxBytesToRead) inputLength = maxBytesToRead;
...
bool BitStream::ReadAlignedBytesSafeAlloc(char **outByteArray, unsigned int &inputLength,
                                          const unsigned int maxBytesToRead) {
...
void BitStream::WriteBits(const unsigned char *inByteArray, BitSize_t numberOfBitsToWrite,
   ...
   // do
   {
      ...
      if (numberOfBitsToWrite < 8 && rightAlignedBits)  // rightAlignedBits means in the case of a partial byte, the
                                                        // bits are aligned from the right (bit 0) rather than the left
                                                        // (as in the normal internal representation)
         dataByte <<= 8 - numberOfBitsToWrite;          // shift left to get the bits on the left, as in our
      ...
      if (numberOfBitsUsedMod8 == 0)
         ...
         *(data + (numberOfBitsUsed >> 3)) |= dataByte >> (numberOfBitsUsedMod8);  // First half
         ...
         if (8 - (numberOfBitsUsedMod8) < 8 &&
             8 - (numberOfBitsUsedMod8) < numberOfBitsToWrite)  // If we didn't write it all out in the first half (8 -
                                                                // (numberOfBitsUsed%8) is the number we wrote in the
                                                                // first half)
         {
            *(data + (numberOfBitsUsed >> 3) + 1) =
                (unsigned char)(dataByte << (8 - (numberOfBitsUsedMod8)));  // Second half (overlaps
                                                                            // byte boundary)
         }
      }
...
void BitStream::WriteCompressed(const unsigned char *inByteArray, const unsigned int size, const bool unsignedData) {
   ...
   // From high byte to low byte, if high byte is a byteMatch then write a 1 bit. Otherwise write a
   // 0 bit and then write the remaining bytes
   while (currentByte > 0) {
      if (inByteArray[currentByte] == byteMatch)  // If high byte is byteMatch (0 of 0xff) then it
                                                  // would have the same value shifted
      {
         bool b = true;
         Write(b);
      } else {
   ...
   // If the upper half of the last byte is a 0 (positive) or 16 (negative) then write a 1 and the
   // remaining 4 bits.  Otherwise write a 0 and the 8 bites.
...
bool BitStream::ReadBits(unsigned char *inOutByteArray, BitSize_t numberOfBitsToRead, const bool alignBitsToRight) {
...
#endif
   if (numberOfBitsToRead <= 0) return false;
   ...
   while (numberOfBitsToRead > 0) {
      *(inOutByteArray + offset) |= *(data + (readOffset >> 3)) << (readOffsetMod8);  // First half
      ...
      if (readOffsetMod8 > 0 && numberOfBitsToRead > 8 - (readOffsetMod8))  // If we have a second half, we didn't read
                                                                            // enough bytes in the first half
         *(inOutByteArray + offset) |=
      ...
      if (numberOfBitsToRead >= 8) {
         ...
         if (neg < 0)  // Reading a partial byte for the last byte, shift right so the data is
                       // aligned on the right
         {
            if (alignBitsToRight) *(inOutByteArray + offset) >>= -neg;
...
bool BitStream::ReadCompressed(unsigned char *inOutByteArray, const unsigned int size, const bool unsignedData) {
   ...
   while (currentByte > 0) {
      ...
      if (b)  // Check that bit
      {
         ...
         if (ReadBits(inOutByteArray, (currentByte + 1) << 3) == false) return false;

         return true;
      }
   }

   // All but the first bytes are byteMatch.  If the upper half of the last byte is a 0 (positive)
   // or 16 (negative) then what we read will be a 1 and the remaining 4 bits. Otherwise we read a 0
   // and the 8 bytes RakAssert(readOffset+1 <=numberOfBitsUsed); // If this assert is hit the
   // stream wasn't long enough to read from
   ...
   if (b)  // Check that bit
   {
      if (ReadBits(inOutByteArray + currentByte, 4) == false) return false;

      inOutByteArray[currentByte] |=
          halfByteMatch;  // We have to set the high 4 bits since these are set to 0 by ReadBits
   } else {
      if (ReadBits(inOutByteArray + currentByte, 8) == false) return false;
   }
...
void BitStream::AddBitsAndReallocate(const BitSize_t numberOfBitsToWrite) {
   ...
   if (numberOfBitsToWrite + numberOfBitsUsed > 0 &&
       ((numberOfBitsAllocated - 1) >> 3) <
           ((newNumberOfBitsAllocated - 1) >> 3))  // If we need to allocate 1 or more new bytes
   {
#ifdef _DEBUG
      // If this assert hits then we need to specify true for the third parameter in the constructor
      // It needs to reallocate to hold all the data and can't do it unless we allocated to begin
      // with Often hits if you call Write or Serialize on a read-only bitstream
...
#endif
                        //  memset(data+newByteOffset, 0,  ((newNumberOfBitsAllocated-1)>>3) -
                        //  ((numberOfBitsAllocated-1)>>3)); // Set the new data block to 0
   }

   if (newNumberOfBitsAllocated > numberOfBitsAllocated) numberOfBitsAllocated = newNumberOfBitsAllocated;
}

BitSize_t BitStream::GetNumberOfBitsAllocated(void) const { return numberOfBitsAllocated; }
...
void BitStream::IgnoreBits(const BitSize_t numberOfBits) { readOffset += numberOfBits; }

void BitStream::IgnoreBytes(const unsigned int numberOfBytes) { IgnoreBits(BYTES_TO_BITS(numberOfBytes)); }

// Move the write pointer to a position on the array.  Dangerous if you don't know what you are
// doing! Doesn't work with non-aligned data!
void BitStream::SetWriteOffset(const BitSize_t offset) { numberOfBitsUsed = offset; }
...
*/
// If we used the constructor version with copy data off, this makes sure it is set to on and the
// data pointed to is copied.
   ...
   if (copyData == false) {
      ...
      if (numberOfBitsAllocated > 0) {
         unsigned char *newdata =
             (unsigned char *)rakMalloc_Ex((size_t)BITS_TO_BYTES(numberOfBitsAllocated), _FILE_AND_LINE_);
#ifdef _DEBUG
...
void BitStream::ReverseBytes(unsigned char *inByteArray, unsigned char *inOutByteArray, const unsigned int length) {
   for (BitSize_t i = 0; i < length; i++) inOutByteArray[i] = inByteArray[length - i - 1];
}
...
bool BitStream::Read(char *varString) { return RakString::Deserialize(varString, this); }

bool BitStream::Read(unsigned char *varString) { return RakString::Deserialize((char *)varString, this); }
...
bool BitStream::ReadAlignedVar8(char *inOutByteArray) {
   ...
   if (readOffset + 1 * 8 > numberOfBitsUsed) return false;
...
bool BitStream::ReadAlignedVar16(char *inOutByteArray) {
   RakAssert((readOffset & 7) == 0);
   if (readOffset + 2 * 8 > numberOfBitsUsed) return false;
#ifndef __BITSTREAM_NATIVE_END
   if (DoEndianSwap()) {
...
bool BitStream::ReadAlignedVar32(char *inOutByteArray) {
   RakAssert((readOffset & 7) == 0);
   if (readOffset + 4 * 8 > numberOfBitsUsed) return false;
#ifndef __BITSTREAM_NATIVE_END
   if (DoEndianSwap()) {
