...
ConsoleServer::~ConsoleServer() {
   if (prompt) rakFree_Ex(prompt, _FILE_AND_LINE_);
}

void ConsoleServer::SetTransportProvider(TransportInterface *transportInterface, unsigned short port) {
   // Replace the current TransportInterface, stopping the old one, if present, and starting the new
   // one.
   if (transportInterface) {
      ...
      unsigned i;
      for (i = 0; i < commandParserList.Size(); i++) commandParserList[i]->OnTransportChange(transport);

      //  The transport itself might have a command parser - for example password for the RakNet
      //  transport
...
void ConsoleServer::AddCommandParser(CommandParserInterface *commandParserInterface) {
   if (commandParserInterface == 0) return;

   // Non-duplicate insertion
   unsigned i;
   for (i = 0; i < commandParserList.Size(); i++) {
      if (commandParserList[i] == commandParserInterface) return;
   ...
   if (transport) commandParserInterface->OnTransportChange(transport);
}

void ConsoleServer::RemoveCommandParser(CommandParserInterface *commandParserInterface) {
   if (commandParserInterface == 0) return;

   // Overwrite the element we are removing from the back of the list and delete the back of the
   // list
   unsigned i;
   for (i = 0; i < commandParserList.Size(); i++) {
...
void ConsoleServer::Update(void) {
   ...
   while (p) {
      ...
      copy[p->length] = 0;
      RakNet::CommandParserInterface::ParseConsoleString((char *)p->data, COMMAND_DELINATOR, COMMAND_DELINATOR_TOGGLE,
      ...
      if (_stricmp(*parameterList, "help") == 0 && numParameters <= 2) {
         ...
         if (numParameters == 1) {
            ...
            transport->Send(p->systemAddress,
                            "help <ParserName>                           Show help on a particular "
                            "parser.\r\n");
            transport->Send(p->systemAddress,
                            "help <CommandName>                          Show help on a particular "
                            "command.\r\n");
            transport->Send(p->systemAddress,
         ...
         if (numParameters >= 2)  // At minimum <CommandParserName> <Command>
         {
            ...
            if (**parameterList >= '0' && **parameterList <= '9') {
               ...
               // // Prefixing with the name of a command parser directs to that parser.  See if the
               // first word matches a parser
               for (i = 0; i < commandParserList.Size(); i++) {
                  ...
                  if (commandParserList[commandParserIndex]->GetRegisteredCommand(parameterList[1], &rc)) {
                     ...
                     if (rc.parameterCount == CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS ||
                        ...
                        commandParserList[commandParserIndex]->OnCommand(
                            rc.command, numParameters - 2, parameterList + 2, transport, p->systemAddress, copy);
                     else
...
void ConsoleServer::ShowPrompt(SystemAddress systemAddress) { transport->Send(systemAddress, prompt); }
