...
// TODO: optimize the list of teams and team members to be O(1). Store in hashes, use linked lists
// to get ordered traversal
...
/// Teams contain properties including the number of team members per team, whether or not tagged
/// teams must have equal numbers of members, and if a team is locked or not to certain entry
/// conditions Team members contain properties including which teams they are on and which teams
/// they want to join if a team is not immediately joinable Advanced functionality includes the
/// ability for a team member to be on multiple teams simultaneously, the ability to swap teams with
/// other members, and the ability to resize the number of members supported per team
///
...
/// \brief Automates networking and list management for teams
/// \details When used with ReplicaManager3 and FullyConnectedMesh2, provides a complete solution to
/// managing a distributed list of teams and team member objects with support for host migration.
/// \ingroup PLUGINS_GROUP

/// \ingroup TEAM_MANAGER_GROUP
/// \brief A subcategory of not being on a team. For example, 0 may mean no team for a player, while
/// 1 may mean no team for a spectator. Defined by the user.
...
// Bitwise combination of ALLOW_JOIN_ANY_AVAILABLE_TEAM, ALLOW_JOIN_SPECIFIC_TEAM,
...
/// \details Use TeamSelection::AnyAvailable(), TeamSelection::SpecificTeam(), or
/// TeamSelection::NoTeam() \ingroup TEAM_MANAGER_GROUP
struct TeamSelection {
   ...
   /// \brief Join any team that has available slots and is tagged with
   /// ALLOW_JOIN_ANY_AVAILABLE_TEAM \details ID_TEAM_BALANCER_TEAM_ASSIGNED,
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, or ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED will be
   /// returned to all systems.
   ...
   /// \details ID_TEAM_BALANCER_TEAM_ASSIGNED, ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, or
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED will be returned to all systems. \param[in]
   /// specificTeamToJoin Which team to attempt to join.
   static TeamSelection SpecificTeam(TM_Team *specificTeamToJoin);
   /// \brief Do not join a team, or leave all current teams.
   /// \details This always succeeds. ID_TEAM_BALANCER_TEAM_ASSIGNED will be returned to all
   /// systems. \param[in] noTeamSubcategory Even when not on a team, you can internally identify a
   /// subcategory of not being on a team, such as AI or spectator.
   static TeamSelection NoTeam(NoTeamId noTeamSubcategory);
};

/// \brief A member of one or more teams.
/// \details Contains data and operations on data to manage which team your game's team members are
/// on. Best used as a composite member of your "User" or "Player" class(es). When using with
/// ReplicaManager3, call TM_TeamMember::ReferenceTeamMember() in
/// Replica3::DeserializeConstruction() and TM_TeamMember::DeserializeConstruction() in
/// Replica3::PostDeserializeConstruction() There is otherwise no need to manually serialize the
/// class, as operations are networked internally. \ingroup TEAM_MANAGER_GROUP
class RAK_DLL_EXPORT TM_TeamMember {
  public:
   ...
   /// \details Function will return false on invalid operations, such as joining a team you are
   /// already on. Will also fail with TeamSelection::JOIN_ANY_AVAILABLE_TEAM if you are currently
   /// on a team. On success, every system will get ID_TEAM_BALANCER_TEAM_ASSIGNED. Use
   /// TeamManager::DecomposeTeamAssigned() to get details of which team member the message refers
   /// to. On failure, all systems will get ID_TEAM_BALANCER_REQUESTED_TEAM_FULL or
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED. Use TeamManager::DecomposeTeamFull() and
   /// TeamManager::DecomposeTeamLocked() to get details of which team member the message refers to.
   /// \note Joining a specific team with this function may result in being on more than one team at
   /// once, even if you call the function while locally only on one team. If your game depends on
   /// only being on one team at a team, use RequestTeamSwitch() instead with the parameter
   /// teamToLeave set to 0 \param[in] TeamSelection::AnyAvailable(), TeamSelection::SpecificTeam(),
   /// or TeamSelection::NoTeam() \return false On invalid or unnecessary operation. Otherwise
   /// returns true
   bool RequestTeam(TeamSelection teamSelection);

   /// \brief Similar to RequestTeam with TeamSelection::SpecificTeam(), but leave a team
   /// simultaneously when the desired team is joinable \param[in] teamToJoin Which team to join
   /// \param[in] teamToLeave If 0, means leave all current teams. Otherwise, leave the specified
   /// team. \return false On invalid or unnecessary operation. Otherwise returns true
   bool RequestTeamSwitch(TM_Team *teamToJoin, TM_Team *teamToLeave);

   /// \brief Returns the first requested team in the list of requested teams, if you have a
   /// requested team at all. \return TeamSelection::SpecificTeam(), TeamSelection::NoTeam(), or
   /// TeamSelection::AnyAvailable()
   ...
   /// \details Useful if you got ID_TEAM_BALANCER_REQUESTED_TEAM_FULL or
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED and changed your mind about joining the team. \note
   /// This is not guaranteed to work due to latency. To clarify, If the host switches your team at
   /// the same time you call CancelRequestTeam() you may still get ID_TEAM_BALANCER_TEAM_ASSIGNED
   /// for the team you tried to cancel. \param[in] specificTeamToCancel Which team to no longer
   /// join. Use 0 for all. \return false On invalid or unnecessary operation. Otherwise returns
   /// true
   ...
   /// \param[in] _noTeamSubcategory If the team member has been removed from all teams, which
   /// subcategory of NoTeamId to set them to \return false On invalid or unnecessary operation.
   ...
   /// \note This is the same as and just calls
   /// RequestTeam(TeamSelection::NoTeam(noTeamSubcategory)); \return false On invalid or
   /// unnecessary operation. Otherwise returns true
   ...
   /// For each team member, when you get ID_TEAM_BALANCER_TEAM_ASSIGNED for that member, the team
   /// list is saved. Use this function to get that list, for example to determine which teams we
   /// just left or joined \param[out] _teams The previous list of teams we were on
   ...
   /// \details To replicate a TM_TeamMember on another system, first instantiate the object using
   /// your own code, or a system such as ReplicaManager3. Next, call SerializeConstruction() from
   /// whichever system owns the team member Last, call DeserializeConstruction() on the newly
   /// created TM_TeamMember \note You must instantiate and deserialize all TM_Team instances that
   /// the team member refers to before calling DesrializeConstruction().
   /// ReplicaManager3::PostSerializeConstruction() and
   /// ReplicaManager3::PostDeserializeConstruction() will ensure this. \param[out]
   /// constructionBitstream This object serialized to a BitStream
   ...
   /// \note DeserializeConstruction also calls ReferenceTeamMember on the passed \a teamManager
   /// instance, there is no need to do so yourself \param[in] teamManager TeamManager instance
   ...
   /// \param[in] o Stores a void* for your own use. If using composition, this is useful to store a
  ...
  protected:
   NetworkID networkId;
   TM_World *world;
   // Teams we are a member of. We can be on more than one team, but not on the same team more than
   // once
   DataStructures::List<TM_Team *> teams;
   // If teams is empty, which subcategory of noTeam we are on
   NoTeamId noTeamSubcategory;
   // Teams we have requested to join. Mutually exclusive with teams we are already on. Cannot
   // request the same team more than once.
   DataStructures::List<RequestedTeam> teamsRequested;
   // If teamsRequested is not empty, we want to join a specific team
   // If teamsRequested is empty, then joinTeamType is either JOIN_NO_TEAM or
   // JOIN_ANY_AVAILABLE_TEAM
   JoinTeamType joinTeamType;
...
/// When using with ReplicaManager3, call TM_Team::ReferenceTeam() in
/// Replica3::DeserializeConstruction() and TM_Team::DeserializeConstruction() in
/// Replica3::PostDeserializeConstruction() There is otherwise no need to manually serialize the
/// class, as operations are networked internally. \ingroup TEAM_MANAGER_GROUP
class RAK_DLL_EXPORT TM_Team {
  public:
   ...
   /// Defaults to 65535
   /// Setting the limit lower than the existing number of members kicks members out, and assigns
   /// noTeamSubcategory to them if they have no other team to go to Setting the limit higher allows
   /// members to join in. If a member has a pending request to join this team, they join
   /// automatically and ID_TEAM_BALANCER_TEAM_ASSIGNED will be returned for those members.
   /// \param[in] _teamMemberLimit The new limit
   /// \param[in] noTeamSubcategory Which noTeamSubcategory to assign to members that now have no
   /// team. \return false On invalid or unnecessary operation. Otherwise returns true
   bool SetMemberLimit(TeamMemberLimit _teamMemberLimit, NoTeamId noTeamSubcategory);

   /// \return If team balancing is on, the most members that can be on this team that would not
   /// either unbalance it or exceed the value passed to SetMemberLimit(). If team balancing is off,
   /// the same as GetMemberLimitSetting()
   ...
   /// For an invite-only team, use ALLOW_JOIN_SPECIFIC_TEAM only and only allow the requester to
   /// call TM_TeamMember::RequestTeam() upon invitiation through your game code. Defaults to allow
   /// all \param[in] _joinPermissions Bitwise combination of ALLOW_JOIN_ANY_AVAILABLE_TEAM,
   /// ALLOW_JOIN_SPECIFIC_TEAM, ALLOW_JOIN_REBALANCING \return false On invalid or unnecessary
   /// operation. Otherwise returns true
   ...
   /// \details Identical to teamMember->LeaveTeam(this, noTeamSubcategory); See
   /// TeamMember::LeaveTeam() for details. \param[in] teamMember Which team member to remove
   /// \param[in] noTeamSubcategory If the team member has been removed from all teams, which
   /// subcategory of NoTeamId to set them to
   void LeaveTeam(TM_TeamMember *teamMember, NoTeamId noTeamSubcategory);

   /// \return What was passed as the \a applyBalancing parameter TM_World::ReferenceTeam() when
   /// this team was added.
   ...
   /// \details On the host, when sending existing objects to a new system, call
   /// SerializeConstruction() on each of those objects to serialize creation state. Creating the
   /// actual Team and TeamMember objects should be handled by your game code, or a system such as
   /// ReplicaManager3
   void SerializeConstruction(BitStream *constructionBitstream);

   /// \brief Used by non-host systems to read the bitStream written by SerializeConstruction()
   /// \details On non-host systems, after creating existing objects, call DeserializeConstruction()
   /// to read and setup that object Creating the actual Team and TeamMember objects should be
   /// handled by your game code, or a system such as ReplicaManager3
   bool DeserializeConstruction(TeamManager *teamManager, BitStream *constructionBitstream);
  ...
  protected:
   ...
   // Find the member index that wants to join the indicated team, is only on one team, and wants to
   // leave that team
...
/// \details Each TM_World instance is independent of other TM_World world instances. This enables
/// you to host multiple games on a single computer. Not currently supported to have the same
/// TM_Team or TM_TeamMember in more than one world at a time, but easily added on request. \ingroup
/// TEAM_MANAGER_GROUP
class TM_World {
  public:
   TM_World();
   ...
   /// \param[in] rakNetGUID GUID of the system you are adding. See Packet::rakNetGUID or
   /// RakPeerInterface::GetGUIDFromSystemAddress()
   ...
   /// \param[in] rakNetGUID GUID of the system you are removing. See Packet::rakNetGUID or
   /// RakPeerInterface::GetGUIDFromSystemAddress()
   ...
   /// \details Your game should contain instances of TM_Team, for example by using composition with
   /// your game's Team or PlayerList class Tell TeamManager about these instances using
   /// ReferenceTeam(). \note The destrutor of TM_Team calls DereferenceTeam() automatically.
   /// \param[in] team The instance you are registering
   /// \param[in] networkId Identifies this instance. This value is independent of values used by
   /// NetworkIDManager. You can use the same value as the object that contains this instance.
   /// \param[in] applyBalancing Whether or not to include this team for balancing when calling
   /// SetBalanceTeams().
   ...
   /// \param[in] noTeamSubcategory All players on this team are kicked off. If these players then
   /// have no team, they are set to this no team category.
   ...
   /// \return Returns whatever was passed to \a team in the function ReferenceTeam() in the order
   /// it was called.
   TM_Team *GetTeamByIndex(unsigned int index) const;

   /// \param[in] teamId Value passed to ReferenceTeam()
   /// \return Returns whatever was passed to \a team in the function ReferenceTeam() with this
   /// NetworkID.
   ...
   /// \details Your game should contain instances of TM_TeamMember, for example by using
   /// composition with your game's User or Player classes Tell TeamManager about these instances
   /// using ReferenceTeamMember(). \note The destrutor of TM_TeamMember calls
   /// DereferenceTeamMember() automatically. \param[in] teamMember The instance you are registering
   /// \param[in] networkId Identifies this instance. This value is independent of values used by
   /// NetworkIDManager. You can use the same value as the object that contains this instance
   ...
   /// \note This is called by the destructor of TM_TeamMember automatically, so you do not normally
   ...
   /// \return Returns whatever was passed to \a team in the function ReferenceTeamMember() in the
   /// order it was called.
   TM_TeamMember *GetTeamMemberByIndex(unsigned int index) const;

   /// \param[in] index A value between 0 and GetTeamMemberCount()
   /// \return Returns whatever was passed to \a teamMemberID in the function ReferenceTeamMember()
   ...
   /// \return Returns Returns whatever was passed to \a team in the function ReferenceTeamMember()
   /// with this NetworkID
   ...
   /// \details For each team added with ReferenceTeam() and \a applyBalancing set to true, players
   /// on unbalanced teams will be redistributed While active, players can only join balanced teams
   /// if doing so would not cause that team to become unbalanced. If a player on the desired team
   /// also wants to switch, then both players will switch simultaneously. Otherwise,
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_FULL will be returned to the requester and switching will
   /// occur when possible. If balanceTeams is true and later set to false, players waiting on
   /// ID_TEAM_BALANCER_REQUESTED_TEAM_FULL will be able to join the desired team immediately
   /// provided it is not full. \param[in] balanceTeams Whether to activate or deactivate team
   /// balancing. \param[in] noTeamSubcategory If a player is kicked off a team and is no longer on
   /// any team, his or her noTeamSubcategory is set to this value
   ...
   /// \details Operations that can cause conflicts due to latency, such as joining teams, are
   /// operated on by the host. The result is sent to all systems added with AddParticipant() For a
   /// client/server game, call SetHost() with the server's RakNetGUID value on all systems
   /// (including the server itself). If you call TeamManager::SetTopology(TM_CLIENT_SERVER), the
   /// server will also relay messages between participants. For a peer to peer game, call SetHost()
   /// on the same peer when host migration occurs. Use TeamManager::SetTopology(TM_PEER_TO_PEER) in
   /// this case. \note If using FullyConnectedMesh2, SetHost() is called automatically when
   /// ID_FCM2_NEW_HOST is returned. \param[in] _hostGuid The host, which is the system that will
   /// serialize and resolve team disputes and calculate team balancing.
   ...
   /// \details It is up to the user to deallocate pointers passed to ReferenceTeamMember() or
   /// ReferenceTeam(), if so desired.
   void Clear(void);
  ...
  protected:
   ...
   void GetSortedJoinRequests(
       DataStructures::OrderedList<JoinRequestHelper, JoinRequestHelper, JoinRequestHelperComp> &joinRequests);
   ...
   int JoinSpecificTeam(TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch, TM_Team *teamToLeave,
...
/// they want to join if a team is not immediately joinable Advanced functionality includes the
/// ability for a team member to be on multiple teams simultaneously, the ability to swap teams with
/// other members, and the ability to resize the number of members supported per team The
/// architecture is designed for easy integration with ReplicaManager3
///
/// Usage:<BR>
/// 1. Define your game classes to represent teams and team members. Your game classes should hold
/// game-specific information such as team name and color.<BR>
/// 2. Have those game classes contain a corresponding TM_Team or TM_TeamMember instance. Operations
/// on teams will be performed by those instances. Use SetOwner() to refer to the parent object when
/// using composition.<BR>
/// 3. Call TeamManager::SetTopology() for client/server or peer to peer.<BR>
/// 4. Call AddWorld() to instantiate a TM_World object which will contain references to your
/// TM_TeamMember and TM_Team instances.<BR>
/// 5. When you instantiate a TM_TeamMember or TM_Team object, call ReferenceTeam() and
/// ReferenceTeamMember() for each corresponding object<BR>
/// 6. When sending world state to a new connection, for example in
/// ReplicaManager3::SerializeConstruction(), call TM_SerializeConstruction() on the corresponding
/// TM_TeamMember and TM_Team objects. TM_Team instances on the new connection must be created
/// before TM_TeamMember instances.<BR>
/// 7. Call TM_DeserializeConstruction() on your new corresponding TM_TeamMember and TM_Team
/// instances.<BR>
/// 8. Execute team operations. ID_TEAM_BALANCER_REQUESTED_TEAM_FULL,
/// ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED, ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED, and
/// ID_TEAM_BALANCER_TEAM_ASSIGNED are returned to all systems when the corresponding event occurs
/// for a team member.<BR>
/// 9. As the peer to peer session host changes, call SetHost() (Not necessary if using
/// FullyConnectedMesh2). If using client/server, you must set the host<BR> \note This replaces
/// TeamBalancer. You cannot use TeamBalancer and TeamManager at the same time. \ingroup
/// TEAM_MANAGER_GROUP
class RAK_DLL_EXPORT TeamManager : public PluginInterface2 {
  public:
   ...
   /// \param[in] worldId Arbitrary user-defined id of the world to create. Each world instance must
   /// have a unique id.
   ...
   /// \brief When auto managing connections, call TM_World::AddParticipant() on all worlds for all
   /// new connections automatically Defaults to true \note You probably want this set to false if
   /// using multiple worlds \param[in] autoAdd Automatically call TM_World::AddParticipant() all
   /// worlds each new connection. Defaults to true.
   void SetAutoManageConnections(bool autoAdd);

   /// \brief If \a _topology is set to TM_CLIENT_SERVER, the host will relay messages to
   /// participants. \details If topology is set to TM_PEER_TO_PEER, the host assumes the original
   /// message source was connected to all other participants and does not relay messages. \note If
   /// TM_PEER_TO_PEER, this plugin will listen for ID_FCM2_NEW_HOST and call SetHost() on all
   /// worlds automatically \note Defaults to TM_PEER_TO_PEER \param[in] _topology Topology to use
   void SetTopology(TMTopology _topology);

   /// \brief When you get ID_TEAM_BALANCER_REQUESTED_TEAM_FULL, pass the packet to this function to
   /// read out parameters \param[in] A packet where
   /// packet->data[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_FULL \return true on success, false on read
   /// error
   void DecomposeTeamFull(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
                          uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive,
                          JoinPermissions &joinPermissions);

   /// \brief When you get ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED, pass the packet to this function
   /// to read out parameters \param[in] A packet where
   /// packet->data[0]==ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED \return true on success, false on
   /// read error
   void DecomposeTeamLocked(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
                            uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive,
                            JoinPermissions &joinPermissions);

   /// \brief Clear all memory and reset everything.
   /// \details Deallocates TM_World instances. It is up to the user to deallocate pointers passed
   /// to ReferenceTeamMember() or ReferenceTeam(), if so desired.
   void Clear(void);
  ...
  protected:
   ...
   void DecomposeTeamFullOrLocked(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
                                  uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing,
   ...
   void RemoveFromTeamsRequestedAndAddTeam(TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch,
                                           TM_Team *teamToLeave);
   ...
   /// \brief When you get ID_TEAM_BALANCER_TEAM_ASSIGNED, pass the packet to this function to read
   /// out parameters \param[in] A packet where packet->data[0]==ID_TEAM_BALANCER_TEAM_ASSIGNED
   /// \return true on success, false on read error
   void DecodeTeamAssigned(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember,
                           NoTeamId &noTeamSubcategory, JoinTeamType &joinTeamType,
                           DataStructures::List<TM_Team *> &newTeam, DataStructures::List<TM_Team *> &teamsLeft,
