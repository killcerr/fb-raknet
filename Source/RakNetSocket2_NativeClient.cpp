...
RNS2BindResult RNS2_NativeClient::Bind(NativeClientBindParameters *bindParameters, const char *file,
                                       unsigned int line) {
   ...
   if (Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4)) {
      rns2Socket =
          ((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
              ->Create(bindParameters->nativeClientInstance);
      RAKNET_DEBUG_PRINTF("CreateChromeSocket(%d,%s,0x%08x,%d) ==> 0x%08x\n", bindParameters->port,
                          bindParameters->forceHostAddress ? bindParameters->forceHostAddress : "(null)",
      ...
      // bind call)
      ((PPB_UDPSocket_Private_0_4 *)pp::Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
          ->SetSocketFeature(rns2Socket, PP_UDPSOCKETFEATURE_BROADCAST, PP_MakeBool(PP_TRUE));
      ...
      ((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
          ->Bind(rns2Socket, &client_addr, cc);
      return BR_SUCCESS;
   }
...
void RNS2_NativeClient::SendImmediate(RNS2_SendParameters_NativeClient *sp) {
   ...
   NetAddressPrivate::CreateFromIPv6Address(sp->systemAddress.address.addr6.sin6_addr.u.Byte, 0,
                                            sp->systemAddress.GetPort(), &client_addr);
#else
   NetAddressPrivate::CreateFromIPv4Address((const uint8_t *)&sp->systemAddress.address.addr4.sin_addr,
                                            sp->systemAddress.GetPort(), &client_addr);
#endif
   ...
   ((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
       ->SendTo(sp->socket2->rns2Socket, sp->data, sp->length, &client_addr, cc);
}
...
void RNS2_NativeClient::onRecvFrom(void *pData, int32_t dataSize) {
   ...
   // any error codes will be given to us in the dataSize value; see pp_errors.h for a list of
   // errors
   if (dataSize <= 0 || !pData) {
      // Free data
   ...
   bool ok = false;
   if (((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
           ->GetRecvFromAddress(socket2->rns2Socket, &addr) == PP_TRUE) {
...
void RNS2_NativeClient::IssueReceiveCall(void) {
   ...
   if (recvFromStruct != NULL) {
      ...
      ((PPB_UDPSocket_Private_0_4 *)Module::Get()->GetBrowserInterface(PPB_UDPSOCKET_PRIVATE_INTERFACE_0_4))
          ->RecvFrom(rns2Socket, recvFromStruct->data, MAXIMUM_MTU_SIZE, cc);
   }
}
