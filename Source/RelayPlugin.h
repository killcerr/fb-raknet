...
/// \brief A simple class to relay messages from one system to another, identifying remote systems
/// by a string. \ingroup RELAY_PLUGIN_GROUP
class RAK_DLL_EXPORT RelayPlugin : public PluginInterface2 {
  public:
   ...
   /// \brief Forward messages from any system, to the system specified by the combination of key
   /// and guid. The sending system only needs to know the key. \param[in] key A string to identify
   /// the target's RakNetGUID. This is so the sending system does not need to know the RakNetGUID
   /// of the target system. The key should be unique among all guids added. If the key is not
   /// unique, only one system will be sent to (at random). \param[in] guid The RakNetGuid of the
   /// system to send to. If this system disconnects, it is removed from the internal hash \return
   ...
   /// \brief If true, then if the client calls AddParticipantRequestFromClient(), the server will
   /// call AddParticipantOnServer() automatically Defaults to false \param[in] accept true to
   /// accept, false to not.
   ...
   /// \pre The server must have called SetAcceptAddParticipantRequests(true) or the request will be
   /// ignored \param[in] key A string to identify out system. Passed to \a key on
   /// AddParticipantOnServer() \param[in] relayPluginServerGuid the RakNetGUID of the system
   /// running RelayPlugin
   ...
   /// \param[in] destinationGuid The key value passed to AddParticipant() earlier on the server. If
   /// this was not done, the server will not relay the message (it will be silently discarded).
   ...
   void SendToParticipant(const RakNetGUID &relayPluginServerGuid, const RakString &destinationGuid,
                          BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
                          char orderingChannel);

   void SendGroupMessage(const RakNetGUID &relayPluginServerGuid, BitStream *bitStream, PacketPriority priority,
