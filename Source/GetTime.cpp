...
/// See http://support.microsoft.com/kb/274323 where the timer can sometimes jump forward by hours
/// or days This also has the effect where debugging a sending system won't treat the time spent
/// halted past 1 second as elapsed network time
...
//		GetProcessAffinityMask(mProc, &mProcMask, &mSysMask);
   ...
   // 9/26/2010 In China running LuDaShi, QueryPerformanceFrequency has to be called every time
   // because CPU clock speeds can be different
...
RakNet::TimeUS GetTimeUS_Linux(void) {
   ...
   if (initialized == false) {
      gettimeofday(&tp, 0);
      initialized = true;
      // I do this because otherwise RakNet::Time in milliseconds won't work as it will underflow
      // when dividing by 1000 to do the conversion
