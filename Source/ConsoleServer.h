...
/// \brief Contains ConsoleServer , used to plugin to your game to accept remote console-based
/// connections
///
...
/// The TransportInterface will be used to send data between the server and the client.  The
/// connecting client must support the protocol used by your derivation of TransportInterface .
/// TelnetTransport and RakNetTransport are two such derivations . When a command is sent by a
/// remote console, it will be processed by your implementations of CommandParserInterface
class RAK_DLL_EXPORT ConsoleServer {
  public:
   ...
   /// \brief Call this with a derivation of TransportInterface so that the console server can send
   /// and receive commands \param[in] transportInterface Your interface to use. \param[in] port The
   /// port to host on.  Telnet uses port 23 by default.  RakNet can use whatever you want.
   ...
   /// \brief Remove an implementation of CommandParserInterface previously added with
   /// AddCommandParser(). \param[in] commandParserInterface The command parser referred to
   ...
   /// \param[in] _prompt Null-terminated string of the prompt to use. If you want a newline, be
   /// sure to use /r/n
