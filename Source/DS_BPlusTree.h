...
struct RAK_DLL_EXPORT Page {
   ...
   // Used only for leaf nodes.  Data is the actual data, while next is the pointer to the next leaf
   // (for B+)
   DataType data[order];
...
class RAK_DLL_EXPORT BPlusTree {
  ...
  protected:
   ...
   Page<KeyType, DataType, order> *InsertIntoNode(const KeyType key, const DataType &childData, int insertionIndex,
                                                  Page<KeyType, DataType, order> *nodeData,
                                                  Page<KeyType, DataType, order> *cur, ReturnAction *returnAction);
   Page<KeyType, DataType, order> *InsertBranchDown(const KeyType key, const DataType &data,
                                                    Page<KeyType, DataType, order> *cur, ReturnAction *returnAction,
                                                    bool *success);
   Page<KeyType, DataType, order> *GetLeafFromKey(const KeyType key) const;
   bool FindDeleteRebalance(const KeyType key, Page<KeyType, DataType, order> *cur, bool *underflow,
                            KeyType rightRootKey, ReturnAction *returnAction, DataType &out);
   bool FixUnderflow(int branchIndex, Page<KeyType, DataType, order> *cur, KeyType rightRootKey,
...
void BPlusTree<KeyType, DataType, order>::DeleteFromPageAtIndex(const int index, Page<KeyType, DataType, order> *cur) {
   int i;
   for (i = index; i < cur->size - 1; i++) cur->keys[i] = cur->keys[i + 1];
   if (cur->isLeaf) {
      for (i = index; i < cur->size - 1; i++) cur->data[i] = cur->data[i + 1];
   } else {
      for (i = index; i < cur->size - 1; i++) cur->children[i + 1] = cur->children[i + 2];
   }
   cur->size--;
}
template <class KeyType, class DataType, int order>
...
bool BPlusTree<KeyType, DataType, order>::Delete(const KeyType key, DataType &out) {
   ...
   if (root == leftmostLeaf) {
      if (GetIndexOf(key, root, &childIndex) == false) return false;
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::FindDeleteRebalance(const KeyType key, Page<KeyType, DataType, order> *cur,
                                                              bool *underflow, KeyType rightRootKey,
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::FixUnderflow(int branchIndex, Page<KeyType, DataType, order> *cur,
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
      ...
      //
      // To merge two branches, copy the pointers and keys over, using rightRootKey as the key for
      // the extra pointer
      if (branchIndex < cur->size) {
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
      ...
      if (order <= 3 && branchIndex > 0 &&
          cur->children[branchIndex]->isLeaf)  // With order==2 it is possible to delete data[0],
                                               // which is not possible with higher orders.
         cur->keys[branchIndex - 1] = cur->children[branchIndex]->keys[0];
      ...
      if (source == leftmostLeaf) leftmostLeaf = source->next;

      if (source->isLeaf) {
         if (source->previous) source->previous->next = source->next;
         if (source->next) source->next->previous = source->previous;
      }
...
void BPlusTree<KeyType, DataType, order>::ShiftNodeRight(Page<KeyType, DataType, order> *cur) {
   int i;
   for (i = cur->size; i > 0; i--) cur->keys[i] = cur->keys[i - 1];
   if (cur->isLeaf) {
      for (i = cur->size; i > 0; i--) cur->data[i] = cur->data[i - 1];
   } else {
      for (i = cur->size + 1; i > 0; i--) cur->children[i] = cur->children[i - 1];
   }
...
void BPlusTree<KeyType, DataType, order>::ShiftNodeLeft(Page<KeyType, DataType, order> *cur) {
   int i;
   for (i = 0; i < cur->size - 1; i++) cur->keys[i] = cur->keys[i + 1];
   if (cur->isLeaf) {
      for (i = 0; i < cur->size; i++) cur->data[i] = cur->data[i + 1];
   } else {
      for (i = 0; i < cur->size; i++) cur->children[i] = cur->children[i + 1];
   }
   cur->size--;
}
template <class KeyType, class DataType, int order>
Page<KeyType, DataType, order> *BPlusTree<KeyType, DataType, order>::InsertIntoNode(
    const KeyType key, const DataType &leafData, int insertionIndex, Page<KeyType, DataType, order> *nodeData,
    Page<KeyType, DataType, order> *cur, ReturnAction *returnAction) {
   int i;
   if (cur->size < order) {
      for (i = cur->size; i > insertionIndex; i--) cur->keys[i] = cur->keys[i - 1];
      if (cur->isLeaf) {
         for (i = cur->size; i > insertionIndex; i--) cur->data[i] = cur->data[i - 1];
      } else {
         for (i = cur->size + 1; i > insertionIndex + 1; i--) cur->children[i] = cur->children[i - 1];
      }
      ...
      if (cur->isLeaf) {
         ...
         if (cur->next) cur->next->previous = newPage;
      ...
      if (insertionIndex >= (order + 1) / 2) {
         ...
         if (cur->isLeaf) {
            ...
            // sourceIndex+1 is sort of a hack but it works - because there is one extra child than
            // keys skip past the last child for cur
         ...
         for (; sourceIndex < order; sourceIndex++, destIndex++) newPage->keys[destIndex] = cur->keys[sourceIndex];
         destIndex = 0;
         if (cur->isLeaf) {
            sourceIndex = (order + 1) / 2 - 1;
            for (; sourceIndex < order; sourceIndex++, destIndex++) newPage->data[destIndex] = cur->data[sourceIndex];
         } else {
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateLeft(Page<KeyType, DataType, order> *cur, int childIndex,
                                                     ReturnAction *returnAction) {
...
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::RotateRight(Page<KeyType, DataType, order> *cur, int childIndex,
                                                      ReturnAction *returnAction) {
   Page<KeyType, DataType, order> *dest = cur->children[childIndex + 1];
   Page<KeyType, DataType, order> *source = cur->children[childIndex];
   returnAction->key1 = dest->keys[0];
...
template <class KeyType, class DataType, int order>
Page<KeyType, DataType, order> *BPlusTree<KeyType, DataType, order>::InsertBranchDown(
    const KeyType key, const DataType &data, Page<KeyType, DataType, order> *cur, ReturnAction *returnAction,
    bool *success) {
   int childIndex;
   int branchIndex;
   if (GetIndexOf(key, cur, &childIndex))
...
bool BPlusTree<KeyType, DataType, order>::Insert(const KeyType key, const DataType &data) {
   if (root == 0) {
      ...
      if (newPage) {
         KeyType newKey;
         if (newPage->isLeaf == false) {
            // One key is pushed up through the stack.  I store that at keys[0] but it has to be
            // removed for the page to be correct
            RakAssert(returnAction.action == ReturnAction::PUSH_KEY_TO_PARENT);
...
void BPlusTree<KeyType, DataType, order>::ShiftKeysLeft(Page<KeyType, DataType, order> *cur) {
   int i;
   for (i = 0; i < cur->size; i++) cur->keys[i] = cur->keys[i + 1];
}
template <class KeyType, class DataType, int order>
...
template <class KeyType, class DataType, int order>
bool BPlusTree<KeyType, DataType, order>::GetIndexOf(const KeyType key, Page<KeyType, DataType, order> *page,
                                                     int *out) const {
...
void BPlusTree<KeyType, DataType, order>::FreePages(void) {
   ...
   while (queue.Size()) {
      ptr = queue.Pop();
      if (ptr->isLeaf == false) {
         for (i = 0; i < ptr->size + 1; i++) queue.Push(ptr->children[i], _FILE_AND_LINE_);
      }
...
void BPlusTree<KeyType, DataType, order>::ForEachData(void (*func)(DataType input, int index)) {
   ...
   DataStructures::Page<KeyType, DataType, order> *cur = GetListHead();
   while (cur) {
      for (i = 0; i < cur->size; i++) func(cur->data[i], count++);
      cur = cur->next;
   }
}
template <class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::PrintLeaf(Page<KeyType, DataType, order> *leaf, int index) {
   ...
   for (i = 0; i < leaf->size; i++) RAKNET_DEBUG_PRINTF(" %i. %i\n", i + 1, leaf->data[i]);
}
template <class KeyType, class DataType, int order>
...
void BPlusTree<KeyType, DataType, order>::ValidateTreeRecursive(Page<KeyType, DataType, order> *cur) {
   ...
   if (cur->children[0]->isLeaf) {
      ...
      for (int i = 0; i < cur->size + 1; i++) ValidateTreeRecursive(cur->children[i]);
   }
}
   ...
   while (queue.Size()) {
      ...
      if (ptr == 0)
         ...
         for (i = 0; i < ptr->size + 1; i++) {
            ...
            for (j = 0; j < ptr->children[i]->size; j++) RAKNET_DEBUG_PRINTF("%i ", ptr->children[i]->keys[j]);
            RAKNET_DEBUG_PRINTF(") ");
...
void main(void)
{
        ...
        for (testSize=0; testSize < 514; testSize++)
        {
                ...
                for (i=0; i<testSize; i++)
                {
                        btree.Delete(haveList[0]); // Asserts on 8th call.  Fails on going to remove
8 (7th call) removedList.Insert(haveList[0]); haveList.RemoveAtIndex(0); for (j=0; j <
removedList.Size(); j++)
                        {
