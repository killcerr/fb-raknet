...
void CloudClient::Post(CloudKey *cloudKey, const unsigned char *data, uint32_t dataLengthBytes,
   ...
   if (data == 0) dataLengthBytes = 0;
   bsOut.Write(dataLengthBytes);
   if (dataLengthBytes > 0) bsOut.WriteAlignedBytes((const unsigned char *)data, dataLengthBytes);
...
bool CloudClient::Get(CloudQuery *keyQuery, DataStructures::List<RakNetGUID> &specificSystems,
                      RakNetGUID systemIdentifier) {
   RakNet::BitStream bsOut;
   bsOut.Write((MessageID)ID_CLOUD_GET_REQUEST);
   keyQuery->Serialize(true, &bsOut);
...
void CloudClient::OnGetReponse(Packet *packet, CloudClientCallback *_callback, CloudAllocator *_allocator) {
   if (_callback == 0) _callback = callback;
   if (_allocator == 0) _allocator = allocator;
...
void CloudClient::OnSubscriptionNotification(Packet *packet, CloudClientCallback *_callback,
...
void CloudClient::OnSubscriptionNotification(bool *wasUpdated, CloudQueryRow *row, Packet *packet,
                                             CloudAllocator *_allocator) {
   if (_allocator == 0) _allocator = allocator;
...
void CloudClient::DeallocateWithDefaultAllocator(CloudQueryRow *row) { allocator->DeallocateRowData(row->data); }
#endif
