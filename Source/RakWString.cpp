...
RakWString::~RakWString() { rakFree_Ex(c_str, _FILE_AND_LINE_); }
...
RakWString &RakWString::operator=(const RakString &right) { return *this = right.C_String(); }

RakWString &RakWString::operator=(const wchar_t *const str) {
   Clear();
   if (str == 0) return *this;
   c_strCharLength = wcslen(str);
   if (c_strCharLength == 0) return *this;
...
bool RakWString::operator<(const RakWString &right) const { return wcscmp(C_String(), right.C_String()) < 0; }
...
bool RakWString::operator!=(const RakWString &right) const {
   if (GetLength() != right.GetLength()) return true;
   return wcscmp(C_String(), right.C_String()) != 0;
}

void RakWString::Set(wchar_t *str) { *this = str; }

bool RakWString::IsEmpty(void) const { return GetLength() == 0; }

size_t RakWString::GetLength(void) const { return c_strCharLength; }
...
int RakWString::StrCmp(const RakWString &right) const { return wcscmp(C_String(), right.C_String()); }
...
void RakWString::Printf(void) { printf("%ls", C_String()); }

void RakWString::FPrintf(FILE *fp) { fprintf(fp, "%ls", C_String()); }

void RakWString::Serialize(BitStream *bs) const { Serialize(C_String(), bs); }
