...
FileListReceiver::~FileListReceiver() {
   unsigned int i = 0;
   for (i = 0; i < pushedFiles.Size(); i++) rakFree_Ex(pushedFiles[i].flrMemoryBlock, _FILE_AND_LINE_);
}
...
void FileListTransfer::FileToPushRecipient::DeleteThis(void) {
   ...
   for (unsigned int j = 0; j < filesToPush.Size(); j++) RakNet::OP_DELETE(filesToPush[j], _FILE_AND_LINE_);
...
unsigned short FileListTransfer::SetupReceive(FileListTransferCBInterface *handler, bool deleteHandler,
                                              SystemAddress allowedSender) {
   ...
   if (fileListReceivers.Has(setId)) {
      ...
      if (receiver->deleteDownloadHandler) RakNet::OP_DELETE(receiver->downloadHandler, _FILE_AND_LINE_);
   ...
   oldId = setId;
   if (++setId == (unsigned short)-1) setId = 0;
   return oldId;
}
...
void FileListTransfer::Send(FileList *fileList, RakNet::RakPeerInterface *rakPeer, SystemAddress recipient,
                            ...
                            IncrementalReadInterface *_incrementalReadInterface, unsigned int _chunkSize) {
   ...
   // Write the chunk header, which contains the frequency table, the total number of files, and the
...
bool FileListTransfer::DecodeFile(Packet *packet, bool isTheFullFile) {
   ...
   if (isTheFullFile == false) {
      ...
      inBitStream.IgnoreBits(8);
      // The header is appended to every chunk, which we continue to read after this statement
      // flrMemoryBlock
   }
   ...
   if (isTheFullFile) {
      ...
      onFileStruct.bytesDownloadedForThisSet =
          fileListReceiver->setTotalDownloadedLength + onFileStruct.bytesDownloadedForThisFile;
   }
   ...
   if (isTheFullFile) {
      ...
      if (fileListReceiver->downloadHandler->OnFile(&onFileStruct)) rakFree_Ex(onFileStruct.fileData, _FILE_AND_LINE_);
      ...
      // Remote system is sending a complete file, but the file is large enough that we get
      // ID_PROGRESS_NOTIFICATION from the transport layer
      fileListReceiver->downloadHandler->OnFileProgress(&fps);
   }
...
void FileListTransfer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void FileListTransfer::CancelReceive(unsigned short setId) {
   ...
   fileListReceiver->downloadHandler->OnDereference();
   if (fileListReceiver->deleteDownloadHandler) RakNet::OP_DELETE(fileListReceiver->downloadHandler, _FILE_AND_LINE_);
   RakNet::OP_DELETE(fileListReceiver, _FILE_AND_LINE_);
   fileListReceivers.Delete(setId);
}
...
bool FileListTransfer::IsHandlerActive(unsigned short setId) { return fileListReceivers.Has(setId); }
...
void FileListTransfer::ClearCallbacks(void) { fileListProgressCallbacks.Clear(true, _FILE_AND_LINE_); }
...
void FileListTransfer::OnReferencePush(Packet *packet, bool isTheFullFile) {
   ...
   if (isTheFullFile == false) {
      ...
      // 2/16/2012 I care because a reference push is 16 megabytes by default. Also, if it is the
      // last file "if (ftpr->filesToPush.Size()<2)" or total file size exceeds smallFileTotalSize
      // it always sends a reference push.
      //		return;
   }
   ...
   if (isTheFullFile == false) fileListReceiver->partLength = partLength;
   ...
   if (isTheFullFile) {
      if (mb.flrMemoryBlock) {
         // Either the very first block, or a subsequent block and allocateIrIDataChunkAutomatically
         // was true for the first block
         memcpy(mb.flrMemoryBlock + offset, inBitStream.GetData() + BITS_TO_BYTES(inBitStream.GetReadOffset()),
                amountToRead);
         fps.iriDataChunk = mb.flrMemoryBlock + offset;
      } else {
         // In here mb.flrMemoryBlock is null
         // This means the first block explicitly deallocated the memory, and no blocks will be
         // permanently held by RakNet
   ...
   if (finished) {
      ...
      if (fps.partCount == 0) fps.firstDataChunk = fps.iriDataChunk;
      if (fps.partTotal == 1) fps.onFileStruct->fileData = fps.iriDataChunk;
      fileListReceiver->downloadHandler->OnFileProgress(&fps);

      // Incremental read interface sent us a file chunk
      // This is the last file chunk we were waiting for to consider the file done
      if (fileListReceiver->downloadHandler->OnFile(&onFileStruct)) rakFree_Ex(oldFileData, _FILE_AND_LINE_);
      ...
      if (isTheFullFile) {
         ...
         // This is a download progress notification for a file chunk using incremental read
         // interface We don't have all the data for this chunk yet
         ...
         unreadBytes =
             onFileStruct.byteLengthOfThisFile - ((currentNotificationIndex + 1) * fileListReceiver->partLength);
         fps.partCount = currentNotificationIndex;
         fps.partTotal = totalNotifications;
...
File sender:
Got ID_FILE_LIST_REFERENCE_PUSH_ACK. Calls OnReferencePushAck, calls SendIRIToAddress, calls
SendIRIToAddressCB
*/
...
int SendIRIToAddressCB(FileListTransfer::ThreadData threadData, bool *returnOutput, void *perThreadData) {
   ...
   *returnOutput = false;
   ...
   // Was previously using GetStatistics to get outgoing buffer size, but TCP with UnifiedSend
   // doesn't have this
   ...
   for (ftpIndex = 0; ftpIndex < fileListTransfer->fileToPushRecipientList.Size(); ftpIndex++) {
      ...
      if (ftpr->systemAddress == systemAddress && ftpr->setId == setId) {
         ...
         bytesRead = ftp->incrementalReadInterface->GetFilePart(ftp->fileListNode.fullPathToFile, ftp->currentOffset,
                                                                ftp->chunkSize, buff, ftp->fileListNode.context);
         ...
         while (done && ftp->currentOffset == 0 && smallFileTotalSize < ftp->chunkSize) {
            ////ftpr->filesToPushMutex.Lock();
            // The reason for 2 is that ID_FILE_LIST_REFERENCE_PUSH gets
            // ID_FILE_LIST_REFERENCE_PUSH_ACK. WIthout ID_FILE_LIST_REFERENCE_PUSH_ACK,
            // SendIRIToAddressCB would not be called again
            ...
            // Send all small files at once, rather than wait for ID_FILE_LIST_REFERENCE_PUSH. But
            // at least one ID_FILE_LIST_REFERENCE_PUSH must be sent
            outBitstream.Reset();
            outBitstream.Write((MessageID)ID_FILE_LIST_TRANSFER_FILE);
         ...
         for (unsigned int flpcIndex = 0; flpcIndex < fileListTransfer->fileListProgressCallbacks.Size(); flpcIndex++)
            fileListTransfer->fileListProgressCallbacks[flpcIndex]->OnFilePush(
                ftp->fileListNode.filename, ftp->fileListNode.fileLengthBytes, ftp->currentOffset - bytesRead,
                bytesRead, done, systemAddress, setId);

         dataBlocks[0] = (char *)outBitstream.GetData();
         lengths[0] = outBitstream.GetNumberOfBytesUsed();
         dataBlocks[1] = (char *)buff;
         lengths[1] = bytesRead;
         // rakPeerInterface->SendList(dataBlocks,lengths,2,ftp->packetPriority, RELIABLE_ORDERED,
         // ftp->orderingChannel, ftp->systemAddress, false);
         char orderingChannel = ftp->orderingChannel;
         PacketPriority packetPriority = ftp->packetPriority;

         // Mutex state: FileToPushRecipient (ftpr) has AddRef. fileToPushRecipientListMutex not
         // locked.
         if (done) {
            // Done
            ...
            if (ftpr->filesToPush.Size() == 0) {
               ...
               for (unsigned int flpcIndex = 0; flpcIndex < fileListTransfer->fileListProgressCallbacks.Size();
                    flpcIndex++)
         ...
         fileListTransfer->SendListUnified(dataBlocks, lengths, 2, packetPriority, RELIABLE_ORDERED, orderingChannel,
                                           systemAddress, false);
