...
// Sucks but this struct has to be outside the class.  Inside and DevCPP won't let you refer to the
// struct as RakPeer::RemoteSystemIndex while GCC forces you to do RakPeer::RemoteSystemIndex
struct RemoteSystemIndex {
   unsigned index;
   RemoteSystemIndex *next;
};
// int RAK_DLL_EXPORT SystemAddressAndIndexComp( const SystemAddress &key, const RemoteSystemIndex
// &data ); // GCC requires RakPeer::RemoteSystemIndex or it won't compile
...
class RAK_DLL_EXPORT RakPeer : public RakPeerInterface, public RNS2EventHandler {
  public:
   ...
   // --------------------------------------------------------------------------------------------Major
   // Low Level Functions - Functions needed by most
   // users--------------------------------------------------------------------------------------------
   ...
   /// \note Multiple calls while already active are ignored.  To call this function again with
   /// different settings, you must first call Shutdown(). \note Call SetMaximumIncomingConnections
   /// if you want to accept incoming connections. \param[in] maxConnections Maximum number of
   /// connections between this instance of RakPeer and another instance of RakPeer. Required so
   /// that the network can preallocate and for thread safety. A pure client would set this to 1.  A
   /// pure server would set it to the number of allowed clients.A hybrid would set it to the sum of
   /// both types of connections. \param[in] localPort The port to listen for connections on. On
   /// linux the system may be set up so thast ports under 1024 are restricted for everything but
   /// the root user. Use a higher port for maximum compatibility. \param[in] socketDescriptors An
   /// array of SocketDescriptor structures to force RakNet to listen on a particular IP address or
   /// port (or both).  Each SocketDescriptor will represent one unique socket.  Do not pass
   /// redundant structures.  To listen on a specific port, you can pass SocketDescriptor(myPort,0);
   /// such as for a server.  For a client, it is usually OK to just pass SocketDescriptor();
   /// However, on the XBOX be sure to use IPPROTO_VDP \param[in] socketDescriptorCount The size of
   /// the \a socketDescriptors array.  Pass 1 if you are not sure what to pass. \param[in]
   /// threadPriority Passed to the thread creation routine. Use THREAD_PRIORITY_NORMAL for Windows.
   /// For Linux based systems, you MUST pass something reasonable based on the thread priorities
   /// for your application. \return RAKNET_STARTED on success, otherwise appropriate failure
   /// enumeration.
   StartupResult Startup(unsigned int maxConnections, SocketDescriptor *socketDescriptors,
                         unsigned socketDescriptorCount, int threadPriority = -99999);

   /// If you accept connections, you must call this or else security will not be enabled for
   /// incoming connections. This feature requires more round trips, bandwidth, and CPU time for the
   /// connection handshake x64 builds require under 25% of the CPU time of other builds See the
   /// Encryption sample for example usage \pre Must be called while offline \pre LIBCAT_SECURITY
   /// must be defined to 1 in NativeFeatureIncludes.h for this function to have any effect
   ...
   /// \param[in] bRequireClientKey: Should be set to false for most servers.  Allows the server to
   /// accept a public key from connecting clients as a proof of identity but eats twice as much CPU
   /// time as a normal connection
   ...
   ///
   ///  Secure connections are determined by the recipient of an incoming connection. This has no
   ///  effect if called on the system attempting to connect.
   ...
   /// \param[in] ip IP address to remove. Pass 0 to remove all IP addresses. * wildcards are
   /// supported.
   ...
   /// \details If the number of incoming connections is less than the number of players currently
   /// connected, no more players will be allowed to connect.  If this is greater than the maximum
   /// number of peers allowed, it will be reduced to the maximum number of peers allowed.
   ///
   ...
   /// This is a way to set a low level password for all incoming connections.  To selectively
   /// reject connections, implement your own scheme using CloseConnection() to remove unwanted
   /// connections. \param[in] passwordData A data block that incoming connections must match.  This
   /// can be just a password, or can be a stream of data. Specify 0 for no password data \param[in]
   ...
   /// \param[out] passwordData  Should point to a block large enough to hold the password data you
   /// passed to SetIncomingPassword() \param[in,out] passwordDataLength Maximum size of the
   /// passwordData array.  Modified to hold the number of bytes actually written.
   ...
   /// \details Calling Connect and not calling SetMaximumIncomingConnections acts as a dedicated
   /// client. Calling both acts as a true peer.
   ///
   /// This is a non-blocking connection.
   ///
   /// The connection is successful when GetConnectionState() returns IS_CONNECTED or Receive() gets
   /// a message with the type identifier ID_CONNECTION_REQUEST_ACCEPTED. If the connection is not
   /// successful, such as a rejected connection or no response then neither of these things will
   /// happen. \pre Requires that you first call Startup(). \param[in] host Either a dotted IP
   /// address or a domain name. \param[in] remotePort Port to connect to on the remote machine.
   /// \param[in] passwordData A data block that must match the data block on the server passed to
   /// SetIncomingPassword().  This can be a string or can be a stream of data.  Use 0 for no
   /// password. \param[in] passwordDataLength The length in bytes of passwordData. \param[in]
   /// publicKey The public key the server is using. If 0, the server is not using security. If
   /// non-zero, the publicKeyMode member determines how to connect \param[in] connectionSocketIndex
   /// Index into the array of socket descriptors passed to socketDescriptors in RakPeer::Startup()
   /// to determine the one to send on. \param[in] sendConnectionAttemptCount Number of datagrams to
   /// send to the other system to try to connect. \param[in] timeBetweenSendConnectionAttemptsMS
   /// Time to elapse before a datagram is sent to the other system to try to connect. After
   /// sendConnectionAttemptCount number of attempts, ID_CONNECTION_ATTEMPT_FAILED is returned.
   /// Under low bandwidth conditions with multiple simultaneous outgoing connections, this value
   /// should be raised to 1000 or higher, or else the MTU detection can overrun the available
   /// bandwidth. \param[in] timeoutTime Time to elapse before dropping the connection if a reliable
   /// message could not be sent. 0 to use the default value from
   /// SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS); \return CONNECTION_ATTEMPT_STARTED on successful
   /// initiation. Otherwise, an appropriate enumeration indicating failure. \note
   /// CONNECTION_ATTEMPT_STARTED does not mean you are already connected! \note It is possible to
   /// immediately get back ID_CONNECTION_ATTEMPT_FAILED if you exceed the maxConnections parameter
   /// passed to Startup(). This could happen if you call CloseConnection() with
   /// sendDisconnectionNotificaiton true, then immediately call Connect() before the connection has
   /// closed.
   ConnectionAttemptResult Connect(const char *host, unsigned short remotePort, const char *passwordData,
                                   int passwordDataLength, PublicKey *publicKey = 0, unsigned connectionSocketIndex = 0,
                                   unsigned sendConnectionAttemptCount = 6,
                                   unsigned timeBetweenSendConnectionAttemptsMS = 1000, RakNet::TimeMS timeoutTime = 0);
   ...
   /// SetIncomingPassword().  This can be a string or can be a stream of data.  Use 0 for no
   /// password. \param[in] passwordDataLength The length in bytes of passwordData. \param[in]
   /// socket A bound socket returned by another instance of RakPeerInterface. \param[in]
   /// sendConnectionAttemptCount Number of datagrams to send to the other system to try to connect.
   /// \param[in] timeBetweenSendConnectionAttemptsMS Time to elapse before a datagram is sent to
   /// the other system to try to connect. After sendConnectionAttemptCount number of attempts,
   /// ID_CONNECTION_ATTEMPT_FAILED is returned.. Under low bandwidth conditions with multiple
   /// simultaneous outgoing connections, this value should be raised to 1000 or higher, or else the
   /// MTU detection can overrun the available bandwidth. \param[in] timeoutTime Time to elapse
   /// before dropping the connection if a reliable message could not be sent. 0 to use the default
   /// from SetTimeoutTime(UNASSIGNED_SYSTEM_ADDRESS); \return CONNECTION_ATTEMPT_STARTED on
   /// successful initiation. Otherwise, an appropriate enumeration indicating failure. \note
   /// CONNECTION_ATTEMPT_STARTED does not mean you are already connected!
   virtual ConnectionAttemptResult ConnectWithSocket(const char *host, unsigned short remotePort,
                                                     ...
                                                     RakNetSocket2 *socket, PublicKey *publicKey = 0,
                                                     unsigned sendConnectionAttemptCount = 6,
   ...
   /// \param[in] passwordData A data block that must match the data block on the server passed to
   SetIncomingPassword().  This can be a string or can be a stream of data.  Use 0 for no password.
   /// \param[in] passwordDataLength The length in bytes of passwordData.
   //bool Console2LobbyConnect( void *networkServiceId, const char *passwordData, int
   passwordDataLength );*/

   /// \brief Stops the network threads and closes all connections.
   /// \param[in] blockDuration Wait time(milli seconds) for all remaining messages to go out,
   /// including ID_DISCONNECTION_NOTIFICATION.  If 0, it doesn't wait at all. \param[in]
   /// orderingChannel Channel on which ID_DISCONNECTION_NOTIFICATION will be sent, if blockDuration
   /// > 0. \param[in] disconnectionNotificationPriority Priority of sending
   /// ID_DISCONNECTION_NOTIFICATION. If set to 0, the disconnection notification won't be sent.
   ...
   /// \brief Fills the array remoteSystems with the SystemAddress of all the systems we are
   /// connected to. \param[out] remoteSystems An array of SystemAddress structures, to be filled
   /// with the SystemAddresss of the systems we are connected to. Pass 0 to remoteSystems to get
   /// the number of systems we are connected to. \param[in, out] numberOfSystems As input, the size
   /// of remoteSystems array.  As output, the number of elements put into the array.
   ...
   /// \note If using RakPeer from multiple threads, this may not be accurate for your thread. Use
   /// IncrementNextSendReceipt() in that case. \return The next uint32_t that Send() or SendList
   /// will return
   ...
   /// \param[in] orderingChannel When using ordered or sequenced messages, the channel to order
   /// these on. Messages are only ordered relative to other messages on the same stream. \param[in]
   /// systemIdentifier Who to send this packet to, or in the case of broadcasting who not to send
   /// it to. Pass either a SystemAddress structure or a RakNetGUID structure. Use
   /// UNASSIGNED_SYSTEM_ADDRESS or to specify none \param[in] broadcast True to send this packet to
   /// all connected systems. If true, then systemAddress specifies who not to send the packet to.
   /// \param[in] forceReceipt If 0, will automatically determine the receipt number to return. If
   /// non-zero, will return what you give it. \return 0 on bad input. Otherwise a number that
   /// identifies this message. If \a reliability is a type that returns a receipt, on a later call
   /// to Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4
   /// inclusive containing this number
   uint32_t Send(const char *data, const int length, PacketPriority priority, PacketReliability reliability,
   ...
   /// \param[in] priority Priority level to send on.  See PacketPriority.h
   /// \param[in] reliability How reliably to send this data.  See PacketPriority.h
   /// \param[in] orderingChannel Channel to order the messages on, when using ordered or sequenced
   /// messages. Messages are only ordered relative to other messages on the same stream. \param[in]
   /// systemIdentifier System Address or RakNetGUID to send this packet to, or in the case of
   /// broadcasting, the address not to send it to.  Use UNASSIGNED_SYSTEM_ADDRESS to specify none.
   /// \param[in] broadcast True to send this packet to all connected systems. If true, then
   /// systemAddress specifies who not to send the packet to. \param[in] forceReceipt If 0, will
   /// automatically determine the receipt number to return. If non-zero, will return what you give
   /// it. \return 0 on bad input. Otherwise a number that identifies this message. If \a
   /// reliability is a type that returns a receipt, on a later call to Receive() you will get
   /// ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
   /// \note COMMON MISTAKE: When writing the first byte, bitStream->Write((unsigned char)
   /// ID_MY_TYPE) be sure it is casted to a byte, and you are not writing a 4 byte enumeration.
   uint32_t Send(const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
                 char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast,
                 uint32_t forceReceiptNumber = 0);
   ...
   /// reliability is a type that returns a receipt, on a later call to Receive() you will get
   /// ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
   uint32_t SendList(const char **data, const int *lengths, const int numParameters, PacketPriority priority,
                     PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                     bool broadcast, uint32_t forceReceiptNumber = 0);
   ...
   /// User-thread functions, such as RPC calls and the plugin function PluginInterface::Update
   /// occur here. \return 0 if no packets are waiting to be handled, otherwise a pointer to a
   /// packet. \note COMMON MISTAKE: Be sure to call this in a loop, once per game tick, until it
   /// returns 0. If you only process one packet per game tick they will buffer up. \sa
   ...
   // Functions--------------------------------------------------------------------------------------------
   /// \brief Close the connection to another host (if we initiated the connection it will
   /// disconnect, if they did it will kick them out). \details This method closes the connection
   /// irrespective of who initiated the connection. \param[in] target Which system to close the
   /// connection to. \param[in] sendDisconnectionNotification True to send
   /// ID_DISCONNECTION_NOTIFICATION to the recipient.  False to close it silently. \param[in]
   /// channel Which ordering channel to send the disconnection notification on, if any \param[in]
   /// disconnectionNotificationPriority Priority to send ID_DISCONNECTION_NOTIFICATION on.
   ...
   /// Returns if a system is connected, disconnected, connecting in progress, or various other
   /// states \param[in] systemIdentifier The system we are referring to \note This locks a mutex,
   /// do not call too frequently during connection attempts or the attempt will take longer and
   /// possibly even timeout \return What state the remote system is in
   ...
   /// \brief Same as calling GetSystemAddressFromIndex and GetGUIDFromIndex for all systems, but
   /// more efficient Indices match each other, so \a addresses[0] and \a guids[0] refer to the same
   /// system \param[out] addresses All system addresses. Size of the list is the number of
   /// connections. Size of the \a addresses list will match the size of the \a guids list.
   /// \param[out] guids All guids. Size of the list is the number of connections. Size of the list
   /// will match the size of the \a addresses list.
   ...
   /// \details Banned IPs persist between connections but are not saved on shutdown nor loaded on
   /// startup. \param[in] IP Dotted IP address. You can use * for a wildcard address, such as
   /// 128.0.0. * will ban all IP addresses starting with 128.0.0. \param[in] milliseconds Gives
   /// time in milli seconds for a temporary ban of the IP address.  Use 0 for a permanent ban.
   void AddToBanList(const char *IP, RakNet::TimeMS milliseconds = 0);

   /// \brief Allows a previously banned IP to connect.
   /// param[in] Dotted IP address. You can use * as a wildcard. An IP such as 128.0.0.* will ban
   ...
   /// \return True if IP matches any IPs in the ban list, accounting for any wildcards. False
   /// otherwise.
   bool IsBanned(const char *IP);

   /// \brief Enable or disable allowing frequent connections from the same IP adderss
   /// \details This is a security measure which is disabled by default, but can be set to true to
   /// prevent attackers from using up all connection slots. \param[in] b True to limit connections
   /// from the same ip to at most 1 per 100 milliseconds.
   void SetLimitIPConnectionFrequency(bool b);

   // --------------------------------------------------------------------------------------------Pinging
   // Functions - Functions dealing with the automatic ping
   // mechanism--------------------------------------------------------------------------------------------
   ...
   /// \details The remote system, if it is Initialized, will respond with ID_PONG followed by
   /// sizeof(RakNet::TimeMS) containing the system time the ping was sent. Default is 4 bytes - See
   /// __GET_TIME_64BIT in RakNetTypes.h System should reply with ID_PONG if it is active \param[in]
   /// host Either a dotted IP address or a domain name.  Can be 255.255.255.255 for LAN broadcast.
   /// \param[in] remotePort Which port to connect to on the remote machine.
   /// \param[in] onlyReplyOnAcceptingConnections Only request a reply if the remote system is
   /// accepting connections \param[in] connectionSocketIndex Index into the array of socket
   /// descriptors passed to socketDescriptors in RakPeer::Startup() to send on. \return true on
   /// success, false on failure (unknown hostname)
   bool Ping(const char *host, unsigned short remotePort, bool onlyReplyOnAcceptingConnections,
             unsigned connectionSocketIndex = 0);

   /// \brief Returns the average of all ping times read for the specific system or -1 if none read
   /// yet \param[in] systemAddress Which system we are referring to \return The ping time for this
   /// system, or -1
   ...
   /// By default this is true. Recommended to leave on, because congestion control uses it to
   /// determine how often to resend lost packets. It would be true by default to prevent timestamp
   /// drift, since in the event of a clock spike, the timestamp deltas would no longer be accurate
   ...
   /// Subtract GetClockDifferential() from a time returned by the remote system to get that time
   /// relative to your own system Returns 0 if the system is unknown \param[in] systemIdentifier
   /// Which system we are referring to
   RakNet::Time GetClockDifferential(const AddressOrGUID systemIdentifier);

   // --------------------------------------------------------------------------------------------Static
   // Data Functions - Functions dealing with API defined synchronized
   // memory--------------------------------------------------------------------------------------------
   ...
   // general--------------------------------------------------------------------------------------------
   /// \brief Returns the unique address identifier that represents you or another system on the the
   /// network \note Not supported by the XBOX \param[in] systemAddress Use
   /// UNASSIGNED_SYSTEM_ADDRESS to get your behind-LAN address. Use a connected system to get their
   /// behind-LAN address. This does not return the port. \param[in] index When you have multiple
   /// internal IDs, which index to return? Currently limited to MAXIMUM_NUMBER_OF_INTERNAL_IDS (so
   /// the maximum value of this variable is MAXIMUM_NUMBER_OF_INTERNAL_IDS-1) \return Identifier of
   /// your system internally, which may not be how other systems see if you if you are behind a NAT
   /// or proxy.
   ...
   /// override a value \param[in] systemAddress. The address to set. Use
   /// SystemAddress::FromString() if you want to use a dotted string \param[in] index When you have
   /// multiple internal IDs, which index to set?
   void SetInternalID(SystemAddress systemAddress, int index = 0);

   /// \brief Returns the unique address identifier that represents the target on the the network
   /// and is based on the target's external IP / port. \param[in] target The SystemAddress of the
   /// remote system. Usually the same for all systems, unless you have two or more network cards.
   ...
   /// \brief  Given a connected system address, this method gives the unique GUID representing that
   /// instance of RakPeer. This will be the same on all systems connected to that instance of
   /// RakPeer, even if the external system addresses are different. Complexity is O(log2(n)). If \a
   /// input is UNASSIGNED_SYSTEM_ADDRESS, will return your own GUID \pre Call Startup() first, or
   /// the function will return UNASSIGNED_RAKNET_GUID \param[in] input The system address of the
   /// target system we are connected to.
   ...
   /// The GUID will be the same on all systems connected to that instance of RakPeer, even if the
   /// external system addresses are different. Currently O(log(n)), but this may be improved in the
   /// future If \a input is UNASSIGNED_RAKNET_GUID, UNASSIGNED_SYSTEM_ADDRESS is returned.
   /// \param[in] input The RakNetGUID of the target system.
   SystemAddress GetSystemAddressFromGuid(const RakNetGUID input) const;

   /// Given the SystemAddress of a connected system, get the public key they provided as an
   /// identity Returns false if system address was not found or client public key is not known
   /// \param[in] input The RakNetGUID of the system
   /// \param[in] client_public_key The connected client's public key is copied to this address.
   /// Buffer must be cat::EasyHandshake::PUBLIC_KEY_BYTES bytes in length.
   bool GetClientPublicKeyFromSystemAddress(const SystemAddress input, char *client_public_key) const;

   /// \brief Set the time, in MS, to use before considering ourselves disconnected after not being
   /// able to deliver a reliable message.

   /// Set the time, in MS, to use before considering ourselves disconnected after not being able to
   /// deliver a reliable message. Default time is 10,000 or 10 seconds in release and 30,000 or 30
   /// seconds in debug. Do not set different values for different computers that are connected to
   /// each other, or you won't be able to reconnect after ID_CONNECTION_LOST
   /// \param[in] timeMS Time, in MS
   /// \param[in] target SystemAddress structure of the target system. Pass
   /// UNASSIGNED_SYSTEM_ADDRESS for all systems.
   void SetTimeoutTime(RakNet::TimeMS timeMS, const SystemAddress target);

   /// \brief Returns the Timeout time for the given system.
   /// \param[in] target Target system to get the TimeoutTime for. Pass UNASSIGNED_SYSTEM_ADDRESS to
   /// get the default value. \return Timeout time for a given system.
   ...
   /// \details Normally this should be false, but may be necessary when connecting to servers with
   /// multiple IP addresses. \param[in] allow - True to allow this behavior, false to not allow.
   ...
   /// This will send our external IP outside the LAN along with some user data to the remote
   /// system. \pre The sender and recipient must already be started via a successful call to
   /// Initialize \param[in] host Either a dotted IP address or a domain name \param[in] remotePort
   /// Which port to connect to on the remote machine. \param[in] data Optional data to append to
   /// the packet. \param[in] dataLength Length of data in bytes.  Use 0 if no data. \param[in]
   /// connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors
   /// in RakPeer::Startup() to send on. \return False if IsActive()==false or the host is
   /// unresolvable. True otherwise.
   bool AdvertiseSystem(const char *host, unsigned short remotePort, const char *data, int dataLength,
                        unsigned connectionSocketIndex = 0);

   /// \brief Controls how often to return ID_DOWNLOAD_PROGRESS for large message downloads.
   /// \details ID_DOWNLOAD_PROGRESS is returned to indicate a new partial message chunk, roughly
   /// the MTU size, has arrived. As it can be slow or cumbersome to get this notification for every
   /// chunk, you can set the interval at which it is returned. Defaults to 0 (never return this
   /// notification). \param[in] interval How many messages to use as an interval before a download
   /// progress notification is returned.
   void SetSplitMessageProgressInterval(int interval);

   /// \brief Returns what was passed to SetSplitMessageProgressInterval().
   /// \return Number of messages to be recieved before a download progress notification is
   /// returned. Default to 0.
   ...
   // Plugin
   // Functions--------------------------------------------------------------------------------------------
   /// \brief Attaches a Plugin interface to an instance of the base class (RakPeer or
   /// PacketizedTCP) to run code automatically on message receipt in the Receive call. If the
   /// plugin returns false from PluginInterface::UsesReliabilityLayer(), which is the case for all
   /// plugins except PacketLogger, you can call AttachPlugin() and DetachPlugin() for this plugin
   /// while RakPeer is active. \param[in] messageHandler Pointer to the plugin to attach.
   ...
   // --------------------------------------------------------------------------------------------Miscellaneous
   // Functions--------------------------------------------------------------------------------------------
   /// \brief Puts a message back in the receive queue in case you don't want to deal with it
   /// immediately. \param[in] packet The pointer to the packet you want to push back. \param[in]
   /// pushAtHead True to push the packet at the start of the queue so that the next receive call
   /// returns it.  False to push it at the end of the queue. \note Setting pushAtHead to false end
   /// makes the packets out of order.
   ...
   /// The smart pointer reference counts the RakNetSocket object, so the socket will remain active
   /// as long as the smart pointer does, even if RakNet were to shutdown or close the connection.
   /// \note This sends a query to the thread and blocks on the return value for up to one second.
   /// In practice it should only take a millisecond or so. \param[in] target Which system. \return
   /// A smart pointer object containing the socket information about the target. Be sure to check
   /// IsNull() which is returned if the update thread is unresponsive, shutting down, or if this
   /// system is not connected.
   virtual RakNetSocket2 *GetSocket(const SystemAddress target);

   /// \brief Gets all sockets in use.
   /// \note This sends a query to the thread and blocks on the return value for up to one second.
   /// In practice it should only take a millisecond or so. \param[out] sockets List of RakNetSocket
   /// structures in use.
   ...
   /// If you need code to run in the same thread as RakNet's update thread, this function can be
   /// used for that \param[in] _userUpdateThreadPtr C callback function \param[in]
   /// _userUpdateThreadData Passed to C callback function
   virtual void SetUserUpdateThread(void (*_userUpdateThreadPtr)(RakPeerInterface *, void *),
                                    void *_userUpdateThreadData);

   /// Set a C callback to be called whenever a datagram arrives
   /// Return true from the callback to have RakPeer handle the datagram. Return false and RakPeer
   /// will ignore the datagram. This can be used to filter incoming datagrams by system, or to
   /// share a recvfrom socket with RakPeer RNS2RecvStruct will only remain valid for the duration
   /// of the call
   ...
   /// To simulate bi-directional ping and packet loss, you should call this on both the sender and
   /// the recipient, with half the total ping and packetloss value on each. You can exclude network
   /// simulator code with the _RELEASE #define to decrease code size \deprecated Use
   /// http://www.jenkinssoftware.com/forum/index.php?topic=1671.0 instead. \note Doesn't work past
   /// version 3.6201 \param[in] packetloss Chance to lose a packet. Ranges from 0 to 1. \param[in]
   /// minExtraPing The minimum time to delay sends. \param[in] extraPingVariance The additional
   /// random time to delay sends.
   ...
   /// \param[in] maxBitsPerSecond Maximum bits per second to send.  Use 0 for unlimited (default).
   /// Once set, it takes effect immedately and persists until called again.
   ...
   // --------------------------------------------------------------------------------------------Statistical
   // Functions - Functions dealing with API
   // performance--------------------------------------------------------------------------------------------

   /// \brief Returns a structure containing a large set of network statistics for the specified
   /// system. You can map this data to a string using the C style StatisticsToString() function
   /// \param[in] systemAddress Which connected system to get statistics for.
   /// \param[in] rns If you supply this structure,the network statistics will be written to it.
   /// Otherwise the method uses a static struct to write the data, which is not threadsafe. \return
   /// 0 if the specified system can't be found. Otherwise a pointer to the struct containing the
   /// specified system's network statistics. \sa RakNetStatistics.h
   ...
   // --------------------------------------------------------------------------------------------EVERYTHING
   // AFTER THIS COMMENT IS FOR INTERNAL USE
   // ONLY--------------------------------------------------------------------------------------------
   ...
   // remotePortRakNetWasStartedOn_PS3 and extraSocketOptions are from SocketDescriptor when the
   // socket was created bool RunRecvFromOnce( RakNetSocket *s );

   /// \internal
   bool SendOutOfBand(const char *host, unsigned short remotePort, const char *data, BitSize_t dataLength,
                      unsigned connectionSocketIndex = 0);
   ...
   struct RemoteSystemStruct {
      ...
      SystemAddress theirInternalSystemAddress[MAXIMUM_NUMBER_OF_INTERNAL_IDS];  /// Their internal IP,
                                                                                 /// behind the LAN
      ReliabilityLayer reliabilityLayer;  /// The reliability layer associated with this player
      bool weInitiatedTheConnection;      /// True if we started this connection via Connect.  False if
                                          /// someone else connected to us.
      PingAndClockDifferential pingAndClockDifferential[PING_TIMES_ARRAY_SIZE];  /// last x ping times and calculated
                                                                                 /// clock differentials with it
      RakNet::Time pingAndClockDifferentialWriteIndex;                           /// The index we are writing into the
                                                        /// pingAndClockDifferential circular buffer
      unsigned short lowestPing;      /// The lowest ping value encountered
      RakNet::Time nextPingTime;      /// When to next ping this player
      RakNet::Time lastReliableSend;  /// When did the last reliable send occur.  Reliable sends must occur
                                      /// at least once every timeoutTime/2 units to notice disconnects
...
#if LIBCAT_SECURITY == 1
      // Cached answer used internally by RakPeer to prevent DoS attacks based on the connexion
      // handshake
      char answer[cat::EasyHandshake::ANSWER_BYTES];

      // If the server has bRequireClientKey = true, then this is set to the validated public key of
      // the connected client Valid after connectMode reaches HANDLING_CONNECTION_REQUEST
   ...
   // DS_APR
   // void ProcessChromePacket(RakNetSocket2 *s, const char *buffer, int dataSize, const
  ...
  protected:
   ...
   friend bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length,
                                           RakPeer *rakPeer, RakNetSocket2 *rakNetSocket, bool *isOfflineMessage,
                                           RakNet::TimeUS timeRead);
   friend void ProcessNetworkPacket(const SystemAddress systemAddress, const char *data, const int length,
                                    RakPeer *rakPeer, RakNet::TimeUS timeRead, BitStream &updateBitStream);
   friend void ProcessNetworkPacket(const SystemAddress systemAddress, const char *data, const int length,
                                    RakPeer *rakPeer, RakNetSocket2 *rakNetSocket, RakNet::TimeUS timeRead,
   ...
   //  Two versions needed because some buggy compilers strip the last parameter if unused, and
   //  crashes
   ConnectionAttemptResult SendConnectionRequest(const char *host, unsigned short remotePort, const char *passwordData,
                                                 int passwordDataLength, PublicKey *publicKey,
   ...
   ///  \return 0 if none
   RemoteSystemStruct *GetRemoteSystemFromSystemAddress(const SystemAddress systemAddress, bool calledFromNetworkThread,
                                                        bool onlyActive) const;
   RakPeer::RemoteSystemStruct *GetRemoteSystem(const AddressOrGUID systemIdentifier, bool calledFromNetworkThread,
                                                bool onlyActive) const;
   ...
   void ParseConnectionRequestPacket(RakPeer::RemoteSystemStruct *remoteSystem, const SystemAddress &systemAddress,
                                     const char *data, int byteSize);
   ...
   RemoteSystemStruct *AssignSystemAddressToRemoteSystemList(const SystemAddress systemAddress,
                                                             RemoteSystemStruct::ConnectMode connectionMode,
                                                             RakNetSocket2 *incomingRakNetSocket,
                                                             bool *thisIPConnectedRecently,
                                                             SystemAddress bindingAddress, int incomingMTU,
   ...
   /// Store the maximum number of peers able to connect, including reserved connection slots for
   /// pings, etc.
   ...
   /// This allows us to preallocate the list when starting, so we don't have to allocate or delete
   /// at runtime. Another benefit is that is lets us add and remove active players simply by
   /// setting systemAddress and moving elements in the list by copying pointers variables without
   /// affecting running threads, even if they are in the reliability layer
   RemoteSystemStruct *remoteSystemList;
   /// activeSystemList holds a list of pointers and is preallocated to be the same size as
   /// remoteSystemList. It is updated only by the network thread, but read by both threads When the
   /// isActive member of RemoteSystemStruct is set to true or false, that system is added to this
   /// list of pointers Threadsafe because RemoteSystemStruct is preallocated, and the list is only
   ...
   /// RunUpdateCycle is not thread safe but we don't need to mutex calls. Just skip calls if it is
   /// running already
   ...
   // bool continualPing,isRecvfromThreadActive,isMainLoopThreadActive, endThreads,
   // isSocketLayerBlocking;
   ...
   // DataStructures::Queue<Packet *> incomingpacketSingleProducerConsumer; //,
   // synchronizedMemorypacketSingleProducerConsumer;
...
//	char random[16];
   ...
   // DataStructures::List<DataStructures::List<MemoryBlock>* >
   // automaticVariableSynchronizationList;
   ...
   // This stores the user send calls to be handled by the update thread.  This way we don't have
   // thread contention over systemAddresss
   ...
   void SendBuffered(const char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
                     PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                     bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt);
   void SendBufferedList(const char **data, const int *lengths, const int numParameters, PacketPriority priority,
                         PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                         bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt);
   bool SendImmediate(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability,
                      char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast,
   ...
   /// How long it has been since things were updated by a call to receiveUpdate thread uses this to
   /// determine how long to sleep for
   // unsigned int lastUserUpdateCycle;
   /// True to allow connection accepted packets from anyone.  False to only allow these packets
   /// from servers we requested a connection to.
   ...
   // Systems in this list will not go through the secure connection process, even when secure
   // connections are turned on. Wildcards are accepted.
   ...
   /// This number will be returned back with ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS and is
