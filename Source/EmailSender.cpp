...
const char *EmailSender::Send(const char *hostAddress, unsigned short hostPort, const char *sender,
   ...
   if (tcpInterface.Start(0, 0) == false) return "Unknown error starting TCP";
   emailServer = tcpInterface.Connect(hostAddress, hostPort, true);
   if (emailServer == UNASSIGNED_SYSTEM_ADDRESS) return "Failed to connect to host";
   ...
   if (packet == 0) return "Timeout while waiting for initial data from server.";
...
#pragma warning(disable : 4127)  // conditional expression is constant
   ...
   while (1) {
      ...
      if (response != 0 && strcmp(response, "CONTINUE") != 0) return response;
   ...
   if (authenticate) {
      ...
      if (password == 0) return "Password needed";
      char *outputData =
          RakNet::OP_NEW_ARRAY<char>((const int)(strlen(sender) + strlen(password) + 2) * 3, _FILE_AND_LINE_);
      RakNet::BitStream bs;
      char zero = 0;
      bs.Write(&zero, 1);
   ...
   if (attachedFiles && attachedFiles->fileList.Size()) {
      ...
      for (i = 0; i < boundarySize; i++) boundary[i] = Base64Map()[rakNetRandom.RandomMT() % 64];
   ...
   // Write the body of the email, doing some lame shitty shit where I have to make periods at the
   // start of a newline have a second period.
   char *newBody;
   int bodyLength;
   bodyLength = (int)strlen(body);
   newBody = (char *)rakMalloc_Ex(bodyLength * 3, _FILE_AND_LINE_);
   if (bodyLength >= 0) newBody[0] = body[0];
   for (i = 1, j = 1; i < bodyLength; i++) {
      ...
      // Transform \n . \n into \n . . \r \n (this is a bug in the mail server - the spec says do
      // not count \n alone but it does)
      ...
      // Transform \n . . \n into \n . . . \r \n (this is a bug in the mail server - the spec says
      // do not count \n alone but it does) In fact having to process .. is a bug too - because the
   ...
   // What a pain in the rear.  I have to map the binary to printable characters using 6 bits per
   // character.
   if (attachedFiles && attachedFiles->fileList.Size()) {
      for (i = 0; i < (int)attachedFiles->fileList.Size(); i++) {
         ...
         sprintf(query,
                 ...
                 attachedFiles->fileList[i].dataLengthBytes, attachedFiles->fileList[i].filename.C_String(),
         ...
         outputOffset = Base64Encoding((const unsigned char *)attachedFiles->fileList[i].data,
...
#endif
   while (1) {
      if (tcpInterface->HasLostConnection() == emailServer) return "Connection to server lost.";
      packet = tcpInterface->Receive();
      if (packet) {
         if (doPrintf) {
            RAKNET_DEBUG_PRINTF("%s", packet->data);
...
// 			}
#endif
         if (strstr((const char *)packet->data, "235")) return 0;  // Authentication accepted
         if (strstr((const char *)packet->data, "354")) return 0;  // Go ahead
#if OPEN_SSL_CLIENT_SUPPORT == 1
...
#endif
         if (strstr((const char *)packet->data, "250")) return 0;  // OK
         if (strstr((const char *)packet->data, "550")) return "Failed on error code 550";
         if (strstr((const char *)packet->data, "553")) return "Failed on error code 553";
      }
      if (RakNet::GetTimeMS() > timeout) return "Timed out";
      RakSleep(100);
   }
}
