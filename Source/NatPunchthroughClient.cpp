...
void NatPunchthroughClient::FindRouterPortStride(const SystemAddress &facilitator) {
   ...
   if (cs != IS_CONNECTED) return;
   if (hasPortStride != UNKNOWN_PORT_STRIDE) return;
   ...
   if (natPunchthroughDebugInterface) {
      natPunchthroughDebugInterface->OnClientMessage(
          RakString("Calculating port stride from %s", facilitator.ToString(true)));
   }
...
/*
bool NatPunchthroughClient::OpenNATGroup(DataStructures::List<RakNetGUID> destinationSystems, const
...
*/
void NatPunchthroughClient::SetDebugInterface(NatPunchthroughDebugInterface *i) { natPunchthroughDebugInterface = i; }
...
void NatPunchthroughClient::Update(void) {
   ...
   if (sp.nextActionTime && sp.nextActionTime < time) {
      ...
      {
              // Send to unused port. We do not want the message to arrive, just to open our
      router's table SystemAddress sa=sp.targetAddress; int ttlSendIndex; for (ttlSendIndex=0;
      ttlSendIndex <= pc.MAX_PREDICTIVE_PORT_RANGE; ttlSendIndex++)
              {
                      sa.SetPortHostOrder((unsigned short)
      (sp.targetAddress.GetPort()+ttlSendIndex)); SendTTL(sa);
              }

              // Only do this stage once
              // Wait 250 milliseconds for next stage. The delay is so that even with timing errors
      both systems send out the
      ...
      */
         ...
         if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
            if (natPunchthroughDebugInterface) {
               ...
               natPunchthroughDebugInterface->OnClientMessage(
                   RakNet::RakString("Likely bidirectional punchthrough failure to guid %s, system address %s.",
                                     guidString, ipAddressString));
            }
      ...
      if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
         ...
         if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
            if (natPunchthroughDebugInterface) {
               ...
               sp.targetGuid.ToString(guidString);
               natPunchthroughDebugInterface->OnClientMessage(
                   RakNet::RakString("Likely unidirectional punchthrough failure to guid %s, system address %s.",
                                     guidString, ipAddressString));
            }
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
   if (pc.retryOnFailure == false) {
      if (natPunchthroughDebugInterface) {
         ...
         sp.targetGuid.ToString(guidString);
         natPunchthroughDebugInterface->OnClientMessage(
             RakNet::RakString("Failed punchthrough once. Returning failure to guid %s, system address %s to user.",
                               guidString, ipAddressString));
      }
   ...
   for (i = 0; i < failedAttemptList.Size(); i++) {
      if (failedAttemptList[i].guid == sp.targetGuid) {
         if (natPunchthroughDebugInterface) {
            ...
            sp.targetGuid.ToString(guidString);
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("Failed punchthrough twice. Returning failure to guid %s, system "
                                  "address %s to user.",
                                  guidString, ipAddressString));
         }
   ...
   if (rakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
      if (natPunchthroughDebugInterface) {
         ...
         sp.targetGuid.ToString(guidString);
         natPunchthroughDebugInterface->OnClientMessage(
             RakNet::RakString("Not connected to facilitator, so cannot retry punchthrough after first failure. "
                               "Returning failure onj guid %s, system address %s to user.",
                               guidString, ipAddressString));
      }
   ...
   if (natPunchthroughDebugInterface) {
      ...
      char guidString[128];
      sp.targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
          "First punchthrough failure on guid %s, system address %s. Reattempting.", guidString, ipAddressString));
   }
   ...
   // If we are the sender, try again, immediately if possible, else added to the queue on the
   // faciltiator
   if (sp.weAreSender) SendPunchthrough(sp.targetGuid, sp.facilitator);
}
   ...
   switch (packet->data[0]) {
      ...
      case ID_NAT_RESPOND_BOUND_ADDRESSES: {
         ...
         if (boundAddressCount < 2) {
            if (natPunchthroughDebugInterface)
               natPunchthroughDebugInterface->OnClientMessage(
                   RakString("INCAPABLE_PORT_STRIDE. My external ID is %s",
                             rakPeerInterface->GetExternalID(packet->systemAddress).ToString()));
         ...
         for (int i = 0; i < boundAddressCount && i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
            ...
            if (boundAddresses[i] != packet->systemAddress) {
               ...
               rakPeerInterface->SendOutOfBand((const char *)boundAddresses[i].ToString(false),
                                               boundAddresses[i].GetPort(), (const char *)outgoingBs.GetData(),
      ...
      case ID_OUT_OF_BAND_INTERNAL:
         if (packet->length >= 2 && packet->data[1] == ID_NAT_PONG) {
            ...
            if (natPunchthroughDebugInterface)
               natPunchthroughDebugInterface->OnClientMessage(
                   RakString("HAS_PORT_STRIDE %i. First external port %i. Second external port %i.", portStride,
                             externalPort, externalPort2));

            SendQueuedOpenNAT();
            return RR_STOP_PROCESSING_AND_DEALLOCATE;
         } else if (packet->length >= 2 &&
                    (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL ||
                     packet->data[1] == ID_NAT_ESTABLISH_BIDIRECTIONAL) &&
                    sp.nextActionTime != 0) {
            RakNet::BitStream bs(packet->data, packet->length, false);
            bs.IgnoreBytes(2);
            uint16_t sessionId;
            ...
            if (sessionId != sp.sessionId) break;

            char ipAddressString[32];
            packet->systemAddress.ToString(true, ipAddressString);
            // sp.targetGuid==packet->guid is because the internal IP addresses reported may include
            // loopbacks not reported by RakPeer::IsLocalIP()
            if (packet->data[1] == ID_NAT_ESTABLISH_UNIDIRECTIONAL && sp.targetGuid == packet->guid) {
               if (sp.testMode != SendPing::PUNCHING_FIXED_PORT) {
                  ...
                  if (natPunchthroughDebugInterface) {
                     ...
                     sp.targetGuid.ToString(guidString);
                     natPunchthroughDebugInterface->OnClientMessage(
                         RakNet::RakString("PUNCHING_FIXED_PORT: Received ID_NAT_ESTABLISH_UNIDIRECTIONAL "
                                           "from guid %s, system address %s.",
                                           guidString, ipAddressString));
                  }
               } else {
                  if (natPunchthroughDebugInterface) {
                     ...
                     sp.targetGuid.ToString(guidString);
                     natPunchthroughDebugInterface->OnClientMessage(
                         RakNet::RakString("Received ID_NAT_ESTABLISH_UNIDIRECTIONAL from guid %s, "
                                           "system address %s.",
                                           guidString, ipAddressString));
                  }
               }
               ...
               SendOutOfBand(sp.targetAddress, ID_NAT_ESTABLISH_BIDIRECTIONAL);
            } else if (packet->data[1] == ID_NAT_ESTABLISH_BIDIRECTIONAL && sp.targetGuid == packet->guid) {
               ...
               if (mostRecentExternalPort == 0) {
                  ...
                  if (natPunchthroughDebugInterface) {
                     natPunchthroughDebugInterface->OnClientMessage(
                         RakNet::RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL mostRecentExternalPort "
                                           "first time set to %i",
                  ...
                  if (sp.testMode != SendPing::TESTING_INTERNAL_IPS &&
                     ...
                     if (hasPortStride != HAS_PORT_STRIDE) {
                        ...
                        if (natPunchthroughDebugInterface) {
                           natPunchthroughDebugInterface->OnClientMessage(
                               RakString("ID_NAT_ESTABLISH_BIDIRECTIONAL: Estimated port stride from "
                                         "incoming "
                                         "connection at %i. ourExternalPort=%i mostRecentExternalPort=%i",
                                         portStride, ourExternalPort, mostRecentExternalPort));
                        }
                     ...
                     if (natPunchthroughDebugInterface) {
                        natPunchthroughDebugInterface->OnClientMessage(RakString(
                            "ID_NAT_ESTABLISH_BIDIRECTIONAL: New mostRecentExternalPort %i", mostRecentExternalPort));
                     }
                  }
               }
               ...
               if (natPunchthroughDebugInterface) {
                  ...
                  if (removedFromFailureQueue)
                     natPunchthroughDebugInterface->OnClientMessage(
                         RakNet::RakString("Punchthrough to guid %s, system address %s succeeded on 2nd attempt.",
                                           guidString, ipAddressString));
                  else
                     natPunchthroughDebugInterface->OnClientMessage(
                         RakNet::RakString("Punchthrough to guid %s, system address %s succeeded on 1st attempt.",
                                           guidString, ipAddressString));
               }
            }
      ...
      case ID_NAT_ALREADY_IN_PROGRESS: {
         ...
         // UpdateGroupPunchOnNatResult(packet->systemAddress, targetGuid,
         // UNASSIGNED_SYSTEM_ADDRESS, 2);
         if (natPunchthroughDebugInterface) {
            char guidString[128];
            targetGuid.ToString(guidString);
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("Punchthrough retry to guid %s failed due to "
                                  "ID_NAT_ALREADY_IN_PROGRESS. Returning failure.",
                                  guidString));
         }
      ...
      case ID_NAT_TARGET_UNRESPONSIVE: {
         ...
         if (packet->data[0] == ID_NAT_CONNECTION_TO_TARGET_LOST || packet->data[0] == ID_NAT_TARGET_UNRESPONSIVE) {
            uint16_t sessionId;
         ...
         for (i = 0; i < failedAttemptList.Size(); i++) {
            if (failedAttemptList[i].guid == targetGuid) {
               if (natPunchthroughDebugInterface) {
                  ...
                  targetGuid.ToString(guidString);
                  natPunchthroughDebugInterface->OnClientMessage(
                      RakNet::RakString("Punchthrough retry to guid %s failed due to %s.", guidString, reason));
               }

               // If the retry target is not connected, or loses connection, or is not responsive,
               // then previous failures cannot be retried.
         ...
         if (natPunchthroughDebugInterface) {
            ...
            targetGuid.ToString(guidString);
            natPunchthroughDebugInterface->OnClientMessage(
                RakNet::RakString("Punchthrough attempt to guid %s failed due to %s.", guidString, reason));
         }
...
void NatPunchthroughClient::OnConnectAtTime(Packet *packet) {
   ...
   //	int k;
   //	k=0;
   for (j = 0; j < MAXIMUM_NUMBER_OF_INTERNAL_IDS; j++) bs.Read(sp.internalIds[j]);
   ...
   */
   ...
   if (pc.MAXIMUM_NUMBER_OF_INTERNAL_IDS_TO_CHECK > 0) {
      ...
      // TESTING: Try sending to unused ports on the remote system to reserve our own ports while
      // not getting banned sp.testMode=SendPing::SEND_WITH_TTL;
      sp.testMode = SendPing::TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_FACILITATOR_PORT;
      sp.attemptCount = 0;
      sp.sentTTL = false;
   }
...
void NatPunchthroughClient::SendTTL(const SystemAddress &sa) {
   if (sa == UNASSIGNED_SYSTEM_ADDRESS) return;
   if (sa.GetPort() == 0) return;
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
   ...
   if (oobId == ID_NAT_ESTABLISH_BIDIRECTIONAL) oob.Write(sa.GetPort());
   char ipAddressString[32];
   sa.ToString(false, ipAddressString);
   rakPeerInterface->SendOutOfBand((const char *)ipAddressString, sa.GetPort(), (const char *)oob.GetData(),
                                   oob.GetNumberOfBytesUsed());
   ...
   if (natPunchthroughDebugInterface) {
      ...
      if (oobId == ID_NAT_ESTABLISH_UNIDIRECTIONAL)
         natPunchthroughDebugInterface->OnClientMessage(
             RakNet::RakString("%lld: %s: OOB ID_NAT_ESTABLISH_UNIDIRECTIONAL to guid %s, system address %s.\n",
                               serverTime, TestModeToString(sp.testMode), guidString, ipAddressString));
      else
         natPunchthroughDebugInterface->OnClientMessage(
             RakNet::RakString("%lld: %s: OOB ID_NAT_ESTABLISH_BIDIRECTIONAL to guid %s, system address %s.\n",
                               serverTime, TestModeToString(sp.testMode), guidString, ipAddressString));
   }
}

void NatPunchthroughClient::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
                                            bool isIncoming) {
   (void)rakNetGUID;
   (void)isIncoming;
   ...
   if (ourExternalId != UNASSIGNED_SYSTEM_ADDRESS && mostRecentExternalPort == 0) {
      ...
      if (natPunchthroughDebugInterface) {
         natPunchthroughDebugInterface->OnClientMessage(
             RakNet::RakString("OnNewConnection mostRecentExternalPort first time set to %i", mostRecentExternalPort));
      }
   }
...
void NatPunchthroughClient::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
   ...
   if (sp.facilitator == systemAddress) {
      // If we lose the connection to the facilitator, all previous failures not currently in
      // progress are returned as such
      unsigned int i = 0;
...
void NatPunchthroughClient::GetUPNPPortMappings(char *externalPort, char *internalPort,
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet *packet) {
   ...
   if (mostRecentExternalPort == 0) {
      ...
      if (natPunchthroughDebugInterface) {
         natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
             "OnGetMostRecentPort mostRecentExternalPort first time set to %i", mostRecentExternalPort));
      }
   }
...
/*
unsigned int NatPunchthroughClient::GetPendingOpenNATIndex(RakNetGUID destination, const
SystemAddress &facilitator)
{
        unsigned int i;
        for (i=0; i < pendingOpenNAT.Size(); i++)
        {
                if (pendingOpenNAT[i].destination==destination &&
pendingOpenNAT[i].facilitator==facilitator) return i;
        }
...
void NatPunchthroughClient::SendPunchthrough(RakNetGUID destination, const SystemAddress &facilitator) {
   ...
   if (natPunchthroughDebugInterface) {
      ...
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Starting ID_NAT_PUNCHTHROUGH_REQUEST to guid %s.", guidString));
   }
}

void NatPunchthroughClient::OnAttach(void) { Clear(); }

void NatPunchthroughClient::OnDetach(void) { Clear(); }

void NatPunchthroughClient::OnRakPeerShutdown(void) { Clear(); }
...
PunchthroughConfiguration *NatPunchthroughClient::GetPunchthroughConfiguration(void) { return &pc; }

void NatPunchthroughClient::OnReadyForNextPunchthrough(void) {
   if (rakPeerInterface == 0) return;
...
void NatPunchthroughClient::UpdateGroupPunchOnNatResult(SystemAddress facilitator, RakNetGUID
targetSystem, SystemAddress targetSystemAddress, int result)
{
...
_FILE_AND_LINE_); gpr->passedListAddress.Push(targetSystemAddress, _FILE_AND_LINE_);
                                        }
                                        else
                                        {
