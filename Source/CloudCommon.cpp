...
int RakNet::CloudKeyComp(const CloudKey &key, const CloudKey &data) {
   if (key.primaryKey < data.primaryKey) return -1;
   if (key.primaryKey > data.primaryKey) return 1;
   if (key.secondaryKey < data.secondaryKey) return -1;
   if (key.secondaryKey > data.secondaryKey) return 1;
   return 0;
}

CloudQueryRow *CloudAllocator::AllocateCloudQueryRow(void) { return RakNet::OP_NEW<CloudQueryRow>(_FILE_AND_LINE_); }

void CloudAllocator::DeallocateCloudQueryRow(CloudQueryRow *row) { RakNet::OP_DELETE(row, _FILE_AND_LINE_); }
...
void CloudAllocator::DeallocateRowData(void *data) { rakFree_Ex(data, _FILE_AND_LINE_); }
...
void CloudQuery::Serialize(bool writeToBitstream, BitStream *bitStream) {
   ...
   if (startingRowIndexIsZero == false) bitStream->Serialize(writeToBitstream, startingRowIndex);
   if (maxRowsToReturnIsZero == false) bitStream->Serialize(writeToBitstream, maxRowsToReturn);
...
void CloudQueryResult::SerializeCloudQueryRows(bool writeToBitstream, uint32_t &numRows, BitStream *bitStream,
