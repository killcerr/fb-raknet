...
class RAK_DLL_EXPORT Heap {
  public:
   ...
   /// If you are going to push a list of items, where the weights of the items on the list are in
   /// order and follow the heap order, PushSeries is faster than Push()
...
void Heap<weight_type, data_type, isMaxHeap>::PushSeries(const weight_type &weight, const data_type &data,
   ...
   if (optimizeNextSeriesPush == false) {
      // If the weight of what we are inserting is greater than / less than in order of the heap of
      // every sibling and sibling of parent, then can optimize next push
...
#pragma warning(disable : 4127)  // conditional expression is constant
      ...
      // Parent's subsequent siblings and this row's siblings all are less than / greater than
      // inserted element. Can insert all further elements straight to the end
...
template <class weight_type, class data_type, bool isMaxHeap>
void Heap<weight_type, data_type, isMaxHeap>::Push(const weight_type &weight, const data_type &data, const char *file,
                                                   unsigned int line) {
   unsigned currentIndex = heap.Size();
   unsigned parentIndex;
   heap.Insert(HeapNode(weight, data), file, line);
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
   ...
   while (1) {
      ...
      if (rightChild >= heap.Size()) {
         ...
         if (isMaxHeap) {
            if (heap[leftChild].weight <= currentWeight && heap[rightChild].weight <= currentWeight) return returnValue;
...
template <class weight_type, class data_type, bool isMaxHeap>
void Heap<weight_type, data_type, isMaxHeap>::Clear(bool doNotDeallocateSmallBlocks, const char *file,
                                                    unsigned int line) {
