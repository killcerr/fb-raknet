...
namespace RakNet {
extern void ProcessNetworkPacket(const SystemAddress systemAddress, const char *data, const int length,
                                 RakPeer *rakPeer, RakNet::TimeUS timeRead);
// extern void ProcessNetworkPacket( const SystemAddress systemAddress, const char *data, const int
// length, RakPeer *rakPeer, RakNetSocket* rakNetSocket, RakNet::TimeUS timeRead );
}  // namespace RakNet
...
void SocketLayer::SetSocketOptions(__UDPSOCKET__ listenSocket, bool blockingSocket, bool setBroadcast) {
   ...
   // Immediate hard close. Don't linger the socket, or recreating the socket quickly on Vista
   // fails. Fail with voice and xbox
...
RakNet::RakString SocketLayer::GetSubNetForSocketAndIp(__UDPSOCKET__ inSock, RakNet::RakString inIpString) {
   ...
   if (WSAIoctl(inSock, SIO_GET_INTERFACE_LIST, 0, 0, &InterfaceList, sizeof(InterfaceList), &nBytesReturned, 0, 0) ==
       SOCKET_ERROR) {
      return "";
   }
...
void GetMyIP_WinRT(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
   // Perhaps DatagramSocket.BindEndpointAsynch, use localHostName as an empty string, then query
   // what it bound to?
   RakAssert("Not yet supported" && 0);
}
#else
void GetMyIP_Win32(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
   ...
   if (gethostname(ac, sizeof(ac)) == -1) {
      ...
      FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
                    dwIOError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),  // Default language
                    (LPTSTR)&messageBuffer, 0, NULL);
      // something has gone wrong here...
...
bool SocketLayer::GetFirstBindableIP(char firstBindable[128], int ipProto) {
   ...
   for (l = 0; l < MAXIMUM_NUMBER_OF_INTERNAL_IDS; l++) {
      if (ipList[l] == UNASSIGNED_SYSTEM_ADDRESS) break;
      if (ipList[l].GetIPVersion() == 4 && ipProto == AF_INET) break;
      if (ipList[l].GetIPVersion() == 6 && ipProto == AF_INET6) break;
   }

   if (ipList[l] == UNASSIGNED_SYSTEM_ADDRESS || l == MAXIMUM_NUMBER_OF_INTERNAL_IDS) return false;
