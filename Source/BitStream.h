...
/// \details BitStream is used extensively throughout RakNet and is designed to be used by users as
/// well.
///
...
namespace RakNet {
/// This class allows you to write and read native types as a string of bits.  BitStream is used
/// extensively throughout RakNet and is designed to be used by users as well. \sa
/// BitStreamSample.txt
class RAK_DLL_EXPORT BitStream {
  public:
   ...
   /// \details There is no benefit to calling this, unless you know exactly how many bytes you need
   /// and it is greater than BITSTREAM_STACK_ALLOCATION_SIZE. In that case all it does is save you
   /// one or more realloc calls. \param[in] initialBytesToAllocate the number of bytes to
   /// pre-allocate.
   BitStream(const unsigned int initialBytesToAllocate);

   /// \brief Initialize the BitStream, immediately setting the data it contains to a predefined
   /// pointer. \details Set \a _copyData to true if you want to make an internal copy of the data
   /// you are passing. Set it to false to just save a pointer to the data. You shouldn't call Write
   /// functions with \a _copyData as false, as this will write to unallocated memory 99% of the
   /// time you will use this function to cast Packet::data to a bitstream for reading, in which
   /// case you should write something as follows: \code RakNet::BitStream bs(packet->data,
   /// packet->length, false); \endcode \param[in] _data An array of bytes. \param[in] lengthInBytes
   /// Size of the \a _data. \param[in] _copyData true or false to make a copy of \a _data or not.
   ...
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] inOutTemplateVar The value to write
   /// \return true if \a writeToBitstream is true.  true if \a writeToBitstream is false and the
   /// read was successful.  false if \a writeToBitstream is false and the read was not successful.
   ...
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] inOutCurrentValue The current value to
   /// write \param[in] lastValue The last value to compare against.  Only used if \a
   /// writeToBitstream is true. \return true if \a writeToBitstream is true.  true if \a
   /// writeToBitstream is false and the read was successful.  false if \a writeToBitstream is false
   /// and the read was not successful.
   template <class templateType>
   bool SerializeDelta(bool writeToBitstream, templateType &inOutCurrentValue, const templateType &lastValue);

   /// \brief Bidirectional version of SerializeDelta when you don't know what the last value is, or
   /// there is no last value. \param[in] writeToBitstream true to write from your data to this
   /// bitstream.  False to read from this bitstream and write to your data \param[in]
   /// inOutCurrentValue The current value to write \return true if \a writeToBitstream is true.
   /// true if \a writeToBitstream is false and the read was successful.  false if \a
   /// writeToBitstream is false and the read was not successful.
   template <class templateType>
   ...
   /// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type \param[in] writeToBitstream true to write from
   /// your data to this bitstream.  False to read from this bitstream and write to your data
   ...
   /// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the
   /// opposite is true for types larger than 1 byte \param[in] writeToBitstream true to write from
   /// your data to this bitstream.  False to read from this bitstream and write to your data
   /// \param[in] inOutCurrentValue The current value to write
   /// \param[in] lastValue The last value to compare against.  Only used if \a writeToBitstream is
   /// true. \return true if \a writeToBitstream is true.  true if \a writeToBitstream is false and
   /// the read was successful.  false if \a writeToBitstream is false and the read was not
   /// successful.
   template <class templateType>
   bool SerializeCompressedDelta(bool writeToBitstream, templateType &inOutCurrentValue, const templateType &lastValue);

   /// \brief Save as SerializeCompressedDelta(templateType &currentValue, const templateType
   /// &lastValue) when we have an unknown second parameter \return true on data read. False on
   /// insufficient data in bitstream
   template <class templateType>
   bool SerializeCompressedDelta(bool writeToBitstream, templateType &inOutTemplateVar);

   /// \brief Bidirectional serialize/deserialize an array or casted stream or raw data.  This does
   /// NOT do endian swapping. \param[in] writeToBitstream true to write from your data to this
   /// bitstream.  False to read from this bitstream and write to your data \param[in]
   /// inOutByteArray a byte buffer \param[in] numberOfBytes the size of \a input in bytes \return
   /// true if \a writeToBitstream is true.  true if \a writeToBitstream is false and the read was
   /// successful. false if \a writeToBitstream is false and the read was not successful.
   ...
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] inOutFloat The float to write
   /// \param[in] floatMin Predetermined minimum value of f \param[in] floatMax Predetermined
   /// maximum value of f
   ...
   /// Example: int num=53; SerializeCasted<uint8_t>(true, num); would use 1 byte to write what
   /// would otherwise be an integer (4 or 8 bytes) \param[in] writeToBitstream true to write from
   /// your data to this bitstream.  False to read from this bitstream and write to your data
   ...
   /// Given the minimum and maximum values for an integer type, figure out the minimum number of
   /// bits to represent the range Then serialize only those bits \note A static is used so that the
   /// required number of bits for (maximum-minimum) is only calculated once. This does require that
   /// \a minimum and \maximum are fixed values for a given line of code for the life of the program
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] value Integer value to write, which
   /// should be between \a minimum and \a maximum \param[in] minimum Minimum value of \a value
   /// \param[in] maximum Maximum value of \a value
   /// \param[in] allowOutsideRange If true, all sends will take an extra bit, however value can
   /// deviate from outside \a minimum and \a maximum. If false, will assert if the value deviates
   template <class templateType>
   ...
   /// \param[in] requiredBits Primarily for internal use, called from above function() after
   /// calculating number of bits needed to represent maximum-minimum
   template <class templateType>
   bool SerializeBitsFromIntegerRange(bool writeToBitstream, templateType &value, const templateType minimum,
                                      const templateType maximum, const int requiredBits,
                                      bool allowOutsideRange = false);

   /// \brief Bidirectional serialize/deserialize a normalized 3D vector, using (at most) 4 bytes +
   /// 3 bits instead of 12-24 bytes. \details Will further compress y or z axis aligned vectors.
   /// Accurate to 1/32767.5.
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] x x \param[in] y y \param[in] z z
   /// \return true if \a writeToBitstream is true.  true if \a writeToBitstream is false and the
   /// read was successful.  false if \a writeToBitstream is false and the read was not successful.
   ...
   /// \details Loses accuracy to about 3/10ths and only saves 2 bytes, so only use if accuracy is
   /// not important. \param[in] writeToBitstream true to write from your data to this bitstream.
   /// False to read from this bitstream and write to your data \param[in] x x \param[in] y y
   /// \param[in] z z
   /// \return true if \a writeToBitstream is true.  true if \a writeToBitstream is false and the
   /// read was successful.  false if \a writeToBitstream is false and the read was not successful.
   template <class templateType>  // templateType for this function must be a float or double
   ...
   /// \brief Bidirectional serialize/deserialize a normalized quaternion in 6 bytes + 4 bits
   /// instead of 16 bytes. Slightly lossy. \param[in] writeToBitstream true to write from your data
   /// to this bitstream.  False to read from this bitstream and write to your data \param[in] w w
   /// \param[in] x x \param[in] y y \param[in] z z \return true if \a writeToBitstream is true.
   /// true if \a writeToBitstream is false and the read was successful.  false if \a
   /// writeToBitstream is false and the read was not successful.
   template <class templateType>  // templateType for this function must be a float or double
   bool SerializeNormQuat(bool writeToBitstream, templateType &w, templateType &x, templateType &y, templateType &z);

   /// \brief Bidirectional serialize/deserialize an orthogonal matrix by creating a quaternion, and
   /// writing 3 components of the quaternion in 2 bytes each. \details Use 6 bytes instead of 36
   /// Lossy, although the result is renormalized
   /// \return true on success, false on failure.
   template <class templateType>  // templateType for this function must be a float or double
   bool SerializeOrthMatrix(bool writeToBitstream, templateType &m00, templateType &m01, templateType &m02,
   ...
   /// \param[in] writeToBitstream true to write from your data to this bitstream.  False to read
   /// from this bitstream and write to your data \param[in] inOutByteArray The data \param[in]
   /// numberOfBitsToSerialize The number of bits to write \param[in] rightAlignedBits if true data
   /// will be right aligned \return true if \a writeToBitstream is true.  true if \a
   /// writeToBitstream is false and the read was successful.  false if \a writeToBitstream is false
   /// and the read was not successful.
   bool SerializeBits(bool writeToBitstream, unsigned char *inOutByteArray, const BitSize_t numberOfBitsToSerialize,
   ...
   /// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type \param[in] inTemplateVar The value to write
   template <class templateType>
   ...
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the
   /// opposite is true for types larger than 1 byte \param[in] currentValue The current value to
   /// write \param[in] lastValue The last value to compare against
   template <class templateType>
   void WriteCompressedDelta(const templateType &currentValue, const templateType &lastValue);

   /// \brief Save as WriteCompressedDelta(const templateType &currentValue, const templateType
   /// &lastValue) when we have an unknown second parameter
   template <class templateType>
   ...
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the
   /// opposite is true for types larger than 1 byte \param[in] outTemplateVar The value to read
   /// \return true on success, false on failure.
   template <class templateType>
   ...
   /// must be between -1 and +1. For non-floating point, this is lossless, but only has benefit if
   /// you use less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the
   /// opposite is true for types larger than 1 byte ReadCompressedDelta is only valid from a
   /// previous call to WriteDelta \param[in] outTemplateVar The value to read \return true on
   ...
   /// serializationType should be uint8_t, uint16_t, uint24_t, or uint32_t
   /// Example: int num=53; WriteCasted<uint8_t>(num); would use 1 byte to write what would
   /// otherwise be an integer (4 or 8 bytes) \param[in] value The value to write
   template <class serializationType, class sourceType>
   void WriteCasted(const sourceType &value);

   /// Given the minimum and maximum values for an integer type, figure out the minimum number of
   /// bits to represent the range Then write only those bits \note A static is used so that the
   /// required number of bits for (maximum-minimum) is only calculated once. This does require that
   /// \a minimum and \maximum are fixed values for a given line of code for the life of the program
   ...
   /// \brief Write a normalized quaternion in 6 bytes + 4 bits instead of 16 bytes.  Slightly
   /// lossy. \param[in] w w \param[in] x x \param[in] y y \param[in] z z
   template <class templateType>  // templateType for this function must be a float or double
   void WriteNormQuat(templateType w, templateType x, templateType y, templateType z);

   /// \brief Write an orthogonal matrix by creating a quaternion, and writing 3 components of the
   /// quaternion in 2 bytes each. \details Use 6 bytes instead of 36 Lossy, although the result is
   /// renormalized
   template <class templateType>  // templateType for this function must be a float or double
   void WriteOrthMatrix(templateType m00, templateType m01, templateType m02, templateType m10, templateType m11,
                        templateType m12, templateType m20, templateType m21, templateType m22);
   ...
   /// serializationType should be uint8_t, uint16_t, uint24_t, or uint32_t
   /// Example: int num; ReadCasted<uint8_t>(num); would read 1 bytefrom the stream, and put the
   /// value in an integer \param[in] value The value to write
   template <class serializationType, class sourceType>
   bool ReadCasted(sourceType &value);

   /// Given the minimum and maximum values for an integer type, figure out the minimum number of
   /// bits to represent the range Then read only those bits \note A static is used so that the
   /// required number of bits for (maximum-minimum) is only calculated once. This does require that
   /// \a minimum and \maximum are fixed values for a given line of code for the life of the program
   ...
   /// \param[in] allowOutsideRange If true, all sends will take an extra bit, however value can
   /// deviate from outside \a minimum and \a maximum. If false, will assert if the value deviates.
   /// This should match the corresponding value passed to Write().
   template <class templateType>
   ...
   /// \brief Read 3 floats or doubles, using 10 bytes, where those float or doubles comprise a
   /// vector. \details Loses accuracy to about 3/10ths and only saves 2 bytes, so only use if
   /// accuracy is not important. \param[in] x x \param[in] y y \param[in] z z \return true on
   /// success, false on failure.
   template <class templateType>  // templateType for this function must be a float or double
   ...
   /// \brief Read an orthogonal matrix from a quaternion, reading 3 components of the quaternion in
   /// 2 bytes each and extrapolatig the 4th. \details Use 6 bytes instead of 36 Lossy, although the
   /// result is renormalized \return true on success, false on failure.
   template <class templateType>  // templateType for this function must be a float or double
   bool ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02, templateType &m10, templateType &m11,
                       templateType &m12, templateType &m20, templateType &m21, templateType &m22);
   ...
   /// \brief Aligns the bitstream, writes inputLength, and writes input. Won't write beyond
   /// maxBytesToWrite \param[in] inByteArray The data \param[in] inputLength The size of input.
   /// \param[in] maxBytesToWrite Max bytes to write
   void WriteAlignedBytesSafe(const char *inByteArray, const unsigned int inputLength,
   ...
   /// \brief Same as ReadAlignedBytesSafe() but allocates the memory for you using new, rather than
   /// assuming it is safe to write to \param[in] outByteArray outByteArray will be deleted if it is
   /// not a pointer to 0 \return true on success, false on failure.
   ...
   inline void Write(const wchar_t *const inStringVar) { RakWString::Serialize(inStringVar, this); }
   ...
   inline void WriteCompressed(const wchar_t *const inStringVar) { RakWString::Serialize(inStringVar, this); }
  ...
  private:
   ...
   /// BitStreams that use less than BITSTREAM_STACK_ALLOCATION_SIZE use the stack, rather than the
   /// heap to store data.  It switches over if BITSTREAM_STACK_ALLOCATION_SIZE is exceeded
...
bool BitStream::SerializeCasted(bool writeToBitstream, sourceType &value) {
   if (writeToBitstream)
      ...
      return ReadCasted<serializationType>(value);
   return true;
}

template <class templateType>
bool BitStream::SerializeBitsFromIntegerRange(bool writeToBitstream, templateType &value, const templateType minimum,
...
template <class templateType>
inline bool BitStream::SerializeOrthMatrix(bool writeToBitstream, templateType &m00, templateType &m01,
...
inline bool BitStream::SerializeBits(bool writeToBitstream, unsigned char *inOutByteArray,
                                     const BitSize_t numberOfBitsToSerialize, const bool rightAlignedBits) {
   if (writeToBitstream)
...
/// \brief Write any integral type to a bitstream.
/// \details Undefine __BITSTREAM_NATIVE_END if you need endian swapping.
/// For floating point, this is lossy, using 2 bytes for a float and 4 for a double.  The range must
/// be between -1 and +1. For non-floating point, this is lossless, but only has benefit if you use
/// less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the opposite is
/// true for types larger than 1 byte \param[in] inTemplateVar The value to write
template <class templateType>
...
/// be between -1 and +1. For non-floating point, this is lossless, but only has benefit if you use
/// less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the opposite is
/// true for types larger than 1 byte \param[in] currentValue The current value to write \param[in]
/// lastValue The last value to compare against
template <class templateType>
...
/// \brief Read any integral type from a bitstream.  Define __BITSTREAM_NATIVE_END if you need
/// endian swapping. \param[in] outTemplateVar The value to read
template <class templateType>
...
inline bool BitStream::Read(uint24_t &outTemplateVar) {
   ...
   if (readOffset + 3 * 8 > numberOfBitsUsed) return false;
...
inline bool BitStream::ReadDelta(templateType &outTemplateVar) {
   ...
   if (dataWritten) success = Read(outTemplateVar);
   return success;
}
...
/// be between -1 and +1. For non-floating point, this is lossless, but only has benefit if you use
/// less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the opposite is
/// true for types larger than 1 byte \param[in] outTemplateVar The value to read
template <class templateType>
...
/// be between -1 and +1. For non-floating point, this is lossless, but only has benefit if you use
/// less than half the bits of the type If you are not using __BITSTREAM_NATIVE_END the opposite is
/// true for types larger than 1 byte ReadCompressedDelta is only valid from a previous call to
/// WriteDelta \param[in] outTemplateVar The value to read
template <class templateType>
inline bool BitStream::ReadCompressedDelta(templateType &outTemplateVar) {
   bool dataWritten;
   bool success;
   success = Read(dataWritten);
   if (dataWritten) success = ReadCompressed(outTemplateVar);
   return success;
}
...
template <class templateType>
void BitStream::WriteBitsFromIntegerRange(const templateType value, const templateType minimum,
                                          const templateType maximum, const int requiredBits, bool allowOutsideRange) {
...
template <class templateType>  // templateType for this function must be a float or double
void BitStream::WriteOrthMatrix(templateType m00, templateType m01, templateType m02, templateType m10,
...
template <class templateType>
bool BitStream::ReadBitsFromIntegerRange(templateType &value, const templateType minimum, const templateType maximum,
                                         bool allowOutsideRange) {
   int requiredBits = BYTES_TO_BITS(sizeof(templateType)) - NumberOfLeadingZeroes(templateType(maximum - minimum));
   return ReadBitsFromIntegerRange(value, minimum, maximum, requiredBits, allowOutsideRange);
}
...
bool BitStream::ReadBitsFromIntegerRange(templateType &value, const templateType minimum, const templateType maximum,
   ...
   if (allowOutsideRange) {
      bool isOutsideRange;
      Read(isOutsideRange);
      if (isOutsideRange) return Read(value);
   }
   unsigned char output[sizeof(templateType)];
   memset(output, 0, sizeof(output));
   bool success = ReadBits(output, requiredBits);
   if (success) {
      if (IsBigEndian() == true) ReverseBytesInPlace(output, sizeof(output));
...
template <class templateType>  // templateType for this function must be a float or double
bool BitStream::ReadOrthMatrix(templateType &m00, templateType &m01, templateType &m02, templateType &m10,
                               templateType &m11, templateType &m12, templateType &m20, templateType &m21,
                               templateType &m22) {
   float qw, qx, qy, qz;
