...
void PacketizedTCP::Stop(void) {
   unsigned int i;
   TCPInterface::Stop();
   for (i = 0; i < waitingPackets.Size(); i++) DeallocatePacket(waitingPackets[i]);
   ClearAllConnections();
}
...
bool PacketizedTCP::SendList(const char **data, const unsigned int *lengths, const int numParameters,
   ...
   for (i = 0; i < numParameters; i++) {
      if (lengths[i] > 0) totalLengthOfUserData += lengths[i];
   }
   if (totalLengthOfUserData == 0) return false;

   PTCPHeader dataLength;
#ifndef __BITSTREAM_NATIVE_END
   if (RakNet::BitStream::DoEndianSwap())
      RakNet::BitStream::ReverseBytes((unsigned char *)&totalLengthOfUserData, (unsigned char *)&dataLength,
                                      sizeof(dataLength));
#else
...
Packet *PacketizedTCP::Receive(void) {
   ...
   if (outgoingPacket) return outgoingPacket;
   ...
   while (incomingPacket) {
      ...
      if (incomingPacket->deleteData == true) {
         ...
         if (index < connections.Size()) {
            ...
            // Header indicates packet length. If enough data is available, read out and return one
            // packet
            if (bq->GetBytesWritten() >= dataLength + sizeof(PTCPHeader)) {
               ...
               if (newWritten / 65536 != oldWritten / 65536) {
                  outgoingPacket = RakNet::OP_NEW<Packet>(_FILE_AND_LINE_);
                  outgoingPacket->length = sizeof(MessageID) + sizeof(unsigned int) * 2 + sizeof(unsigned int) + 65536;
                  ...
                  memcpy(outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 1, &totalParts,
                         sizeof(unsigned int));
                  memcpy(outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 2, &oneChunkSize,
                         sizeof(unsigned int));
                  bq->IncrementReadOffset(sizeof(PTCPHeader));
                  bq->ReadBytes((char *)outgoingPacket->data + sizeof(MessageID) + sizeof(unsigned int) * 3,
                                oneChunkSize, true);
...
void PacketizedTCP::ClearAllConnections(void) {
   unsigned int i;
   for (i = 0; i < connections.Size(); i++) RakNet::OP_DELETE(connections[i], _FILE_AND_LINE_);
   connections.Clear();
}
...
SystemAddress PacketizedTCP::HasCompletedConnectionAttempt(void) {
   ...
   if (_completedConnectionAttempts.IsEmpty() == false) return _completedConnectionAttempts.Pop();
...
SystemAddress PacketizedTCP::HasFailedConnectionAttempt(void) {
   ...
   if (_failedConnectionAttempts.IsEmpty() == false) return _failedConnectionAttempts.Pop();
   return UNASSIGNED_SYSTEM_ADDRESS;
}
...
SystemAddress PacketizedTCP::HasNewIncomingConnection(void) {
   ...
   if (_newIncomingConnections.IsEmpty() == false) return _newIncomingConnections.Pop();
   return UNASSIGNED_SYSTEM_ADDRESS;
}
...
SystemAddress PacketizedTCP::HasLostConnection(void) {
   ...
   if (_lostConnections.IsEmpty() == false) return _lostConnections.Pop();
   return UNASSIGNED_SYSTEM_ADDRESS;
}
