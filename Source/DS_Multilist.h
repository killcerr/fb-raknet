...
/// \file DS_Multilist.h
/// \internal
/// \brief ADT that can represent an unordered list, ordered list, stack, or queue with a common
/// interface
///
...
enum MultilistType {
   /// Removing from the middle of the list will swap the end of the list rather than shift the
   /// elements. Push and Pop operate on the tail.
   ML_UNORDERED_LIST,
   /// A normal list, with the list order preserved. Push and Pop operate on the tail.
   ML_STACK,
   /// A queue. Push and Pop operate on the head
   ML_QUEUE,
   /// A list that is always kept in order. Elements must be unique, and compare against each other
   /// consistently using <, ==, and >
   ML_ORDERED_LIST,
   /// A list whose type can change at runtime
   ML_VARIABLE_DURING_RUNTIME
};

/// The namespace DataStructures was only added to avoid compiler errors for commonly named data
/// structures As these data structures are stand-alone, you can use them outside of RakNet for your
...
/// bool operator<( const MyClass *myClass, const int &inputKey ) {return myClass->value <
/// inputKey;} At least one type has to be a reference to a class MLKeyRef is a helper class to turn
/// a native type into a class, so you can compare that native type against a pointer to a different
/// class Used for he Multilist, when _DataType != _KeyType
template <class templateType>
class MLKeyRef {
  public:
...
/// For the Multilist, when _DataType != _KeyType, you must define the comparison operators between
/// the key and the data This is non-trivial due to the need to use MLKeyRef in case the type held
/// is a pointer to a structure or class and the key type is not a class For convenience, this macro
/// will implement the comparison operators under the following conditions
...
#define DEFINE_MULTILIST_PTR_TO_MEMBER_COMPARISONS(_CLASS_NAME_, _KEY_TYPE_, _MEMBER_VARIABLE_NAME_) \
   bool operator<(const DataStructures::MLKeyRef<_KEY_TYPE_> &inputKey, const _CLASS_NAME_ *cls) {   \
...
/// \param[in] _KeyType If a function takes a key to sort on, what type of key this is. The
/// comparison operator between _DataType and _KeyType must be defined \param[in] _IndexType What
/// variable type to use for indices
template <const MultilistType _MultilistType, class _DataType, class _KeyType = _DataType,
...
class RAK_DLL_EXPORT Multilist {
  public:
   ...
   /// \brief Gets or removes and gets an element from the list, according to the same rules as
   /// Push(). Ordered list is LIFO for the purposes of Pop and Peek.
   ...
   void PushOpposite(const _DataType &d, const _KeyType &key, const char *file = __FILE__,
   ...
   /// \brief Finds the index of \a key. Return 0 if the key is not found. Useful if _DataType is
   /// always non-zero pointers.
   _DataType GetPtr(_KeyType key) const;

   /// \brief Iterate over the list, calling the function pointer on each element.
   void ForEach(void (*func)(_DataType &item, const char *file, unsigned int line), const char *file,
                unsigned int line);
   ...
   /// \brief Sorts the list unless it is an ordered list, in which it does nothing as the list is
   /// assumed to already be sorted. \details However, if \a force is true, it will also resort the
   /// ordered list, useful if the comparison operator between _KeyType and _DataType would now
   /// return different results Once the list is sorted, further operations to lookup by key will be
   /// log2(n) until the list is modified
   ...
   static void FindIntersection(Multilist &source1, Multilist &source2, Multilist &intersection,
                                Multilist &uniqueToSource1, Multilist &uniqueToSource2);
...
Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Multilist() {
   ...
   if (_MultilistType == ML_VARIABLE_DURING_RUNTIME) variableMultilistType = ML_UNORDERED_LIST;
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::~Multilist() {
   if (data != 0) RakNet::OP_DELETE_ARRAY(data, _FILE_AND_LINE_);
}
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
Multilist<_MultilistType, _DataType, _KeyType, _IndexType>
    &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::operator=(const Multilist &source) {
   Clear(true);
   CopySource(source);
   return *this;
}
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::CopySource(const Multilist &source) {
   ...
   if (source.data == 0) {
      ...
      _IndexType i;
      for (i = 0; i < dataSize; i++) data[i] = source[i];
   }
}
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Push(const _DataType &d, const char *file,
                                                                      unsigned int line) {
   Push(d, d, file, line);
}
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Push(const _DataType &d, const _KeyType &key,
   ...
   if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK) {
      ...
      if (queueTail == allocationSize) queueTail = 0;
      dataSize++;
   } else {
   ...
   if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK || GetMultilistType() == ML_QUEUE) {
      ...
      if (sortState != ML_UNSORTED && dataSize > 1) {
         if (ascendingSort) {
            if (MLKeyRef<_KeyType>(key) < operator[](dataSize - 2)) sortState = ML_UNSORTED;
         } else {
            if (MLKeyRef<_KeyType>(key) > operator[](dataSize - 2)) sortState = ML_UNSORTED;
         }
...
_DataType &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Pop(const char *file, unsigned int line) {
   ...
   if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK ||
       GetMultilistType() == ML_ORDERED_LIST) {
      dataSize--;
      return data[dataSize];
   } else {
      RakAssert(GetMultilistType() == ML_QUEUE);

      if (++queueHead == allocationSize) queueHead = 0;

      if (queueHead == 0) return data[allocationSize - 1];
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::PushOpposite(const _DataType &d, const char *file,
                                                                              unsigned int line) {
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::PushOpposite(const _DataType &d, const _KeyType &key,
   ...
   if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK || GetMultilistType() == ML_QUEUE) {
      ...
      if (sortState != ML_UNSORTED && dataSize > 1) {
         if (ascendingSort) {
            if (MLKeyRef<_KeyType>(key) > operator[](1)) sortState = ML_UNSORTED;
         } else {
            if (MLKeyRef<_KeyType>(key) < operator[](1)) sortState = ML_UNSORTED;
         }
      }
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
_DataType &Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::PopOpposite(const char *file,
                                                                                   unsigned int line) {
   RakAssert(IsEmpty() == false);
   if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK ||
       GetMultilistType() == ML_ORDERED_LIST) {
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::InsertAtIndex(const _DataType &d, _IndexType index,
   ...
   if (_MultilistType != ML_ORDERED_LIST) sortState = ML_UNSORTED;
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::RemoveAtIndex(_IndexType position, const char *file,
                                                                               unsigned int line) {
...
_IndexType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetIndexOf(_KeyType key) const {
   ...
   if (IsSorted()) {
      ...
      for (i = 0; i < dataSize; i++) {
         if (MLKeyRef<_KeyType>(key) == operator[](i)) return i;
      }
      return (_IndexType)-1;
   }
}
...
_IndexType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetInsertionIndex(_KeyType key) const {
   ...
   if (IsSorted()) {
      bool objectExists;
      i = GetIndexFromKeyInSortedList(key, &objectExists);
      if (objectExists) return (_IndexType)-1;
      return i;
   } else if (GetMultilistType() == ML_UNORDERED_LIST || GetMultilistType() == ML_STACK) {
      for (i = 0; i < dataSize; i++) {
         if (MLKeyRef<_KeyType>(key) == data[i]) return (_IndexType)-1;
      }
      return dataSize;
   } else {
      RakAssert(GetMultilistType() == ML_QUEUE);
      ...
      for (i = 0; i < dataSize; i++) {
         if (MLKeyRef<_KeyType>(key) == operator[](i)) return (_IndexType)-1;
      }
      return dataSize;
   }
}
...
_DataType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetPtr(_KeyType key) const {
   ...
   if (i == (_IndexType)-1) return 0;
   return data[i];
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ForEach(void (*func)(_DataType &item, const char *file,
                                                                                      unsigned int line),
                                                                         const char *file, unsigned int line) {
   _IndexType i;
   for (i = 0; i < dataSize; i++) func(operator[](i), file, line);
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ForEach(void (*func)(_DataType &item)) {
   _IndexType i;
   for (i = 0; i < dataSize; i++) func(operator[](i));
}
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Clear(bool deallocateSmallBlocks, const char *file,
                                                                       unsigned int line) {
   dataSize = 0;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ClearPointers(bool deallocateSmallBlocks,
                                                                               const char *file, unsigned int line) {
   _IndexType i;
   for (i = 0; i < dataSize; i++) RakNet::OP_DELETE(operator[](i), file, line);
   Clear(deallocateSmallBlocks, file, line);
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ClearPointer(_KeyType key, const char *file,
                                                                              unsigned int line) {
   _IndexType i;
   i = GetIndexOf(key);
   if (i != -1) {
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ReverseList(void) {
   if (IsSorted()) ascendingSort = !ascendingSort;

   ReverseListInternal();
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Reallocate(_IndexType size, const char *file,
                                                                            unsigned int line) {
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::Sort(bool force) {
   if (IsSorted() && force == false) return;
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::QSortAscending(_IndexType leftEdge,
                                                                                _IndexType rightEdge) {
   _DataType temp;
   ...
   if (left != leftEdge) QSortAscending(leftEdge, left);

   if (right != rightEdge) QSortAscending(right, rightEdge);
}

template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::QSortDescending(_IndexType leftEdge,
                                                                                 _IndexType rightEdge) {
   _DataType temp;
   _IndexType left = leftEdge;
   _IndexType right = rightEdge;
   ...
   if (left != leftEdge) QSortDescending(leftEdge, left);

   if (right != rightEdge) QSortDescending(right, rightEdge);
}
...
MultilistType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetMultilistType(void) const {
   if (_MultilistType == ML_VARIABLE_DURING_RUNTIME) return variableMultilistType;
         ...
         switch (newType) {
            ...
            case ML_ORDERED_LIST:
               if (queueTail < queueHead) {
                  ...
                  _IndexType i;
                  for (i = 0; i < dataSize; i++) data[i] = operator[](i);
               }
               if (newType == ML_ORDERED_LIST) Sort(false);
               break;
            case ML_QUEUE:
               // No change
               break;
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::FindIntersection(Multilist &source1,
                                                                                  Multilist &source2,
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ReallocateIfNeeded(const char *file,
                                                                                    unsigned int line) {
   if (dataSize < allocationSize) return;
   ...
   if (allocationSize < 16)
      ...
      if (newAllocationSize < allocationSize) newAllocationSize = allocationSize + 65536;
   }
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::DeallocateIfNeeded(const char *file,
                                                                                    unsigned int line) {
   if (allocationSize < 512) return;
   if (dataSize >= allocationSize / 3) return;
   if (dataSize <= preallocationSize) return;
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::ReallocToSize(_IndexType newAllocationSize,
   ...
   _IndexType i;
   for (i = 0; i < dataSize; i++) newData[i] = operator[](i);
   if (dataSize > 0) {
      RakNet::OP_DELETE_ARRAY(data, file, line);
      if (GetMultilistType() == ML_QUEUE) {
         queueHead = 0;
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::InsertInOrderedList(const _DataType &d,
                                                                                     const _KeyType &key) {
   RakAssert(GetMultilistType() == ML_ORDERED_LIST);
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
_IndexType Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::GetIndexFromKeyInSortedList(
    const _KeyType &key, bool *objectExists) const {
   RakAssert(IsSorted());
...
template <const MultilistType _MultilistType, class _DataType, class _KeyType, class _IndexType>
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::InsertShiftArrayRight(const _DataType &d,
                                                                                       _IndexType index) {
   RakAssert(_MultilistType != ML_QUEUE);

   // Move the elements in the list to make room
   _IndexType i;
   for (i = dataSize; i != index; i--) data[i] = data[i - 1];
...
void Multilist<_MultilistType, _DataType, _KeyType, _IndexType>::DeleteShiftArrayLeft(_IndexType index) {
   ...
   _IndexType i;
   for (i = index; i < dataSize - 1; i++) data[i] = data[i + 1];
}
};  // namespace DataStructures
