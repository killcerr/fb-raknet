...
struct RAK_DLL_EXPORT PunchthroughConfiguration {
   ...
   /// This sometimes works because the remote router was looking for an incoming message on a
   /// higher numbered port before responding to a lower numbered port from the other system
   bool retryOnFailure;
};
...
/// \details Maintain connection to NatPunchthroughServer to process incoming connection attempts
/// through NatPunchthroughClient<BR> Client will send datagrams to port to estimate next port<BR>
...
class RAK_DLL_EXPORT NatPunchthroughClient : public PluginInterface2 {
  public:
   ...
   /// If the instance of RakPeer running NATPunchthroughServer was bound to two IP addresses, then
   /// you can call FindRouterPortStride() This will determine the stride that your router uses when
   /// assigning ports, if your router is full-cone This function is also called automatically when
   /// you call OpenNAT - however, calling it earlier when you are connected to the facilitator will
   /// speed up the process \param[in] destination The system to punch. Must already be connected to
   /// \a facilitator
   ...
   /// \param[in] facilitator A system we are already connected to running the NatPunchthroughServer
   /// plugin \sa OpenNATGroup() You will get ID_NAT_PUNCHTHROUGH_SUCCEEDED on success You will get
   /// ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_TARGET_UNRESPONSIVE, ID_NAT_CONNECTION_TO_TARGET_LOST,
   ...
   /// This is useful for peer to peer games where you want to connect to every system in the remote
   session, not just one particular system
   /// \note For cloud computing, all systems in the group must be connected to the same facilitator
   since we're only specifying one
   /// You will get ID_NAT_GROUP_PUNCH_SUCCEEDED on success
   /// You will get ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_ALREADY_IN_PROGRESS, or
   ID_NAT_GROUP_PUNCH_FAILED on failures of various types
   /// However, if you lose connection to the facilitator, you may not necessarily get above
   bool OpenNATGroup(DataStructures::List<RakNetGUID> destinationSystems, const SystemAddress
   &facilitator);
   */
   ...
   /// \param[in] i Pointer to an interface. The pointer is stored, so don't delete it while in
   /// progress. Pass 0 to clear.
   void SetDebugInterface(NatPunchthroughDebugInterface *i);

   /// Get the port mappings you should pass to UPNP (for miniupnpc-1.6.20120410, for the function
   /// UPNP_AddPortMapping)
   ...
   struct SendPing {
      ...
      uint16_t sessionId;
      bool sentTTL;
      // Give priority to internal IP addresses because if we are on a LAN, we don't want to try to
      ...
      enum TestMode {
         ...
         // If you continue punching on a different port for the same IP it bans you and the
         // communication becomes unidirectioal
         PUNCHING_FIXED_PORT,
  ...
  protected:
   ...
   // The first time we fail a NAT attempt, we add it to failedAttemptList and try again, since
   // sometimes trying again later fixes the problem The second time we fail, we return
   ...
   enum { HAS_PORT_STRIDE, UNKNOWN_PORT_STRIDE, CALCULATING_PORT_STRIDE, INCAPABLE_PORT_STRIDE } hasPortStride;
   ...
   void UpdateGroupPunchOnNatResult(SystemAddress facilitator, RakNetGUID targetSystem,
   SystemAddress targetSystemAddress, int result); // 0=failed, 1=success, 2=ignore
   */
};
