...
int RakNet::MessageFilterStrComp(char *const &key, char *const &data) { return strcmp(key, data); }
...
MessageFilter::MessageFilter() { whenLastTimeoutCheck = RakNet::GetTime(); }

MessageFilter::~MessageFilter() { Clear(); }

void MessageFilter::SetAutoAddNewConnectionsToFilter(int filterSetID) { autoAddNewConnectionsToFilter = filterSetID; }
...
void MessageFilter::SetAllowMessageID(bool allow, int messageIDStart, int messageIDEnd, int filterSetID) {
   ...
   int i;
   for (i = messageIDStart; i <= messageIDEnd; ++i) filterSet->allowedIDs[i] = allow;
}
...
void MessageFilter::SetActionOnDisallowedMessage(bool kickOnDisallowed, bool banOnDisallowed, RakNet::TimeMS banTimeMS,
                                                 int filterSetID) {
   FilterSet *filterSet = GetFilterSetByID(filterSetID);
...
void MessageFilter::SetDisallowedMessageCallback(int filterSetID, void *userData,
...
void MessageFilter::SetTimeoutCallback(int filterSetID, void *userData,
                                       void (*invalidMessageCallback)(RakPeerInterface *peer,
...
unsigned MessageFilter::GetSystemCount(int filterSetID) const {
   if (filterSetID == -1) {
      ...
      for (i = 0; i < itemList.Size(); i++)
         if (itemList[i].filter->filterSetID == filterSetID) ++count;
      return count;
   }
}

unsigned MessageFilter::GetFilterSetCount(void) const { return filterList.Size(); }

int MessageFilter::GetFilterSetIDByIndex(unsigned index) { return filterList[index]->filterSetID; }
...
void MessageFilter::Clear(void) {
   ...
   for (i = 0; i < filterList.Size(); i++) DeallocateFilterSet(filterList[i]);
   filterList.Clear(false, _FILE_AND_LINE_);
}

void MessageFilter::DeallocateFilterSet(FilterSet *filterSet) { RakNet::OP_DELETE(filterSet, _FILE_AND_LINE_); }
...
void MessageFilter::OnInvalidMessage(FilterSet *filterSet, AddressOrGUID systemAddress, unsigned char messageID) {
   if (filterSet->invalidMessageCallback)
      filterSet->invalidMessageCallback(rakPeerInterface, systemAddress, filterSet->filterSetID,
                                        filterSet->disallowedCallbackUserData, messageID);
   ...
   if (filterSet->kickOnDisallowedMessage) {
      if (rakPeerInterface) rakPeerInterface->CloseConnection(systemAddress, true, 0);
...
void MessageFilter::Update(void) {
   // Update all timers for all systems.  If those systems' filter sets are expired, take the
   // appropriate action.
      ...
      for (index = 0; index < itemList.Size(); index++) {
         if (itemList[index].filter && itemList[index].filter->maxMemberTimeMS > 0 &&
            ...
            if (itemList[index].filter->timeoutCallback)
               itemList[index].filter->timeoutCallback(rakPeerInterface, keyList[index],
            ...
            if (rakPeerInterface) rakPeerInterface->CloseConnection(keyList[index], true, 0);
#if _RAKNET_SUPPORT_PacketizedTCP == 1 && _RAKNET_SUPPORT_TCPInterface == 1
            else
...
void MessageFilter::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
PluginReceiveResult MessageFilter::OnReceive(Packet *packet) {
   ...
   switch (packet->data[0]) {
      ...
      default:
         ...
         // If this system is filtered, check if this message is allowed.  If not allowed, return
         // RR_STOP_PROCESSING_AND_DEALLOCATE index =
         // systemList.GetIndexFromKey(packet->addressOrGUID, &objectExists);
         index = systemList.GetIndexOf(packet);
         if (index.IsInvalid()) break;
