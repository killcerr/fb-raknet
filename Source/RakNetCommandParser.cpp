...
RakNetCommandParser::RakNetCommandParser() {
   ...
   RegisterCommand(4, "Connect",
                   "( const char* host, unsigned short remotePort, const char *passwordData, int "
                   "passwordDataLength );");
   ...
   RegisterCommand(3, "CloseConnection",
                   "( const SystemAddress target, bool sendDisconnectionNotification, unsigned char "
                   "orderingChannel=0 );");
   ...
   RegisterCommand(4, "AdvertiseSystem",
...
void RakNetCommandParser::SetRakPeerInterface(RakNet::RakPeerInterface *rakPeer) { peer = rakPeer; }

bool RakNetCommandParser::OnCommand(const char *command, unsigned numParameters, char **parameterList,
   ...
   if (strcmp(command, "Startup") == 0) {
      ...
      ReturnResult(peer->Startup((unsigned short)atoi(parameterList[0]), &socketDescriptor, 1), command, transport,
                   systemAddress);
      ...
      ReturnResult(peer->Connect(parameterList[0], (unsigned short)atoi(parameterList[1]), parameterList[2],
                                 atoi(parameterList[3])) == RakNet::CONNECTION_ATTEMPT_STARTED,
      ...
      peer->CloseConnection(SystemAddress(parameterList[0]), atoi(parameterList[1]) != 0,
                            (unsigned char)atoi(parameterList[2]));
      ReturnResult(command, transport, systemAddress);
   ...
   {
           ReturnResult(peer->SetMTUSize(atoi(parameterList[0]), UNASSIGNED_SYSTEM_ADDRESS),
   command, transport, systemAddress);
   }
   */
      ...
      peer->AdvertiseSystem(parameterList[0], (unsigned short)atoi(parameterList[1]), parameterList[2],
                            atoi(parameterList[3]));
      ReturnResult(command, transport, systemAddress);
...
const char *RakNetCommandParser::GetName(void) const { return "RakNet"; }
