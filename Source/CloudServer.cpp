...
int CloudServer::RemoteServerComp(const RakNetGUID &key, RemoteServer *const &data) {
   if (key < data->serverAddress) return -1;
   if (key > data->serverAddress) return 1;
   return 0;
}

int CloudServer::KeySubscriberIDComp(const CloudKey &key, KeySubscriberID *const &data) {
   if (key.primaryKey < data->key.primaryKey) return -1;
   if (key.primaryKey > data->key.primaryKey) return 1;
   if (key.secondaryKey < data->key.secondaryKey) return -1;
   if (key.secondaryKey > data->key.secondaryKey) return 1;
   return 0;
}

int CloudServer::KeyDataPtrComp(const RakNetGUID &key, CloudData *const &data) {
   if (key < data->clientGUID) return -1;
   if (key > data->clientGUID) return 1;
   return 0;
}
...
int CloudServer::GetRequestComp(const uint32_t &key, CloudServer::GetRequest *const &data) {
   if (key < data->requestId) return -1;
   if (key > data->requestId) return -1;
   return 0;
}
...
bool CloudServer::GetRequest::AllRemoteServersHaveResponded(void) const {
   ...
   for (i = 0; i < remoteServerResponses.Size(); i++)
      if (remoteServerResponses[i]->gotResult == false) return false;
   return true;
}
...
CloudServer::~CloudServer() { Clear(); }

void CloudServer::SetMaxUploadBytesPerClient(uint64_t bytes) { maxUploadBytesPerClient = bytes; }

void CloudServer::SetMaxBytesPerDownload(uint64_t bytes) { maxBytesPerDowload = bytes; }
...
void CloudServer::OnPostRequest(Packet *packet) {
   ...
   if (maxUploadBytesPerClient > 0 && dataLengthBytes > maxUploadBytesPerClient) return;  // Exceeded max upload bytes
   ...
   for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
      if (queryFilters[filterIndex]->OnPostRequest(
              ...
              (const char *)bsIn.GetData() + BITS_TO_BYTES(bsIn.GetReadOffset())) == false)
         return;
   }
   ...
   if (keyDataListExists == false) {
      if (maxUploadBytesPerClient > 0 && remoteCloudClient->uploadedBytes + dataLengthBytes > maxUploadBytesPerClient) {
         ...
         if (dataLengthBytes > CLOUD_SERVER_DATA_STACK_SIZE) rakFree_Ex(data, _FILE_AND_LINE_);
      ...
      if (cloudData->serverSystemAddress.GetPort() == 0) {
         // Fix localhost port
         cloudData->serverSystemAddress.SetPortHostOrder(
             rakPeerInterface->GetSocket(UNASSIGNED_SYSTEM_ADDRESS)->GetBoundAddress().GetPort());
      }
      ...
      if (cloudData->allocatedData != 0) rakFree_Ex(cloudData->allocatedData, _FILE_AND_LINE_);
   }
   ...
   if (dataLengthBytes > CLOUD_SERVER_DATA_STACK_SIZE) {
      ...
      // Read to stack
      if (dataLengthBytes > 0) bsIn.ReadAlignedBytes(cloudData->stackData, dataLengthBytes);
...
void CloudServer::OnReleaseRequest(Packet *packet) {
   ...
   if (remoteSystemIndex.IsInvalid() == true) return;
   ...
   for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
      if (queryFilters[filterIndex]->OnReleaseRequest(packet->guid, packet->systemAddress, cloudKeys) == false) return;
   }
      ...
      if (objectExists) {
         ...
         if (cloudData->IsUnused()) {
            ...
            if (cloudDataList->IsNotUploaded()) {
               // Tell other servers that this key is no longer uploaded, so they do not request it
               // from us
...
void CloudServer::OnGetRequest(Packet *packet) {
   ...
   if (remoteServersWithData.Size() == 0) {
      ...
      for (unsigned int remoteServerIndex = 0; remoteServerIndex < remoteServersWithData.Size(); remoteServerIndex++) {
         ...
         getRequest->remoteServerResponses.Insert(remoteServersWithData[remoteServerIndex]->serverAddress,
   ...
   if (getRequest->cloudQueryWithAddresses.cloudQuery.subscribeToResults) {
      // Add to key subscription list for the client, which contains a keyId / specificUploaderList
      // pair
      DataStructures::HashIndex remoteSystemsHashIndex = remoteSystems.GetIndexOf(packet->guid);
      RemoteCloudClient *remoteCloudClient;
      if (remoteSystemsHashIndex.IsInvalid()) {
      ...
      for (keyIndex = 0; keyIndex < getRequest->cloudQueryWithAddresses.cloudQuery.keys.Size(); keyIndex++) {
         ...
         for (specificSystemIndex = 0; specificSystemIndex < getRequest->cloudQueryWithAddresses.specificSystems.Size();
              specificSystemIndex++) {
            keySubscriberId->specificSystemsSubscribedTo.Insert(
                getRequest->cloudQueryWithAddresses.specificSystems[specificSystemIndex],
                getRequest->cloudQueryWithAddresses.specificSystems[specificSystemIndex], true, _FILE_AND_LINE_);
         }
         ...
         CloudDataList *cloudDataList =
             GetOrAllocateCloudDataList(cloudKey, &dataRepositoryExists, dataRepositoryIndex);

         // If this is the first local client to subscribe to this key, call
         // SendSubscribedKeyToServers
         if (cloudDataList->subscriberCount == 0) SendSubscribedKeyToServers(cloudKey);
         ...
         if (getRequest->cloudQueryWithAddresses.specificSystems.Size()) {
            ...
            for (specificSystemIndex = 0;
                 specificSystemIndex < getRequest->cloudQueryWithAddresses.specificSystems.Size();
                 specificSystemIndex++) {
               ...
               unsigned int keyDataListIndex =
                   cloudDataList->keyData.GetIndexFromKey(specificSystem, &keyDataListExists);
               if (keyDataListExists == false) {
                  cloudData = RakNet::OP_NEW<CloudData>(_FILE_AND_LINE_);
            ...
            // Remove packet->guid from CloudData::specificSubscribers among all instances of
            // cloudDataList->keyData
            unsigned int subscribedKeysIndex;
            bool subscribedKeysIndexExists;
            subscribedKeysIndex =
                remoteCloudClient->subscribedKeys.GetIndexFromKey(cloudDataList->key, &subscribedKeysIndexExists);
            if (subscribedKeysIndexExists) {
               KeySubscriberID *keySubscriberId;
               keySubscriberId = remoteCloudClient->subscribedKeys[subscribedKeysIndex];
               unsigned int specificSystemIndex;
               for (specificSystemIndex = 0; specificSystemIndex < keySubscriberId->specificSystemsSubscribedTo.Size();
                    specificSystemIndex++) {
                  bool keyDataExists;
   ...
   if (remoteServersWithData.Size() == 0) RakNet::OP_DELETE(getRequest, _FILE_AND_LINE_);
}
...
void CloudServer::OnUnsubscribeRequest(Packet *packet) {
   ...
   for (unsigned int filterIndex = 0; filterIndex < queryFilters.Size(); filterIndex++) {
      if (queryFilters[filterIndex]->OnUnsubscribeRequest(packet->guid, packet->systemAddress, cloudKeys,
                                                          specificSystems) == false)
         return;
   }
   ...
   for (index = 0; index < keyCount; index++) {
      ...
      if (hasKeySubscriber == false) continue;
...
void CloudServer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
   ...
   if (objectExists) {
      // Update remoteServerResponses by removing this server and sending the response if it is now
      // complete
      ...
      while (getRequestIndex < getRequests.Size()) {
         ...
         unsigned int remoteServerResponsesIndex =
             getRequest->remoteServerResponses.GetIndexFromKey(rakNetGUID, &waitingForThisServer);
   ...
   if (remoteSystemIndex.IsInvalid() == false) {
      ...
      for (uploadedKeysIndex = 0; uploadedKeysIndex < remoteCloudClient->uploadedKeys.Size(); uploadedKeysIndex++) {
         ...
         unsigned int dataRepositoryIndex = dataRepository.GetIndexFromKey(
             remoteCloudClient->uploadedKeys[uploadedKeysIndex], &keyDataRepositoryExists);
         if (keyDataRepositoryExists) {
            ...
            if (keyDataExists) {
               ...
               NotifyClientSubscribersOfDataChange(cloudData, cloudDataList->key, cloudData->specificSubscribers,
                                                   false);
               NotifyClientSubscribersOfDataChange(cloudData, cloudDataList->key, cloudDataList->nonSpecificSubscribers,
                                                   false);
               NotifyServerSubscribersOfDataChange(cloudData, cloudDataList->key, false);
               ...
               if (cloudData->IsUnused()) {
                  ...
                  if (cloudDataList->IsNotUploaded()) {
                     // Tell other servers that this key is no longer uploaded, so they do not
                     // request it from us
                     RemoveUploadedKeyFromServers(cloudDataList->key);
                  }
      ...
      unsigned int subscribedKeysIndex;
      for (subscribedKeysIndex = 0; subscribedKeysIndex < remoteCloudClient->subscribedKeys.Size();
           subscribedKeysIndex++) {
         KeySubscriberID *keySubscriberId;
         keySubscriberId = remoteCloudClient->subscribedKeys[subscribedKeysIndex];

         bool keyDataRepositoryExists;
         unsigned int keyDataRepositoryIndex = dataRepository.GetIndexFromKey(
...
void CloudServer::OnRakPeerShutdown(void) { Clear(); }
...
void CloudServer::AddServer(RakNetGUID systemIdentifier) {
   ...
   if (cs == IS_DISCONNECTED || cs == IS_NOT_CONNECTED) return;
   bool objectExists;
...
void CloudServer::ProcessAndTransmitGetRequest(GetRequest *getRequest) {
   ...
   if (unlimitedRows == false && localNumRows > getRequest->cloudQueryWithAddresses.cloudQuery.startingRowIndex &&
   ...
   if (unlimitedRows == true || getRequest->cloudQueryWithAddresses.cloudQuery.maxRowsToReturn > localRowsToWrite) {
      ...
      for (remoteServerResponseIndex = 0; remoteServerResponseIndex < getRequest->remoteServerResponses.Size();
         ...
         for (cloudQueryRowIndex = 0;
              cloudQueryRowIndex < bufferedGetResponseFromServer->queryResult.rowsReturned.Size();
              cloudQueryRowIndex++) {
            if (skipRows > 0) {
               --skipRows;
               continue;
            ...
            if (unlimitedRows == false && --remainingRows == 0) break;
         }
...
void CloudServer::ProcessCloudQueryWithAddresses(CloudServer::CloudQueryWithAddresses &cloudQueryWithAddresses,
                                                 DataStructures::List<CloudData *> &cloudDataResultList,
                                                 DataStructures::List<CloudKey> &cloudKeyResultList) {
   CloudQueryResult cloudQueryResult;
   CloudQueryRow cloudQueryRow;
      ...
      if (dataRepositoryExists) {
         ...
         if (cloudDataList->uploaderCount > 0) {
            ...
            if (cloudQueryWithAddresses.specificSystems.Size() > 0) {
               ...
               unsigned int specificSystemIndex;
               for (specificSystemIndex = 0; specificSystemIndex < cloudQueryWithAddresses.specificSystems.Size();
                    specificSystemIndex++) {
                  bool uploaderExists;
                  keyDataIndex = cloudDataList->keyData.GetIndexFromKey(
                      cloudQueryWithAddresses.specificSystems[specificSystemIndex], &uploaderExists);
                  if (uploaderExists) {
...
void CloudServer::SendUploadedAndSubscribedKeysToServer(RakNetGUID systemAddress) {
   ...
   for (unsigned int i = 0; i < dataRepository.Size(); i++) dataRepository[i]->key.Serialize(true, &bsOut);
...
void CloudServer::OnSendUploadedAndSubscribedKeysToServer(Packet *packet) {
   ...
   for (uint16_t i = 0; i < numUploadedKeys; i++) {
      ...
      if (alreadyHasKey == false) remoteServer->uploadedKeys.Insert(cloudKey, cloudKey, true, _FILE_AND_LINE_);
   }
   ...
   for (uint16_t i = 0; i < numSubscribedKeys; i++) {
      ...
      if (alreadyHasKey == false) remoteServer->subscribedKeys.Insert(cloudKey, cloudKey, true, _FILE_AND_LINE_);
   }
...
void CloudServer::OnSendSubscribedKeyToServers(Packet *packet) {
   ...
   // Do not need to send current values, the Get request will do that as the Get request is sent at
   // the same time
   if (alreadyHasKey == false) remoteServer->subscribedKeys.Insert(cloudKey, cloudKey, true, _FILE_AND_LINE_);
}
...
void CloudServer::OnRemoveUploadedKeyFromServers(Packet *packet) {
   ...
   if (alreadyHasKey == true) remoteServer->uploadedKeys.RemoveAtIndex(insertionIndex);
}
...
void CloudServer::OnRemoveSubscribedKeyFromServers(Packet *packet) {
   ...
   if (alreadyHasKey == true) remoteServer->subscribedKeys.RemoveAtIndex(insertionIndex);
}
...
CloudServer::CloudDataList *CloudServer::GetOrAllocateCloudDataList(CloudKey key, bool *dataRepositoryExists,
                                                                    unsigned int &dataRepositoryIndex) {
   CloudDataList *cloudDataList;
...
void CloudServer::UnsubscribeFromKey(RemoteCloudClient *remoteCloudClient, RakNetGUID remoteCloudClientGuid,
   ...
   if (keySubscriberId->specificSystemsSubscribedTo.Size() == 0 && specificSystems.Size() > 0) return;

   bool dataRepositoryExists;
   CloudDataList *cloudDataList;
   unsigned int dataRepositoryIndex = dataRepository.GetIndexFromKey(cloudKey, &dataRepositoryExists);
   if (dataRepositoryExists == false) return;
   ...
   if (specificSystems.Size() == 0) {
      ...
      if (cloudDataList->RemoveSubscriber(remoteCloudClientGuid) == false) {
         for (i = 0; i < keySubscriberId->specificSystemsSubscribedTo.Size(); i++) {
            RemoveSpecificSubscriber(keySubscriberId->specificSystemsSubscribedTo[i], cloudDataList,
      ...
      for (j = 0; j < specificSystems.Size(); j++) {
         ...
         specificSystemsSubscribedToIndex = keySubscriberId->specificSystemsSubscribedTo.GetIndexFromKey(
   ...
   if (cloudDataList->subscriberCount == 0) RemoveSubscribedKeyFromServers(cloudKey);
...
void CloudServer::RemoveSpecificSubscriber(RakNetGUID specificSubscriber, CloudDataList *cloudDataList,
   ...
   if (keyDataListExists == false) return;
   CloudData *cloudData = cloudDataList->keyData[keyDataListIndex];
   bool hasSpecificSubscriber;
   unsigned int specificSubscriberIndex =
       cloudData->specificSubscribers.GetIndexFromKey(remoteCloudClientGuid, &hasSpecificSubscriber);
...
void CloudServer::ForceExternalSystemAddress(SystemAddress forcedAddress) { forceAddress = forcedAddress; }

void CloudServer::AddQueryFilter(CloudServerQueryFilter *filter) {
   if (queryFilters.GetIndexOf(filter) != (unsigned int)-1) return;
...
void CloudServer::RemoveQueryFilter(CloudServerQueryFilter *filter) {
   ...
   if (index != (unsigned int)-1) queryFilters.RemoveAtIndex(index);
}

void CloudServer::RemoveAllQueryFilters(void) { queryFilters.Clear(true, _FILE_AND_LINE_); }
