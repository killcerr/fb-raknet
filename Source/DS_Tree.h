...
void Tree<TreeType>::LevelOrderTraversal(DataStructures::List<Tree *> &output) {
   ...
   for (i = 0; i < children.Size(); i++) queue.Push(children[i]);

   while (queue.Size()) {
      node = queue.Pop();
      output.Insert(node, _FILE_AND_LINE_);
      for (i = 0; i < node->children.Size(); i++) queue.Push(node->children[i]);
   }
}
 ...
 */
   ...
   unsigned int i;
   for (i = 0; i < children.Size(); i++) RakNet::OP_DELETE(children[i], _FILE_AND_LINE_);
}
}  // namespace DataStructures
