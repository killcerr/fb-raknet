...
int TM_World::JoinRequestHelperComp(const TM_World::JoinRequestHelper &key, const TM_World::JoinRequestHelper &data) {
   if (key.whenRequestMade < data.whenRequestMade) return -1;
   if (key.whenRequestMade > data.whenRequestMade) return 1;
   if (key.requestIndex < data.requestIndex) return -1;
   if (key.requestIndex > data.requestIndex) return 1;
   return 0;
}
...
TeamSelection::TeamSelection(JoinTeamType itt, TM_Team *param) : joinTeamType(itt) {
   teamParameter.specificTeamToJoin = param;
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TeamSelection::TeamSelection(JoinTeamType itt, NoTeamId param) : joinTeamType(itt) {
   teamParameter.noTeamSubcategory = param;
}
...
TeamSelection TeamSelection::SpecificTeam(TM_Team *specificTeamToJoin) {
   return TeamSelection(JOIN_SPECIFIC_TEAM, specificTeamToJoin);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TeamSelection TeamSelection::NoTeam(NoTeamId noTeamSubcategory) {
   return TeamSelection(JOIN_NO_TEAM, noTeamSubcategory);
}
...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection) {
   if (teamSelection.joinTeamType == JOIN_NO_TEAM) {
      ...
      // - remote Host executes JoinNoTeam() and broadcasts event. This may cause may cause
      // rebalance if team balancing is on.
      ...
      // - JoinAnyTeam(requesterGuid): Attempt to join any team immediately.  If fails, send to all
      // except requestGuid UpdateTeamsRequestedToAny(). Else sends out new team, including to
      // caller. On remote host, execute JoinAnyTeamCheck(). If fails, this was because you were
      // added to a team simultaneously on host. This is OK, just ignore the call. Assuming
      // JoinAnyTeamCheck() passed on remote host, call UpdateTeamsRequestedToAny() for this player.
      // execute JoinAnyTeam(packet->guid).

      if (JoinAnyTeamCheck() == false) return false;
      ...
      // - JoinSpecificTeam(requesterGuid): Attempt to join specific team immediately. If fails,
      // send to all except requesterGuid to execute AddSpecificToRequested(). Else sends out new
      // team, including to caller. On remote host, execute JoinSpecificTeamCheck(). If fails, just
      // ignore. Assuming JoinSpecificTeamCheck() passed on host, call AddSpecificToRequestedList().
      // Execute JoinSpecificTeam(packet->guid)

      if (JoinSpecificTeamCheck(teamSelection.teamParameter.specificTeamToJoin, false) == false) return false;
...
bool TM_TeamMember::RequestTeamSwitch(TM_Team *teamToJoin, TM_Team *teamToLeave) {
   if (SwitchSpecificTeamCheck(teamToJoin, teamToLeave, false) == false) return false;
...
unsigned int TM_TeamMember::GetRequestedTeamIndex(TM_Team *team) const {
   ...
   for (i = 0; i < teamsRequested.Size(); i++) {
      if (teamsRequested[i].requested == team) return i;
   }
...
unsigned int TM_TeamMember::GetRequestedTeamCount(void) const { return teamsRequested.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

bool TM_TeamMember::CancelTeamRequest(TM_Team *specificTeamToCancel) {
   if (RemoveFromRequestedTeams(specificTeamToCancel) == false) return false;
...
bool TM_TeamMember::LeaveTeam(TM_Team *team, NoTeamId _noTeamSubcategory) {
   if (LeaveTeamCheck(team) == false) return false;
   ...
   // On remote host, execute check. If check passes, execute non-host changes, followed by host
   // changes. Relay to all but sender.
   BitStream bsOut;
   bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
...
unsigned int TM_TeamMember::GetCurrentTeamCount(void) const { return teams.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_Team *TM_TeamMember::GetCurrentTeamByIndex(unsigned int index) { return teams[index]; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_TeamMember::GetCurrentTeams(DataStructures::List<TM_Team *> &_teams) const { _teams = teams; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_TeamMember::GetLastTeams(DataStructures::List<TM_Team *> &_teams) const { _teams = lastTeams; }
...
NetworkID TM_TeamMember::GetNetworkID(void) const { return networkId; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_World *TM_TeamMember::GetTM_World(void) const { return world; }
...
bool TM_TeamMember::DeserializeConstruction(TeamManager *teamManager, BitStream *constructionBitstream) {
   ...
   for (unsigned int i = 0; i < teamsRequestedSize; i++) {
      ...
      rt.requestIndex = world->teamRequestIndex++;  // In case whenRequested is the same between two
                                                    // teams when sorting team requests
      if ((hasTeamToLeave == false || (hasTeamToLeave == true && rt.teamToLeave != 0)) &&
...
void *TM_TeamMember::GetOwner(void) const { return owner; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_TeamMember::SetOwner(void *o) { owner = o; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

NoTeamId TM_TeamMember::GetNoTeamId(void) const { return noTeamSubcategory; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

unsigned int TM_TeamMember::GetWorldIndex(void) const { return world->GetTeamMemberIndex(this); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

unsigned long TM_TeamMember::ToUint32(const NetworkID &g) { return g & 0xFFFFFFFF; }
...
bool TM_TeamMember::JoinAnyTeamCheck(void) const {
   ...
   // - - If any team is already in requested teams, return false.
   if (teamsRequested.Size() == 0 && joinTeamType == JOIN_ANY_AVAILABLE_TEAM) return false;
...
bool TM_TeamMember::JoinSpecificTeamCheck(TM_Team *specificTeamToJoin, bool ignoreRequested) const {
   // - If already on specific team, return false
   if (IsOnTeam(specificTeamToJoin)) return false;

   if (ignoreRequested) return true;

   unsigned int i;
   for (i = 0; i < teamsRequested.Size(); i++) {
      if (teamsRequested[i].requested == specificTeamToJoin) {
         if (teamsRequested[i].isTeamSwitch == true) return true;  // Turn off team switch
...
bool TM_TeamMember::SwitchSpecificTeamCheck(TM_Team *teamToJoin, TM_Team *teamToLeave, bool ignoreRequested) const {
   ...
   // - If already on specific team, return false
   if (IsOnTeam(teamToJoin)) return false;

   if (teamToLeave != 0 && IsOnTeam(teamToLeave) == false) return false;

   if (teamToJoin == teamToLeave) return false;
   ...
   for (i = 0; i < teamsRequested.Size(); i++) {
      if (teamsRequested[i].requested == teamToJoin) {
         if (teamsRequested[i].isTeamSwitch == false) return true;  // Different - leave team was off, turn on

         if (teamsRequested[i].teamToLeave == teamToLeave) return false;  // Same thing - leave all or a specific team
...
bool TM_TeamMember::LeaveTeamCheck(TM_Team *team) const {
   if (IsOnTeam(team) == false) return false;
   return true;
}
...
void TM_TeamMember::UpdateTeamsRequestedToAny(void) {
   ...
   joinAnyRequestIndex = world->teamRequestIndex++;  // In case whenRequested is the same between
                                                     // two teams when sorting team requests
}
...
void TM_TeamMember::StoreLastTeams(void) { lastTeams = teams; }
...
TM_Team::~TM_Team() {
   if (world) world->DereferenceTeam(this, 0);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

bool TM_Team::SetMemberLimit(TeamMemberLimit _teamMemberLimit, NoTeamId noTeamId) {
   if (teamMemberLimit == _teamMemberLimit) return false;
...
TeamMemberLimit TM_Team::GetMemberLimit(void) const {
   if (world->GetBalanceTeams() == false) {
      ...
      if (limitWithBalancing < teamMemberLimit) return limitWithBalancing;
      return teamMemberLimit;
   }
}
...
TeamMemberLimit TM_Team::GetMemberLimitSetting(void) const { return teamMemberLimit; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

bool TM_Team::SetJoinPermissions(JoinPermissions _joinPermissions) {
   if (joinPermissions == _joinPermissions) return false;
...
JoinPermissions TM_Team::GetJoinPermissions(void) const { return joinPermissions; }
...
bool TM_Team::GetBalancingApplies(void) const { return balancingApplies; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_Team::GetTeamMembers(DataStructures::List<TM_TeamMember *> &_teamMembers) const { _teamMembers = teamMembers; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

unsigned int TM_Team::GetTeamMembersCount(void) const { return teamMembers.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_TeamMember *TM_Team::GetTeamMemberByIndex(unsigned int index) const { return teamMembers[index]; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

NetworkID TM_Team::GetNetworkID(void) const { return ID; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_World *TM_Team::GetTM_World(void) const { return world; }
...
unsigned long TM_Team::ToUint32(const NetworkID &g) { return g & 0xFFFFFFFF; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void *TM_Team::GetOwner(void) const { return owner; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

unsigned int TM_Team::GetWorldIndex(void) const { return world->GetTeamIndex(this); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_Team::SetOwner(void *o) { owner = o; }
...
TM_World::~TM_World() { Clear(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TeamManager *TM_World::GetTeamManager(void) const { return teamManager; }
...
void TM_World::RemoveParticipant(RakNetGUID rakNetGUID) {
   ...
   if (i != (unsigned int)-1) participants.RemoveAtIndex(i);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_World::SetAutoManageConnections(bool autoAdd) { autoAddParticipants = autoAdd; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_World::GetParticipantList(DataStructures::List<RakNetGUID> &participantList) { participantList = participants; }
...
unsigned int TM_World::GetTeamCount(void) const { return teams.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_Team *TM_World::GetTeamByIndex(unsigned int index) const { return teams[index]; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_Team *TM_World::GetTeamByNetworkID(NetworkID teamId) {
   DataStructures::HashIndex hi = teamsHash.GetIndexOf(teamId);
   if (hi.IsInvalid()) return 0;
...
void TM_World::ReferenceTeamMember(TM_TeamMember *teamMember, NetworkID networkId) {
   unsigned int i;
   for (i = 0; i < teamMembers.Size(); i++) {
      if (teamMembers[i] == teamMember) return;
   }
...
unsigned int TM_World::GetTeamMemberCount(void) const { return teamMembers.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_TeamMember *TM_World::GetTeamMemberByIndex(unsigned int index) const { return teamMembers[index]; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

NetworkID TM_World::GetTeamMemberIDByIndex(unsigned int index) const { return teamMembers[index]->GetNetworkID(); }
...
unsigned int TM_World::GetTeamMemberIndex(const TM_TeamMember *teamMember) const {
   unsigned int i;
   for (i = 0; i < teamMembers.Size(); i++) {
      if (teamMembers[i] == teamMember) return i;
   }
   return (unsigned int)-1;
}
...
bool TM_World::SetBalanceTeams(bool balanceTeams, NoTeamId noTeamId) {
   if (balanceTeams == balanceTeamsIsActive) return false;
...
bool TM_World::GetBalanceTeams(void) const { return balanceTeamsIsActive; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_World::SetHost(RakNetGUID _hostGuid) {
   if (hostGuid == _hostGuid) return;

   RakAssert(_hostGuid != UNASSIGNED_RAKNET_GUID);

   hostGuid = _hostGuid;

   if (GetHost() == GetTeamManager()->GetMyGUIDUnified()) FillRequestedSlots();
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

RakNetGUID TM_World::GetHost(void) const { return hostGuid; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

WorldId TM_World::GetWorldId(void) const { return worldId; }
...
void TM_World::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void TM_World::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming) {
   ...
   if (autoAddParticipants) AddParticipant(rakNetGUID);
}
...
void TM_World::KickExcessMembers(NoTeamId noTeamId) {
   ...
   // For each team that applies balancing, if the team is overfull, put on a team that is not
   // overfull if the team has ALLOW_JOIN_REBALANCING set If cannot move the player to another team,
   // just take the player off the team and set to noTeamId if they have no team at that point
...
void TM_World::FillRequestedSlots(void) {
   ...
   DataStructures::OrderedList<TM_World::JoinRequestHelper, TM_World::JoinRequestHelper, JoinRequestHelperComp>
       joinRequests;
...
unsigned int TM_World::GetAvailableTeamIndexWithFewestMembers(TeamMemberLimit secondaryLimit,
...
void TM_World::GetSortedJoinRequests(
    DataStructures::OrderedList<TM_World::JoinRequestHelper, TM_World::JoinRequestHelper, JoinRequestHelperComp>
        &joinRequests) {
   unsigned int i;
...
void TM_World::BroadcastToParticipants(RakNet::BitStream *bsOut, RakNetGUID exclusionGuid) {
   for (unsigned int i = 0; i < participants.Size(); i++) {
      if (participants[i] == exclusionGuid) continue;
...
int TM_World::JoinSpecificTeam(TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch, TM_Team *teamToLeave,
                               DataStructures::List<TM_Team *> &teamsWeAreLeaving) {
      ...
      if (balanceTeamsIsActive == false || teamsWeAreLeaving.Size() == 0) {
         ...
         if (teamsWeAreLeaving.Size() == 1) {
            ...
            if (j != (unsigned int)-1) {
               ...
               // Calling function sends ID_RUN_RemoveFromTeamsRequestedAndAddTeam which pushes
               // ID_TEAM_BALANCER_TEAM_ASSIGNED for teamMember
...
TeamMemberLimit TM_World::GetBalancedTeamLimit(void) const {
   ...
   if (balanceTeamsIsActive == false) return (TeamMemberLimit)-1;
...
TeamManager::TeamManager() {
   for (unsigned int i = 0; i < 255; i++) worldsArray[i] = 0;
   autoAddParticipants = true;
...
unsigned int TeamManager::GetWorldCount(void) const { return worldsList.Size(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_World *TeamManager::GetWorldAtIndex(unsigned int index) const { return worldsList[index]; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

TM_World *TeamManager::GetWorldWithId(WorldId worldId) const { return worldsArray[worldId]; }
...
void TeamManager::SetTopology(TMTopology _topology) { topology = _topology; }
...
void TeamManager::DecomposeTeamFull(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
   ...
   DecomposeTeamFullOrLocked(&bsIn, world, teamMember, team, currentMembers, memberLimitIncludingBalancing,
                             balancingIsActive, joinPermissions);
}
...
void TeamManager::DecomposeTeamFullOrLocked(RakNet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember,
                                            TM_Team **team, uint16_t &currentMembers,
                                            uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive,
                                            JoinPermissions &joinPermissions) {
   WorldId worldId;
...
void TeamManager::DecomposeTeamLocked(Packet *packet, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
                                      uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing,
                                      bool &balancingIsActive, JoinPermissions &joinPermissions) {
   BitStream bsIn(packet->data, packet->length, false);
   bsIn.IgnoreBytes(sizeof(MessageID));
...
void TeamManager::DecodeTeamCancelled(Packet *packet, TM_World **world, TM_TeamMember **teamMember,
   ...
   if (*world) {
      *teamMember = (*world)->GetTeamMemberByNetworkID(teamMemberId);
      ...
      for (unsigned int i = 0; i < teamsCount; i++) {
         ...
         RakAssert(team);
         if (team) newTeam.Push(team, _FILE_AND_LINE_);
      ...
      if (*teamMember) {
         for (unsigned int i = 0; i < (*teamMember)->teams.Size(); i++) {
            ...
            if (newTeam.GetIndexOf(team) == (unsigned int)-1) teamsLeft.Push(team, _FILE_AND_LINE_);
         }
      }

      for (unsigned int i = 0; i < newTeam.Size(); i++) {
         TM_Team *team = newTeam[i];
         if ((*teamMember)->teams.GetIndexOf(team) == (unsigned int)-1) teamsJoined.Push(team, _FILE_AND_LINE_);
      }
...
PluginReceiveResult TeamManager::OnReceive(Packet *packet) {
   switch (packet->data[0]) {
      case ID_FCM2_NEW_HOST: {
         unsigned int i;
         for (i = 0; i < worldsList.Size(); i++) worldsList[i]->SetHost(packet->guid);
      } break;
      ...
      case ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED: {
         ...
         if (world == 0) return RR_STOP_PROCESSING_AND_DEALLOCATE;
         bool validPacket = OnRemoveFromRequestedTeams(packet, world);
         if (validPacket == false) return RR_STOP_PROCESSING_AND_DEALLOCATE;
         break;
      }
...
void TeamManager::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
                                     PI2_LostConnectionReason lostConnectionReason) {
   for (unsigned int i = 0; i < worldsList.Size(); i++) {
...
void TeamManager::RemoveFromTeamsRequestedAndAddTeam(TM_TeamMember *teamMember, TM_Team *team, bool isTeamSwitch,
                                                     TM_Team *teamToLeave) {
...
void TeamManager::OnJoinAnyTeam(Packet *packet, TM_World *world) {
   ...
   if (teamMember) {
      ...
      if (resultCode == 1) {
         // Broadcast packet - remote systems should clear requested teams to none, and add the team
         // we joined. Broadcast includes non-host sender (all participants)
         BitStream bsOut;
         bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
         bsOut.WriteCasted<MessageID>(ID_RUN_UpdateTeamsRequestedToNoneAndAddTeam);
         bsOut.Write(world->GetWorldId());
         ...
         world->BroadcastToParticipants(&bsOut, UNASSIGNED_RAKNET_GUID);
         if (packet->guid != GetMyGUIDUnified()) PushBitStream(&bsOut);
      }
   }
}
...
void TeamManager::OnJoinRequestedTeam(Packet *packet, TM_World *world) {
   ...
   if (isTeamSwitch) {
      ...
      if (switchSpecificTeam) {
         ...
         if (teamToLeave == 0) isTeamSwitch = false;
      }
   }
   if (teamToJoin && teamMember) {
      if (isTeamSwitch) {
         ...
         if (teamMember->JoinSpecificTeamCheck(teamToJoin, packet->guid == GetMyGUIDUnified()) == false) return;
      ...
      if (isTeamSwitch) {
         if (teamToLeave == 0) {
            ...
            if (teamMember->IsOnTeam(teamToLeave)) teamsWeAreLeaving.Push(teamToLeave, _FILE_AND_LINE_);
         }

         if (teamsWeAreLeaving.Size() == 0) isTeamSwitch = false;
      }

      int resultCode = world->JoinSpecificTeam(teamMember, teamToJoin, isTeamSwitch, teamToLeave, teamsWeAreLeaving);

      if (resultCode == 1) {
         // Broadcast packet - remote systems should remove from requested teams and add the team we
         // joined. Broadcast includes non-host sender (all participants)
         BitStream bsOut;
         bsOut.WriteCasted<MessageID>(ID_TEAM_BALANCER_INTERNAL);
         bsOut.WriteCasted<MessageID>(ID_RUN_RemoveFromTeamsRequestedAndAddTeam);
         bsOut.Write(world->GetWorldId());
         ...
         if (isTeamSwitch) {
            ...
            if (switchSpecificTeam) bsOut.Write(teamToLeaveNetworkId);
         }
         world->BroadcastToParticipants(&bsOut, packet->guid);
...
void TeamManager::OnLeaveTeam(Packet *packet, TM_World *world) {
   ...
   if (team && teamMember) {
      if (teamMember->LeaveTeamCheck(team) == false) return;
      ...
      if (world->GetHost() == world->GetTeamManager()->GetMyGUIDUnified()) {
         ...
         // Relay as host
         if (topology == TM_CLIENT_SERVER) world->BroadcastToParticipants(packet->data, packet->length, packet->guid);
      }
   }
}
