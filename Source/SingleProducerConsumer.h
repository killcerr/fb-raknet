...
/// \file
/// \brief \b [Internal] Passes queued data between threads using a circular buffer with read and
/// write pointers
///
...
class RAK_DLL_EXPORT SingleProducerConsumer {
  public:
   ...
   /// WriteLock must be immediately followed by WriteUnlock.  These two functions must be called in
   /// the same thread. \return A pointer to a block of data you can write to.
   SingleProducerConsumerType *WriteLock(void);

   /// Call if you don't want to write to a block of data from WriteLock() after all.
   /// Cancelling locks cancels all locks back up to the data passed.  So if you lock twice and
   /// cancel using the first lock, the second lock is ignored \param[in] cancelToLocation Which
   /// WriteLock() to cancel.
   ...
   /// ReadLock must be immediately followed by ReadUnlock. These two functions must be called in
   /// the same thread. \retval 0 No data is availble to read \retval Non-zero The data previously
   /// written to, in another thread, by WriteLock followed by WriteUnlock.
   SingleProducerConsumerType *ReadLock(void);

   // Cancelling locks cancels all locks back up to the data passed.  So if you lock twice and
   // cancel using the first lock, the second lock is ignored
   ...
   /// Clear is not thread-safe and none of the lock or unlock functions should be called while it
   /// is running.
   void Clear(void);

   /// This function will estimate how many elements are waiting to be read.  It's threadsafe enough
   /// that the value returned is stable, but not threadsafe enough to give accurate results.
  ...
  private:
   struct DataPlusPtr {
      ...
      // Ready to read is so we can use an equality boolean comparison, in case the writePointer var
      // is trashed while context switching.
...
SingleProducerConsumerType *SingleProducerConsumer<SingleProducerConsumerType>::WriteLock(void) {
   if (writeAheadPointer->next == readPointer || writeAheadPointer->next->readyToRead == true) {
