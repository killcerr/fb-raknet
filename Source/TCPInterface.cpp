...
/// \file
/// \brief A simple TCP based server allowing sends and receives.  Can be connected to by a telnet
/// client.
///
...
bool TCPInterface::CreateListenSocket(unsigned short port, unsigned short maxIncomingConnections,
   ...
   if ((int)listenSocket == -1) return false;
   ...
   if (bind__(listenSocket, (struct sockaddr *)&serverAddress, sizeof(serverAddress)) < 0) return false;
...
#if defined(WINDOWS_STORE_RT)
bool TCPInterface::CreateListenSocket_WinStore8(unsigned short port, unsigned short maxIncomingConnections,
                                                unsigned short socketFamily, const char *bindAddress) {
...
bool TCPInterface::Start(unsigned short port, unsigned short maxIncomingConnections, unsigned short maxConnections,
   ...
   if (isStarted.GetValue() > 0) return false;
   ...
   isStarted.Increment();
   if (maxConnections == 0) maxConnections = maxIncomingConnections;
   ...
   while (threadRunning.GetValue() == 0) RakSleep(0);

   unsigned int i;
   for (i = 0; i < messageHandlerList.Size(); i++) messageHandlerList[i]->OnRakPeerStartup();
...
void TCPInterface::Stop(void) {
   unsigned int i;
   for (i = 0; i < messageHandlerList.Size(); i++) messageHandlerList[i]->OnRakPeerShutdown();

#ifndef __native_client__
   if (isStarted.GetValue() == 0) return;

#if OPEN_SSL_CLIENT_SUPPORT == 1
   for (i = 0; i < remoteClientsLength; i++) remoteClients[i].DisconnectSSL();
#endif
   ...
   while (threadRunning.GetValue() > 0) RakSleep(15);
   ...
   failedConnectionAttempts.Clear(_FILE_AND_LINE_);
   for (i = 0; i < headPush.Size(); i++) DeallocatePacket(headPush[i]);
   headPush.Clear(_FILE_AND_LINE_);
   for (i = 0; i < tailPush.Size(); i++) DeallocatePacket(tailPush[i]);
...
SystemAddress TCPInterface::Connect(const char *host, unsigned short remotePort, bool block,
                                    unsigned short socketFamily, const char *bindAddress) {
   if (threadRunning.GetValue() == 0) return UNASSIGNED_SYSTEM_ADDRESS;
   ...
   if (newRemoteClientIndex == -1) return UNASSIGNED_SYSTEM_ADDRESS;
...
void TCPInterface::StartSSLClient(SystemAddress systemAddress) {
   ...
   *id = systemAddress;
   ...
   if (index == (unsigned)-1) activeSSLConnections.Insert(systemAddress, _FILE_AND_LINE_);
}
...
bool TCPInterface::SendList(const char **data, const unsigned int *lengths, const int numParameters,
   ...
   if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS && broadcast == false) return false;
...
Packet *TCPInterface::Receive(void) {
   unsigned int i;
   for (i = 0; i < messageHandlerList.Size(); i++) messageHandlerList[i]->Update();
...
Packet *TCPInterface::ReceiveInt(void) {
   ...
   if (headPush.IsEmpty() == false) return headPush.Pop();
   Packet *p = incomingMessages.PopInaccurate();
   if (p) return p;
   if (tailPush.IsEmpty() == false) return tailPush.Pop();
   return 0;
}
...
void TCPInterface::CloseConnection(SystemAddress systemAddress) {
   if (isStarted.GetValue() == 0) return;
   if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS) return;
...
#if OPEN_SSL_CLIENT_SUPPORT == 1
   unsigned index = activeSSLConnections.GetIndexOf(systemAddress);
   if (index != (unsigned)-1) activeSSLConnections.RemoveAtIndex(index);
#endif
}
...
bool TCPInterface::WasStarted(void) const { return threadRunning.GetValue() > 0; }

SystemAddress TCPInterface::HasCompletedConnectionAttempt(void) {
   SystemAddress sysAddr = UNASSIGNED_SYSTEM_ADDRESS;
   completedConnectionAttemptMutex.Lock();
   if (completedConnectionAttempts.IsEmpty() == false) sysAddr = completedConnectionAttempts.Pop();
   completedConnectionAttemptMutex.Unlock();
...
SystemAddress TCPInterface::HasFailedConnectionAttempt(void) {
   SystemAddress sysAddr = UNASSIGNED_SYSTEM_ADDRESS;
   failedConnectionAttemptMutex.Lock();
   if (failedConnectionAttempts.IsEmpty() == false) sysAddr = failedConnectionAttempts.Pop();
   failedConnectionAttemptMutex.Unlock();
...
void TCPInterface::GetConnectionList(SystemAddress *remoteSystems, unsigned short *numberOfSystems) const {
   ...
   for (int i = 0; i < remoteClientsLength; i++) {
      if (remoteClients[i].isActive) {
         if (systemCount < maxToWrite) remoteSystems[systemCount] = remoteClients[i].systemAddress;
         systemCount++;
      }
   }
...
unsigned short TCPInterface::GetConnectionCount(void) const {
   unsigned short systemCount = 0;
   for (int i = 0; i < remoteClientsLength; i++) {
      if (remoteClients[i].isActive) systemCount++;
   }
   return systemCount;
}
...
unsigned int TCPInterface::GetOutgoingDataBufferSize(SystemAddress systemAddress) const {
   ...
   if (systemAddress.systemIndex < remoteClientsLength && remoteClients[systemAddress.systemIndex].isActive &&
       remoteClients[systemAddress.systemIndex].systemAddress == systemAddress) {
...
__TCPSOCKET__ TCPInterface::SocketConnect(const char *host, unsigned short remotePort, unsigned short socketFamily,
   ...
   if (connectResult == -1) {
      ...
      if (sockfdIndex != (unsigned)-1) blockingSocketList.RemoveAtIndexFast(sockfdIndex);
      blockingSocketListMutex.Unlock();
...
RAK_THREAD_DECLARATION(RakNet::UpdateTCPInterfaceLoop) {
   ...
   while (sts->isStarted.GetValue() > 0) {
      ...
      if (sslSystemAddress) {
         if (sslSystemAddress->systemIndex >= 0 && sslSystemAddress->systemIndex < sts->remoteClientsLength &&
            ...
            for (int i = 0; i < sts->remoteClientsLength; i++) {
               ...
               if (sts->remoteClients[i].isActive && sts->remoteClients[i].systemAddress == *sslSystemAddress) {
                  if (sts->remoteClients[i].ssl == 0) sts->remoteClients[i].InitSSL(sts->ctx, sts->meth);
               }
      ...
      while (1) {
         ...
         for (i = 0; i < (unsigned int)sts->remoteClientsLength; i++) {
            ...
            if (sts->remoteClients[i].isActive) {
               // calling FD_ISSET with -1 as socket (that’s what 0 is set to) produces a bus error
               // under Linux 64-Bit
               __TCPSOCKET__ socketCopy = sts->remoteClients[i].socket;
               if (socketCopy != 0) {
                  FD_SET(socketCopy, &readFD);
                  FD_SET(socketCopy, &exceptionFD);
                  if (sts->remoteClients[i].outgoingData.GetBytesWritten() > 0) FD_SET(socketCopy, &writeFD);
               ...
               for (newRemoteClientIndex = 0; newRemoteClientIndex < sts->remoteClientsLength; newRemoteClientIndex++) {
                  ...
                  if (sts->remoteClients[newRemoteClientIndex].isActive == false) {
...
#if RAKNET_SUPPORT_IPV6 != 1
                     sts->remoteClients[newRemoteClientIndex].systemAddress.address.addr4.sin_addr.s_addr =
                     ...
                     if (sockAddr.ss_family == AF_INET) {
                        memcpy(&sts->remoteClients[newRemoteClientIndex].systemAddress.address.addr4,
                        ...
                        memcpy(&sts->remoteClients[newRemoteClientIndex].systemAddress.address.addr6,
...

         {
            ...
            while (i < (unsigned int)sts->remoteClientsLength) {
               ...
               if (FD_ISSET(socketCopy, &exceptionFD)) {
                  ...
                  // 							int err;
                  // 							int errlen = sizeof(err);
                  // 							getsockopt__(sts->listenSocket,
                  // SOL_SOCKET, SO_ERROR,(char*)&err, &errlen);
                  // in_addr in; 							in.s_addr =
                  ...
                  *lostConnectionSystemAddress = sts->remoteClients[i].systemAddress;
                  ...
                  if (FD_ISSET(socketCopy, &readFD)) {
                     ...
                     if (len > 0) {
                        ...
                        incomingMessage->data[len] =
                            0;  // Null terminate this so we can print it out as regular strings.
                                // This is different from RakNet which does not do this.
                        ...
                        */
                        ...
                        incomingMessage->deleteData =
                            true;  // actually means came from SPSC, rather than AllocatePacket
                  ...
                  if (FD_ISSET(socketCopy, &writeFD)) {
                     ...
                     if (bytesInBuffer > 0) {
                        ...
                        if (bytesSent > 0) rc->outgoingData.IncrementReadOffset(bytesSent);
                        bytesInBuffer = rc->outgoingData.GetBytesWritten();
                     }
...
bool RemoteClient::InitSSL(SSL_CTX *ctx, SSL_METHOD *meth) {
   ...
   if (res < 0) {
      ...
      // The TLS/SSL handshake was not successful but was shut down controlled and by the
      // specifications of the TLS/SSL protocol. Call SSL_get_error() with the return value ret to
      // find out the reason.
