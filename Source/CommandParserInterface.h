...
/// \details CommandParserInterface provides a set of functions and interfaces that plug into the
/// ConsoleServer class. Each CommandParserInterface works at the same time as other interfaces in
/// the system.
class RAK_DLL_EXPORT CommandParserInterface {
  public:
   ...
   /// \param[in] transport The transport interface that sent us this information.  Can be used to
   /// send messages to this or other players.
   ...
   /// \brief A callback for when you are expected to send a brief description of your parser to \a
   ...
   /// \param[in] parameterList The list of parameters.  parameterList[0] is the first parameter and
   /// so on. \param[in] transport The transport interface we can use to write to \param[in]
   ...
   virtual bool OnCommand(const char *command, unsigned numParameters, char **parameterList,
   ...
   /// \param[out] parameterList An array of pointers to characters.  Will hold pointers to
   /// locations inside \a str \param[in] parameterListLength How big the \a parameterList array is
   static void ParseConsoleString(char *str, const char delineator, unsigned char delineatorToggle,
                                  unsigned *numParameters, char **parameterList, unsigned parameterListLength);
   ...
   /// Registers a command.
   /// \param[in] parameterCount How many parameters your command requires.  If you want to accept a
   /// variable number of commands, pass CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS
   /// \param[in] command A pointer to a STATIC string that has your command.  I keep a copy of the
   /// pointer here so don't deallocate the string. \param[in] commandHelp A pointer to a STATIC
   /// string that has the help information for your command.  I keep a copy of the pointer here so
   ...
   /// \brief Just writes a string to the remote system based on the result ( \a res ) of your
   /// operation \details This is not necessary to call, but makes it easier to return results of
   /// function calls. \param[in] res The result to write \param[in] command The command that this
   /// result came from \param[in] transport The transport interface that will be written to
   /// \param[in] systemAddress The player this result will be sent to
   virtual void ReturnResult(bool res, const char *command, TransportInterface *transport,
                             const SystemAddress &systemAddress);
   virtual void ReturnResult(char *res, const char *command, TransportInterface *transport,
                             const SystemAddress &systemAddress);
   virtual void ReturnResult(SystemAddress res, const char *command, TransportInterface *transport,
                             const SystemAddress &systemAddress);
   virtual void ReturnResult(int res, const char *command, TransportInterface *transport,
                             const SystemAddress &systemAddress);

   /// \brief Just writes a string to the remote system when you are calling a function that has no
   /// return value. \details This is not necessary to call, but makes it easier to return results
   /// of function calls. \param[in] res The result to write \param[in] command The command that
   /// this result came from \param[in] transport The transport interface that will be written to
   /// \param[in] systemAddress The player this result will be sent to
