...
RelayPlugin::RelayPlugin() { acceptAddParticipantRequests = false; }
...
RelayPlugin::~RelayPlugin() {
   ...
   for (unsigned int i = 0; i < itemList.Size(); i++) RakNet::OP_DELETE(itemList[i], _FILE_AND_LINE_);
...
RelayPluginEnums RelayPlugin::AddParticipantOnServer(const RakString &key, const RakNetGUID &guid) {
   ...
   if (cs != IS_CONNECTED) return RPE_ADD_CLIENT_TARGET_NOT_CONNECTED;

   if (strToGuidHash.HasData(key) == true) return RPE_ADD_CLIENT_NAME_ALREADY_IN_USE;  // Name already in use
...
void RelayPlugin::SetAcceptAddParticipantRequests(bool accept) { acceptAddParticipantRequests = accept; }
...
// Send a message to a server running RelayPlugin, to forward a message to the system identified by
// \a key
void RelayPlugin::SendToParticipant(const RakNetGUID &relayPluginServerGuid, const RakString &key, BitStream *bitStream,
...
void RelayPlugin::SendGroupMessage(const RakNetGUID &relayPluginServerGuid, BitStream *bitStream,
                                   PacketPriority priority, PacketReliability reliability, char orderingChannel) {
   BitStream bsOut;
   bsOut.WriteCasted<MessageID>(ID_RELAY_PLUGIN);
...
void RelayPlugin::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
RelayPlugin::RP_Group *RelayPlugin::JoinGroup(RP_Group *room, StrAndGuidAndRoom **strAndGuidSender) {
   if (strAndGuidSender == 0) return 0;
...
RelayPlugin::RP_Group *RelayPlugin::JoinGroup(RakNetGUID userGuid, RakString roomName) {
   ...
   if (strAndGuidSender) {
      if (roomName.IsEmpty()) return 0;

      if ((*strAndGuidSender)->currentRoom == roomName) return 0;

      if ((*strAndGuidSender)->currentRoom.IsEmpty() == false) LeaveGroup(strAndGuidSender);
...
void RelayPlugin::SendMessageToRoom(StrAndGuidAndRoom **strAndGuidSender, BitStream *message) {
   if ((*strAndGuidSender)->currentRoom.IsEmpty()) return;
...
void RelayPlugin::OnLeaveGroupRequestFromClient(Packet *packet) {
   ...
   bsIn.IgnoreBytes(sizeof(MessageID) * 2);
   StrAndGuidAndRoom **strAndGuidSender = guidToStrHash.Peek(packet->guid);
   if (strAndGuidSender) LeaveGroup(strAndGuidSender);
}
