...
/// \details This should not be used at the same time as RPC3. This is a less functional version of
/// RPC3, and is here for users that do not want the Boost dependency of RPC3. \ingroup
/// PLUGINS_GROUP
...
/// \brief Instantiate this class globally if you want to register a function with RPC4 at the
/// global space
class RAK_DLL_EXPORT RPC4GlobalRegistration {
  public:
   /// \brief Queue a call to RPC4::RegisterFunction() globally. Actual call occurs once RPC4 is
   /// attached to an instance of RakPeer or TCPInterface.
   RPC4GlobalRegistration(const char *uniqueID, void (*functionPointer)(RakNet::BitStream *userData, Packet *packet));

   /// \brief Queue a call to RPC4::RegisterSlot() globally. Actual call occurs once RPC4 is
   /// attached to an instance of RakPeer or TCPInterface.
   RPC4GlobalRegistration(const char *uniqueID, void (*functionPointer)(RakNet::BitStream *userData, Packet *packet),
                          int callPriority);

   /// \brief Queue a call to RPC4::RegisterBlockingFunction() globally. Actual call occurs once
   /// RPC4 is attached to an instance of RakPeer or TCPInterface.
   RPC4GlobalRegistration(const char *uniqueID, void (*functionPointer)(RakNet::BitStream *userData,
                                                                        RakNet::BitStream *returnData, Packet *packet));

   /// \brief Queue a call to RPC4::RegisterLocalCallback() globally. Actual call occurs once RPC4
   /// is attached to an instance of RakPeer or TCPInterface.
...
/// You do not have the automatic serialization or other features of RPC3, and C++ member calls are
/// not supported. \note You cannot use RPC4 at the same time as RPC3Plugin \ingroup
/// RPC_PLUGIN_GROUP
class RAK_DLL_EXPORT RPC4 : public PluginInterface2 {
  public:
   ...
   /// \details The hash of the function name will be stored as an association with the function
   /// pointer When a call is made to call this function from the \a Call() or CallLoopback()
   /// function, the function pointer will be invoked with the passed bitStream to Call() and the
   /// actual Packet that RakNet got. \sa RegisterPacketCallback() \param[in] uniqueID Identifier to
   /// be associated with \a functionPointer. If this identifier is already in use, the call will
   /// return false. \param[in] functionPointer C function pointer to be called \return True if the
   /// hash of uniqueID is not in use, false otherwise.
   bool RegisterFunction(const char *uniqueID, void (*functionPointer)(RakNet::BitStream *userData, Packet *packet));

   /// Register a slot, which is a function pointer to one or more implementations that supports
   /// this function signature When a signal occurs, all slots with the same identifier are called.
   /// \param[in] sharedIdentifier A string to identify the slot. Recommended to be the same as the
   /// name of the function. \param[in] functionPtr Pointer to the function. For C, just pass the
   /// name of the function. For C++, use ARPC_REGISTER_CPP_FUNCTION \param[in] callPriority Slots
   /// are called by order of the highest callPriority first. For slots with the same priority, they
   /// are called in the order they are registered
   void RegisterSlot(const char *sharedIdentifier, void (*functionPointer)(RakNet::BitStream *userData, Packet *packet),
                     int callPriority);

   /// \brief Same as \a RegisterFunction, but is called with CallBlocking() instead of Call() and
   /// returns a value to the caller
   bool RegisterBlockingFunction(const char *uniqueID,
                                 void (*functionPointer)(RakNet::BitStream *userData, RakNet::BitStream *returnData,
                                                         Packet *packet));

   /// \deprecated Use RegisterSlot and invoke on self only when the packet you want arrives
   /// When a RakNet Packet with the specified identifier is returned, execute CallLoopback() on a
   /// function previously registered with RegisterFunction() For example, you could call
   /// "OnClosedConnection" whenever you get ID_DISCONNECTION_NOTIFICATION or ID_CONNECTION_LOST
   /// \param[in] uniqueID Identifier passed to RegisterFunction()
   /// \param[in] messageId What RakNet packet ID to call on, for example
   /// ID_DISCONNECTION_NOTIFICATION or ID_CONNECTION_LOST
   ...
   /// \param[in] uniqueID Identifier originally passed to RegisterFunction() on the remote
   /// system(s) \param[in] bitStream bitStream encoded data to send to the function callback
   ...
   void Call(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
   ...
   /// \note This function does not return until the remote system responds, disconnects, or was
   /// never connected to begin with \param[in] Identifier originally passed to
   /// RegisterBlockingFunction() on the remote system(s) \param[in] bitStream bitStream encoded
   /// data to send to the function callback \param[in] priority See RakPeerInterface::Send()
   /// \param[in] reliability See RakPeerInterface::Send() \param[in] orderingChannel See
   /// RakPeerInterface::Send() \param[in] systemIdentifier See RakPeerInterface::Send() \param[out]
   /// returnData Written to by the function registered with RegisterBlockingFunction. \return true
   /// if successfully called. False on disconnect, function not registered, or not connected to
   /// begin with
   bool CallBlocking(const char *uniqueID, RakNet::BitStream *bitStream, PacketPriority priority,
   ...
   /// \param[in] sharedIdentifier parameter of the same name passed to RegisterSlot() on the remote
   /// system \param[in] bitStream bitStream encoded data to send to the function callback
   /// \param[in] priority See RakPeerInterface::Send() \param[in] reliability See
   /// RakPeerInterface::Send() \param[in] orderingChannel See RakPeerInterface::Send() \param[in]
   /// systemIdentifier See RakPeerInterface::Send() \param[in] broadcast See
   /// RakPeerInterface::Send() \param[in] invokeLocal If true, also sends to self.
   void Signal(const char *sharedIdentifier, RakNet::BitStream *bitStream, PacketPriority priority,
               PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
               bool broadcast, bool invokeLocal);

   /// If called while processing a slot, no further slots for the currently executing signal will
   /// be executed
  ...
  protected:
   ...
   DataStructures::Hash<RakNet::RakString, void (*)(RakNet::BitStream *, Packet *), 64, RakNet::RakString::ToInteger>
       registeredNonblockingFunctions;
   DataStructures::Hash<RakNet::RakString, void (*)(RakNet::BitStream *, RakNet::BitStream *, Packet *), 64,
