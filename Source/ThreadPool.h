...
/// This class does not allocate or deallocate memory.  It is up to the user to handle memory
/// management. InputType and OutputType are stored directly in a queue.  For large structures, if
/// you plan to delete from the middle of the queue, you might wish to store pointers rather than
/// the structures themselves so the array can shift efficiently.
...
struct RAK_DLL_EXPORT ThreadPool {
   ...
   /// \param[in] _perThreadInit User callback to return data stored per thread.  Pass 0 if not
   /// needed. \param[in] _perThreadDeinit User callback to destroy data stored per thread, created
   /// by _perThreadInit.  Pass 0 if not needed. \return True on success, false on failure.
   bool StartThreads(int numThreads, int stackSize, void *(*_perThreadInit)() = 0,
   ...
   /// Adds a function to a queue with data to pass to that function.  This function will be called
   /// from the thread Memory management is your responsibility!  This class does not allocate or
   /// deallocate memory. The best way to deallocate \a inputData is in userCallback.  If you call
   /// EndThreads such that callbacks were not called, you can iterate through the inputQueue and
   /// deallocate all pending input data there The best way to deallocate output is as it is
   /// returned to you from GetOutput.  Similarly, if you end the threads such that not all output
   /// was returned, you can iterate through outputQueue and deallocate it there. \param[in]
   /// workerThreadCallback The function to call from the thread \param[in] inputData The parameter
   /// to pass to \a userCallback
   void AddInput(OutputType (*workerThreadCallback)(InputType, bool *returnOutput, void *perThreadData),
                 InputType inputData);

   /// Adds to the output queue
   /// Use it if you want to inject output into the same queue that the system uses. Normally you
   /// would not use this. Consider it a convenience function. \param[in] outputData The output to
   /// inject
   ...
   /// Inaccurate but fast version of HasOutput.  If this returns true, you should still check
   /// HasOutput for the real value. \return true if output is probably waiting, false otherwise
   ...
   /// Inaccurate but fast version of HasInput.  If this returns true, you should still check
   /// HasInput for the real value. \return true if input is probably waiting, false otherwise
   ...
   /// \return The output of \a userCallback.  If you have different output signatures, it is up to
   /// you to encode the data to indicate this
   OutputType GetOutput(void);

   /// Clears internal buffers
   void Clear(void);

   /// Lock the input buffer before calling the functions InputSize, InputAtIndex, and
   /// RemoveInputAtIndex It is only necessary to lock the input or output while the threads are
   /// running
   void LockInput(void);
   ...
   /// Remove input from a specific index.  This does NOT do memory deallocation - it only removes
   /// the item from the queue
   void RemoveInputAtIndex(unsigned index);

   /// Lock the output buffer before calling the functions OutputSize, OutputAtIndex, and
   /// RemoveOutputAtIndex It is only necessary to lock the input or output while the threads are
   /// running
   void LockOutput(void);

   /// Unlock the output buffer after you are done with the functions OutputSize, GetOutputAtIndex,
   /// and RemoveOutputAtIndex
   ...
   /// Remove output from a specific index.  This does NOT do memory deallocation - it only removes
   /// the item from the queue
  ...
  protected:
   // It is valid to cancel input before it is processed.  To do so, lock the inputQueue with
   // inputQueueMutex, Scan the list, and remove the item you don't want.
   ...
   // inputFunctionQueue & inputQueue are paired arrays so if you delete from one at a particular
   // index you must delete from the other at the same index
...
bool ThreadPool<InputType, OutputType>::StartThreads(int numThreads, int stackSize, void *(*_perThreadDataFactory)(),
   ...
   while (done == false) {
      ...
      if (numThreadsRunning == numThreads) done = true;
...
void ThreadPool<InputType, OutputType>::StopThreads(void) {
   ...
   while (done == false) {
      ...
      RakSleep(50);
      numThreadsRunningMutex.Lock();
      if (numThreadsRunning == 0) done = true;
      numThreadsRunningMutex.Unlock();
   }
...
template <class InputType, class OutputType>
void ThreadPool<InputType, OutputType>::AddInput(OutputType (*workerThreadCallback)(InputType, bool *returnOutput,
                                                                                    void *perThreadData),
                                                 InputType inputData) {
...
bool ThreadPool<InputType, OutputType>::IsWorking(void) {
   ...
   // It's possible with the thread timing that working could have been false, then it picks up the
   // data in the other thread, then it checks here and sees there is no data.  So it thinks the
   // thread is not working when it was.
