...
class RakStringCleanup {
  public:
   ~RakStringCleanup() { RakNet::RakString::FreeMemoryNoMutex(); }
};
...
int RakNet::RakString::RakStringComp(RakString const &key, RakString const &data) { return key.StrCmp(data); }

RakString::RakString() { sharedString = &emptyString; }

RakString::RakString(RakString::SharedString *_sharedString) { sharedString = _sharedString; }
...
RakNet::RakString::RakString(std::string &string) { RakString(string.c_str()); }

RakString::~RakString() { Free(); }

RakString &RakString::operator=(const RakString &rhs) {
   Free();
   if (rhs.sharedString == &emptyString) return *this;
...
RakString &RakString::operator=(char *str) { return operator=((const char *)str); }

RakString &RakString::operator=(const unsigned char *str) { return operator=((const char *)str); }

RakString &RakString::operator=(char unsigned *str) { return operator=((const char *)str); }
...
void RakString::Realloc(SharedString *sharedString, size_t bytes) {
   if (bytes <= sharedString->bytesUsed) return;
...
RakString &RakString::operator+=(unsigned char *str) { return operator+=((const char *)str); }
...
bool RakString::operator==(const char *str) const { return strcmp(sharedString->c_str, str) == 0; }

bool RakString::operator==(char *str) const { return strcmp(sharedString->c_str, str) == 0; }

bool RakString::operator<(const RakString &right) const { return strcmp(sharedString->c_str, right.C_String()) < 0; }
...
const char *RakString::ToLower(void) {
   ...
   unsigned i;
   for (i = 0; i < strLen; i++) sharedString->c_str[i] = ToLower(sharedString->c_str[i]);
...
const char *RakString::ToUpper(void) {
   ...
   size_t strLen = strlen(sharedString->c_str);
   unsigned i;
   for (i = 0; i < strLen; i++) sharedString->c_str[i] = ToUpper(sharedString->c_str[i]);
   return sharedString->c_str;
}
...
bool RakString::IsEmpty(void) const { return sharedString == &emptyString; }

size_t RakString::GetLength(void) const { return strlen(sharedString->c_str); }
...
size_t RakString::GetLengthUTF8(void) const { return porges_strlen2(sharedString->c_str); }
...
void RakString::DeallocWideChar(WCHAR *w) { RakNet::OP_DELETE_ARRAY(w, __FILE__, __LINE__); }
...
RakString RakString::SubStr(unsigned int index, unsigned int count) const {
   ...
   if (index >= length || count == 0) return RakString();
   RakString copy;
   size_t numBytes = length - index;
   if (count < numBytes) numBytes = count;
   copy.Allocate(numBytes + 1);
   size_t i;
   for (i = 0; i < numBytes; i++) copy.sharedString->c_str[i] = sharedString->c_str[index + i];
...
void RakString::RemoveCharacter(char c) {
   ...
   if (writeIndex == 0) Clear();
}

int RakString::StrCmp(const RakString &rhs) const { return strcmp(sharedString->c_str, rhs.C_String()); }

int RakString::StrNCmp(const RakString &rhs, size_t num) const {
   return strncmp(sharedString->c_str, rhs.C_String(), num);
}

int RakString::StrICmp(const RakString &rhs) const { return _stricmp(sharedString->c_str, rhs.C_String()); }

void RakString::Printf(void) { RAKNET_DEBUG_PRINTF("%s", sharedString->c_str); }

void RakString::FPrintf(FILE *fp) { fprintf(fp, "%s", sharedString->c_str); }
...
bool RakString::ContainsNonprintableExceptSpaces(void) const {
   ...
   unsigned i;
   for (i = 0; i < strLen; i++) {
      if (sharedString->c_str[i] < ' ' || sharedString->c_str[i] > 126) return true;
   }
   return false;
}
...
bool RakString::IsEmailAddress(void) const {
   ...
   // Has non-printable?
   for (i = 0; i < strLen; i++) {
      if (sharedString->c_str[i] <= ' ' || sharedString->c_str[i] > 126) return false;
   }
   int atCount = 0;
   for (i = 0; i < strLen; i++) {
...
RakNet::RakString &RakString::URLDecode(void) {
   ...
   for (i = 0; i < strLen; i++) {
      ...
      if (c == '%') {
         ...
         if (hexDigits[0] == ' ') hexValues[0] = 0;

         if (hexDigits[0] >= 'A' && hexDigits[0] <= 'F') hexValues[0] = hexDigits[0] - 'A' + 10;
         if (hexDigits[0] >= 'a' && hexDigits[0] <= 'f')
            hexValues[0] = hexDigits[0] - 'a' + 10;
         else
            hexValues[0] = hexDigits[0] - '0';

         if (hexDigits[1] >= 'A' && hexDigits[1] <= 'F') hexValues[1] = hexDigits[1] - 'A' + 10;
...
void RakString::SplitURI(RakNet::RakString &header, RakNet::RakString &domain, RakNet::RakString &path) {
   ...
   if (strncmp(sharedString->c_str, "www.", 4) == 0) i += (unsigned int)strlen("www.");
...
RakNet::RakString &RakString::SQLEscape(void) {
   ...
   for (index = 0; index < strLen; index++) {
      if (sharedString->c_str[index] == '\'' || sharedString->c_str[index] == '"' || sharedString->c_str[index] == '\\')
         escapedCharacterCount++;
   }
   if (escapedCharacterCount == 0) return *this;
   ...
   readIndex = strLen;
   while (readIndex >= 0) {
      if (sharedString->c_str[readIndex] == '\'' || sharedString->c_str[readIndex] == '"' ||
...
RakNet::RakString RakString::FormatForPUTOrPost(const char *type, const char *uri, const char *contentType,
   ...
   if (host.IsEmpty() || remotePath.IsEmpty()) return out;
   ...
   if (extraHeaders != 0 && extraHeaders[0]) {
      out.Set(
          ...
          "\r\n"
          "%s",
          type, remotePath.C_String(), extraHeaders, host.C_String(), contentType,
      ...
      out.Set(
          ...
          "\r\n"
          "%s",
          type, remotePath.C_String(), host.C_String(), contentType,
          // bodyEncoded.GetLength(),
          // bodyEncoded.C_String());
          strlen(body), body);
   }
...
RakString RakString::FormatForPOST(const char *uri, const char *contentType, const char *body,
                                   const char *extraHeaders) {
   return FormatForPUTOrPost("POST", uri, contentType, body, extraHeaders);
}

RakString RakString::FormatForPUT(const char *uri, const char *contentType, const char *body,
                                  const char *extraHeaders) {
...
RakString RakString::FormatForGET(const char *uri, const char *extraHeaders) {
   ...
   if (extraHeaders && extraHeaders[0]) {
      out.Set(
          ...
          "Host: %s\r\n"
          "\r\n",
          remotePath.C_String(), extraHeaders, host.C_String());
   } else {
      out.Set(
          "GET %s HTTP/1.1\r\n"
          "Host: %s\r\n"
...
RakNet::RakString &RakString::MakeFilePath(void) {
...
#ifdef _WIN32
      if (fixedString.sharedString->c_str[i] == '/') fixedString.sharedString->c_str[i] = '\\';
#else
      if (fixedString.sharedString->c_str[i] == '\\') fixedString.sharedString->c_str[i] = '/';
#endif
   }
   ...
   if (fixedString != *this) *this = fixedString;
   return *this;
}
...
void RakString::Serialize(BitStream *bs) const { Serialize(sharedString->c_str, bs); }
...
void RakString::SerializeCompressed(const char *str, BitStream *bs, uint8_t languageId, bool writeLanguageId) {
   if (writeLanguageId) bs->WriteCompressed(languageId);
...
bool RakString::Deserialize(char *str, BitStream *bs) {
   ...
   unsigned short l;
   b = bs->Read(l);
   if (b && l > 0) b = bs->ReadAlignedBytes((unsigned char *)str, l);
...
void RakString::Clear(void) { Free(); }
...
void RakString::Allocate(size_t len) {
   ...
   if (RakString::freeList.Size() == 0) {
      ...
      for (i = 0; i < 128; i++) {
         //	RakString::freeList.Insert(RakString::sharedStringFreeList+i+RakString::sharedStringFreeListAllocationCount);
         //		RakString::freeList.Insert((RakString::SharedString*)rakMalloc_Ex(sizeof(RakString::SharedString),
         //_FILE_AND_LINE_), _FILE_AND_LINE_);
...
unsigned long RakString::ToInteger(const RakString &rs) { return RakString::ToInteger(rs.C_String()); }

int RakString::ReadIntFromSubstring(const char *str, size_t pos, size_t n) {
   char tmp[32];
   if (n >= 32) return 0;
   for (size_t i = 0; i < n; i++) tmp[i] = str[i + pos];
   return atoi(tmp);
}
...
void RakString::Free(void) {
   ...
   if (sharedString->refCount == 0) {
      sharedString->refCountMutex->Unlock();
      const size_t smallStringSize = 128 - sizeof(unsigned int) - sizeof(size_t) - sizeof(char *) * 2;
      if (sharedString->bytesUsed > smallStringSize) rakFree_Ex(sharedString->bigString, _FILE_AND_LINE_);
      /*
      poolMutex->Lock();
...
void RakString::LockMutex(void) { GetPoolMutex().Lock(); }

void RakString::UnlockMutex(void) { GetPoolMutex().Unlock(); }
...
beforeRakString-beforeReferenceList, beforeStdString-beforeRakString,
...
_FILE_AND_LINE_); referenceStringList.RemoveAtIndex(referenceStringList.Size()-1);
                }
                beforeRakString=RakNet::GetTimeMS();
                for (i=0; i < repeatCount; i++)
