...
void Table::Cell::Set(unsigned int input) { Set((int)input); }
...
void Table::Cell::Get(char *output, int *outputLength) {
   ...
   if (outputLength) *outputLength = (int)i;
}

RakNet::RakString Table::Cell::ToString(ColumnType columnType) {
   if (isEmpty) return RakNet::RakString();
...
unsigned Table::AddColumn(const char columnName[_TABLE_MAX_COLUMN_NAME_LENGTH], ColumnType columnType) {
   if (columnName[0] == 0) return (unsigned)-1;
...
unsigned Table::ColumnIndex(const char *columnName) const {
   ...
   for (columnIndex = 0; columnIndex < columns.Size(); columnIndex++)
      if (strcmp(columnName, columns[columnIndex].columnName) == 0) return columnIndex;
   return (unsigned)-1;
}
...
unsigned Table::GetColumnCount(void) const { return columns.Size(); }

unsigned Table::GetRowCount(void) const { return rows.Size(); }
...
Table::Row *Table::AddRow(unsigned rowId, DataStructures::List<Cell *> &initialCellValues, bool copyCells) {
   ...
   for (rowIndex = 0; rowIndex < columns.Size(); rowIndex++) {
      if (rowIndex < initialCellValues.Size() && initialCellValues[rowIndex] &&
          initialCellValues[rowIndex]->isEmpty == false) {
         if (copyCells == false)
            newRow->cells.Insert(RakNet::OP_NEW_4<Table::Cell>(
                                     _FILE_AND_LINE_, initialCellValues[rowIndex]->i, initialCellValues[rowIndex]->c,
...
Table::Row *Table::AddRowColumns(unsigned rowId, Row *row, DataStructures::List<unsigned> columnIndices) {
   ...
   for (columnIndex = 0; columnIndex < columnIndices.Size(); columnIndex++) {
      if (row->cells[columnIndices[columnIndex]]->isEmpty == false) {
         newRow->cells.Insert(RakNet::OP_NEW_4<Table::Cell>(_FILE_AND_LINE_, row->cells[columnIndices[columnIndex]]->i,
...
Table::FilterQuery::FilterQuery() { columnName[0] = 0; }
...
Table::Row *Table::GetRowByID(unsigned rowId) const {
   Row *row;
   if (rows.Get(rowId, row)) return row;
   return 0;
}
...
void Table::QueryTable(unsigned *columnIndicesSubset, unsigned numColumnSubset, FilterQuery *inclusionFilters,
   ...
   if (columnIndicesSubset && numColumnSubset > 0) {
      ...
      for (i = 0; i < columns.Size(); i++) columnIndicesToReturn.Insert(i, _FILE_AND_LINE_);
   }

   if (columnIndicesToReturn.Size() == 0) return;  // No valid columns specified
   ...
   if (rowIds == 0 || numRowIDs == 0) {
      ...
      while (cur) {
         for (i = 0; i < (unsigned)cur->size; i++) {
            QueryRow(inclusionFilterColumnIndices, columnIndicesToReturn, cur->keys[i], cur->data[i], inclusionFilters,
                     result);
         }
         cur = cur->next;
      }
      ...
      for (j = 0; j < inclusionFilterColumnIndices.Size(); j++) {
         ...
         if (columnIndex != (unsigned)-1 && row->cells[columnIndex]->isEmpty == false) {
            if (columns[inclusionFilterColumnIndices[j]].columnType == STRING &&
                (row->cells[columnIndex]->c == 0 || inclusionFilters[j].cellValue->c == 0))
               continue;
   ...
   for (i = 0; i < _numSortQueries; i++) {
      ...
      if (_sortQueries[i].operation == Table::QS_INCREASING_ORDER) {
         if ((*_columns)[columnIndex].columnType == Table::NUMERIC) {
            if (first->cells[columnIndex]->i > second->cells[columnIndex]->i) return 1;
            if (first->cells[columnIndex]->i < second->cells[columnIndex]->i) return -1;
         } else {
            // String
            if (strcmp(first->cells[columnIndex]->c, second->cells[columnIndex]->c) > 0) return 1;
            if (strcmp(first->cells[columnIndex]->c, second->cells[columnIndex]->c) < 0) return -1;
         }
      } else {
         if ((*_columns)[columnIndex].columnType == Table::NUMERIC) {
...
void Table::SortTable(Table::SortQuery *sortQueries, unsigned numSortQueries, Table::Row **out) {
   ...
   outLength = 0;
   for (i = 0; i < orderedList.Size(); i++) out[(outLength)++] = orderedList[i];
}
   ...
   for (i = 0; i < columns.Size(); i++) {
      if (columns[i].columnType == NUMERIC) {
         ...
         if (printDelineatorForBinary) {
            if (i + 1 != columns.Size()) buff[0] = columnDelineator;
            buff[1] = 0;
         } else
            buff[0] = 0;
      }
...
const List<Table::ColumnDescriptor> &Table::GetColumns(void) const { return columns; }
...
Table &Table::operator=(const Table &input) {
   ...
   unsigned int i;
   for (i = 0; i < input.GetColumnCount(); i++) AddColumn(input.ColumnName(i), input.GetColumnType(i));
