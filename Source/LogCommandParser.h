...
class RAK_DLL_EXPORT LogCommandParser : public CommandParserInterface {
  public:
   ...
   bool OnCommand(const char *command, unsigned numParameters, char **parameterList, TransportInterface *transport,
   ...
   /// All logs must be associated with a channel.  This is a filter so that remote clients only get
   /// logs for a system they care about.
   ...
   /// Logs are not buffered, so only remote consoles connected and subscribing at the time you
   /// write will get the output. \param[in] format Same as RAKNET_DEBUG_PRINTF() \param[in] ...
   /// Same as RAKNET_DEBUG_PRINTF()
   ...
   /// This is called every time transport interface is registered.  If you want to save a copy of
   /// the TransportInterface pointer This is the place to do it \param[in] transport The new
   /// TransportInterface
  ...
  protected:
   ...
   /// \param[in] channelName If 0, then unsubscribe from all channels.  Otherwise unsubscribe from
   /// the named channel
   ...
   /// \param[in] channelName If 0, then subscribe from all channels.  Otherwise subscribe to the
   /// named channel
   ...
   struct SystemAddressAndChannel {
      ...
      /// Bitwise representations of the channels subscribed to.  If bit 0 is set, then we subscribe
      /// to channelNames[0] and so on.
      unsigned channels;
   };

   /// The list of remote users.  Added to when users subscribe, removed when they disconnect or
   /// unsubscribe
   ...
   /// This is so I can save the current transport provider, solely so I can use it without having
   /// the user pass it to Log
