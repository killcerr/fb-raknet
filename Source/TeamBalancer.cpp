...
void TeamBalancer::SetLockTeams(bool lock) {
   if (lock == lockTeams) return;

   // Set flag to indicate that teams can no longer be changed.
   lockTeams = lock;

   // If lock is false, and teams were set to be forced as even, then run through the even team
   // algorithm
   if (lockTeams == false) {
      // Process even swaps
      TeamId i, j;
...
void TeamBalancer::RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam) {
   ...
   // Also flag that we have requested a team, and record desiredTeam in case the host changes and
...
PluginReceiveResult TeamBalancer::OnReceive(Packet *packet) {
   ...
   // - This user does not already have a team. Assign a team as if the user called
   // RequestAnyTeam(), with a preference for the requested team. Return
   // ID_TEAM_BALANCER_TEAM_ASSIGNED once the team has been assigned. If teams are not locked
   // - If even team balancing is on, only assign this user if this would not cause teams to be
   // unbalanced. If teams WOULD be unbalanced, then flag this user as wanting to join this team.
   // Return ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING
   // - If the destination team is full, flag this user as wanting to join this team. Return
...
void TeamBalancer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void TeamBalancer::OnAttach(void) { hostGuid = rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS); }
         ...
         if (teamMembers[droppedMemberIndex].memberGuid == rakNetGUID) {
            ...
            if (lockTeams == false) {
               if (forceTeamsToBeEven) {
                  ...
                  // - teams were NOT forced to be even, and the team the dropped player on was
                  // full, then move users wanting to join that team (if any)
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet *packet) {
   ...
   for (uint8_t i = 0; i < requestedTeamChangeListSize; i++) {
      ...
      if (tm.currentTeam == UNASSIGNED_TEAM_ID && tm.requestedTeam == UNASSIGNED_TEAM_ID) return;
      ...
      if (memberIndex == (unsigned int)-1) {
         ...
         // Do not process balancing on requested teams, since we don't necessarily have all data
         // from all systems yet and hopefully the state during the host migration was stable.
...
void TeamBalancer::OnCancelTeamRequest(Packet *packet) {
   ...
   if (memberIndex != (unsigned int)-1) teamMembers[memberIndex].requestedTeam = UNASSIGNED_TEAM_ID;
}
...
void TeamBalancer::OnRequestSpecificTeam(Packet *packet) {
   ...
   if (tm.requestedTeam == UNASSIGNED_TEAM_ID) {
      ...
      if (memberIndex != (unsigned int)-1) RemoveTeamMember(memberIndex);
      return;
   }
   ...
   if (memberIndex == (unsigned int)-1) {
      ...
      if (TeamsWouldBeEvenOnSwitch(tm.requestedTeam, oldTeamThisUserWasOn) == true) {
         ...
         // If someone wants to join this user's old team, and we want to join their team, they can
         // swap
...
unsigned int TeamBalancer::GetMemberIndex(NetworkID memberId, RakNetGUID guid) const {
   for (unsigned int i = 0; i < teamMembers.Size(); i++) {
      if (teamMembers[i].memberGuid == guid && teamMembers[i].memberId == memberId) return i;
   }
...
void TeamBalancer::EvenTeams(void) {
   // Ensure all teams are even. If not, pick players at random from overpopulated teams, and move
   // to underpopulated teams.
   ...
   // First select among players that have requested to switch teams, if any, before choosing
   // players that did not want to switch teams. Players that are moved should be notified of
   ...
   for (teamMemberCountsIndex = 0; teamMemberCountsIndex < teamMemberCounts.Size(); teamMemberCountsIndex++) {
      while (teamMemberCounts[teamMemberCountsIndex] < minMembersOnASingleTeam &&
             teamMemberCounts[teamMemberCountsIndex] < teamLimits[teamMemberCountsIndex]) {
...
void TeamBalancer::GetOverpopulatedTeams(DataStructures::List<TeamId> &overpopulatedTeams, int maxTeamSize) {
   ...
   for (TeamId i = 0; i < teamMemberCounts.Size(); i++) {
      if (teamMemberCounts[i] >= maxTeamSize) overpopulatedTeams.Push(i, _FILE_AND_LINE_);
   }
}

void TeamBalancer::NotifyTeamAssigment(unsigned int teamMemberIndex) {
   RakAssert(teamMemberIndex < teamMembers.Size());
   if (teamMemberIndex >= teamMembers.Size()) return;
...
PluginReceiveResult TeamBalancer::OnTeamAssigned(Packet *packet) {
   if (packet->guid != hostGuid) return RR_STOP_PROCESSING_AND_DEALLOCATE;
   ...
   for (unsigned int i = 0; i < myTeamMembers.Size(); i++) {
      if (myTeamMembers[i].memberId == mtm.memberId) {
         foundMatch = true;
         if (myTeamMembers[i].requestedTeam == mtm.currentTeam) myTeamMembers[i].requestedTeam = UNASSIGNED_TEAM_ID;
...
bool TeamBalancer::TeamWouldBeOverpopulatedOnAddition(TeamId teamId, unsigned int teamMemberSize) {
   // Accounting for team balancing and team limits, would this team be overpopulated if a member
   // was added to it?
...
TeamId TeamBalancer::MoveMemberThatWantsToJoinTeamInternal(TeamId teamId) {
   ...
   for (TeamId i = 0; i < teamMembers.Size(); i++) {
      if (teamMembers[i].requestedTeam == teamId) membersThatWantToJoinTheTeam.Push(i, _FILE_AND_LINE_);
   }
