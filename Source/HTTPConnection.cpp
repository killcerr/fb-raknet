...
bool HTTPConnection::HasBadResponse(int *code, RakNet::RakString *data) {
   if (badResponses.IsEmpty()) return false;

   if (code) *code = badResponses.Peek().code;
   if (data) *data = badResponses.Pop().data;
   return true;
}

void HTTPConnection::CloseConnection() { connectionState = CS_DISCONNECTING; }
      ...
      case CS_CONNECTED: {
         ...
         if (currentProcessingCommand.isPost) {
            request.Set(
                ...
                currentProcessingCommand.contentType.C_String(), (unsigned)currentProcessingCommand.data.GetLength(),
...
RakString HTTPConnection::Read(void) {
   if (results.IsEmpty()) return RakString();
...
SystemAddress HTTPConnection::GetServerAddress(void) const { return server; }
...
void HTTPConnection::ProcessTCPPacket(Packet *packet) {
   ...
   if (packet->systemAddress == server) {
      ...
      if (
          ...
          connectionState == CS_PROCESSING) {
         /*
         // The stupid programmer that wrote this originally didn't think that just because the
         header contains this value doesn't mean you got the whole message if (strstr((const char*)
         packet->data, "\r\nConnection: close\r\n"))
         {
                 CloseConnection();
         }
