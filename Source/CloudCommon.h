...
struct RAK_DLL_EXPORT CloudQuery {
   ...
   /// primaryKey and secondaryKey are potentially returned If you pass more than one key at a time,
   /// the results are concatenated so if you need to differentiate between queries then send two
   /// different queries
