...
void CSHA1::Update(const UINT_8 *pbData, UINT_32 uLen) {
   ...
   if ((j + uLen) > 63) {
      ...
      for (; (i + 63) < uLen; i += 64) Transform(m_state, &pbData[i]);
...
void CSHA1::Final() {
   ...
   for (i = 0; i < 8; ++i)
      pbFinalCount[i] =
          static_cast<UINT_8>((m_count[((i >= 4) ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 0xFF);  // Endian independent

   Update((UINT_8 *)"\200", 1);

   while ((m_count[0] & 504) != 448) Update((UINT_8 *)"\0", 1);

   Update(pbFinalCount, 8);  // Cause a Transform()

   for (i = 0; i < 20; ++i) m_digest[i] = static_cast<UINT_8>((m_state[i >> 2] >> ((3 - (i & 3)) * 8)) & 0xFF);
...
void CSHA1::HMAC(unsigned char *sharedKey, int sharedKeyLength, unsigned char *data, int dataLength,
   ...
   if (sharedKeyLength > sha1BlockLength) sharedKeyLength = sha1BlockLength;
