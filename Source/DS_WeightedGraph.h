...
/// \details I'm assuming the indices are complex map types, rather than sequential numbers (which
...
class RAK_DLL_EXPORT WeightedGraph {
  public:
   ...
   void Print(void);
   void Clear(void);
   bool GetShortestPath(DataStructures::List<node_type> &path, node_type startNode, node_type endNode,
                        weight_type INFINITE_WEIGHT);
   bool GetSpanningTree(DataStructures::Tree<node_type> &outTree, DataStructures::List<node_type> *inputNodes,
...
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::WeightedGraph(const WeightedGraph &original_copy) {
   ...
   if (isValidPath) {
      ...
      costMatrix =
          RakNet::OP_NEW_ARRAY<weight_type>(costMatrixIndices.Size() * costMatrixIndices.Size(), _FILE_AND_LINE_);
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
WeightedGraph<node_type, weight_type, allow_unlinkedNodes>
    &WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::operator=(const WeightedGraph &original_copy) {
   adjacencyLists = original_copy.adjacencyLists;
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::HasConnection(const node_type &node1,
                                                                               const node_type &node2) {
   if (node1 == node2) return false;
   if (adjacencyLists.Has(node1) == false) return false;
   return adjacencyLists.Get(node1)->Has(node2);
}

template <class node_type, class weight_type, bool allow_unlinkedNodes>
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::AddConnection(const node_type &node1,
                                                                               const node_type &node2,
                                                                               weight_type weight) {
   if (node1 == node2) return;

   if (adjacencyLists.Has(node1) == false) AddNode(node1);
   adjacencyLists.Get(node1)->Set(node2, weight);
   if (adjacencyLists.Has(node2) == false) AddNode(node2);
   adjacencyLists.Get(node2)->Set(node1, weight);
}

template <class node_type, class weight_type, bool allow_unlinkedNodes>
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::RemoveConnection(const node_type &node1,
                                                                                  const node_type &node2) {
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
                                      ...
                                      // connections, remove the node
   {
      ...
      if (adjacencyLists.Has(node2) && adjacencyLists.Get(node2)->Size() == 0) RemoveNode(node2);
   }
...
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::Clear(void) {
   unsigned i;
   for (i = 0; i < adjacencyLists.Size(); i++) RakNet::OP_DELETE(adjacencyLists[i], _FILE_AND_LINE_);
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetShortestPath(DataStructures::List<node_type> &path,
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
   ...
   while (1) {
      while (costMatrix[row * adjacencyLists.Size() + col] == currentWeight) {
         if (row == 0) {
            path.Insert(startNode, _FILE_AND_LINE_);
            while (!outputQueue.Empty()) path.Insert(outputQueue.Pop(), _FILE_AND_LINE_);
            return true;
         }
         --row;
      }
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetConnectionAtIndex(unsigned nodeIndex,
...
bool WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GetSpanningTree(
    DataStructures::Tree<node_type> &outTree, DataStructures::List<node_type> *inputNodes, node_type startNode,
    weight_type INFINITE_WEIGHT) {
   // Find the shortest path from the start node to each of the input nodes.  Add this path to a new
   // WeightedGraph if the result is reachable
   ...
   current = &outTree;
   if (outGraph.adjacencyLists.Has(startNode) == false) return false;
...
template <class node_type, class weight_type, bool allow_unlinkedNodes>
void WeightedGraph<node_type, weight_type, allow_unlinkedNodes>::GenerateDisjktraMatrix(node_type startNode,
                                                                                        weight_type INFINITE_WEIGHT) {
   if (adjacencyLists.Size() == 0) return;
   ...
   for (col = 0; col < adjacencyLists.Size(); col++) {
      // This should be already sorted, so it's a bit inefficient to do an insertion sort, but what
      // the heck
      costMatrixIndices.Insert(adjacencyLists.GetKeyAtIndex(col), adjacencyLists.GetKeyAtIndex(col), true,
                               _FILE_AND_LINE_);
   }
   for (col = 0; col < adjacencyLists.Size() * adjacencyLists.Size(); col++) costMatrix[col] = INFINITE_WEIGHT;
   ...
   if (adjacencyLists.Size()) {
      ...
      for (row2 = 0; row2 < adjacencyLists.Size(); row2++) costMatrix[row2 * adjacencyLists.Size() + adjacentIndex] = 0;
   }

   while (row < adjacencyLists.Size() - 1) {
      adjacencyList = adjacencyLists.Get(currentNode);
      // Go through all connections from the current node.  If the new weight is less than the
      // current weight, then update that weight.
   ...
   for (i = 0; i < adjacencyLists.Size(); i++) {
      ...
      if (adjacencyLists[i]->Size() == 0)
         ...
         for (j = 0; j < adjacencyLists[i]->Size(); j++)
            ...
            RAKNET_DEBUG_PRINTF("%s (%.2f) ", adjacencyLists[i]->GetKeyAtIndex(j).systemAddress.ToString(),
