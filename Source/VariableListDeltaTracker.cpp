...
VariableListDeltaTracker::~VariableListDeltaTracker() {
   unsigned int i;
   for (i = 0; i < variableList.Size(); i++) rakFree_Ex(variableList[i].lastData, _FILE_AND_LINE_);
}
...
void VariableListDeltaTracker::FlagDirtyFromBitArray(unsigned char *bArray) {
   ...
   for (readOffset = 0; readOffset < variableList.Size(); readOffset++) {
      ...
      if (result == true) variableList[readOffset].isDirty = true;
   }
}

VariableListDeltaTracker::VariableLastValueNode::VariableLastValueNode() { lastData = 0; }
