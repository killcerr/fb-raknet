...
bool TableSerializer::DeserializeTable(unsigned char *serializedTable, unsigned int dataLength,
...
bool TableSerializer::DeserializeTable(RakNet::BitStream *in, DataStructures::Table *out) {
   ...
   for (rowIndex = 0; rowIndex < rowSize; rowIndex++) {
      if (DeserializeRow(in, out) == false) return false;
   }
   return true;
}
...
bool TableSerializer::DeserializeColumns(RakNet::BitStream *in, DataStructures::Table *out) {
   ...
   if (in->Read(columnSize) == false || columnSize > 10000) return false;  // Hacker crash prevention
...
bool TableSerializer::DeserializeRow(RakNet::BitStream *in, DataStructures::Table *out) {
   ...
   unsigned key;
   if (in->Read(key) == false) return false;
   row = out->AddRow(key);
   unsigned int cnt;
   in->Read(numEntries);
...
bool TableSerializer::DeserializeCell(RakNet::BitStream *in, DataStructures::Table::Cell *cell,
   ...
   if (in->Read(isEmpty) == false) return false;
   if (isEmpty == false) {
      if (columnType == DataStructures::Table::NUMERIC) {
         if (in->Read(value) == false) return false;
         cell->Set(value);
      } else if (columnType == DataStructures::Table::STRING) {
         if (StringCompressor::Instance()->DecodeString(tempString, 65535, in) == false) return false;
         cell->Set(tempString);
      } else if (columnType == DataStructures::Table::POINTER) {
         if (in->Read(ptr) == false) return false;
         cell->SetPtr(ptr);
      } else {
         unsigned binaryLength;
         // Binary
         ...
         if (BITS_TO_BYTES(in->GetNumberOfUnreadBits()) < (BitSize_t)binaryLength) return false;
...
void TableSerializer::SerializeFilterQuery(RakNet::BitStream *in, DataStructures::Table::FilterQuery *query) {
   ...
   if (query->cellValue->isEmpty == false) {
      ...
      in->WriteAlignedBytesSafe((const char *)query->cellValue->c, (const unsigned int)query->cellValue->i,
...
bool TableSerializer::DeserializeFilterQuery(RakNet::BitStream *out, DataStructures::Table::FilterQuery *query) {
   ...
   if (query->cellValue->isEmpty == false) {
      // HACK - cellValue->i is used for integer, character, and binary data. However, for character
      // and binary c will be 0. So use that to determine if the data was integer or not.
      out->Read(query->cellValue->i);
      unsigned int inputLength;
      out->ReadAlignedBytesSafeAlloc(&query->cellValue->c, inputLength,
                                     10000000);  // Sanity check to max binary cell of 10 megabytes
      if (query->cellValue->c) query->cellValue->i = inputLength;
...
void TableSerializer::SerializeFilterQueryList(RakNet::BitStream *in, DataStructures::Table::FilterQuery *query,
   ...
   if (query == 0 || numQueries <= 0) return;
...
void TableSerializer::DeallocateQueryList(DataStructures::Table::FilterQuery *query, unsigned int numQueries) {
   ...
   unsigned i;
   for (i = 0; i < numQueries; i++) RakNet::OP_DELETE(query[i].cellValue, _FILE_AND_LINE_);
