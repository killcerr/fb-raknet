...
// Note to self - if I change this it might affect RECIPIENT_OFFLINE_MESSAGE_INTERVAL in
// Natpunchthrough.cpp
...
static const unsigned int MAX_OFFLINE_DATA_LENGTH = 400;  // I set this because I limit ID_CONNECTION_REQUEST to 512
                                                          // bytes, and the password is appended to that packet.
...
static const unsigned char OFFLINE_MESSAGE_DATA_ID[16] = {0x00, 0xFF, 0xFF, 0x00, 0xFE, 0xFE, 0xFE, 0xFE,
...
RakPeer::RakPeer() {
   ...
   CAT_AUDIT_PRINTF(
       "AUDIT: Initializing RakPeer security flags: using_security = false, "
       "server_handshake = null, cookie_jar = null\n");
   ...
   for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) ipList[i] = UNASSIGNED_SYSTEM_ADDRESS;
...
// Multiple calls while already active are ignored.  To call this function again with different
// settings, you must first call Shutdown(). \note Call SetMaximumIncomingConnections if you want to
// accept incoming connections \param[in] maxConnections The maximum number of connections between
// this instance of RakPeer and another instance of RakPeer. Required so the network can preallocate
// and for thread safety. A pure client would set this to 1.  A pure server would set it to the
// number of allowed clients.- A hybrid would set it to the sum of both types of connections
// \param[in] localPort The port to listen for connections on.
// \param[in] _threadSleepTimer How many ms to Sleep each internal update cycle. With new congestion
// control, the best results will be obtained by passing 10. \param[in] socketDescriptors An array
// of SocketDescriptor structures to force RakNet to listen on a particular IP address or port (or
// both).  Each SocketDescriptor will represent one unique socket.  Do not pass redundant
// structures.  To listen on a specific port, you can pass &socketDescriptor,
// 1SocketDescriptor(myPort,0); such as for a server.  For a client, it is usually OK to just pass
// SocketDescriptor(); \param[in] socketDescriptorCount The size of the \a socketDescriptors array.
// Pass 1 if you are not sure what to pass. \return False on failure (can't create socket or
...
StartupResult RakPeer::Startup(unsigned int maxConnections, SocketDescriptor *socketDescriptors,
   ...
   if (IsActive()) return RAKNET_ALREADY_STARTED;

   // If getting the guid failed in the constructor, try again
   if (myGuid.g == 0) {
      GenerateGUID();
      if (myGuid.g == 0) return COULD_NOT_GENERATE_GUID;
   }
   ...
   if (socketDescriptors == 0 || socketDescriptorCount < 1) return INVALID_SOCKET_DESCRIPTORS;
   ...
   if (maxConnections <= 0) return INVALID_MAX_CONNECTIONS;
   ...
   for (i = 0; i < (int)socketDescriptorCount; i++) {
      ...
      if (SocketLayer::IsSocketFamilySupported(addrToBind,
  socketDescriptors[i].socketFamily)==false) return SOCKET_FAMILY_NOT_SUPPORTED; #endif

      if (socketDescriptors[i].port!=0 && SocketLayer::IsPortInUse(socketDescriptors[i].port,
  addrToBind, socketDescriptors[i].socketFamily)==true)
      {
              DerefAllSockets();
      ...
      {
              rns = SocketLayer::CreateBoundSocket( this, socketDescriptors[i].port,
  socketDescriptors[i].blockingSocket, addrToBind, 100, socketDescriptors[i].extraSocketOptions,
  socketDescriptors[i].socketFamily, socketDescriptors[i].chromeInstance );
      }
      else
      {
  #if defined(_PS3) || defined(__PS3__) || defined(SN_TARGET_PS3) || defined(_PS4)
              rns = SocketLayer::CreateBoundSocket_PS3Lobby( socketDescriptors[i].port,
  socketDescriptors[i].blockingSocket, addrToBind, socketDescriptors[i].socketFamily ); #elif
  defined(SN_TARGET_PSP2) rns = SocketLayer::CreateBoundSocket_PSP2( socketDescriptors[i].port,
  socketDescriptors[i].blockingSocket, addrToBind, socketDescriptors[i].socketFamily ); #endif
      }
      */
      ...
      #if !defined(__native_client__)
                      int zero=0;
                      if (SocketLayer::SendTo(rns, (const char*) &zero,4, rns->GetBoundAddress(),
      _FILE_AND_LINE_)!=0)
                      {
                              DerefAllSockets();
                              return SOCKET_FAILED_TEST_SEND;
   ...
   for (i = 0; i < (int)socketDescriptorCount; i++) {
      if (socketList[i]->IsBerkleySocket()) ((RNS2_Berkley *)socketList[i])->CreateRecvPollingThread(threadPriority);
   }
#endif

   // #if !defined(_XBOX) && !defined(_XBOX_720_COMPILE_AS_WINDOWS) && !defined(X360)
   for (i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) {
      if (ipList[i] == UNASSIGNED_SYSTEM_ADDRESS) break;
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
   ...
   if (maximumNumberOfPeers == 0) {
      ...
      if (maximumIncomingConnections > maxConnections) maximumIncomingConnections = maxConnections;
      ...
      // Allocate 10% extra to handle new connections from players trying to connect when the server
      // is full
      // remoteSystemListSize = maxConnections;// * 11 / 10 + 1;

      // remoteSystemList in Single thread
      // remoteSystemList = RakNet::OP_NEW<RemoteSystemStruct[ remoteSystemListSize ]>(
      // _FILE_AND_LINE_ );
      remoteSystemList = RakNet::OP_NEW_ARRAY<RemoteSystemStruct>(maximumNumberOfPeers, _FILE_AND_LINE_);

      remoteSystemLookup = RakNet::OP_NEW_ARRAY<RemoteSystemIndex *>(
          (unsigned int)maximumNumberOfPeers * REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE, _FILE_AND_LINE_);
...
//					RakAssert(isRecvFromLoopThreadActive.GetValue()==0);
   ...
   //errorCode = RakNet::RakThread::Create(RecvFromLoop, rpai, threadPriority, threadName, 1+i,
   runtime); errorCode = RakNet::RakThread::Create(RecvFromLoop, rpai, threadPriority, threadName,
   1024*1); #else errorCode = RakNet::RakThread::Create(RecvFromLoop, rpai, threadPriority); #endif
...
#if RAKPEER_USER_THREADED != 1
      // Wait for the threads to activate.  When they are active they will set these variables to
      // true
      while (isMainLoopThreadActive == false) RakSleep(10);
#endif  // RAKPEER_USER_THREADED!=1
   }
...
//
// If you accept connections, you must call this or else security will not be enabled for incoming
// connections.
//
...
// bRequireClientKey: Should be set to false for most servers.  Allows the server to accept a public
// key from connecting clients as a proof of identity but eats twice as much CPU time as a normal
// connection
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::InitializeSecurity(const char *public_key, const char *private_key, bool bRequireClientKey) {
   ...
   if (_server_handshake->Initialize(public_key, private_key)) {
      CAT_AUDIT_PRINTF(
          "AUDIT: Successfully initialized, filling cookie jar with goodies, storing "
          "public key and setting using security flag to true\n");
   ...
   CAT_AUDIT_PRINTF(
       "AUDIT: Failure to initialize so deleting server handshake and cookie jar; also "
       "setting using_security flag = false\n");
...
void RakPeer::RemoveFromSecurityExceptionList(const char *ip) {
   if (securityExceptionList.Size() == 0) return;
...
bool RakPeer::IsInSecurityExceptionList(const char *ip) {
   if (securityExceptionList.Size() == 0) return false;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Sets how many incoming connections are allowed.  If this is less than the number of players
// currently connected, no more players will be allowed to connect.  If this is greater than the
// maximum number of peers allowed, it will be reduced to the maximum number of peers allowed.
// Defaults to 0.
//
// Parameters:
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned int RakPeer::GetMaximumIncomingConnections(void) const { return maximumIncomingConnections; }
...
//
// Parameters:
// passwordData: A data block that incoming connections must match.  This can be just a password, or
// can be a stream of data.
...
void RakPeer::SetIncomingPassword(const char *passwordData, int passwordDataLength) {
   ...
   // Not threadsafe but it's not important enough to lock.  Who is going to change the password a
   // lot during runtime? It won't overflow at least because incomingPasswordLength is an unsigned
   // char
   if (passwordDataLength > 0) memcpy(incomingPassword, passwordData, passwordDataLength);
...
void RakPeer::GetIncomingPassword(char *passwordData, int *passwordDataLength) {
   ...
   if (*passwordDataLength > incomingPasswordLength) *passwordDataLength = incomingPasswordLength;

   if (*passwordDataLength > 0) memcpy(passwordData, incomingPassword, *passwordDataLength);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Call this to connect to the specified host (ip or domain name) and server port.
// Calling Connect and not calling SetMaximumIncomingConnections acts as a dedicated client. Calling
// both acts as a true peer. This is a non-blocking connection.  You know the connection is
// successful when IsConnected() returns true or receive gets a packet with the type identifier
// ID_CONNECTION_REQUEST_ACCEPTED.  If the connection is not successful, such as rejected connection
// or no response then neither of these things will happen. Requires that you first call Initialize
//
// Parameters:
// host: Either a dotted IP address or a domain name
// remotePort: Which port to connect to on the remote machine.
// passwordData: A data block that must match the data block on the server.  This can be just a
// password, or can be a stream of data passwordDataLength: The length in bytes of passwordData
//
// Returns:
// True on successful initiation. False on incorrect parameters, internal error, or too many
// existing peers
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::Connect(const char *host, unsigned short remotePort, const char *passwordData,
   ...
   if (host == 0 || endThreads || connectionSocketIndex >= socketList.Size()) return INVALID_PARAMETER;
   ...
   // Not threadsafe but it's not important enough to lock.  Who is going to change the password a
   // lot during runtime? It won't overflow at least because outgoingPasswordLength is an unsigned
   // char
   ...
   return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, connectionSocketIndex, 0,
...
ConnectionAttemptResult RakPeer::ConnectWithSocket(const char *host, unsigned short remotePort,
   ...
   if (host == 0 || endThreads || socket == 0) return INVALID_PARAMETER;

   if (passwordDataLength > 255) passwordDataLength = 255;

   if (passwordData == 0) passwordDataLength = 0;

   return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, 0, 0,
...
//	RakNet::TimeMS timeout;
   ...
   for (i=0; i < socketList.Size(); i++)
   {
           if (SocketLayer::SendTo(socketList[i], (const char*)
   &i,1,socketList[i]->GetBoundAddress(), _FILE_AND_LINE_)!=0) break;
   }
   */
   ...
   while ( isRecvFromLoopThreadActive.GetValue()>0 && RakNet::GetTimeMS()<timeout )
   {
           ...
           for (i=0; i < socketList.Size(); i++)
           {
                   SocketLayer::SendTo(socketList[i], (const char*)
   &i,1,socketList[i]->GetBoundAddress(), _FILE_AND_LINE_);
           }
   ...
   */
   ...
   for (i = 0; i < packetReturnQueue.Size(); i++) DeallocatePacket(packetReturnQueue[i]);
   ...
   */
   ...
   // Clear out the reliability layer list in case we want to reallocate it in a successive call to
   // Init.
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
inline bool RakPeer::IsActive(void) const { return endThreads == false; }
...
//
// Parameters:
// remoteSystems (out): An array of SystemAddress structures to be filled with the SystemAddresss of
// the systems we are connected to
// - pass 0 to remoteSystems to only get the number of systems we are connected to
// numberOfSystems (int, out): As input, the size of remoteSystems array.  As output, the number of
// elements put into the array
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::GetConnectionList(SystemAddress *remoteSystems, unsigned short *numberOfSystems) const {
   if (numberOfSystems == 0) return false;

   if (remoteSystemList == 0 || endThreads == true) {
      if (numberOfSystems) *numberOfSystems = 0;
      return false;
   }
   ...
   if (remoteSystems) {
      unsigned short i;
      for (i = 0; i < *numberOfSystems && i < addresses.Size(); i++) remoteSystems[i] = addresses[i];
      *numberOfSystems = i;
   } else {
...
uint32_t RakPeer::IncrementNextSendReceipt(void) {
   ...
   if (++sendReceiptSerial == 0) sendReceiptSerial = 1;
   sendReceiptSerialMutex.Unlock();
   return returned;
}
...
// systemAddress: Who to send this packet to, or in the case of broadcasting who not to send it to.
// Use UNASSIGNED_SYSTEM_ADDRESS to specify none broadcast: True to send this packet to all
// connected systems.  If true, then systemAddress specifies who not to send the packet to. Returns:
// \return 0 on bad input. Otherwise a number that identifies this message. If \a reliability is a
// type that returns a receipt, on a later call to Receive() you will get ID_SND_RECEIPT_ACKED or
// ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::Send(const char *data, const int length, PacketPriority priority, PacketReliability reliability,
   ...
   if (broadcast == false && systemIdentifier.IsUndefined()) return 0;
   ...
   SendBuffered(data, length * 8, priority, reliability, orderingChannel, systemIdentifier, broadcast,
...
uint32_t RakPeer::Send(const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability,
   ...
   // associated with the reliability layer can change, from that thread, resulting in a send to the
   // wrong player!  While I could mutex the systemAddress, that is much slower than doing this
   SendBuffered((const char *)bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority, reliability,
                orderingChannel, systemIdentifier, broadcast, RemoteSystemStruct::NO_ACTION, usedSendReceipt);
...
// \param[in] orderingChannel When using ordered or sequenced messages, what channel to order these
// on. Messages are only ordered relative to other messages on the same stream \param[in]
// systemIdentifier Who to send this packet to, or in the case of broadcasting who not to send it
// to. Pass either a SystemAddress structure or a RakNetGUID structure. Use
// UNASSIGNED_SYSTEM_ADDRESS or to specify none \param[in] broadcast True to send this packet to all
// connected systems. If true, then systemAddress specifies who not to send the packet to. \return
// False if we are not connected to the specified recipient.  True otherwise
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::SendList(const char **data, const int *lengths, const int numParameters, PacketPriority priority,
                           PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                           bool broadcast, uint32_t forceReceiptNumber) {
#ifdef _DEBUG
   RakAssert(data);
#endif

   if (data == 0 || lengths == 0) return 0;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Gets a packet from the incoming packet queue. Use DeallocatePacket to deallocate the packet after
// you are done with it. Check the Packet struct at the top of CoreNetworkStructures.h for the
// format of the struct
//
// Returns:
// 0 if no packets are waiting to be handled, otherwise an allocated packet
// If the client is not active this will also return 0, as all waiting packets are flushed when the
// client is Disconnected This also updates all memory blocks associated with synchronized memory
// and distributed objects
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#ifdef _MSC_VER
#pragma warning(disable : 4701)  // warning C4701: local variable <variable name> may be used
   ...
   for (i=0; i < socketList.Size(); i++)
   {
           while (1)
           {
                   ...
                   SocketLayer::RecvFromBlocking(
                           recvFromStruct->s, this,
 recvFromStruct->remotePortRakNetWasStartedOn_PS3, recvFromStruct->extraSocketOptions,
 recvFromStruct->data, &recvFromStruct->bytesRead, &recvFromStruct->systemAddress,
 &recvFromStruct->timeRead); if (recvFromStruct->bytesRead<=0)
                   {
   ...
   */
   ...
   do {
      ...
      // Some locally generated packets need to be processed by plugins, for example
      // ID_FCM2_NEW_HOST The plugin itself should intercept these messages generated remotely
      // if (packet->wasGeneratedLocally) 			return packet;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned int RakPeer::GetMaximumNumberOfPeers(void) const { return maximumNumberOfPeers; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Close the connection to another host (if we initiated the connection it will disconnect, if they
// did it will kick them out).
//
// Parameters:
// target: Which connection to close
// sendDisconnectionNotification: True to send ID_DISCONNECTION_NOTIFICATION to the recipient. False
// to close it silently. channel: If blockDuration > 0, the disconnect packet will be sent on this
// channel
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::CloseConnection(const AddressOrGUID target, bool sendDisconnectionNotification,
   ...
   // From the network thread, this should occur because of ID_DISCONNECTION_NOTIFICATION and
   ID_CONNECTION_LOST unsigned j; for (j=0; j < messageHandlerList.Size(); j++)
   {
           messageHandlerList[j]->OnClosedConnection(
                   target.systemAddress==UNASSIGNED_SYSTEM_ADDRESS ?
   GetSystemAddressFromGuid(target.rakNetGuid) : target.systemAddress,
                   target.rakNetGuid==UNASSIGNED_RAKNET_GUID ?
   GetGuidFromSystemAddress(target.systemAddress) : target.rakNetGuid, LCR_CLOSED_BY_USER);
   }
   */
   ...
   // 12/14/09 Return ID_CONNECTION_LOST when calling CloseConnection with
   ...
   if (sendDisconnectionNotification == false && GetConnectionState(target) == IS_CONNECTED) {
      ...
      packet->systemAddress = target.systemAddress == UNASSIGNED_SYSTEM_ADDRESS
                                  ? GetSystemAddressFromGuid(target.rakNetGuid)
      ...
      while (i < requestedConnectionQueue.Size()) {
         if (requestedConnectionQueue[i]->systemAddress == requestedConnectionCancelQueue[0]) {
            ...
            CAT_AUDIT_PRINTF("AUDIT: Deleting requestedConnectionQueue %i client_handshake %x\n", i,
                             requestedConnectionQueue[i]->client_handshake);
...
ConnectionState RakPeer::GetConnectionState(const AddressOrGUID systemIdentifier) {
   ...
   if (index == -1) return IS_NOT_CONNECTED;

   if (remoteSystemList[index].isActive == false) return IS_DISCONNECTED;
...
SystemAddress RakPeer::GetSystemAddressFromIndex(unsigned int index) {
   ...
   if (index < maximumNumberOfPeers)
      if (remoteSystemList[index].isActive &&
          remoteSystemList[index].connectMode ==
              RakPeer::RemoteSystemStruct::CONNECTED)  // Don't give the user players that aren't
                                                       // fully connected, since sends will fail
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Same as calling GetSystemAddressFromIndex and GetGUIDFromIndex for all systems, but more
// efficient Indices match each other, so \a addresses[0] and \a guids[0] refer to the same system
// \param[out] addresses All system addresses. Size of the list is the number of connections. Size
// of the list will match the size of the \a guids list. \param[out] guids All guids. Size of the
// list is the number of connections. Size of the list will match the size of the \a addresses list.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::GetSystemList(DataStructures::List<SystemAddress> &addresses,
   ...
   if (remoteSystemList == 0 || endThreads == true) return;
...
void RakPeer::AddToBanList(const char *IP, RakNet::TimeMS milliseconds) {
   ...
   if (IP == 0 || IP[0] == 0 || strlen(IP) > 15) return;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetLimitIPConnectionFrequency(bool b) { limitConnectionFrequencyFromTheSameIP = b; }
...
bool RakPeer::IsBanned(const char *IP) {
   ...
   if (IP == 0 || IP[0] == 0 || strlen(IP) > 15) return false;

   banListIndex = 0;

   if (banList.Size() == 0) return false;  // Skip the mutex if possible
...
// target - who to ping
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::Ping(const SystemAddress target) { PingInternal(target, false, UNRELIABLE); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Send a ping to the specified unconnected system.
// The remote system, if it is Initialized, will respond with ID_UNCONNECTED_PONG.
// The final ping time will be encoded in the following sizeof(RakNet::TimeMS) bytes.  (Default is 4
// bytes - See __GET_TIME_64BIT in RakNetTypes.h
//
// Parameters:
...
// connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors in
// RakPeer::Startup() to send on.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Ping(const char *host, unsigned short remotePort, bool onlyReplyOnAcceptingConnections,
   ...
   unsigned i;
   for (i=0; i < pluginListNTS.Size(); i++)
           pluginListNTS[i]->OnDirectSocketSend((const char*)bitStream.GetData(),
   bitStream.GetNumberOfBitsUsed(), systemAddress); SocketLayer::SendTo( socketList[realIndex],
   (const char*)bitStream.GetData(), (int) bitStream.GetNumberOfBytesUsed(), systemAddress,
   _FILE_AND_LINE_ );
   */
   ...
   if (bsp.systemAddress == UNASSIGNED_SYSTEM_ADDRESS) return false;
   bsp.systemAddress.FixForIPVersion(socketList[realIndex]->GetBoundAddress());
   unsigned i;
   for (i = 0; i < pluginListNTS.Size(); i++)
      pluginListNTS[i]->OnDirectSocketSend((const char *)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(),
                                           bsp.systemAddress);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetOccasionalPing(bool doPing) { occasionalPing = doPing; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/// Return the clock difference between your system and the specified system
/// Subtract the time from a time returned by the remote system to get that time relative to your
/// own system Returns 0 if the system is unknown
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
RakNet::Time RakPeer::GetClockDifferentialInt(RemoteSystemStruct *remoteSystem) const {
   ...
   for (counter = 0; counter < PING_TIMES_ARRAY_SIZE; counter++) {
      if (remoteSystem->pingAndClockDifferential[counter].pingTime == 65535) break;
...
void RakPeer::SetOfflinePingResponse(const char *data, const unsigned int length) {
   ...
   if (data && length > 0) offlinePingResponse.Write(data, length);
...
// \param[out] length A pointer filled in with the length parameter passed to
// SetOfflinePingResponse() \sa SetOfflinePingResponse
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
SystemAddress RakPeer::GetInternalID(const SystemAddress systemAddress, const int index) const {
   if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      if (remoteSystem == 0) return UNASSIGNED_SYSTEM_ADDRESS;

      return remoteSystem->theirInternalSystemAddress[index];
      /*
      sockaddr_in sa;
      socklen_t len = sizeof(sa);
      if (getsockname__(connectionSockets[remoteSystem->connectionSocketIndex], (sockaddr*)&sa,
      &len)!=0) return UNASSIGNED_SYSTEM_ADDRESS; returnValue.port=ntohs(sa.sin_port);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/// \brief Sets your internal IP address, for platforms that do not support reading it, or to
/// override a value \param[in] systemAddress. The address to set. Use SystemAddress::FromString()
/// if you want to use a dotted string \param[in] index When you have multiple internal IDs, which
/// index to set?
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Return the unique address identifier that represents you on the the network and is based on your
// external IP / port (the IP / port the specified player uses to communicate with you) Note that
// unlike in previous versions, this is a struct and is not sequential
//
// Parameters:
...
SystemAddress RakPeer::GetExternalID(const SystemAddress target) const {
   ...
   if (target == UNASSIGNED_SYSTEM_ADDRESS) return firstExternalID;
...
const RakNetGUID RakPeer::GetMyGUID(void) const { return myGuid; }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
const RakNetGUID &RakPeer::GetGuidFromSystemAddress(const SystemAddress input) const {
   if (input == UNASSIGNED_SYSTEM_ADDRESS) return myGuid;

   if (input.systemIndex != (SystemIndex)-1 && input.systemIndex < maximumNumberOfPeers &&
...
unsigned int RakPeer::GetSystemIndexFromGuid(const RakNetGUID input) const {
   if (input == UNASSIGNED_RAKNET_GUID) return (unsigned int)-1;

   if (input == myGuid) return (unsigned int)-1;
...
SystemAddress RakPeer::GetSystemAddressFromGuid(const RakNetGUID input) const {
   if (input == UNASSIGNED_RAKNET_GUID) return UNASSIGNED_SYSTEM_ADDRESS;

   if (input == myGuid) return GetInternalID(UNASSIGNED_SYSTEM_ADDRESS);
...
bool RakPeer::GetClientPublicKeyFromSystemAddress(const SystemAddress input, char *client_public_key) const {
#if LIBCAT_SECURITY == 1
   if (input == UNASSIGNED_SYSTEM_ADDRESS) return false;
   ...
   if (copy_source) {
      // Verify that at least one byte in the public key is non-zero to indicate that the key was
      // received
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Set the time, in MS, to use before considering ourselves disconnected after not being able to
// deliver a reliable packet \param[in] time Time, in MS
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetTimeoutTime(RakNet::TimeMS timeMS, const SystemAddress target) {
   if (target == UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      for (i = 0; i < maximumNumberOfPeers; i++) {
         if (remoteSystemList[i].isActive) {
            if (remoteSystemList[i].isActive) remoteSystemList[i].reliabilityLayer.SetTimeoutTime(timeMS);
         }
      }
   } else {
      RemoteSystemStruct *remoteSystem = GetRemoteSystemFromSystemAddress(target, false, true);

      if (remoteSystem != 0) remoteSystem->reliabilityLayer.SetTimeoutTime(timeMS);
   }
}
...
RakNet::TimeMS RakPeer::GetTimeoutTime(const SystemAddress target) {
   if (target == UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      if (remoteSystem != 0) return remoteSystem->reliabilityLayer.GetTimeoutTime();
   }
   return defaultTimeoutTime;
}
...
unsigned int RakPeer::GetNumberOfAddresses(void) {
   ...
   while (ipList[i] != UNASSIGNED_SYSTEM_ADDRESS) i++;
...
bool RakPeer::IsLocalIP(const char *ip) {
   ...
   int i;
   for (i = 0; i < num; i++) {
      if (strcmp(ip, GetLocalIP(i)) == 0) return true;
   }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Description:
// Allow or disallow connection responses from any IP. Normally this should be false, but may be
// necessary when connection to servers with multiple IP addresses
//
// Parameters:
// allow - True to allow this behavior, false to not allow.  Defaults to false.  Value persists
// between connections
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// This will tell the remote system our external IP outside the LAN, and can be used for NAT punch
// through
//
// Requires:
...
// ID_DOWNLOAD_PROGRESS is returned to indicate a new partial message chunk, roughly the MTU size,
// has arrived As it can be slow or cumbersome to get this notification for every chunk, you can set
// the interval at which it is returned. Defaults to 0 (never return this notification)
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
int RakPeer::GetSplitMessageProgressInterval(void) const { return splitMessageProgressInterval; }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Put a packet back at the end of the receive queue in case you don't want to deal with it
// immediately
//
// packet The packet you want to push back.
// pushAtHead True to push the packet so that the next receive call returns it.  False to push it at
// the end of the queue (obviously pushing it at the end makes the packets out of order)
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Packet *RakPeer::AllocatePacket(unsigned dataSize) { return AllocPacket(dataSize, _FILE_AND_LINE_); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakNetSocket2 *RakPeer::GetSocket(const SystemAddress target) {
   ...
   while (RakNet::GetTimeMS() < stopWaiting) {
      if (isMainLoopThreadActive == false) return 0;
      ...
      if (sqo) {
         ...
         if (output.Size()) return output[0];
         break;
      }
   }
   return 0;
...
void RakPeer::GetSockets(DataStructures::List<RakNetSocket2 *> &sockets) {
   ...
   while (1) {
      if (isMainLoopThreadActive == false) return;
...
void RakPeer::ReleaseSockets(DataStructures::List<RakNetSocket2 *> &sockets) { sockets.Clear(false, _FILE_AND_LINE_); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Adds simulated ping and packet loss to the outgoing data flow.
// To simulate bi-directional ping and packet loss, you should call this on both the sender and the
// recipient, with half the total ping and maxSendBPS value on each.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void RakPeer::SetPerConnectionOutgoingBandwidthLimit(unsigned maxBitsPerSecond) { maxOutgoingBPS = maxBitsPerSecond; }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SetUserUpdateThread(void (*_userUpdateThreadPtr)(RakPeerInterface *, void *),
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendOutOfBand(const char *host, unsigned short remotePort, const char *data, BitSize_t dataLength,
                            unsigned connectionSocketIndex) {
   if (IsActive() == false) return false;

   if (host == 0 || host[0] == 0) return false;

   // If this assert hits then Startup wasn't called or the call failed.
   RakAssert(connectionSocketIndex < socketList.Size());

   // This is a security measure.  Don't send data longer than this value
   RakAssert(dataLength <= (MAX_OFFLINE_DATA_LENGTH + sizeof(unsigned char) + sizeof(RakNet::Time) +
   ...
   for (i=0; i < pluginListNTS.Size(); i++)
           pluginListNTS[i]->OnDirectSocketSend((const char*)bitStream.GetData(),
   bitStream.GetNumberOfBitsUsed(), systemAddress);

   SocketLayer::SendTo( socketList[realIndex], (const char*)bitStream.GetData(), (int)
   bitStream.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
   */
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::GetStatisticsList(DataStructures::List<SystemAddress> &addresses, DataStructures::List<RakNetGUID> &guids,
...
int RakPeer::GetIndexFromSystemAddress(const SystemAddress systemAddress, bool calledFromNetworkThread) const {
   ...
   if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS) return -1;

   if (systemAddress.systemIndex != (SystemIndex)-1 && systemAddress.systemIndex < maximumNumberOfPeers &&
       remoteSystemList[systemAddress.systemIndex].systemAddress == systemAddress &&
   ...
   if (calledFromNetworkThread) {
      ...
      for (i = 0; i < maximumNumberOfPeers; i++)
         if (remoteSystemList[i].isActive && remoteSystemList[i].systemAddress == systemAddress) return i;

      // If no active results found, try previously active results.
      for (i = 0; i < maximumNumberOfPeers; i++)
         if (remoteSystemList[i].systemAddress == systemAddress) return i;
   }
...
int RakPeer::GetIndexFromGuid(const RakNetGUID guid) {
   ...
   if (guid == UNASSIGNED_RAKNET_GUID) return -1;

   if (guid.systemIndex != (SystemIndex)-1 && guid.systemIndex < maximumNumberOfPeers &&
       remoteSystemList[guid.systemIndex].guid == guid && remoteSystemList[guid.systemIndex].isActive)
      return guid.systemIndex;

   // remoteSystemList in user and network thread
   for (i = 0; i < maximumNumberOfPeers; i++)
      if (remoteSystemList[i].isActive && remoteSystemList[i].guid == guid) return i;

   // If no active results found, try previously active results.
   for (i = 0; i < maximumNumberOfPeers; i++)
      if (remoteSystemList[i].guid == guid) return i;
...
bool RakPeer::GenerateConnectionRequestChallenge(RequestedConnectionStruct *rcs, PublicKey *publicKey) {
   ...
   switch (publicKey->publicKeyMode) {
      ...
      case PKM_USE_TWO_WAY_AUTHENTICATION:
         if (publicKey->myPublicKey == 0 || publicKey->myPrivateKey == 0 || publicKey->remoteServerPublicKey == 0) {
            return false;
         }
         ...
         if (!rcs->client_handshake->Initialize(publicKey->remoteServerPublicKey) ||
            ...
            CAT_AUDIT_PRINTF(
                "AUDIT: Failure initializing new client_handshake object with identity for "
                "this RequestedConnectionStruct\n");
         ...
         CAT_AUDIT_PRINTF(
             "AUDIT: Success initializing new client handshake object with identity for "
             "this RequestedConnectionStruct -- pre-generated challenge\n");

         rcs->publicKeyMode = PKM_USE_TWO_WAY_AUTHENTICATION;
         break;

      case PKM_USE_KNOWN_PUBLIC_KEY:
         if (publicKey->remoteServerPublicKey == 0) return false;
         ...
         if (!rcs->client_handshake->Initialize(publicKey->remoteServerPublicKey) ||
             !rcs->client_handshake->GenerateChallenge(rcs->handshakeChallenge)) {
            CAT_AUDIT_PRINTF(
                "AUDIT: Failure initializing new client_handshake object for this "
         ...
         CAT_AUDIT_PRINTF(
             "AUDIT: Success initializing new client handshake object for this "
             "RequestedConnectionStruct -- pre-generated challenge\n");
...
ConnectionAttemptResult RakPeer::SendConnectionRequest(const char *host, unsigned short remotePort,
                                                       const char *passwordData, int passwordDataLength,
                                                       PublicKey *publicKey, unsigned connectionSocketIndex,
   ...
   RakAssert(remotePort != 0);
   SystemAddress systemAddress;
   if (!systemAddress.FromStringExplicitPort(host, remotePort,
                                             socketList[connectionSocketIndex]->GetBoundAddress().GetIPVersion()))
      return CANNOT_RESOLVE_DOMAIN_NAME;

   // Already connected?
   if (GetRemoteSystemFromSystemAddress(systemAddress, false, true)) return ALREADY_CONNECTED_TO_ENDPOINT;

   // RequestedConnectionStruct *rcs = (RequestedConnectionStruct *)
   // rakMalloc_Ex(sizeof(RequestedConnectionStruct), _FILE_AND_LINE_);
   ...
   if (!GenerateConnectionRequestChallenge(rcs, publicKey)) return SECURITY_INITIALIZATION_FAILED;
#else
   (void)publicKey;
#endif
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystem(const AddressOrGUID systemIdentifier,
                                                      bool calledFromNetworkThread, bool onlyActive) const {
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromSystemAddress(const SystemAddress systemAddress,
   ...
   if (systemAddress == UNASSIGNED_SYSTEM_ADDRESS) return 0;
   ...
   if (calledFromNetworkThread) {
      ...
      // Active connections take priority.  But if there are no active connections, return the first
      // systemAddress match found
      for (i = 0; i < maximumNumberOfPeers; i++) {
      ...
      if (deadConnectionIndex != -1 && onlyActive == false) return remoteSystemList + deadConnectionIndex;
   }
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystemFromGUID(const RakNetGUID guid, bool onlyActive) const {
   if (guid == UNASSIGNED_RAKNET_GUID) return 0;
...
void RakPeer::ParseConnectionRequestPacket(RakPeer::RemoteSystemStruct *remoteSystem,
                                           const SystemAddress &systemAddress, const char *data, int byteSize) {
   ...
   if (_using_security) {
      ...
      if (doSecurity != 1 || !remoteSystem->reliabilityLayer.GetAuthenticatedEncryption()) return;
      ...
      if (doClientKey == 1) {
         ...
         if (_require_client_public_key) {
            ...
            if (!_server_handshake->VerifyInitiatorIdentity(remoteSystem->answer, ident,
               ...
               RakNet::BitStream bitStream;
               bitStream.Write((MessageID)ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY);  // Report an error since the client
                                                                                  // is not providing an identity
         ...
         if (_require_client_public_key) {
            RakNet::BitStream bitStream;
            bitStream.Write((MessageID)ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY);  // Report an error since the client
                                                                               // is not providing an identity when
                                                                               // it is necessary to connect
            bitStream.Write((unsigned char)1);                                 // Indicate client identity is missing
            SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), IMMEDIATE_PRIORITY, RELIABLE,
                          0, systemAddress, false, false, RakNet::GetTimeUS(), 0);
            remoteSystem->connectMode = RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
            return;
         }
      }
...
void RakPeer::OnConnectionRequest(RakPeer::RemoteSystemStruct *remoteSystem, RakNet::Time incomingTimestamp) {
   ...
   for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) bitStream.Write(ipList[i]);
   bitStream.Write(incomingTimestamp);
   bitStream.Write(RakNet::GetTime());

   SendImmediate((char *)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0,
                 remoteSystem->systemAddress, false, false, RakNet::GetTimeUS(), 0);
}
...
void RakPeer::NotifyAndFlagForShutdown(const SystemAddress systemAddress, bool performImmediate,
   ...
   if (performImmediate) {
      ...
      SendBuffered((const char *)temp.GetData(), temp.GetNumberOfBitsUsed(), disconnectionNotificationPriority,
                   RELIABLE_ORDERED, orderingChannel, systemAddress, false, RemoteSystemStruct::DISCONNECT_ASAP, 0);
   }
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
RakPeer::RemoteSystemStruct *RakPeer::AssignSystemAddressToRemoteSystemList(
    const SystemAddress systemAddress, RemoteSystemStruct::ConnectMode connectionMode,
    RakNetSocket2 *incomingRakNetSocket, bool *thisIPConnectedRecently, SystemAddress bindingAddress, int incomingMTU,
    RakNetGUID guid, bool useSecurity) {
      ...
      if (IsLoopbackAddress(systemAddress, false) == false) {
         for (i = 0; i < maximumNumberOfPeers; i++) {
            if (remoteSystemList[i].isActive == true &&
                ...
                time >= remoteSystemList[i].connectionTime && time - remoteSystemList[i].connectionTime < 100) {
               // 4/13/09 Attackers can flood ID_OPEN_CONNECTION_REQUEST and use up all available
               // connection slots Ignore connection attempts if this IP address connected within
   ...
   *thisIPConnectedRecently = false;
   for (assignedIndex = 0; assignedIndex < maximumNumberOfPeers; assignedIndex++) {
      if (remoteSystemList[assignedIndex].isActive == false) {
         ...
         remoteSystem->isActive = true;  // This one line causes future incoming packets to go
                                         // through the reliability layer
         // Reserve this reliability layer for ourselves.
         if (incomingMTU > remoteSystem->MTUSize) remoteSystem->MTUSize = incomingMTU;
         ...
         if (incomingRakNetSocket->GetBoundAddress() == bindingAddress) {
            ...
            for (ipListIndex = 0; ipListIndex < MAXIMUM_NUMBER_OF_INTERNAL_IDS; ipListIndex++) {
               if (ipList[ipListIndex] == UNASSIGNED_SYSTEM_ADDRESS) break;
            ...
            // 06/26/09 Unconfirmed report that Vista firewall blocks the reply if we force a
            // binding For now use the incoming socket only Originally this code was to force a
            // machine with multiple IP addresses to reply back on the IP that the datagram came in
            // on
            if (1 || foundIndex == (unsigned int)-1) {
               ...
               if (socketListIndex==socketList.Size())
               {
                       ...
                       if (incomingRakNetSocket->GetRemotePortRakNetWasStartedOn()==0)
                               rns = SocketLayer::CreateBoundSocket( this, bindingAddress.GetPort(),
     incomingRakNetSocket->GetBlockingSocket(), ipListFoundIndexStr, 0,
     incomingRakNetSocket->GetExtraSocketOptions(), incomingRakNetSocket->GetSocketFamily(),
     incomingRakNetSocket->GetChromeInstance() ); else rns =
     SocketLayer::CreateBoundSocket_PS3Lobby( bindingAddress.GetPort(),
     incomingRakNetSocket->GetBlockingSocket(), ipListFoundIndexStr,
...
void RakPeer::ReferenceRemoteSystem(const SystemAddress &sa, unsigned int remoteSystemListIndex) {
   // #ifdef _DEBUG
   // 	for ( int remoteSystemIndex = 0; remoteSystemIndex < maximumNumberOfPeers;
   // ++remoteSystemIndex )
   // 	{
   // 		if (remoteSystemList[remoteSystemIndex].isActive )
   // 		{
   // 			unsigned int hashIndex =
   // GetRemoteSystemIndex(remoteSystemList[remoteSystemIndex].systemAddress);
   ...
   if (oldAddress != UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      if (GetRemoteSystem(oldAddress) == &remoteSystemList[remoteSystemListIndex]) DereferenceRemoteSystem(oldAddress);
   }
   ...
   // 		{
   // 			unsigned int hashIndex =
   // GetRemoteSystemIndex(remoteSystemList[remoteSystemIndex].systemAddress); 			if
   // (hashIndex!=remoteSystemIndex)
   // 			{
   // 				RakAssert(hashIndex==remoteSystemIndex);
   // 			}
   // 		}
...
unsigned int RakPeer::GetRemoteSystemIndex(const SystemAddress &sa) const {
   ...
   RemoteSystemIndex *cur = remoteSystemLookup[hashIndex];
   while (cur != 0) {
      if (remoteSystemList[cur->index].systemAddress == sa) return cur->index;
      cur = cur->next;
   }
   return (unsigned int)-1;
}
...
RakPeer::RemoteSystemStruct *RakPeer::GetRemoteSystem(const SystemAddress &sa) const {
   ...
   if (remoteSystemIndex == (unsigned int)-1) return 0;
...
void RakPeer::RemoveFromActiveSystemList(const SystemAddress &sa) {
   ...
   RakAssert(
       "activeSystemList invalid, entry not found in RemoveFromActiveSystemList. Ensure that "
       "AddToActiveSystemList and RemoveFromActiveSystemList are called by the same thread." &&
       0);
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
...
unsigned int RakPeer::LookupIndexUsingHashIndex(const SystemAddress &sa) const
{
        ...
        while (remoteSystemList[remoteSystemLookup[index].index].systemAddress!=sa)
        {
                if (++index==(unsigned int) maximumNumberOfPeers*REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE)
                        index=0;
                if (++scanCount>(unsigned int)
maximumNumberOfPeers*REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE) return (unsigned int) -1; if
(remoteSystemLookup[index].index==-1) return (unsigned int) -1;
        }
        return index;
}
//
...
bool RakPeer::IsLoopbackAddress(const AddressOrGUID &systemIdentifier, bool matchPort) const {
   if (systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID) return systemIdentifier.rakNetGuid == myGuid;

   for (int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS && ipList[i] != UNASSIGNED_SYSTEM_ADDRESS; i++) {
      if (matchPort) {
         if (ipList[i] == systemIdentifier.systemAddress) return true;
      } else {
         if (ipList[i].EqualsExcludingPort(systemIdentifier.systemAddress)) return true;
      }
   }
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SystemAddress RakPeer::GetLoopbackAddress(void) const { return ipList[0]; }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void RakPeer::ClearBufferedPackets(void) {
   bufferedPacketsFreePoolMutex.Lock();
   while (bufferedPacketsFreePool.Size() > 0) RakNet::OP_DELETE(bufferedPacketsFreePool.Pop(), _FILE_AND_LINE_);
   bufferedPacketsFreePoolMutex.Unlock();

   bufferedPacketsQueueMutex.Lock();
   while (bufferedPacketsQueue.Size() > 0) RakNet::OP_DELETE(bufferedPacketsQueue.Pop(), _FILE_AND_LINE_);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::CloseConnectionInternal(const AddressOrGUID &systemIdentifier, bool sendDisconnectionNotification,
                                      bool performImmediate, unsigned char orderingChannel,
                                      PacketPriority disconnectionNotificationPriority) {
#ifdef _DEBUG
   RakAssert(orderingChannel < 32);
#endif

   if (systemIdentifier.IsUndefined()) return;
      ...
      if (performImmediate) {
         ...
         if (index != (unsigned int)-1) {
            if (remoteSystemList[index].isActive) {
               ...
               // Reserve this reliability layer for ourselves
               // remoteSystemList[ remoteSystemLookup[index].index ].systemAddress =
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBuffered(const char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
                           PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                           bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt) {
   BufferedCommandStruct *bcs;

   bcs = bufferedCommands.Allocate(_FILE_AND_LINE_);
   bcs->data = (char *)rakMalloc_Ex((size_t)BITS_TO_BYTES(numberOfBitsToSend),
                                    _FILE_AND_LINE_);  // Making a copy doesn't lose efficiency because I tell the
                                                       // reliability layer to use this allocation for its own copy
   if (bcs->data == 0) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBufferedList(const char **data, const int *lengths, const int numParameters, PacketPriority priority,
                               PacketReliability reliability, char orderingChannel,
                               const AddressOrGUID systemIdentifier, bool broadcast,
                               RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt) {
   BufferedCommandStruct *bcs;
   unsigned int totalLength = 0;
   unsigned int lengthOffset;
   int i;
   for (i = 0; i < numParameters; i++) {
      if (lengths[i] > 0) totalLength += lengths[i];
   }
   if (totalLength == 0) return;

   char *dataAggregate;
   dataAggregate = (char *)rakMalloc_Ex((size_t)totalLength,
                                        _FILE_AND_LINE_);  // Making a copy doesn't lose efficiency because I tell the
                                                           // reliability layer to use this allocation for its own copy
   if (dataAggregate == 0) {
      notifyOutOfMemory(_FILE_AND_LINE_);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendImmediate(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
                            PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier,
                            bool broadcast, bool useCallerDataAllocation, RakNet::TimeUS currentTime,
                            uint32_t receipt) {
   unsigned *sendList;
...
//			int debugIndex = GetRemoteSystemIndex(systemIdentifier.systemAddress);
      ...
      for (idx = 0; idx < maximumNumberOfPeers; idx++) {
         if (remoteSystemIndex != (unsigned int)-1 && idx == remoteSystemIndex) continue;
   ...
   for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++) {
      // Send may split the packet and thus deallocate data.  Don't assume data is valid if we use
      // the callerAllocationData
      ...
      remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(
          data, numberOfBitsToSend, priority, reliability, orderingChannel, useData == false,
          remoteSystemList[sendList[sendListIndex]].MTUSize, currentTime, receipt);
      ...
      if (reliability == RELIABLE || reliability == RELIABLE_ORDERED || reliability == RELIABLE_SEQUENCED ||
          reliability == RELIABLE_WITH_ACK_RECEIPT || reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT
          //			||
          //			reliability==RELIABLE_SEQUENCED_WITH_ACK_RECEIPT
      )
         remoteSystemList[sendList[sendListIndex]].lastReliableSend =
             (RakNet::TimeMS)(currentTime / (RakNet::TimeUS)1000);
   }
   ...
   // Return value only meaningful if true was passed for useCallerDataAllocation.  Means the
   // reliability layer used that data copy, so the caller should not deallocate it
...
void RakPeer::OnConnectedPong(RakNet::Time sendPingTime, RakNet::Time sendPongTime, RemoteSystemStruct *remoteSystem) {
   ...
   remoteSystem->pingAndClockDifferential[remoteSystem->pingAndClockDifferentialWriteIndex].pingTime =
       (unsigned short)ping;
   // Thanks to Chris Taylor (cat02e@fsu.edu) for the improved timestamping algorithm
   // Divide each integer by 2, rather than the sum by 2, to prevent overflow
   remoteSystem->pingAndClockDifferential[remoteSystem->pingAndClockDifferentialWriteIndex].clockDifferential =
       sendPongTime - (time / 2 + sendPingTime / 2);
...
void RakPeer::ClearBufferedCommands(void) {
   ...
   while ((bcs = bufferedCommands.Pop()) != 0) {
      if (bcs->data) rakFree_Ex(bcs->data, _FILE_AND_LINE_);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ClearSocketQueryOutput(void) { socketQueryOutput.Clear(_FILE_AND_LINE_); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ClearRequestedConnectionList(void) {
   DataStructures::Queue<RequestedConnectionStruct *> freeQueue;
   requestedConnectionQueueMutex.Lock();
   while (requestedConnectionQueue.Size()) freeQueue.Push(requestedConnectionQueue.Pop(), _FILE_AND_LINE_);
   requestedConnectionQueueMutex.Unlock();
   unsigned i;
   for (i = 0; i < freeQueue.Size(); i++) {
#if LIBCAT_SECURITY == 1
      CAT_AUDIT_PRINTF(
          "AUDIT: In ClearRequestedConnectionList(), Deleting freeQueue index %i "
          "client_handshake %x\n",
...
uint64_t RakPeerInterface::Get64BitUniqueRandomNumber(void) {
   // Mac address is a poor solution because you can't have multiple connections from the same
   // system
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::GenerateGUID(void) { myGuid.g = Get64BitUniqueRandomNumber(); }
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
namespace RakNet {
bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer,
   ...
   if (rakPeer->IsBanned(str1)) {
      ...
      /*
                      unsigned i;
                      for (i=0; i < rakPeer->pluginListNTS.Size(); i++)
                              rakPeer->pluginListNTS[i]->OnDirectSocketSend((char*) bs.GetData(),
         bs.GetNumberOfBitsUsed(), systemAddress); SocketLayer::SendTo( rakNetSocket, (char*)
         bs.GetData(), bs.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
                      */
   ...
   // messages that arrived late for a player that is now connected, and a regular encoding. So I
   // insert OFFLINE_MESSAGE_DATA_ID into the stream, the encoding of which is essentially
   // impossible to hit by chance
   if (length <= 2) {
      ...
      *isOfflineMessage = memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time), OFFLINE_MESSAGE_DATA_ID,
              ...
              (size_t)length >= sizeof(unsigned char) + sizeof(RakNet::TimeMS) + RakNetGUID::size() +
      ...
      *isOfflineMessage = memcmp(data + sizeof(MessageID) + RakNetGUID::size(), OFFLINE_MESSAGE_DATA_ID,
                                 sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
   } else if (((unsigned char)data[0] == ID_OPEN_CONNECTION_REPLY_1 ||
               ...
               (unsigned char)data[0] == ID_CONNECTION_BANNED || (unsigned char)data[0] == ID_ALREADY_CONNECTED ||
               (unsigned char)data[0] == ID_IP_RECENTLY_CONNECTED) &&
              (size_t)length >= sizeof(MessageID) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)) {
      *isOfflineMessage =
          memcmp(data + sizeof(MessageID), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
      ...
      *isOfflineMessage =
          memcmp(data + sizeof(MessageID) * 2, OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
   } else {
   ...
   if (*isOfflineMessage) {
      ...
      // These are all messages from unconnected systems.  Messages here can be any size, but are
      // never processed from connected systems.
      if (((unsigned char)data[0] == ID_UNCONNECTED_PING_OPEN_CONNECTIONS ||
             ...
             rakPeer->AllowIncomingConnections())  // Open connections with players
         {
            ...
            outBitStream.WriteAlignedBytes((const unsigned char *)OFFLINE_MESSAGE_DATA_ID,
                                           sizeof(OFFLINE_MESSAGE_DATA_ID));
            ...
            for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
               rakPeer->pluginListNTS[i]->OnDirectSocketSend((const char *)outBitStream.GetData(),
                                                             outBitStream.GetNumberOfBytesUsed(), systemAddress);
            ...
            // SocketLayer::SendTo( rakNetSocket, (const char*)outBitStream.GetData(), (unsigned
            // int) outBitStream.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
      ...
      // UNCONNECTED MESSAGE Pong with no data.
         ...
         bsOut.Write(pingMS);
         bsOut.WriteAlignedBytes((const unsigned char *)data + sizeof(unsigned char) + sizeof(RakNet::Time) +
                                     RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID),
                                 length - sizeof(unsigned char) - sizeof(RakNet::Time) - RakNetGUID::size() -
                                     sizeof(OFFLINE_MESSAGE_DATA_ID));
         ...
         rakPeer->AddPacketToProducer(packet);
                 ...
                 (size_t)length < MAX_OFFLINE_DATA_LENGTH + sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) +
                                      RakNetGUID::size()) {
         unsigned int dataLength =
             (unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) - RakNetGUID::size() - sizeof(MessageID));
         ...
         if (data[sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) + RakNetGUID::size()] == ID_ADVERTISE_SYSTEM) {
            ...
            memcpy(packet->data + 1,
                   data + sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) * 2 + RakNetGUID::size(), dataLength - 1);
         } else {
            packet->data[0] = ID_OUT_OF_BAND_INTERNAL;
            memcpy(packet->data + 1, data + sizeof(OFFLINE_MESSAGE_DATA_ID) + sizeof(MessageID) + RakNetGUID::size(),
                   dataLength);
         }
         ...
         // Even if the server has security, it may not be required of us if we are in the security
         // exception list
         ...
         if (serverHasSecurity) bsOut.Write(cookie);
         ...
         for (i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++) {
            ...
            if (rcs->systemAddress == systemAddress) {
               if (serverHasSecurity) {
                  ...
                  if (cat::SecureEqual(public_key, rcs->remote_public_key, cat::EasyHandshake::PUBLIC_KEY_BYTES) ==
                      false) {
                     rakPeer->requestedConnectionQueueMutex.Unlock();
                     CAT_AUDIT_PRINTF(
                         "AUDIT: Expected public key does not match what was sent by server "
                         "-- Reporting back ID_PUBLIC_KEY_MISMATCH to user\n");
        ...
        // Message does not contain a challenge
                  ...
                  if (rcs->client_handshake != 0) {
                     rakPeer->requestedConnectionQueueMutex.Unlock();
                     CAT_AUDIT_PRINTF(
                         "AUDIT: Security disabled by server but we expected security "
                         "(indicated by client_handshake not null) so failing!\n");
               ...
               for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
                  rakPeer->pluginListNTS[i]->OnDirectSocketSend((const char *)bsOut.GetData(),
                                                                bsOut.GetNumberOfBitsUsed(), rcs->systemAddress);

               // SocketLayer::SendTo( rakPeer->socketList[rcs->socketIndex], (const char*)
               // bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), rcs->systemAddress, _FILE_AND_LINE_
               // );
...
#if LIBCAT_SECURITY == 1
               CAT_AUDIT_PRINTF(
                   "AUDIT: System address matches an entry in the requestedConnectionQueue "
                   "and doSecurity=%i\n",
                   (int)doSecurity);
               if (doSecurity) {
                  if (rcs->client_handshake == 0) {
                     CAT_AUDIT_PRINTF(
                         "AUDIT: Server wants security but we didn't set a public key -- "
                         "Reporting back ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY to user\n");
                     rakPeer->requestedConnectionQueueMutex.Unlock();

                     packet = rakPeer->AllocPacket(2, _FILE_AND_LINE_);
                     packet->data[0] = ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY;  // Attempted a connection and
                                                                              // couldn't
                  ...
                  CAT_AUDIT_PRINTF(
                      "AUDIT: Looks good, preparing to send challenge to server! client_handshake "
                      "= %x\n",
                      client_handshake);
               }
               ...
               if (remoteSystem == 0) {
                  if (rcs->socket == 0) {
                     remoteSystem = rakPeer->AssignSystemAddressToRemoteSystemList(
                         systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rakNetSocket,
                         &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                  } else {
                     remoteSystem = rakPeer->AssignSystemAddressToRemoteSystemList(
                         systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rcs->socket,
                         &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                  }
               }
                  ...
                  if (remoteSystem) {
                     ...
                     if (rcs->client_handshake) {
                        ...
                        if (rcs->publicKeyMode == PKM_USE_TWO_WAY_AUTHENTICATION) {
                           if (!rcs->client_handshake->ProcessAnswerWithIdentity(
                                   answer, ident, remoteSystem->reliabilityLayer.GetAuthenticatedEncryption())) {
                           ...
                           if (!rcs->client_handshake->ProcessAnswer(
                                   answer, remoteSystem->reliabilityLayer.GetAuthenticatedEncryption())) {
                              CAT_AUDIT_PRINTF("AUDIT: Processing answer -- Invalid Answer\n");
                              rakPeer->requestedConnectionQueueMutex.Unlock();
                     ...
                     if (rcs->timeoutTime != 0) remoteSystem->reliabilityLayer.SetTimeoutTime(rcs->timeoutTime);
                     ...
                     if (doSecurity) {
                        ...
                        remoteSystem->reliabilityLayer.GetAuthenticatedEncryption()->GenerateProof(proof,
                                                                                                   sizeof(proof));
                     ...
                     rakPeer->SendImmediate((char *)temp.GetData(), temp.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
                                            RELIABLE, 0, systemAddress, false, false, timeRead, 0);
                  } else {
...
#if LIBCAT_SECURITY == 1
               CAT_AUDIT_PRINTF(
                   "AUDIT: Deleting client_handshake object %x and rcs->client_handshake object "
                   "%x\n",
         ...
         if (unlock) rakPeer->requestedConnectionQueueMutex.Unlock();
         ...
         for (i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++) {
            rcs = rakPeer->requestedConnectionQueue[i];
            if (rcs->actionToTake == RakPeer::RequestedConnectionStruct::CONNECT &&
                rcs->systemAddress == systemAddress) {
...
#if LIBCAT_SECURITY == 1
               CAT_AUDIT_PRINTF(
                   "AUDIT: Connection attempt canceled so deleting rcs->client_handshake object "
                   "%x\n",
         ...
         if (connectionAttemptCancelled) {
            ...
            rakPeer->AddPacketToProducer(packet);
                 ...
                 length > (int)(1 + sizeof(OFFLINE_MESSAGE_DATA_ID))) { /*
                                                                               static int x = 0;
                                                                               ++x;
         ...
         if (remoteProtocol != RAKNET_PROTOCOL_VERSION) {
            ...
            for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
               rakPeer->pluginListNTS[i]->OnDirectSocketSend((char *)bs.GetData(), bs.GetNumberOfBitsUsed(),
                                                             systemAddress);

            // SocketLayer::SendTo( rakNetSocket, (char*)bs.GetData(), bs.GetNumberOfBytesUsed(),
         ...
         // Pad response with zeros to MTU size so the connection's MTU will be tested in both
         // directions
         bsOut.PadWithZeroToByteLength(mtu - bsOut.GetNumberOfBytesUsed());

         for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
            rakPeer->pluginListNTS[i]->OnDirectSocketSend((const char *)bsOut.GetData(), bsOut.GetNumberOfBitsUsed(),
                                                          systemAddress);
         // SocketLayer::SendTo( rakNetSocket, (const char*) bsOut.GetData(),
         // bsOut.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
         ...
         bsp.systemAddress = systemAddress;
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
         if (rakNetSocket->IsBerkleySocket()) ((RNS2_Berkley *)rakNetSocket)->SetDoNotFragment(1);
#endif
         rakNetSocket->Send(&bsp, _FILE_AND_LINE_);
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
         if (rakNetSocket->IsBerkleySocket()) ((RNS2_Berkley *)rakNetSocket)->SetDoNotFragment(0);
#endif
         ...
         // TRUE,	  , TRUE	 , ID_OPEN_CONNECTION_REPLY if they are the same, else
         // ID_ALREADY_CONNECTED FALSE,     , TRUE     , ID_ALREADY_CONNECTED (someone else took
         // this guid) TRUE,	  , FALSE	 , ID_ALREADY_CONNECTED (silently disconnected,
         // restarted rakNet) FALSE	  , FALSE	 , Allow connection
         ...
         if (IPAddrInUse & GUIDInUse) {
            if (rssFromSA == rssFromGuid && rssFromSA->connectMode == RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER) {
               ...
               // Note to self: If REQUESTED_CONNECTION, this means two systems attempted to connect
               // to each other at the same time, and one finished first. Returns
               // ID)_CONNECTION_REQUEST_ACCEPTED to one system, and ID_ALREADY_CONNECTED followed
               // by ID_NEW_INCOMING_CONNECTION to another
            } else {
               // ID_ALREADY_CONNECTED (restarted raknet, connected again from same ip, plus someone
               // else took this guid)
               outcome = 2;
            }
         ...
         if (outcome == 1) {
            ...
            if (requiresSecurityOfThisClient) {
               CAT_AUDIT_PRINTF(
                   "AUDIT: Resending public key and answer from packetloss.  Sending "
            ...
            unsigned int i;
            for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
               rakPeer->pluginListNTS[i]->OnDirectSocketSend((const char *)bsAnswer.GetData(),
                                                             bsAnswer.GetNumberOfBitsUsed(), systemAddress);
            // SocketLayer::SendTo( rakNetSocket, (const char*) bsAnswer.GetData(),
            // bsAnswer.GetNumberOfBytesUsed(), systemAddress, _FILE_AND_LINE_ );
         ...
         bool thisIPConnectedRecently = false;
         rssFromSA = rakPeer->AssignSystemAddressToRemoteSystemList(
             systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rakNetSocket, &thisIPConnectedRecently,
             bindingAddress, mtu, guid, requiresSecurityOfThisClient);
         ...
         if (requiresSecurityOfThisClient) {
            ...
            if (rakPeer->_server_handshake->ProcessChallenge(
                    remoteHandshakeChallenge, rssFromSA->answer,
                    rssFromSA->reliabilityLayer.GetAuthenticatedEncryption())) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void ProcessNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer,
...
void ProcessNetworkPacket(SystemAddress systemAddress, const char *data, const int length, RakPeer *rakPeer,
   ...
   if (remoteSystem) {
      ...
      // HandleSocketReceiveFromConnectedPlayer is only safe to be called from the same thread as
      // Update, which is this thread
      if (isOfflineMessage == false) {
         remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(
             data, length, systemAddress, rakPeer->pluginListNTS, remoteSystem->MTUSize, rakNetSocket, &rnr, timeRead,
             updateBitStream);
      }
   } else {
      // int a=5;
...
unsigned int RakPeer::GenerateSeedFromGuid(void) {
   /*
   // Construct a random seed based on the initial guid value, and the last digits of the difference
   to each subsequent number
   // This assumes that only the last 3 bits of each guidId integer has a meaningful amount of
   randomness between it and the prior number unsigned int t = guid.g[0]; unsigned int i; for (i=1;
   i < sizeof(guid.g) / sizeof(guid.g[0]); i++)
   {
...
unsigned int RakPeer::GetRakNetSocketFromUserConnectionSocketIndex(unsigned int userIndex) const {
   unsigned int i;
   for (i = 0; i < socketList.Size(); i++) {
      if (socketList[i]->GetUserConnectionSocketIndex() == userIndex) return i;
   }
...
/*
// DS_APR
void RakPeer::ProcessChromePacket(RakNetSocket2 *s, const char *buffer, int dataSize, const
SystemAddress& recvFromAddress, RakNet::TimeUS timeRead)
{
        RakAssert(buffer);
...
bool RakPeer::RunRecvFromOnce( RakNetSocket *s )
{
        ...
        {
                recvFromStruct->s=s;
                SocketLayer::RecvFromBlocking(s, this, recvFromStruct->data,
&recvFromStruct->bytesRead, &recvFromStruct->systemAddress, &recvFromStruct->timeRead);
...
bool RakPeer::RunUpdateCycle(BitStream &updateBitStream) {
   ...
   while ((recvFromStruct = PopBufferedPacket()) != 0) {
      ...
      if (socketListIndex!=socketList.Size())
      */
      ProcessNetworkPacket(recvFromStruct->systemAddress, recvFromStruct->data, recvFromStruct->bytesRead, this,
                           recvFromStruct->socket, recvFromStruct->timeRead, updateBitStream);
   ...
   while ((bcs = bufferedCommands.PopInaccurate()) != 0) {
      if (bcs->command == BufferedCommandStruct::BCS_SEND) {
         ...
         callerDataAllocationUsed =
             SendImmediate((char *)bcs->data, bcs->numberOfBitsToSend, bcs->priority, bcs->reliability,
                           bcs->orderingChannel, bcs->systemIdentifier, bcs->broadcast, true, timeNS, bcs->receipt);
         if (callerDataAllocationUsed == false) rakFree_Ex(bcs->data, _FILE_AND_LINE_);

         // Set the new connection state AFTER we call sendImmediate in case we are setting it to a
         // disconnection state, which does not allow further sends
         if (bcs->connectionMode != RemoteSystemStruct::NO_ACTION) {
            remoteSystem = GetRemoteSystem(bcs->systemIdentifier, true, true);
            if (remoteSystem) remoteSystem->connectMode = bcs->connectionMode;
         }
   ...
   if (requestedConnectionQueue.IsEmpty() == false) {
      ...
      while (requestedConnectionQueueIndex < requestedConnectionQueue.Size()) {
         ...
         if (rcs->nextRequestTime < timeMS) {
            ...
            // If too many requests made or a hole then remove this if possible, otherwise
            // invalidate it
            if (condition1 || condition2) {
               ...
               int denom = rcs->sendConnectionAttemptCount / NUM_MTU_SIZES;
               if (!denom) denom = 1;
               int MTUSizeIndex = rcs->requestsMade / denom;
               if (MTUSizeIndex >= NUM_MTU_SIZES) MTUSizeIndex = NUM_MTU_SIZES - 1;
               rcs->requestsMade++;
               ...
               bitStream.WriteAlignedBytes((const unsigned char *)OFFLINE_MESSAGE_DATA_ID,
                                           sizeof(OFFLINE_MESSAGE_DATA_ID));
               ...
               for (i = 0; i < pluginListNTS.Size(); i++)
                  pluginListNTS[i]->OnDirectSocketSend((const char *)bitStream.GetData(),
                                                       bitStream.GetNumberOfBitsUsed(), rcs->systemAddress);
...
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
               if (socketToUse->IsBerkleySocket()) ((RNS2_Berkley *)socketToUse)->SetDoNotFragment(1);
#endif

               //					SocketLayer::SetDoNotFragment(socketToUse,
               // 1);
               ...
               // if (SocketLayer::SendTo( socketToUse, (const char*) bitStream.GetData(),
               // bitStream.GetNumberOfBytesUsed(), rcs->systemAddress, _FILE_AND_LINE_ )==-10040)
               {
                  // Don't use this MTU size again
                  rcs->requestsMade =
                      (unsigned char)((MTUSizeIndex + 1) * (rcs->sendConnectionAttemptCount / NUM_MTU_SIZES));
...
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
               if (socketToUse->IsBerkleySocket()) ((RNS2_Berkley *)socketToUse)->SetDoNotFragment(0);
#endif
   ...
   {
      // I'm using systemAddress from remoteSystemList but am not locking it because this loop is
      // called very frequently and it doesn't matter if we miss or do an extra update.  The
      // reliability layers themselves never care which player they are associated with
      // systemAddress = remoteSystemList[ remoteSystemIndex ].systemAddress;
      // Allow the systemAddress for this remote system list to change.  We don't care if it changes
      // now.
      ...
      RakAssert(systemAddress != UNASSIGNED_SYSTEM_ADDRESS);
      // Update is only safe to call from the same thread that calls
      // HandleSocketReceiveFromConnectedPlayer, which is this thread
      ...
      if (timeMS > remoteSystem->lastReliableSend &&
          timeMS - remoteSystem->lastReliableSend > remoteSystem->reliabilityLayer.GetTimeoutTime() / 2 &&
          remoteSystem->connectMode == RemoteSystemStruct::CONNECTED) {
         // If no reliable packets are waiting for an ack, do a one byte reliable send so that
         // disconnections are noticed
      ...
      remoteSystem->reliabilityLayer.Update(
          remoteSystem->rakNetSocket, systemAddress, remoteSystem->MTUSize, timeNS, maxOutgoingBPS, pluginListNTS, &rnr,
          updateBitStream);  // systemAddress only used for the internet simulator test

      // Check for failure conditions
      if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
          ((remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP ||
            remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY) &&
           remoteSystem->reliabilityLayer.IsOutgoingDataWaiting() == false) ||
          (remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ON_NO_ACK &&
           (remoteSystem->reliabilityLayer.AreAcksWaiting() == false ||
            remoteSystem->reliabilityLayer.AckTimeout(timeMS) == true)) ||
         ...
         // TODO - RakNet 4.0 - Return a different message identifier for DISCONNECT_ASAP_SILENTLY
         // and DISCONNECT_ASAP than for DISCONNECT_ON_NO_ACK The first two mean we called
         // CloseConnection(), the last means the other system sent us ID_DISCONNECTION_NOTIFICATION
         if (remoteSystem->connectMode == RemoteSystemStruct::CONNECTED ||
            ...
            //					memcpy(packet->data+1,
            // undeliveredMessages.GetData(), undeliveredMessages.GetNumberOfBytesUsed());
      ...
      if (remoteSystem->connectMode == RemoteSystemStruct::CONNECTED && timeMS > remoteSystem->nextPingTime &&
          (occasionalPing || remoteSystem->lowestPing == (unsigned short)-1)) {
      ...
      while (bitSize > 0) {
         ...
         if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER) {
            ...
            // However, if we are connected we still take a connection request in case both systems
            // are trying to connect to each other at the same time
            if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST) {
               // 04/27/06 This is wrong.  With cross connections, we can both have initiated the
               // connection are in state REQUESTED_CONNECTION 04/28/06 Downgrading connections from
               // connected will close the connection due to security at
               // ((remoteSystem->connectMode!=RemoteSystemStruct::CONNECTED && time >
               // remoteSystem->connectionTime && time - remoteSystem->connectionTime > 10000))
               if (remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION) {
                  ParseConnectionRequestPacket(remoteSystem, systemAddress, (const char *)data, byteSize);
               } else {
                  RakNet::BitStream bs((unsigned char *)data, byteSize, false);
                  ...
                  // Got a connection request message from someone we are already connected to. Just
                  // reply normally. This can happen due to race conditions with the fully connected
                  // mesh
                  OnConnectionRequest(remoteSystem, incomingTimestamp);
               }
               rakFree_Ex(data, _FILE_AND_LINE_);
            } else if ((unsigned char)data[0] == ID_NEW_INCOMING_CONNECTION &&
                       byteSize > sizeof(unsigned char) + sizeof(unsigned int) + sizeof(unsigned short) +
               ...
               if (remoteSystem->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST) {
                  ...
                  //					NewIncomingConnectionStruct
                  // newIncomingConnectionStruct; RakNet::BitStream nICS_BS( data,
                  ...
                  // Bug: If A connects to B through R, A's firstExternalID is set to R. If A tries
                  // to send to R, sends to loopback because R==firstExternalID Correct fix is to
                  // specify in Connect() if target is through a proxy. However, in practice you
                  // have to connect to something else first anyway to know about the proxy. So
                  // setting once only is good enough
                  ...
                  // Send to game even if already connected. This could happen when connecting to
                  // 127.0.0.1 Ignore, already connected
                  //	rakFree_Ex(data, _FILE_AND_LINE_ );
               }
            } else if ((unsigned char)data[0] == ID_CONNECTED_PONG &&
                       byteSize == sizeof(unsigned char) + sizeof(RakNet::Time) * 2) {
               RakNet::Time sendPingTime, sendPongTime;
               ...
               rakFree_Ex(data, _FILE_AND_LINE_);
            } else if ((unsigned char)data[0] == ID_CONNECTED_PING &&
                       byteSize == sizeof(unsigned char) + sizeof(RakNet::Time)) {
               RakNet::BitStream inBitStream((unsigned char *)data, byteSize, false);
               inBitStream.IgnoreBits(8);
               RakNet::Time sendPingTime;
               inBitStream.Read(sendPingTime);
               ...
               SendImmediate((char *)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY,
                             UNRELIABLE, 0, systemAddress, false, false, RakNet::GetTimeUS(), 0);
               ...
               // We shouldn't close the connection immediately because we need to ack the
               ...
               if (byteSize > sizeof(MessageID) + sizeof(unsigned int) + sizeof(unsigned short) + sizeof(SystemIndex) +
                  ...
                  if (allowConnection) {
                     ...
                     //						RakNet::BitStream casBitS(data,
                     // byteSize, false); ConnectionAcceptStruct cas;
                     ...
                     // Bug: If A connects to B through R, A's firstExternalID is set to R. If A
                     // tries to send to R, sends to loopback because R==firstExternalID Correct fix
                     // is to specify in Connect() if target is through a proxy. However, in
                     // practice you have to connect to something else first anyway to know about
                     // the proxy. So setting once only is good enough
                     if (firstExternalID == UNASSIGNED_SYSTEM_ADDRESS) {
                     ...
                     for (unsigned int i = 0; i < MAXIMUM_NUMBER_OF_INTERNAL_IDS; i++) outBitStream.Write(ipList[i]);
               ...
               if ((data[0] >= (MessageID)ID_TIMESTAMP || data[0] == ID_SND_RECEIPT_ACKED ||
                    data[0] == ID_SND_RECEIPT_LOSS) &&
...
void RakPeer::OnRNS2Recv(RNS2RecvStruct *recvStruct) {
   if (incomingDatagramEventHandler) {
      if (incomingDatagramEventHandler(recvStruct) != true) return;
   }
...
{
#if defined(SN_TARGET_PSP2)
        RakPeerAndIndex *rpai = ( RakPeerAndIndex * )
RakThread::GetRealThreadArgument(callGetRealThreadArgument); #else RakPeerAndIndex *rpai = (
RakPeerAndIndex * ) arguments; #endif RakPeer * rakPeer = rpai->rakPeer; RakNetSocket *s = rpai->s;
...
RAK_THREAD_DECLARATION(RakNet::UpdateNetworkLoop) {
   ...
   /*
           ...
           BOOL success = SetWaitableTimer( timerHandle, &dueTime, rakPeer->threadSleepTimer, NULL,
   NULL, FALSE ); (void) success; RakAssert( success );
   ...
   */
   ...
   while (rakPeer->endThreads == false) {
      // #ifdef _DEBUG
      // 		// Sanity check, make sure RunUpdateCycle does not block or not otherwise
      // get called for a long time 		RakNetTime thisCall=RakNet::GetTime();
      // RakAssert(thisCall-lastCall<250); 		lastCall=thisCall; #endif
      if (rakPeer->userUpdateThreadPtr) rakPeer->userUpdateThreadPtr(rakPeer, rakPeer->userUpdateThreadData);
  ...
  // #if ((_WIN32_WINNT >= 0x0400) || (_WIN32_WINDOWS > 0x0400)) &&
      ...
      if (rakPeer->threadSleepTimer>0)
      {
              ...
              WSAWaitForMultipleEvents(eventArrayIndex,(const HANDLE*)
  &eventArray,FALSE,rakPeer->threadSleepTimer,FALSE);
      }
      else
      {
              RakSleep(0);
  ...
  #else // ((_WIN32_WINNT >= 0x0400) || (_WIN32_WINDOWS > 0x0400)) &&
  defined(USE_WAIT_FOR_MULTIPLE_EVENTS) #pragma message("-- RakNet: Using Sleep(). Uncomment
  USE_WAIT_FOR_MULTIPLE_EVENTS in RakNetDefines.h if you want to use WaitForSingleObject instead.
  --")
...
void RakPeer::FillIPList(void) {
   if (ipList[0] != UNASSIGNED_SYSTEM_ADDRESS) return;
   ...
   while (startingIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 && ipList[startingIdx] != UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      for (int curIdx = startingIdx + 1;
           curIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 && ipList[curIdx] != UNASSIGNED_SYSTEM_ADDRESS; curIdx++) {
