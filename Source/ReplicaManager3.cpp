...
int Connection_RM3::Replica3LSRComp(Replica3 *const &replica3, LastSerializationResult *const &data) {
   ...
   // 7/28/2013 - If GetNetworkID chagned during runtime, the list would be out of order and lookup
   // would always fail or go out of bounds I remember before that I could not directly compare
   if (replica3->referenceIndex < data->replica->referenceIndex) return -1;
   if (replica3->referenceIndex > data->replica->referenceIndex) return 1;
   return 0;
}
...
LastSerializationResult::~LastSerializationResult() {
   if (lastSerializationResultBS) RakNet::OP_DELETE(lastSerializationResultBS, _FILE_AND_LINE_);
}
...
bool ReplicaManager3::GetAutoCreateConnections(void) const { return autoCreateConnections; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

bool ReplicaManager3::GetAutoDestroyConnections(void) const { return autoDestroyConnections; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void ReplicaManager3::AutoCreateConnectionList(DataStructures::List<RakNetGUID> &participantListIn,
   ...
   for (unsigned int index = 0; index < participantListIn.Size(); index++) {
      if (GetConnectionByGUID(participantListIn[index], worldId) == NULL) {
         Connection_RM3 *connection = AllocConnection(
             rakPeerInterface->GetSystemAddressFromGuid(participantListIn[index]), participantListIn[index]);
         if (connection) {
...
bool ReplicaManager3::PushConnection(RakNet::Connection_RM3 *newConnection, WorldId worldId) {
   ...
   if (GetConnectionByGUID(newConnection->GetRakNetGUID(), worldId)) return false;
   // Was this intended?
...
void ReplicaManager3::DeallocReplicaNoBroadcastDestruction(RakNet::Connection_RM3 *connection,
...
RakNet::Connection_RM3 *ReplicaManager3::PopConnection(unsigned int index, WorldId worldId) {
   ...
   for (index2 = 0; index2 < destructionList.Size(); index2++) {
      // Do lookup in case DeallocReplica destroyed one of of the later Replica3 instances in the
      // list
...
void ReplicaManager3::DereferenceList(DataStructures::List<Replica3 *> &replicaListIn, WorldId worldId) {
   unsigned int index;
   for (index = 0; index < replicaListIn.Size(); index++) Dereference(replicaListIn[index], worldId);
}
...
void ReplicaManager3::SetDefaultOrderingChannel(char def) { defaultSendParameters.orderingChannel = def; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void ReplicaManager3::SetDefaultPacketPriority(PacketPriority def) { defaultSendParameters.priority = def; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void ReplicaManager3::SetDefaultPacketReliability(PacketReliability def) { defaultSendParameters.reliability = def; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void ReplicaManager3::SetAutoSerializeInterval(RakNet::Time intervalMS) { autoSerializeInterval = intervalMS; }
...
bool ReplicaManager3::GetAllConnectionDownloadsCompleted(WorldId worldId) const {
   ...
   unsigned int index;
   for (index = 0; index < world->connectionList.Size(); index++) {
      if (world->connectionList[index]->GetDownloadWasCompleted() == false) return false;
   }
   return true;
}
...
void ReplicaManager3::Clear(bool deleteWorlds) {
   ...
   if (deleteWorlds) worldsList.Clear(false, _FILE_AND_LINE_);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

ReplicaManager3::RM3World::RM3World() { networkIDManager = 0; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void ReplicaManager3::RM3World::Clear(ReplicaManager3 *replicaManager3) {
   if (replicaManager3->GetAutoDestroyConnections()) {
      for (unsigned int i = 0; i < connectionList.Size(); i++) replicaManager3->DeallocConnection(connectionList[i]);
   } else {
      // Clear out downloadGroup even if not auto destroying the connection, since the packets need
      // to go back to RakPeer
...
PRO ReplicaManager3::GetDefaultSendParameters(void) const { return defaultSendParameters; }
...
PluginReceiveResult ReplicaManager3::OnReceive(Packet *packet) {
   ...
   if (worldsArray[incomingWorldId] == 0) return RR_CONTINUE_PROCESSING;
...
void Connection_RM3::AutoConstructByQuery(ReplicaManager3 *replicaManager3, WorldId worldId) {
   ...
   if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
      ...
      while (index < queryToConstructReplicaList.Size()) {
         ...
         if (constructionState == RM3CS_ALREADY_EXISTS_REMOTELY ||
             constructionState == RM3CS_ALREADY_EXISTS_REMOTELY_DO_NOT_CONSTRUCT) {
            ...
            if (constructionState==RM3CS_ALREADY_EXISTS_REMOTELY)
            {
                    ...
                    if (bsOut.GetNumberOfBitsUsed()!=bitsWritten)
                            replicaManager3->SendUnified(&bsOut,HIGH_PRIORITY,RELIABLE_ORDERED,0,GetSystemAddress(),
            false);
            }
                    ...
                    res!=RM3SR_SERIALIZED_UNIQUELY)
            {
                    ...
                    if (SendSerialize(replica, allIndices, sp.outputBitstream, sp.messageTimestamp,
            sp.pro, replicaManager3->GetRakPeerInterface(),
            replicaManager3->GetWorldID())==SSICR_SENT_DATA)
   ...
   SendConstruction(constructedReplicasCulled, destroyedReplicasCulled, replicaManager3->defaultSendParameters,
                    replicaManager3->rakPeerInterface, worldId, replicaManager3);
}
...
void ReplicaManager3::Update(void) {
   ...
   for (index3 = 0; index3 < worldsList.Size(); index3++) {
      ...
      for (index = 0; index < world->connectionList.Size(); index++) {
         if (world->connectionList[index]->isValidated == false) continue;
   ...
   if (time - lastAutoSerializeOccurance >= autoSerializeInterval) {
      for (index3 = 0; index3 < worldsList.Size(); index3++) {
         ...
         sp.messageTimestamp = 0;
         for (int i = 0; i < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; i++) sp.pro[i] = defaultSendParameters;
         index2 = 0;
         for (index = 0; index < world->connectionList.Size(); index++) {
            connection = world->connectionList[index];
            sp.bitsWrittenSoFar = 0;
            ...
            if (connection->QuerySerializationList(replicasToSerialize)) {
               ...
               while (index2 < connection->queryToSerializeReplicaList.Size()) {
                  connection->queryToSerializeReplicaList[index2]->replica->lsr =
                      connection->queryToSerializeReplicaList[index2];
                  index2++;
               }
               ...
               while (index2 < replicasToSerialize.Size()) {
                  ...
                  if (ssicr == SSICR_SENT_DATA) lsr->whenLastSerialized = time;
                  index2++;
               }
            } else {
               while (index2 < connection->queryToSerializeReplicaList.Size()) {
...
void ReplicaManager3::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
                                         PI2_LostConnectionReason lostConnectionReason) {
   (void)lostConnectionReason;
   (void)systemAddress;
   if (autoDestroyConnections) {
      Connection_RM3 *connection = PopConnection(rakNetGUID);
      if (connection) DeallocConnection(connection);
   }
}
...
void ReplicaManager3::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming) {
   ...
   if (autoCreateConnections) {
      ...
      if (connection) PushConnection(connection);
   }
}
...
void ReplicaManager3::OnDetach(void) { OnRakPeerShutdown(); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

PluginReceiveResult ReplicaManager3::OnConstruction(Packet *packet, unsigned char *packetData, int packetDataLength,
                                                    RakNetGUID senderGuid, unsigned char packetDataOffset,
                                                    WorldId worldId) {
   RM3World *world = worldsArray[worldId];
   ...
   for (index = 0; index < constructionObjectListSize; index++) {
      ...
      if (actuallyCreateObject) {
         ...
         // Go past the bitStream written to with WriteAllocationID(). Necessary in case the user
         // didn't read out the bitStream the same way it was written bitOffset2 is already aligned
         ...
         if (!replica->QueryRemoteConstruction(connection) || !replica->DeserializeConstruction(&bsIn, connection)) {
...
PluginReceiveResult ReplicaManager3::OnSerialize(Packet *packet, unsigned char *packetData, int packetDataLength,
                                                 RakNetGUID senderGuid, RakNet::Time timestamp,
                                                 unsigned char packetDataOffset, WorldId worldId) {
   Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
   if (connection == 0) return RR_CONTINUE_PROCESSING;
   if (connection->groupConstructionAndSerialize) {
      connection->downloadGroup.Push(packet, __FILE__, __LINE__);
      return RR_STOP_PROCESSING;
   }
...
PluginReceiveResult ReplicaManager3::OnDownloadStarted(Packet *packet, unsigned char *packetData, int packetDataLength,
                                                       RakNetGUID senderGuid, unsigned char packetDataOffset,
                                                       WorldId worldId) {
   Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
   if (connection == 0) return RR_CONTINUE_PROCESSING;
   if (connection->QueryGroupDownloadMessages() &&
       // ID_DOWNLOAD_STARTED will be processed twice, being processed the second time once
       // ID_DOWNLOAD_COMPLETE arrives. However, the second time groupConstructionAndSerialize will
       // be set to true so it won't be processed a third time
...
PluginReceiveResult ReplicaManager3::OnDownloadComplete(Packet *packet, unsigned char *packetData, int packetDataLength,
                                                        RakNetGUID senderGuid, unsigned char packetDataOffset,
                                                        WorldId worldId) {
   Connection_RM3 *connection = GetConnectionByGUID(senderGuid, worldId);
   if (connection == 0) return RR_CONTINUE_PROCESSING;
   ...
   if (connection->groupConstructionAndSerialize == true && connection->downloadGroup.Size() > 0) {
      ...
      // groupConstructionAndSerialize will be false and downloadGroup will be empty, so it will go
      // past this block
...
Replica3 *ReplicaManager3::GetReplicaByNetworkID(NetworkID networkId, WorldId worldId) {
   ...
   for (i = 0; i < world->userReplicaList.Size(); i++) {
      if (world->userReplicaList[i]->GetNetworkID() == networkId) return world->userReplicaList[i];
   }
   return 0;
}
...
void ReplicaManager3::BroadcastDestructionList(DataStructures::List<Replica3 *> &replicaListSource,
                                               const SystemAddress &exclusionAddress, WorldId worldId) {
   RakNet::BitStream bsOut;
   unsigned int i, j;
   ...
   for (i = 0; i < replicaListSource.Size(); i++) {
      if (replicaListSource[i] == currentlyDeallocatingReplica) continue;
      replicaList.Push(replicaListSource[i], __FILE__, __LINE__);
   }

   if (replicaList.Size() == 0) return;

   for (i = 0; i < replicaList.Size(); i++) {
      if (replicaList[i]->deletingSystemGUID == UNASSIGNED_RAKNET_GUID)
         replicaList[i]->deletingSystemGUID =
             GetRakPeerInterface()->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
   }

   for (j = 0; j < world->connectionList.Size(); j++) {
      if (world->connectionList[j]->GetSystemAddress() == exclusionAddress) continue;
      ...
      for (i = 0; i < replicaList.Size(); i++) {
         if (world->connectionList[j]->HasReplicaConstructed(replicaList[i]) == false) continue;
         cnt++;
      ...
      if (cnt > 0) {
         ...
         rakPeerInterface->Send(&bsOut, defaultSendParameters.priority, defaultSendParameters.reliability,
                                defaultSendParameters.orderingChannel, world->connectionList[j]->GetSystemAddress(),
                                false, defaultSendParameters.sendReceipt);
      }
   }
}
...
Connection_RM3::~Connection_RM3() {
   unsigned int i;
   for (i = 0; i < constructedReplicaList.Size(); i++) RakNet::OP_DELETE(constructedReplicaList[i], _FILE_AND_LINE_);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void Connection_RM3::SendSerializeHeader(RakNet::Replica3 *replica, RakNet::Time timestamp, RakNet::BitStream *bs,
                                         WorldId worldId) {
   bs->Reset();
...
void Connection_RM3::ClearDownloadGroup(RakPeerInterface *rakPeerInterface) {
   unsigned int i;
   for (i = 0; i < downloadGroup.Size(); i++) rakPeerInterface->DeallocatePacket(downloadGroup[i]);
...
SendSerializeIfChangedResult Connection_RM3::SendSerialize(
    ...
    PRO sendParameters[RM3_NUM_OUTPUT_BITSTREAM_CHANNELS], RakPeerInterface *rakPeer, unsigned char worldId,
    RakNet::Time curTime) {
   bool channelHasData;
   BitSize_t sum = 0;
   for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
      if (indicesToSend[z]) sum += serializationData[z].GetNumberOfBitsUsed();
   }
   ...
   for (channelIndex = 0; channelIndex < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; channelIndex++) {
      if (channelIndex == 0) {
         ...
         if (anyData == false) return SSICR_SENT_DATA;
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(
    LastSerializationResult *lsr, SerializeParameters *sp, RakNet::RakPeerInterface *rakPeer, unsigned char worldId,
    ReplicaManager3 *replicaManager, RakNet::Time curTime) {
   RakNet::Replica3 *replica = lsr->replica;

   if (replica->GetNetworkID() == UNASSIGNED_NETWORK_ID) return SSICR_DID_NOT_SEND_DATA;
   ...
   if (rm3qsr == RM3QSR_DO_NOT_CALL_SERIALIZE) return SSICR_DID_NOT_SEND_DATA;
   ...
   if (replica->forceSendUntilNextUpdate) {
      ...
      return SendSerialize(replica, replica->lastSentSerialization.indicesToSend,
                           replica->lastSentSerialization.bitStream, sp->messageTimestamp, sp->pro, rakPeer, worldId,
                           curTime);
   }
   ...
   // This is necessary in case the user in the Serialize() function for some reason read the
   // bitstream they also wrote WIthout this code, the Write calls to another bitstream would not
   // write the entire bitstream
   BitSize_t sum = 0;
   for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
   ...
   if (serializationResult == RM3SR_SERIALIZED_ALWAYS_IDENTICALLY) {
      ...
      return SendSerialize(replica, replica->lastSentSerialization.indicesToSend, sp->outputBitstream,
                           sp->messageTimestamp, sp->pro, rakPeer, worldId, curTime);
   }
   ...
   if (serializationResult == RM3SR_BROADCAST_IDENTICALLY ||
         ...
         if (sp->outputBitstream[z].GetNumberOfBitsUsed() > 0 &&
             (serializationResult == RM3SR_BROADCAST_IDENTICALLY_FORCE_SERIALIZATION ||
              ((sp->outputBitstream[z].GetNumberOfBitsUsed() !=
                ...
                memcmp(sp->outputBitstream[z].GetData(), replica->lastSentSerialization.bitStream[z].GetData(),
      ...
      for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
         if (sp->outputBitstream[z].GetNumberOfBitsUsed() > 0 &&
             (sp->outputBitstream[z].GetNumberOfBitsUsed() !=
                  lsr->lastSerializationResultBS->bitStream[z].GetNumberOfBitsUsed() ||
              memcmp(sp->outputBitstream[z].GetData(), lsr->lastSerializationResultBS->bitStream[z].GetData(),
                     sp->outputBitstream[z].GetNumberOfBytesUsed()) != 0)) {
            indicesToSend[z] = true;
            sp->bitsWrittenSoFar += sp->outputBitstream[z].GetNumberOfBitsUsed();
            lsr->lastSerializationResultBS->bitStream[z].Reset();
...
void Connection_RM3::OnDereference(Replica3 *replica3, ReplicaManager3 *replicaManager) {
   ...
   if (replica3->GetNetworkIDManager() == 0) return;
   ...
   if (lsr) RakNet::OP_DELETE(lsr, _FILE_AND_LINE_);
...
void Connection_RM3::ValidateLists(ReplicaManager3 *replicaManager) const {
   ...
   /*
 #ifdef _DEBUG
   // Each object should exist only once in either constructedReplicaList or
...
void Connection_RM3::SendConstruction(DataStructures::List<Replica3 *> &newObjects,
                                      ...
                                      ReplicaManager3 *replicaManager3) {
   if (newObjects.Size() == 0 && deletedObjects.Size() == 0) return;

   // All construction and destruction takes place in the same network message
   // Otherwise, if objects rely on each other being created the same tick to be valid, this won't
   // always be true
   ...
   if (isFirstConstruction) {
      ...
      rakPeer->Send(&bsOut, sendParameters.priority, RELIABLE_ORDERED, sendParameters.orderingChannel, systemAddress,
                    false, sendParameters.sendReceipt);
   }
   ...
   for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
      ...
      if (actuallyCreateObject) {
         ...
         bsOut.AlignWriteToByteBoundary();  // Give the user an aligned bitStream in case they use
                                            // memcpy
   ...
   // If the object was serialized identically, and does not change later on, then the new
   // connection never gets the data
   SerializeParameters sp;
   sp.whenLastSerialized = 0;
   RakNet::BitStream emptyBs;
...
void Replica3::BroadcastDestruction(void) { replicaManager->BroadcastDestruction(this, UNASSIGNED_SYSTEM_ADDRESS); }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

RakNetGUID Replica3::GetCreatingSystemGUID(void) const { return creatingSystemGUID; }
...
RM3ConstructionState Replica3::QueryConstruction_ClientConstruction(RakNet::Connection_RM3 *destinationConnection,
   ...
   if (isThisTheServer) return RM3CS_SEND_CONSTRUCTION;
...
bool Replica3::QueryRemoteConstruction_ServerConstruction(RakNet::Connection_RM3 *sourceConnection,
                                                          bool isThisTheServer) {
   (void)sourceConnection;
   if (isThisTheServer) return false;
   return true;
}
...
RM3ConstructionState Replica3::QueryConstruction_PeerToPeer(RakNet::Connection_RM3 *destinationConnection,
                                                            Replica3P2PMode p2pMode) {
   (void)destinationConnection;

   if (p2pMode == R3P2PM_SINGLE_OWNER) {
      // We send to all, others do nothing
      if (creatingSystemGUID ==
          replicaManager->GetRakPeerInterface()->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS))
         return RM3CS_SEND_CONSTRUCTION;
      ...
      // RM3CS_ALREADY_EXISTS_REMOTELY will not send the object, but WILL call QuerySerialization()
      // and Serialize() on it.
      return RM3CS_ALREADY_EXISTS_REMOTELY;
   }
}
...
RM3QuerySerializationResult Replica3::QuerySerialization_ClientSerializable(
   ...
   if (isThisTheServer && destinationConnection->GetRakNetGUID() != creatingSystemGUID) return RM3QSR_CALL_SERIALIZE;
...
RM3QuerySerializationResult Replica3::QuerySerialization_ServerSerializable(
    RakNet::Connection_RM3 *destinationConnection, bool isThisTheServer) {
   (void)destinationConnection;
   // Server sends to all
   if (isThisTheServer) return RM3QSR_CALL_SERIALIZE;
...
RM3ActionOnPopConnection Replica3::QueryActionOnPopConnection_PeerToPeer(
