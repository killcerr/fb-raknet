...
class GridSectorizer {
  public:
   ...
   // minX, minY, maxX, maxY are the world dimensions (can be changed to dynamically allocate later
   // if needed)
   void Init(const float _maxCellWidth, const float _maxCellHeight, const float minX, const float minY,
   ...
   void MoveEntry(void *entry, const float sourceMinX, const float sourceMinY, const float sourceMaxX,
   ...
   void GetEntries(DataStructures::List<void *> &intersectionList, const float minX, const float minY, const float maxX,
                   const float maxY);
  ...
  protected:
   ...
   // Returns true or false if a position crosses cells in the grid.  If false, you don't need to
   // move entries
   bool PositionCrossesCells(const float originX, const float originY, const float destinationX,
