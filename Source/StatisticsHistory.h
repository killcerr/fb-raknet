...
class RAK_DLL_EXPORT StatisticsHistory {
  public:
   ...
   enum SHDataCategory {
      ...
      /// set
      /// based on nearest two data points longTerm* values are unknown using this method Use to add
      /// two graphs together
      DC_CONTINUOUS
   };
   ...
   struct TimeAndValueQueue {
      ...
      static void MergeSets(const TimeAndValueQueue *lhs, SHDataCategory lhsDataCategory, const TimeAndValueQueue *rhs,
                            SHDataCategory rhsDataCategory, TimeAndValueQueue *output);
      ...
      void ResizeSampleSet(int approximateSamples,
                           ...
                           SHDataCategory dataCategory, Time timeClipStart = 0, Time timeClipEnd = 0);
