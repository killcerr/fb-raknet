...
// BPSTracker::TimeAndValue2::TimeAndValue2(RakNet::TimeUS t, uint64_t v1, uint64_t v2) : time(t),
// value1(v1), value2(v2) {}

BPSTracker::BPSTracker() { Reset(_FILE_AND_LINE_); }

BPSTracker::~BPSTracker() {}
// void BPSTracker::Reset(const char *file, unsigned int line) {total1=total2=lastSec1=lastSec2=0;
...
// void BPSTracker::Push2(RakNetTimeUS time, uint64_t value1, uint64_t value2)
// {dataQueue.Push(TimeAndValue2(time,value1,value2),_FILE_AND_LINE_); total1+=value1;
// lastSec1+=value1;  total2+=value2; lastSec2+=value2;} uint64_t BPSTracker::GetBPS2(RakNetTimeUS
// time) {ClearExpired2(time); return lastSec2;} void BPSTracker::GetBPS1And2(RakNetTimeUS time,
// uint64_t &out1, uint64_t &out2) {ClearExpired2(time); out1=lastSec1; out2=lastSec2;}
...
struct DatagramHeaderFormat {
   ...
   static BitSize_t GetDataHeaderBitLength() { return BYTES_TO_BITS(GetDataHeaderByteLength()); }
   ...
   void Serialize(RakNet::BitStream *b) {
      ...
      //		b->WriteAlignedBytes((const unsigned char*) this,
      // sizeof(DatagramHeaderFormat)); 		return;
...
int RakNet::SplitPacketChannelComp(SplitPacketIdType const &key, SplitPacketChannel *const &data) {
   ...
   if (key < data->returnedPacket->splitPacketId) return -1;
   if (key == data->returnedPacket->splitPacketId) return 0;
#else
   if (key < data->splitPacketList.PacketId()) return -1;
   if (key == data->splitPacketList.PacketId()) return 0;
#endif
   return 1;
}

// DEFINE_MULTILIST_PTR_TO_MEMBER_COMPARISONS( InternalPacket, SplitPacketIndexType,
// splitPacketIndex )
/*
bool operator<( const DataStructures::MLKeyRef<SplitPacketIndexType> &inputKey, const InternalPacket
*cls )
{
...
bool operator==( const DataStructures::MLKeyRef<SplitPacketIndexType> &inputKey, const
InternalPacket *cls )
{
        return inputKey.Get() == cls->splitPacketIndex;
}
/// Semi-hack: This is necessary to call Sort()
bool operator<( const DataStructures::MLKeyRef<InternalPacket *> &inputKey, const InternalPacket
*cls )
{
...
int SplitPacketIndexComp(SplitPacketIndexType const &key, InternalPacket *const &data) {
   if (key < data->splitPacketIndex) return -1;
   if (key == data->splitPacketIndex) return 0;
   return 1;
}
...
// Constructor
//-------------------------------------------------------------------------------------------------------
// Add 21 to the default MTU so if we encrypt it can hold potentially 21 more bytes of extra data +
// padding.
...
void ReliabilityLayer::Reset(bool resetVariables, int MTUSize, bool _useSecurity) {
   FreeMemory(true);  // true because making a memory reset pending in the update cycle causes
                      // resets after reconnects.  Instead, just call Reset from a single thread
   if (resetVariables) {
      InitializeVariables();
      ...
      if (_useSecurity) MTUSize -= cat::AuthenticatedEncryption::OVERHEAD_BYTES;
#else
      (void)_useSecurity;
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SetTimeoutTime(RakNet::TimeMS time) { timeoutTime = time; }

//-------------------------------------------------------------------------------------------------------
// Returns the value passed to SetTimeoutTime. or the default if it was never called
//-------------------------------------------------------------------------------------------------------
RakNet::TimeMS ReliabilityLayer::GetTimeoutTime(void) { return timeoutTime; }
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
#endif
      while (1) {
         if (iter->data) FreeInternalPacketData(iter, _FILE_AND_LINE_);
         prev = iter;
   ...
   for (j = 0; j < outgoingPacketBuffer.Size(); j++) {
      if (outgoingPacketBuffer[j]->data) FreeInternalPacketData(outgoingPacketBuffer[j], _FILE_AND_LINE_);
   ...
   /*
   DataStructures::Page<DatagramSequenceNumberType, DatagramMessageIDList*, RESEND_TREE_ORDER> *cur
   = datagramMessageIDTree.GetListHead(); while (cur)
   {
   int treeIndex;
...
bool ReliabilityLayer::HandleSocketReceiveFromConnectedPlayer(
    ...
    DataStructures::List<PluginInterface2 *> &messageHandlerList, int MTUSize, RakNetSocket2 *s, RakNetRandom *rnr,
    CCTimeType timeRead, BitStream &updateBitStream) {
#ifdef _DEBUG
   RakAssert(!(buffer == 0));
#endif
   ...
   if (length <= 2 || buffer == 0)  // Length of 1 is a connection request resend that we just ignore
   {
      for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
         messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
             "length <= 2 || buffer == 0", BYTES_TO_BITS(length), systemAddress, true);
      return true;
   }
   ...
   if (useSecurity) {
      ...
      if (!auth_enc.Decrypt((cat::u8 *)buffer, received)) return false;
   ...
   if (dhf.isValid == false) {
      for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
         messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
             "dhf.isValid==false", BYTES_TO_BITS(length), systemAddress, true);
   ...
   if (dhf.isACK) {
...
#endif
      {
         // Sanity check. This could happen due to type overflow, especially since I only send the
         // low 4 bytes to reduce bandwidth
...
#endif
      //		congestionManager.OnAck(timeRead, rtt, dhf.hasBAndAS, dhf.B, dhf.AS,
      ...
      if (incomingAcks.Deserialize(&socketData) == false) {
         for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
              messageHandlerIndex++)
            messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                "incomingAcks.Deserialize failed", BYTES_TO_BITS(length), systemAddress, true);

         return false;
      }
      for (i = 0; i < incomingAcks.ranges.Size(); i++) {
         if (incomingAcks.ranges[i].minIndex > incomingAcks.ranges[i].maxIndex ||
             (incomingAcks.ranges[i].maxIndex == (uint24_t)(0xFFFFFFFF))) {
            RakAssert(incomingAcks.ranges[i].minIndex <= incomingAcks.ranges[i].maxIndex);

            for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                 messageHandlerIndex++)
               messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                   "incomingAcks minIndex > maxIndex or maxIndex is max value", BYTES_TO_BITS(length), systemAddress,
                   true);
            return false;
         }
         for (datagramNumber = incomingAcks.ranges[i].minIndex;
            ...
            if (unreliableWithAckReceiptHistory.Size() > 0) {
               ...
               while (k < unreliableWithAckReceiptHistory.Size()) {
                  if (unreliableWithAckReceiptHistory[k].datagramNumber == datagramNumber) {
                     ...
                     memcpy(ackReceipt->data + sizeof(MessageID), &unreliableWithAckReceiptHistory[k].sendReceiptSerial,
                            sizeof(uint32_t));
            ...
            if (messageNumberNode) {
...
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS == 1
               congestionManager.OnAck(timeRead, rtt, dhf.hasBAndAS, 0, dhf.AS, totalUserDataBytesAcked,
                                       bandwidthExceededStatistic, datagramNumber);
#else
               CCTimeType ping;
               if (timeRead > whenSent)
                  ...
                  ping = 0;
               congestionManager.OnAck(timeRead, ping, dhf.hasBAndAS, 0, dhf.AS, totalUserDataBytesAcked,
                                       bandwidthExceededStatistic, datagramNumber);
#endif
               while (messageNumberNode) {
                  // TESTING1
                  // 						printf("Remove %i on ack for
                  // datagramNumber=%i.\n", messageNumberNode->messageNumber.val,
                  // datagramNumber.val);

                  RemovePacketFromResendListAndDeleteOlderReliableSequenced(messageNumberNode->messageNumber, timeRead,
                                                                            messageHandlerList, systemAddress);
            ...
            // 				else if (isReliable)
            // 				{
            // 					// Previously used slot, rather than empty
            // unreliable
            // slot 					printf("%p Ack %i is duplicate\n", this,
            // datagramNumber.val);
            //
            //  					congestionManager.OnDuplicateAck(timeRead,
            //  datagramNumber);
            // 				}
         }
      }
      ...
      if (incomingNAKs.Deserialize(&socketData) == false) {
         for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
              messageHandlerIndex++)
            messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                "incomingNAKs.Deserialize failed", BYTES_TO_BITS(length), systemAddress, true);
      ...
      for (i = 0; i < incomingNAKs.ranges.Size(); i++) {
         if (incomingNAKs.ranges[i].minIndex > incomingNAKs.ranges[i].maxIndex) {
            ...
            for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                 messageHandlerIndex++)
               messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                   "incomingNAKs minIndex>maxIndex", BYTES_TO_BITS(length), systemAddress, true);

            return false;
         }
         // Sanity check
         // RakAssert(incomingNAKs.ranges[i].maxIndex.val-incomingNAKs.ranges[i].minIndex.val<1000);
         for (messageNumber = incomingNAKs.ranges[i].minIndex;
            ...
            while (messageNumberNode) {
               ...
               InternalPacket *internalPacket =
                   resendBuffer[messageNumberNode->messageNumber & (uint32_t)RESEND_BUFFER_ARRAY_MASK];
               if (internalPacket) {
      ...
      if (!congestionManager.OnGotPacket(dhf.datagramNumber, dhf.isContinuousSend, timeRead, length,
                                         &skippedMessageCount)) {
         for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
              messageHandlerIndex++)
            messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                "congestionManager.OnGotPacket failed", BYTES_TO_BITS(length), systemAddress, true);

         return true;
      }
      if (dhf.isPacketPair) congestionManager.OnGotPacketPair(dhf.datagramNumber, length, timeRead);
      ...
      if (internalPacket == 0) {
         for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
              messageHandlerIndex++)
            messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                "CreateInternalPacketFromBitStream failed", BYTES_TO_BITS(length), systemAddress, true);
      ...
      while (internalPacket) {
         for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
              messageHandlerIndex++) {
#if CC_TIME_TYPE_BYTES == 4
            messageHandlerList[messageHandlerIndex]->OnInternalPacket(internalPacket, receivePacketCount, systemAddress,
                                                                      timeRead, false);
#else
            messageHandlerList[messageHandlerIndex]->OnInternalPacket(internalPacket, receivePacketCount, systemAddress,
...

         {
            ...
            // We do the actual reset in this function so the data is not modified by multiple
            // threads
            ...
            if (internalPacket->reliability == RELIABLE_SEQUENCED ||
               ...
               if (internalPacket->orderingChannel >= NUMBER_OF_ORDERED_STREAMS) {
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "internalPacket->orderingChannel >= NUMBER_OF_ORDERED_STREAMS", BYTES_TO_BITS(length),
                         systemAddress, true);

                  bpsMetrics[(int)USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(
                      timeRead, BITS_TO_BYTES(internalPacket->dataBitLength));
            ...
            // 8/12/09 was previously not checking if the message was reliable. However, on
            // packetloss this would mean you'd eventually exceed the hole count because unreliable
            // messages were never resent, and you'd stop getting messages
            if (internalPacket->reliability == RELIABLE || internalPacket->reliability == RELIABLE_SEQUENCED ||
                internalPacket->reliability == RELIABLE_ORDERED) {
               ...
               if (holeCount == (DatagramSequenceNumberType)0) {
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "holeCount > typeRange/(DatagramSequenceNumberType) 2", BYTES_TO_BITS(length), systemAddress,
                         false);
                  ...
                  // Got a higher count out of order packet that was missing in the sequence or we
                  // already got
                  if (hasReceivedPacketQueue[holeCount] != false)  // non-zero means this is a hole
                  {
                     ...
                     for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                          messageHandlerIndex++)
                        messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                            "Higher count pushed to hasReceivedPacketQueue", BYTES_TO_BITS(length), systemAddress,
                            false);
#endif
                     ...
                     for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                          messageHandlerIndex++)
                        messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                            "Duplicate packet ignored", BYTES_TO_BITS(length), systemAddress, false);
#endif
               ...
               } else  // holeCount>=receivedPackets.Size()
               {
                  if (holeCount > (DatagramSequenceNumberType)1000000) {
                     ...
                     for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                          messageHandlerIndex++)
                        messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                            "holeCount > 1000000", BYTES_TO_BITS(length), systemAddress, true);
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "Adding to hasReceivedPacketQueue later ordered message", BYTES_TO_BITS(length), systemAddress,
                         false);
#endif

                  // Fix - sending on a higher priority gives us a very very high received packets
                  // base index if we formerly had pre-split a lot of messages and used that as the
                  // message number.  Because of this, a lot of time is spent in this linear loop
                  // and the timeout time expires because not all of the message is sent in time.
                  // Fixed by late assigning message IDs on the sender

                  // Add 0 times to the queue until (reliableMessageNumber - baseIndex) < queue
                  // size.
                  while ((unsigned int)(holeCount) > hasReceivedPacketQueue.Size())
                     hasReceivedPacketQueue.Push(true,
                                                 _FILE_AND_LINE_);  // time+(CCTimeType)60 * (CCTimeType)1000 *
                                                                    // (CCTimeType)1000);
                                                                    // // Didn't get this packet - set the time to give
                                                                    // up waiting
                  ...
                  RakAssert(hasReceivedPacketQueue.Size() <
                            (unsigned int)((DatagramSequenceNumberType)(const uint32_t)(-1)));
#endif
               }
            ...
            // If the allocated buffer is > DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE and it is 3x
            // greater than the number of elements actually being used
            if (hasReceivedPacketQueue.AllocationSize() > (unsigned int)DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE &&
                hasReceivedPacketQueue.AllocationSize() > hasReceivedPacketQueue.Size() * 3)
               hasReceivedPacketQueue.Compress(_FILE_AND_LINE_);

            /*
            if ( internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability ==
    ...
    messageHandlerList.Size(); messageHandlerIndex++)
                                    messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification("internalPacket->orderingChannel
    >= NUMBER_OF_ORDERED_STREAMS", BYTES_TO_BITS(length), systemAddress);

                            bpsMetrics[(int)
    USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(timeRead,BITS_TO_BYTES(internalPacket->dataBitLength));

                            goto CONTINUE_SOCKET_DATA_PARSE_LOOP;
                    }

                    if ( IsOlderOrderedPacket( internalPacket->orderingIndex,
    waitingForSequencedPacketReadIndex[ internalPacket->orderingChannel ] ) == false )
                    {
                            ...
                            if ( internalPacket->splitPacketCount > 0 )
                            {
                                    ...
                                    bpsMetrics[(int)
    USER_MESSAGE_BYTES_RECEIVED_PROCESSED].Push1(timeRead,BITS_TO_BYTES(internalPacket->dataBitLength));

                                    // Sequenced
                                    internalPacket = BuildPacketFromSplitPacketList(
    internalPacket->splitPacketId, timeRead, s, systemAddress, rnr,
    remotePortRakNetWasStartedOn_PS3, extraSocketOptions);

                                    if ( internalPacket )
                                    {
                                            // Update our index to the newest packet
                                            waitingForSequencedPacketReadIndex[
    internalPacket->orderingChannel ] = internalPacket->orderingIndex + (OrderingIndexType)1;

                                            // If there is a rebuilt packet, add it to the output
    queue outputQueue.Push( internalPacket, _FILE_AND_LINE_  ); internalPacket = 0;
                                    }
    ...
    USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(timeRead,BITS_TO_BYTES(internalPacket->dataBitLength));
            ...
            if ( internalPacket->splitPacketCount > 0 )
            {
                    ...
                    if ( internalPacket->reliability != RELIABLE_ORDERED )
                            internalPacket->orderingChannel = 255; // Use 255 to designate not
    sequenced and not ordered

                    InsertIntoSplitPacketList( internalPacket, timeRead );

                    internalPacket = BuildPacketFromSplitPacketList( internalPacket->splitPacketId,
    timeRead, s, systemAddress, rnr, remotePortRakNetWasStartedOn_PS3, extraSocketOptions);
    ...
    USER_MESSAGE_BYTES_RECEIVED_PROCESSED].Push1(timeRead,BITS_TO_BYTES(internalPacket->dataBitLength));
                    ...
                    if ( waitingForOrderedPacketReadIndex[ internalPacket->orderingChannel ] ==
    internalPacket->orderingIndex )
                    {
                            // Get the list to hold ordered packets for this stream
                            DataStructures::LinkedList<InternalPacket*>
    *orderingListAtOrderingStream; unsigned char orderingChannelCopy =
    internalPacket->orderingChannel;

                            // Push the packet for the user to read
                            outputQueue.Push( internalPacket, _FILE_AND_LINE_  );
                            internalPacket = 0; // Don't reference this any longer since other
    threads access it

                            // Wait for the resendNext ordered packet in sequence
                            waitingForOrderedPacketReadIndex[ orderingChannelCopy ] ++; // This
    wraps

                            orderingListAtOrderingStream = GetOrderingListAtOrderingStream(
    orderingChannelCopy );
                            ...
                            if ( orderingListAtOrderingStream != 0)
                            {
                                    while ( orderingListAtOrderingStream->Size() > 0 )
                                    {
                                            ...
                                            {
                                                    if (
    orderingListAtOrderingStream->Peek()->orderingIndex == waitingForOrderedPacketReadIndex[
    orderingChannelCopy ] )
                                                    {
                                                            outputQueue.Push(
    orderingListAtOrderingStream->Pop(), _FILE_AND_LINE_  ); waitingForOrderedPacketReadIndex[
    orderingChannelCopy ]++; indexFound=true;
                                                    }
                                                    else
                    ...
                    {
                            // This is a newer ordered packet than we are waiting for. Store it for
    future use AddToOrderingList( internalPacket );
                    }
            ...
            */
            ...
            if (internalPacket->splitPacketCount > 0) {
               ...
               internalPacket = BuildPacketFromSplitPacketList(internalPacket->splitPacketId, timeRead, s,
                                                               systemAddress, rnr, updateBitStream);
               ...
               if (internalPacket == 0) {
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "BuildPacketFromSplitPacketList did not return anything.", BYTES_TO_BITS(length),
                         systemAddress, false);
#endif
            ...
            if (internalPacket->reliability == RELIABLE_SEQUENCED ||
               ...
               if (internalPacket->orderingIndex == orderedReadIndex[internalPacket->orderingChannel]) {
                  ...
                  if (internalPacket->reliability == RELIABLE_SEQUENCED ||
                      internalPacket->reliability == UNRELIABLE_SEQUENCED) {
                     // Is sequenced
                     if (IsOlderOrderedPacket(internalPacket->sequencingIndex,
                                              highestSequencedReadIndex[internalPacket->orderingChannel]) == false) {
                        // Expected or highest known value

#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
                        if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
                           fprintf(fp, "Returning %i, %s by fallthrough. OI=%i. SI=%i.\n", receivedPacketNumber, type,
                                   internalPacket->orderingIndex.val, internalPacket->sequencingIndex);
                           fflush(fp);
                        }
                        ...
                        // Means a duplicated RELIABLE_SEQUENCED or UNRELIABLE_SEQUENCED packet
                        // would be returned to the user
                        highestSequencedReadIndex[internalPacket->orderingChannel] =
                            internalPacket->sequencingIndex + (OrderingIndexType)1;
                        ...
                        for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                             messageHandlerIndex++)
                           messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                               "Sequenced rejected: lower than highest known value", BYTES_TO_BITS(length),
                               systemAddress, false);
#endif
                     ...
                     bpsMetrics[(int)USER_MESSAGE_BYTES_RECEIVED_PROCESSED].Push1(
                         timeRead, BITS_TO_BYTES(internalPacket->dataBitLength));
                     outputQueue.Push(internalPacket, _FILE_AND_LINE_);

#ifdef PRINT_TO_FILE_RELIABLE_ORDERED_TEST
                     if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
                        fprintf(fp, "outputting immediate %i, %s. OI=%i. SI=%i.", receivedPacketNumber, type,
                                internalPacket->orderingIndex.val, internalPacket->sequencingIndex);
                     ...
                     while (orderingHeaps[internalPacket->orderingChannel].Size() > 0 &&
                            orderingHeaps[internalPacket->orderingChannel].Peek()->orderingIndex ==
                                orderedReadIndex[internalPacket->orderingChannel]) {
               ...
               } else if (IsOlderOrderedPacket(internalPacket->orderingIndex,
                                               orderedReadIndex[internalPacket->orderingChannel]) == false) {
                  // internalPacket->_orderingIndex is greater
                  // If a message has a greater ordering index, and is sequenced or ordered, buffer
                  // it Sequenced has a lower heap weight, ordered has max sequenced weight

                  // Keep orderedHoleCount count small
                  if (orderingHeaps[internalPacket->orderingChannel].Size() == 0)
                     heapIndexOffsets[internalPacket->orderingChannel] =
                         orderedReadIndex[internalPacket->orderingChannel];

                  reliabilityHeapWeightType orderedHoleCount =
                      internalPacket->orderingIndex - heapIndexOffsets[internalPacket->orderingChannel];
                  ...
                  if (packetId == ID_USER_PACKET_ENUM + 1 && fp) {
                     fprintf(fp,
                             "Heap push %i, %s, weight=%" PRINTF_64_BIT_MODIFIER "u. OI=%i. waiting on %i. SI=%i.\n",
                             receivedPacketNumber, type, weight, internalPacket->orderingIndex.val,
                             orderedReadIndex[internalPacket->orderingChannel].val, internalPacket->sequencingIndex);
                     fflush(fp);
                  }
#endif
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "Larger number ordered packet leaving holes", BYTES_TO_BITS(length), systemAddress, false);
#endif
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                       messageHandlerIndex++)
                     messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                         "Rejected older resend", BYTES_TO_BITS(length), systemAddress, false);
#endif
            ...
            bpsMetrics[(int)USER_MESSAGE_BYTES_RECEIVED_PROCESSED].Push1(timeRead,
...
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::Send(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority,
                            PacketReliability reliability, unsigned char orderingChannel, bool makeDataCopy,
                            int MTUSize, CCTimeType currentTime, uint32_t receipt) {
#ifdef _DEBUG
   ...
   unsigned int maxDataSizeBytes =
       GetMaxDatagramSizeExcludingMessageHeaderBytes() - BITS_TO_BYTES(GetMaxMessageHeaderLengthBits());

   bool splitPacket = numberOfBytesToSend > maxDataSizeBytes;

   // If a split packet, we might have to upgrade the reliability
   if (splitPacket) {
      // Split packets cannot be unreliable, in case that one part doesn't arrive and the whole
      // cannot be reassembled. One part could not arrive either due to packetloss or due to
      // unreliable discard
   ...
   if (internalPacket->reliability == RELIABLE_SEQUENCED || internalPacket->reliability == UNRELIABLE_SEQUENCED
      ...
      // UPDATE:
      // Disabled.  We don't have enough info to consistently do this.  Sometimes newer data does
      // supercede older data such as with constantly declining health, but not in all cases. For
      // example, with sequenced unreliable sound packets just because you send a newer one doesn't
      // mean you don't need the older ones because the odds are they will still arrive in order
      /*
      for (int i=0; i < NUMBER_OF_PRIORITIES; i++)
      {
   ...
   {
      // Must split the packet.  This will also generate the SHA1 if it is required. It also adds it
      // to the send list.
   ...
   RakAssert(outgoingPacketBuffer.Size() == 0 ||
...
void ReliabilityLayer::Update(RakNetSocket2 *s, SystemAddress &systemAddress, int MTUSize, CCTimeType time,
                              unsigned bitsPerSecondLimit, DataStructures::List<PluginInterface2 *> &messageHandlerList,
...

{
   ...
   while (delayList.Size()) {
      if (delayList.Peek()->sendTime <= timeMs) {
         ...
         //			SocketLayer::SendTo( dat->s, dat->data, dat->length, systemAddress,
         //__FILE__, __LINE__  );
   ...
   lastUpdateTime = time;
#if CC_TIME_TYPE_BYTES == 4
   if (timeSinceLastTick > 100) timeSinceLastTick = 100;
#else
   if (timeSinceLastTick > 100000) timeSinceLastTick = 100000;
#endif
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
            ...
            while (1) {
               if (time > cur->creationTime + (CCTimeType)unreliableTimeout) {
                  // Flag invalid, and clear the memory. Still needs to be removed from the
                  // sendPacketSet later This fixes a problem where a remote system disconnects, but
                  // we don't know it yet, and memory consumption increases to a huge value
   ...
   if (unreliableWithAckReceiptHistory.Size() > 0) {
      ...
      while (i < unreliableWithAckReceiptHistory.Size()) {
         ...
         if (time - unreliableWithAckReceiptHistory[i].nextActionTime < (((CCTimeType)-1) / 2)) {
            ...
            memcpy(ackReceipt->data + sizeof(MessageID), &unreliableWithAckReceiptHistory[i].sendReceiptSerial,
                   sizeof(uint32_t));
            outputQueue.Push(ackReceipt, _FILE_AND_LINE_);
   ...
   if (hasDataToSendOrResend == true) {
      ...
      int transmissionBandwidth = congestionManager.GetTransmissionBandwidth(time, timeSinceLastTick,
                                                                             unacknowledgedBytes, dhf.isContinuousSend);
      int retransmissionBandwidth = congestionManager.GetRetransmissionBandwidth(
          time, timeSinceLastTick, unacknowledgedBytes, dhf.isContinuousSend);
         ...
         while ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < retransmissionBandwidth) {
            ...
            while (IsResendQueueEmpty() == false) {
               ...
               if (time - internalPacket->nextActionTime < (((CCTimeType)-1) / 2)) {
                  ...
                  internalPacket->retransmissionTime =
                      congestionManager.GetRTOForRetransmission(internalPacket->timesSent);
                  ...
                  for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size();
                     ...
                     messageHandlerList[messageHandlerIndex]->OnInternalPacket(
                         internalPacket,
                         packetsToSendThisUpdateDatagramBoundaries.Size() +
                             congestionManager.GetNextDatagramSequenceNumber(),
                         systemAddress, (RakNet::TimeMS)time, true);
#else
                     messageHandlerList[messageHandlerIndex]->OnInternalPacket(
                         internalPacket,
                         packetsToSendThisUpdateDatagramBoundaries.Size() +
                             congestionManager.GetNextDatagramSequenceNumber(),
                         systemAddress, (RakNet::TimeMS)(time / (CCTimeType)1000), true);
#endif
                  }
                  ...
                  // Removeme
                  //						printf("Resend:%i ",
                  // internalPacket->reliableMessageNumber);
               } else {
                  // Filled one datagram.
      ...
      if ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < transmissionBandwidth) {
         ...
         while (ResendBufferOverflow() == false &&
                ((int)BITS_TO_BYTES(allDatagramSizesSoFar) < transmissionBandwidth ||
                 // This condition means if we want to send a datagram pair, and only have one
                 // datagram buffered, exceed bandwidth to add another
            ...
            // while ( sendPacketSet[ i ].Size() )
            {
               ...
               if (internalPacket->data == 0) {
                  ...
                  RakAssert(outgoingPacketBuffer.Size() == 0 ||
                            outgoingPacketBuffer.Peek()->dataBitLength < BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
                  statistics.messageInSendBuffer[(int)internalPacket->priority]--;
                  statistics.bytesInSendBuffer[(int)internalPacket->priority] -=
                      (double)BITS_TO_BYTES(internalPacket->dataBitLength);
                  ReleaseToInternalPacketPool(internalPacket);
                  continue;
               }
               ...
               if (isReliable
                   /*
                   I thought about this and agree that UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT and
                   RELIABLE_SEQUENCED_WITH_ACK_RECEIPT is not useful unless you also know if the
                   message was discarded.

                   The problem is that internally, message numbers are only assigned to reliable
                   messages, because message numbers are only used to discard duplicate message
                   receipt and only reliable messages get sent more than once. However, without
                   message numbers getting assigned and transmitted, there is no way to tell the
                   sender about which messages were discarded. In fact, in looking this over I
                   realized that UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT introduced a bug, because the
                   remote system assumes all message numbers are used (no holes). With that send
                   type, on packetloss, a permanent hole would have been created which eventually
                   would cause the system to discard all further packets.

                   So I have two options. Either do not support ack receipts when sending sequenced,
                   or write complex and major new systems. UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
                   would need to send the message ID number on a special channel which allows for
                   non-delivery. And both of them would need to have a special range list to
                   indicate which message numbers were not delivered, so when acks are sent that can
                   be indicated as well. A further problem is that the ack itself can be lost - it
                   is possible that the message can arrive but be discarded, yet the ack is lost. On
                   resend, the resent message would be ignored as duplicate, and you'd never get the
                   discard message either (unless I made a special buffer for that case too).
     */
                   //						||
                  ...
                  internalPacket->retransmissionTime =
                      congestionManager.GetRTOForRetransmission(internalPacket->timesSent + 1);
                  internalPacket->nextActionTime = internalPacket->retransmissionTime + time;
#if CC_TIME_TYPE_BYTES == 4
                  ...
                  if (resendBuffer[internalPacket->reliableMessageNumber & (uint32_t)RESEND_BUFFER_ARRAY_MASK] != 0) {
                     //								bool overflow =
                     // ResendBufferOverflow();
                     RakAssert(0);
                  }
                  resendBuffer[internalPacket->reliableMessageNumber & (uint32_t)RESEND_BUFFER_ARRAY_MASK] =
                      internalPacket;
                  ...
                  unreliableWithAckReceiptHistory.Push(
               ...
               // If isReliable is false, the packet and its contents will be added to a list to be
               // freed in ClearPacketsAndDatagrams However, the internalPacket structure will
               // remain allocated and be in the resendBuffer list if it requires a receipt
               bpsMetrics[(int)USER_MESSAGE_BYTES_SENT].Push1(time, BITS_TO_BYTES(internalPacket->dataBitLength));

               // Testing1
               // 					if
               // (internalPacket->reliability==RELIABLE_ORDERED
               // || internalPacket->reliability==RELIABLE_ORDERED_WITH_ACK_RECEIPT) printf("SEND
               // reliableMessageNumber %i in datagram %i\n",
               // internalPacket->reliableMessageNumber.val,
               // congestionManager.GetNextDatagramSequenceNumber().val);
      ...
      for (unsigned int datagramIndex = 0; datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size();
           datagramIndex++) {
         if (datagramIndex > 0) dhf.isContinuousSend = true;
         ...
         bool isSecondOfPacketPair =
             dhf.isPacketPair && datagramIndex > 0 && datagramsToSendThisUpdateIsPair[datagramIndex - 1];
            ...
            if (packetsToSendThisUpdate[msgIndex]->reliability != UNRELIABLE &&
               ...
               if (messageNumberNode == 0) {
                  messageNumberNode = AddFirstToDatagramHistory(
                      dhf.datagramNumber, packetsToSendThisUpdate[msgIndex]->reliableMessageNumber, time);
               } else {
                  messageNumberNode = AddSubsequentToDatagramHistory(
                      messageNumberNode, packetsToSendThisUpdate[msgIndex]->reliableMessageNumber);
               }
            }
...
void ReliabilityLayer::SendBitStream(RakNetSocket2 *s, SystemAddress &systemAddress, RakNet::BitStream *bitStream,
...
#ifdef _DEBUG
   if (packetloss > 0.0) {
      if (frandomMT() < packetloss) return;
   }
   ...
   if (minExtraPing > 0 || extraPingVariance > 0) {
      ...
      if (extraPingVariance > 0) delay += (randomMT() % extraPingVariance);
      if (delay > 0) {
         DataAndTime *dat = RakNet::OP_NEW<DataAndTime>(__FILE__, __LINE__);
         memcpy(dat->data, (char *)bitStream->GetData(), length);
         dat->s = s;
         ...
         if (dat != 0) delayList.Push(dat, __FILE__, __LINE__);
         return;
      }
#endif
   }
...
#else
   // SocketLayer::SendTo( s, ( char* ) bitStream->GetData(), length, systemAddress, __FILE__,
   // __LINE__  );
...
bool ReliabilityLayer::IsOutgoingDataWaiting(void) {
   if (outgoingPacketBuffer.Size() > 0) return true;
   ...
   return
       ...
       // resendTree.IsEmpty()==false;// || outputQueue.Size() > 0 || orderingList.Size() > 0 ||
       // splitPacketChannelList.Size() > 0;
       statistics.messagesInResendBuffer != 0;
}

bool ReliabilityLayer::AreAcksWaiting(void) { return acknowlegements.Size() > 0; }
//-------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SetSplitMessageProgressInterval(int interval) { splitMessageProgressInterval = interval; }
//-------------------------------------------------------------------------------------------------------
...
bool ReliabilityLayer::IsSendThrottled(int MTUSize) {
   ...
   packetsWaiting = 1 + ((BITS_TO_BYTES(resendListDataSize)) / (MTUSize - UDP_HEADER_SIZE - 10)); //
   10 to roughly estimate the raknet header
...
// We lost a packet
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::UpdateWindowFromPacketloss(CCTimeType time) { (void)time; }

//-------------------------------------------------------------------------------------------------------
// Increase the window size
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::UpdateWindowFromAck(CCTimeType time) { (void)time; }

//-------------------------------------------------------------------------------------------------------
// Does what the function name says
//-------------------------------------------------------------------------------------------------------
unsigned ReliabilityLayer::RemovePacketFromResendListAndDeleteOlderReliableSequenced(
    const MessageNumberType messageNumber, CCTimeType time,
    DataStructures::List<PluginInterface2 *> &messageHandlerList, const SystemAddress &systemAddress) {
   (void)time;
   (void)messageNumber;
   InternalPacket *internalPacket;
   // InternalPacket *temp;
   //	PacketReliability reliability; // What type of reliability algorithm to use with this packet
   //	unsigned char orderingChannel; // What ordering channel this packet is on, if the
   // reliability type uses ordering channels 	OrderingIndexType orderingIndex; // The ID used as
   // identification for ordering channels 	unsigned j;

   for (unsigned int messageHandlerIndex = 0; messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++) {
#if CC_TIME_TYPE_BYTES == 4
      messageHandlerList[messageHandlerIndex]->OnAck(messageNumber, systemAddress, time);
#else
      messageHandlerList[messageHandlerIndex]->OnAck(messageNumber, systemAddress,
                                                     (RakNet::TimeMS)(time / (CCTimeType)1000));
#endif
   }
   ...
   if (internalPacket && internalPacket->reliableMessageNumber == messageNumber) {
      ...
      if (internalPacket->reliability >= RELIABLE_WITH_ACK_RECEIPT &&
         ...
         ackReceipt->dataBitLength = BYTES_TO_BITS(5);
         ackReceipt->data[0] = (MessageID)ID_SND_RECEIPT_ACKED;
         memcpy(ackReceipt->data + sizeof(MessageID), &internalPacket->sendReceiptSerial,
      ...
      bool isReliable;
      if (internalPacket->reliability == RELIABLE || internalPacket->reliability == RELIABLE_SEQUENCED ||
          internalPacket->reliability == RELIABLE_ORDERED || internalPacket->reliability == RELIABLE_WITH_ACK_RECEIPT ||
          //			internalPacket->reliability == RELIABLE_SEQUENCED_WITH_ACK_RECEIPT
          //||
          internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT)
         isReliable = true;
...
BitSize_t ReliabilityLayer::GetMessageHeaderLengthBits(const InternalPacket *const internalPacket) {
   ...
   //	tempChar=(unsigned char)internalPacket->reliability; bitStream->WriteBits( (const unsigned
   // char *)&tempChar, 3, true ); // 3 bits to write reliability. 	bool hasSplitPacket =
   // internalPacket->splitPacketCount>0; bitStream->Write(hasSplitPacket); // Write 1 bit to
   // indicate if splitPacketCount>0
   bitLength = 8 * 1;

   //	bitStream->AlignWriteToByteBoundary();
   //	RakAssert(internalPacket->dataBitLength < 65535);
   //	unsigned short s; s = (unsigned short) internalPacket->dataBitLength;
   ...
   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED) {
      bitLength += 8 * 3;
      ;  // bitStream->Write(internalPacket->_sequencingIndex); // Used for UNRELIABLE_SEQUENCED,
         // RELIABLE_SEQUENCED, RELIABLE_ORDERED.
   }

   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED ||
       internalPacket->reliability == RELIABLE_ORDERED ||
       internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
      bitLength += 8 * 3;  // bitStream->Write(internalPacket->orderingIndex); // Used for
                           // UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED.
      bitLength += 8 * 1;  // tempChar=internalPacket->orderingChannel; bitStream->WriteAlignedVar8((const
                           // char*)& tempChar); // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
                           // RELIABLE_ORDERED. 5 bits needed, write one byte
   }
   if (internalPacket->splitPacketCount > 0) {
      bitLength += 8 * 4;  // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketCount);
                           // RakAssert(sizeof(SplitPacketIndexType)==4); // Only needed if
                           // splitPacketCount>0. 4 bytes
      bitLength += 8 * sizeof(SplitPacketIdType);  // bitStream->WriteAlignedVar16((const char*)&
                                                   // internalPacket->splitPacketId);
                                                   // RakAssert(sizeof(SplitPacketIdType)==2); //
                                                   // Only needed if splitPacketCount>0.
      bitLength += 8 * 4;  // bitStream->WriteAlignedVar32((const char*)& internalPacket->splitPacketIndex);
                           // // Only needed if splitPacketCount>0. 4 bytes
   }
...
BitSize_t ReliabilityLayer::WriteToBitStreamFromInternalPacket(RakNet::BitStream *bitStream,
   ...
   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED) {
      bitStream->Write(internalPacket->sequencingIndex);  // Used for UNRELIABLE_SEQUENCED,
                                                          // RELIABLE_SEQUENCED, RELIABLE_ORDERED.
   }
   ...
   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED ||
       internalPacket->reliability == RELIABLE_ORDERED ||
       internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
      bitStream->Write(internalPacket->orderingIndex);  // Used for UNRELIABLE_SEQUENCED,
                                                        // RELIABLE_SEQUENCED, RELIABLE_ORDERED.
      tempChar = internalPacket->orderingChannel;
      bitStream->WriteAlignedVar8((const char *)&tempChar);  // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
                                                             // RELIABLE_ORDERED. 5 bits needed, write one byte
   }
   ...
   if (internalPacket->splitPacketCount > 0) {
      ...
      bitStream->WriteAlignedVar32(
          (const char *)&internalPacket->splitPacketIndex);  // Only needed if splitPacketCount>0. 4 bytes
...
InternalPacket *ReliabilityLayer::CreateInternalPacketFromBitStream(RakNet::BitStream *bitStream, CCTimeType time) {
   ...
   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED) {
      bitStream->Read(internalPacket->sequencingIndex);  // Used for UNRELIABLE_SEQUENCED,
                                                         // RELIABLE_SEQUENCED, RELIABLE_ORDERED.
   }
   ...
   if (internalPacket->reliability == UNRELIABLE_SEQUENCED || internalPacket->reliability == RELIABLE_SEQUENCED ||
       internalPacket->reliability == RELIABLE_ORDERED ||
       internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT) {
      bitStream->Read(internalPacket->orderingIndex);  // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
                                                       // RELIABLE_ORDERED. 4 bytes.
      readSuccess = bitStream->ReadAlignedVar8(
          (char *)&internalPacket->orderingChannel);  // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED,
                                                      // RELIABLE_ORDERED. 5 bits needed, Read one byte
   } else
   ...
   if (hasSplitPacket) {
      ...
      bitStream->ReadAlignedVar32(
          (char *)&internalPacket->splitPacketCount);  // Only needed if splitPacketCount>0. 4 bytes
      bitStream->ReadAlignedVar16((char *)&internalPacket->splitPacketId);  // Only needed if splitPacketCount>0.
      readSuccess = bitStream->ReadAlignedVar32(
          (char *)&internalPacket->splitPacketIndex);  // Only needed if splitPacketCount>0. 4 bytes
   ...
   if (readSuccess == false || internalPacket->dataBitLength == 0 ||
       internalPacket->reliability >= NUMBER_OF_RELIABILITIES || internalPacket->orderingChannel >= 32 ||
   ...
   bitStreamSucceeded =
       bitStream->ReadAlignedBytes((unsigned char *)internalPacket->data, BITS_TO_BYTES(internalPacket->dataBitLength));
...
bool ReliabilityLayer::CheckSHA1(char code[SHA1_LENGTH], unsigned char *const buffer, unsigned int nbytes) {
   ...
   for (int i = 0; i < SHA1_LENGTH; i++)
      if (code[i] != code2[i]) return false;
...
DataStructures::List<InternalPacket*>&theList, int splitPacketId )
{
        unsigned i = 0;
...
//			theList[ i ]->reliability == UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
                        ...
                        theList[ i ]->orderingChannel == orderingChannel && ( splitPacketId == -1 ||
theList[ i ]->splitPacketId != (unsigned int) splitPacketId ) )
                {
...
//-------------------------------------------------------------------------------------------------------
// Search the specified list for sequenced packets with a value less than orderingIndex and delete
them
// Note - I added functionality so you can use the Queue as a list (in this case for searching) but
it is less efficient to do so than a regular list
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::DeleteSequencedPacketsInList( unsigned char orderingChannel,
...
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::IsOlderOrderedPacket(OrderingIndexType newPacketOrderingIndex,
   ...
   if (waitingForPacketOrderingIndex > maxRange / (OrderingIndexType)2) {
      if (newPacketOrderingIndex >=
              waitingForPacketOrderingIndex - maxRange / (OrderingIndexType)2 + (OrderingIndexType)1 &&
...
//-------------------------------------------------------------------------------------------------------
// Split the passed packet into chunks under MTU_SIZEbytes (including headers) and save those new
// chunks Optimized version
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SplitPacket(InternalPacket *internalPacket) {
   ...
   internalPacket->splitPacketCount =
       1;  // This causes GetMessageHeaderLengthBits to account for the split packet header
...
#endif
      internalPacketArray =
          (InternalPacket **)rakMalloc_Ex(sizeof(InternalPacket *) * internalPacket->splitPacketCount, _FILE_AND_LINE_);
   ...
   for (i = 0; i < (int)internalPacket->splitPacketCount; i++) {
      ...
      // sendPacketSet[internalPacket->priority].WriteLock();
      *internalPacketArray[i] = *internalPacket;
      internalPacketArray[i]->messageNumberAssigned = false;

      if (i != 0) internalPacket->messageInternalOrder = internalOrderIndex++;
   }
   ...
   do {
      ...
      if (bytesToSend > maximumSendBlockBytes) bytesToSend = maximumSendBlockBytes;

      // Copy over our chunk of data

      AllocInternalPacketData(internalPacketArray[splitPacketIndex], &refCounter, internalPacket->data,
                              internalPacket->data + byteOffset);
      //		internalPacketArray[ splitPacketIndex ]->data = (unsigned char*)
      // rakMalloc_Ex(
      // bytesToSend, _FILE_AND_LINE_ ); 		memcpy( internalPacketArray[
      // splitPacketIndex
      // ]->data, internalPacket->data + byteOffset, bytesToSend );

      if (bytesToSend != maximumSendBlockBytes)
         internalPacketArray[splitPacketIndex]->dataBitLength =
             internalPacket->dataBitLength - splitPacketIndex * (maximumSendBlockBytes << 3);
      else
   ...
   // Tell the heap we are going to push a list of elements where each element in the list follows
   // the heap order
   RakAssert(outgoingPacketBuffer.Size() == 0 ||
             outgoingPacketBuffer.Peek()->dataBitLength < BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
   ...
   for (i = 0; i < (int)internalPacket->splitPacketCount; i++) {
      ...
      statistics.bytesInSendBuffer[(int)(int)internalPacketArray[i]->priority] +=
          (double)BITS_TO_BYTES(internalPacketArray[i]->dataBitLength);
   ...
   // Do not delete, original is referenced by all split packets to avoid numerous allocations. See
   // AllocInternalPacketData above
   //	FreeInternalPacketData(internalPacket, _FILE_AND_LINE_ );
   ReleaseToInternalPacketPool(internalPacket);

   if (usedAlloca == false) rakFree_Ex(internalPacketArray, _FILE_AND_LINE_);
}
...
void ReliabilityLayer::InsertIntoSplitPacketList(InternalPacket *internalPacket, CCTimeType time) {
   bool objectExists;
   unsigned index;
   // Find in splitPacketChannelList if a SplitPacketChannel with this splitPacketId was already
   // allocated. If not, allocate and insert the channel into the list.
   ...
   if (objectExists == false) {
      ...
      AllocInternalPacketData(newChannel->returnedPacket,
                              BITS_TO_BYTES(internalPacket->dataBitLength * internalPacket->splitPacketCount), false,
                              __FILE__, __LINE__);
   ...
   if (internalPacket->splitPacketIndex == 0) {
      ...
      for (unsigned int j = 0; j < splitPacketChannelList[index]->splitPacketList.Size(); j++) {
         memcpy(splitPacketChannelList[index]->returnedPacket->data +
                    internalPacket->splitPacketIndex * splitPacketChannelList[index]->stride,
                internalPacket->data, (size_t)BITS_TO_BYTES(internalPacket->dataBitLength));
      ...
      memcpy(splitPacketChannelList[index]->returnedPacket->data, internalPacket->data,
   ...
   if (splitPacketChannelList[index]->gotFirstPacket == true && splitMessageProgressInterval &&
       ...
       splitPacketChannelList[index]->splitPacketsArrived !=
           splitPacketChannelList[index]->returnedPacket->splitPacketCount &&
      ...
      //		unsigned int len = sizeof(MessageID) + sizeof(unsigned int)*2 +
      // sizeof(unsigned int)
      ...
      // memcpy(progressIndicator->data+sizeof(MessageID)+sizeof(unsigned int)*3,
      // splitPacketChannelList[index]->firstPacket->data, (size_t)
      // BITS_TO_BYTES(splitPacketChannelList[index]->firstPacket->dataBitLength));
      memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 3,
             splitPacketChannelList[index]->returnedPacket->data, (size_t)BITS_TO_BYTES(l));
   }
   ...
   // If the index is 0, then this is the first packet. Record this so it can be returned to the
   // user with download progress
   if (internalPacket->splitPacketIndex == 0) splitPacketChannelList[index]->firstPacket = internalPacket;

   // Return download progress if we have the first packet, the list is not complete, and there are
   // enough packets to justify it
   if (splitMessageProgressInterval && splitPacketChannelList[index]->firstPacket &&
       splitPacketChannelList[index]->splitPacketList.AddedPacketsCount() !=
           splitPacketChannelList[index]->firstPacket->splitPacketCount &&
      ...
      memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 3,
             splitPacketChannelList[index]->firstPacket->data,
             (size_t)BITS_TO_BYTES(splitPacketChannelList[index]->firstPacket->dataBitLength));
...
// Optimized version
//-------------------------------------------------------------------------------------------------------
InternalPacket *ReliabilityLayer::BuildPacketFromSplitPacketList(SplitPacketChannel *splitPacketChannel,
                                                                 CCTimeType time) {
#if PREALLOCATE_LARGE_MESSAGES == 1
   ...
   internalPacket->data =
       (unsigned char *)rakMalloc_Ex((size_t)BITS_TO_BYTES(internalPacket->dataBitLength), _FILE_AND_LINE_);
...
#endif
(splitPacketChannelList[i]->splitPacketList[0]->reliability==UNRELIABLE ||
splitPacketChannelList[i]->splitPacketList[0]->reliability==UNRELIABLE_SEQUENCED))
{
...
//-------------------------------------------------------------------------------------------------------
// Creates a copy of the specified internal packet with data copied from the original starting at
// dataByteOffset for dataByteLength bytes. Does not copy any split data parameters as that
// information is always generated does not have any reason to be copied
//-------------------------------------------------------------------------------------------------------
InternalPacket *ReliabilityLayer::CreateInternalPacketCopy(InternalPacket *original, int dataByteOffset,
...
//-------------------------------------------------------------------------------------------------------
/*
DataStructures::LinkedList<InternalPacket*> *ReliabilityLayer::GetOrderingListAtOrderingStream(
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::InsertPacketIntoResendList(InternalPacket *internalPacket, CCTimeType time, bool firstResend,
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::KillConnection(void) { deadConnection = true; }
...
unsigned int ReliabilityLayer::GetResendListDataSize(void) const {
   // Not accurate but thread-safe.  The commented version might crash if the queue is cleared while
   // we loop through it return resendTree.Size();
   return statistics.messagesInResendBuffer;
}

//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::AckTimeout(RakNet::Time curTime) {
   // I check timeLastDatagramArrived-curTime because with threading it is possible that
   // timeLastDatagramArrived is slightly greater than curTime, in which case this is NOT an ack
   // timeout
   return (timeLastDatagramArrived - curTime) > 10000 && curTime - timeLastDatagramArrived > timeoutTime;
}
//-------------------------------------------------------------------------------------------------------
CCTimeType ReliabilityLayer::GetNextSendTime(void) const { return nextSendTime; }
//-------------------------------------------------------------------------------------------------------
CCTimeType ReliabilityLayer::GetTimeBetweenPackets(void) const { return timeBetweenPackets; }
//-------------------------------------------------------------------------------------------------------
#if INCLUDE_TIMESTAMP_WITH_DATAGRAMS == 1
CCTimeType ReliabilityLayer::GetAckPing(void) const { return ackPing; }
#endif
//-------------------------------------------------------------------------------------------------------
...
void ReliabilityLayer::PushPacket(CCTimeType time, InternalPacket *internalPacket, bool isReliable) {
   BitSize_t bitsForThisPacket =
       BYTES_TO_BITS(BITS_TO_BYTES(internalPacket->dataBitLength) + BITS_TO_BYTES(internalPacket->headerLength));
   ...
   // This code tells me how much time elapses between when you send, and when the message actually
   // goes out 	if (internalPacket->data[0]==0)
   // 	{
   // 		RakNet::TimeMS t;
   ...
   congestionManager.OnSendBytes(
       time, BITS_TO_BYTES(internalPacket->dataBitLength) + BITS_TO_BYTES(internalPacket->headerLength));
}
//-------------------------------------------------------------------------------------------------------
...
void ReliabilityLayer::ClearPacketsAndDatagrams(void) {
   ...
   for (i = 0; i < packetsToDeallocThisUpdate.Size(); i++) {
      // packetsToDeallocThisUpdate holds a boolean indicating if packetsToSendThisUpdate at this
      ...
      if (packetsToDeallocThisUpdate[i]) {
         ...
         // if (keepInternalPacketIfNeedsAck==false ||
         // packetsToSendThisUpdate[i]->reliability<RELIABLE_WITH_ACK_RECEIPT)
...
void ReliabilityLayer::MoveToListHead(InternalPacket *internalPacket) {
   if (internalPacket == resendLinkedListHead) return;
...
void ReliabilityLayer::RemoveFromList(InternalPacket *internalPacket, bool modifyUnacknowledgedBytes) {
   ...
   if (internalPacket == resendLinkedListHead) resendLinkedListHead = newPosition;
   if (resendLinkedListHead == internalPacket) resendLinkedListHead = 0;
...
//-------------------------------------------------------------------------------------------------------
bool ReliabilityLayer::IsResendQueueEmpty(void) const { return resendLinkedListHead == 0; }
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress, CCTimeType time, RakNetRandom *rnr,
   ...
   while (acknowlegements.Size() > 0) {
      ...
      // I think this is causing a bug where if the estimated bandwidth is very low for the
      // recipient, only acks ever get sent
...
void ReliabilityLayer::RemoveFromUnreliableLinkedList(InternalPacket *internalPacket) {
   if (internalPacket->reliability == UNRELIABLE || internalPacket->reliability == UNRELIABLE_SEQUENCED ||
      ...
      if (internalPacket == unreliableLinkedListHead) unreliableLinkedListHead = newPosition;
      if (unreliableLinkedListHead == internalPacket) unreliableLinkedListHead = 0;
   }
}
//-------------------------------------------------------------------------------------------------------
...
bool ReliabilityLayer::ResendBufferOverflow(void) const {
   ...
   //	int index2 = (sendReliableMessageNumberIndex+(uint32_t)1) & (uint32_t)
   // RESEND_BUFFER_ARRAY_MASK;
...
ReliabilityLayer::MessageNumberNode *ReliabilityLayer::GetMessageNumberNodeByDatagramIndex(
   ...
   if (datagramHistory.IsEmpty()) return 0;

   if (congestionManager.LessThan(index, datagramHistoryPopCount)) return 0;

   DatagramSequenceNumberType offsetIntoList = index - datagramHistoryPopCount;
   if (offsetIntoList >= datagramHistory.Size()) return 0;
...
void ReliabilityLayer::AddFirstToDatagramHistory(DatagramSequenceNumberType datagramNumber, CCTimeType timeSent) {
   ...
   // datagramHistory.Size()-1);
}
//-------------------------------------------------------------------------------------------------------
ReliabilityLayer::MessageNumberNode *ReliabilityLayer::AddFirstToDatagramHistory(
    DatagramSequenceNumberType datagramNumber, DatagramSequenceNumberType messageNumber, CCTimeType timeSent) {
   (void)datagramNumber;
   ...
   // messageNumber.val, datagramHistory.Size()-1);
   return mnm;
}
//-------------------------------------------------------------------------------------------------------
ReliabilityLayer::MessageNumberNode *ReliabilityLayer::AddSubsequentToDatagramHistory(
    MessageNumberNode *messageNumberNode, DatagramSequenceNumberType messageNumber) {
...
//-------------------------------------------------------------------------------------------------------
void ReliabilityLayer::AllocInternalPacketData(InternalPacket *internalPacket, unsigned int numBytes, bool allowStack,
...
void ReliabilityLayer::FreeInternalPacketData(InternalPacket *internalPacket, const char *file, unsigned int line) {
   if (internalPacket == 0) return;

   if (internalPacket->allocationScheme == InternalPacket::REF_COUNTED) {
      if (internalPacket->refCountedData == 0) return;
      ...
      if (internalPacket->data == 0) return;
...
#if LIBCAT_SECURITY == 1
   if (useSecurity) val -= cat::AuthenticatedEncryption::OVERHEAD_BYTES;
#endif
