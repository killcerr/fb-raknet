...
/// \file
/// \brief \b [Internal] A class which stores a user message, and all information associated with
/// sending and receiving that message.
///
...
/// This is the counter used for holding packet numbers, so we can detect duplicate packets.  It
/// should be large enough that if the variables Internally assumed to be 4 bytes, but written as 3
/// bytes in ReliabilityLayer::WriteToBitStreamFromInternalPacket
...
/// packets.  It should be large enough that if the variables were to wrap, the newly wrapped values
/// would no longer be in use.  Warning: Too large of a value wastes bandwidth!
...
struct InternalPacketFixedSizeTransmissionHeader {
   /// A unique numerical identifier given to this user message. Used to identify reliable messages
   /// on the network
   ...
   /// The ID of the split packet, if we have split packets.  This is the maximum number of split
   /// messages we can send simultaneously per connection.
...
struct InternalPacket : public InternalPacketFixedSizeTransmissionHeader {
   ...
   /// Has this message number been assigned yet?  We don't assign until the message is actually
   /// sent. This fixes a bug where pre-determining message numbers and then sending a message on a
   /// different channel creates a huge gap. This causes performance problems and causes those
   /// messages to timeout.
   bool messageNumberAssigned;
   /// Was this packet number used this update to track windowing drops or increases?  Each packet
   /// number is only used once per update.
   ...
   enum AllocationScheme {
      ...
      /// internalPacketRefCountedData is used in this case
      REF_COUNTED,
   ...
   // Linked list implementation so I can remove from the list via a pointer, without finding it in
   // the list
