...
bool HTTPConnection2::TransmitRequest(const char *stringToTransmit, const char *host, unsigned short port, bool useSSL,
                                      int ipVersion, SystemAddress useAddress, void *userData) {
...
bool HTTPConnection2::GetResponse(RakString &stringTransmitted, RakString &hostTransmitted, RakString &responseReceived,
...
bool HTTPConnection2::IsBusy(void) const { return pendingRequests.Size() > 0 || sentRequests.Size() > 0; }

bool HTTPConnection2::HasResponse(void) const { return completedRequests.Size() > 0; }
...
void ReadChunkBlock(size_t &currentChunkSize, size_t &bytesReadSoFar, char *txtIn, RakString &txtOut) {
   ...
   do {
      ...
      sLen = strlen(txtIn);
      if (sLen < bytesToRead) bytesToRead = sLen;
...
PluginReceiveResult HTTPConnection2::OnReceive(Packet *packet) {
         ...
         */
         ...
         if (isFirstChunk) {
            ...
            if (sentRequest->thisChunkSize == 0) {
               ...
               body_header += 2;
               ReadChunkBlock(sentRequest->thisChunkSize, sentRequest->bytesReadForThisChunk, body_header,
                              sentRequest->stringReceived);
            ...
            ReadChunkBlock(sentRequest->thisChunkSize, sentRequest->bytesReadForThisChunk, (char *)packet->data,
                           sentRequest->stringReceived);
            ...
            if (sentRequest->contentLength == -1) {
               ...
               if (length_header) {
                  ...
                  for (clLength = 0;
                       length_header[clLength] && length_header[clLength] >= '0' && length_header[clLength] <= '9';
                       clLength++)
                     ;
   ...
   if (locked == true) sentRequestsMutex.Unlock();
...
void HTTPConnection2::SendPendingRequestToConnectedSystem(SystemAddress sa) {
   ...
   while (i < pendingRequests.Size()) {
      ...
      if (request->hostEstimatedAddress == sa) {
         ...
         if (request->useSSL) tcpInterface->StartSSLClient(sa);
#endif
...
void HTTPConnection2::OnFailedConnectionAttempt(Packet *packet,
                                                PI2_FailedConnectionAttemptReason failedConnectionAttemptReason) {
   (void)failedConnectionAttemptReason;
   if (packet->systemAddress == UNASSIGNED_SYSTEM_ADDRESS) return;
...
void HTTPConnection2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void HTTPConnection2::SendRequest(Request *request) {
   tcpInterface->Send(request->stringToTransmit.C_String(), (unsigned int)request->stringToTransmit.GetLength(),
                      request->hostCompletedAddress, false);
}
