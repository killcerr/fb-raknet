...
void FileList::AddFile(const char *filepath, const char *filename, FileListNodeContext context) {
   ...
   if (length > (int)((unsigned int)-1 / 8)) {
      // If this assert hits, split up your file. You could also change BitSize_t in RakNetTypes.h
      // to unsigned long long but this is not recommended for performance reasons
...
void FileList::AddFile(const char *filename, const char *fullPathToFile, const char *data, const unsigned dataLength,
                       const unsigned fileLength, FileListNodeContext context, bool isAReference,
         ...
         if ((fileInfo.attrib & (_A_HIDDEN | _A_SUBDIR | _A_SYSTEM)) == 0) {
            ...
            if (writeData && writeHash) {
               ...
               if (fp) {
                  ...
                  //					sha1.Reset();
                  //					sha1.Update( ( unsigned char* )
                  // fileData+HASH_LENGTH, fileInfo.size
                  //); 					sha1.Final();
               ...
               // HASH_LENGTH,
               // fileInfo.size, context);
               AddFile((const char *)fullPath + rootLen, fullPath, (const char *)&hash, HASH_LENGTH, fileInfo.size,
                       context);
            } else if (writeData) {
...
void FileList::GetDeltaToCurrent(FileList *input, FileList *output, const char *dirSubset, const char *remoteSubdir) {
   ...
   for (thisIndex = 0; thisIndex < fileList.Size(); thisIndex++) {
      ...
      // fileList[thisIndex].filename has to match dirSubset and be shorter or equal to it in
      // length.
      if (dirSubsetLen > 0 &&
      ...
      for (inputIndex = 0; inputIndex < input->fileList.Size(); inputIndex++) {
         // If the filenames, hashes, and lengths match then skip this element in fileList.
...
void FileList::WriteDataToDisk(const char *applicationDirectory) {
   ...
   for (i = 0; i < fileList.Size(); i++) {
      ...
      // Security - Don't allow .. in the filename anywhere so you can't write outside of the root
      // directory
...
#ifdef _MSC_VER
#pragma warning(disable : 4996)  // unlink declared deprecated by Microsoft in order to make it
                                 // harder to be cross platform.  I don't agree it's deprecated.
#endif
