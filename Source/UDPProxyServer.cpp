...
void UDPProxyServer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void UDPProxyServer::OnAttach(void) {
   if (rakPeerInterface->IsActive()) OnRakPeerStartup();
}
...
void UDPProxyServer::OnForwardingRequestFromCoordinatorToServer(Packet *packet) {
   ...
   UDPForwarderResult success = udpForwarder.StartForwarding(sourceAddress, targetAddress, timeoutOnNoDataMS, 0,
                                                             socketFamily, &forwardingPort, 0);
   RakNet::BitStream outgoingBs;
   outgoingBs.Write((MessageID)ID_UDP_PROXY_GENERAL);
