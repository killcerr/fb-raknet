...
UDPForwarder::ForwardEntry::~ForwardEntry() {
   if (socket != INVALID_SOCKET) closesocket__(socket);
}
...
void UDPForwarder::Startup(void) {
   if (isRunning.GetValue() > 0) return;
   ...
   while (threadRunning.GetValue() == 0) RakSleep(30);
}
...
void UDPForwarder::Shutdown(void) {
   ...
   unsigned int j;
   for (j = 0; j < forwardListNotUpdated.Size(); j++) RakNet::OP_DELETE(forwardListNotUpdated[j], _FILE_AND_LINE_);
...
int UDPForwarder::GetMaxForwardEntries(void) const { return maxForwardEntries; }

int UDPForwarder::GetUsedForwardEntries(void) const { return (int)forwardListNotUpdated.Size(); }

UDPForwarderResult UDPForwarder::StartForwarding(SystemAddress source, SystemAddress destination,
                                                 RakNet::TimeMS timeoutOnNoDataMS, const char *forceHostAddress,
                                                 unsigned short socketFamily, unsigned short *forwardingPort,
   ...
   if (isRunning.GetValue() == 0) return UDPFORWARDER_NOT_RUNNING;
   ...
   if (forceHostAddress && forceHostAddress[0]) sfis->forceHostAddress = forceHostAddress;
      ...
      for (unsigned int i = 0; i < startForwardingOutput.Size(); i++) {
         if (startForwardingOutput[i].inputId == inputId) {
            if (startForwardingOutput[i].result == UDPFORWARDER_SUCCESS) {
               if (forwardingPort) *forwardingPort = startForwardingOutput[i].forwardingPort;
               if (forwardingSocket) *forwardingSocket = startForwardingOutput[i].forwardingSocket;
            }
...
void UDPForwarder::RecvFrom(RakNet::TimeMS curTime, ForwardEntry *forwardEntry) {
...
#else
   receivedDataLen =
       recvfrom__(forwardEntry->socket, data, MAXIMUM_MTU_SIZE, flag, (sockaddr *)&sockAddrIn, (socklen_t *)&len2);
#endif
   ...
   do {
      ...
      if (forwardTarget.address.addr4.sin_family == AF_INET) {
         do {
            len = sendto__(forwardEntry->socket, data, receivedDataLen, 0,
                           (const sockaddr *)&forwardTarget.address.addr4, sizeof(sockaddr_in));
         } while (len == 0);
      } else {
         do {
            len = sendto__(forwardEntry->socket, data, receivedDataLen, 0,
                           (const sockaddr *)&forwardTarget.address.addr6, sizeof(sockaddr_in6));
         } while (len == 0);
      }
#else
      do {
         len = sendto__(forwardEntry->socket, data, receivedDataLen, 0, (const sockaddr *)&forwardTarget.address.addr4,
                        sizeof(sockaddr_in));
      } while (len == 0);
#endif
