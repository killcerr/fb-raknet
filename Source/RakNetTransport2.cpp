...
RakNetTransport2::~RakNetTransport2() { Stop(); }
...
void RakNetTransport2::Send(SystemAddress systemAddress, const char *data, ...) {
   ...
   rakPeerInterface->Send(&str, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0, systemAddress,
...
SystemAddress RakNetTransport2::HasNewIncomingConnection(void) {
   if (newConnections.Size()) return newConnections.Pop();
   return UNASSIGNED_SYSTEM_ADDRESS;
}

SystemAddress RakNetTransport2::HasLostConnection(void) {
   if (lostConnections.Size()) return lostConnections.Pop();
   return UNASSIGNED_SYSTEM_ADDRESS;
}
...
void RakNetTransport2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
