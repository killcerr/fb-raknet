...
enum RNSPerSecondMetrics {
   ...
   /// How many user message bytes were sent via a call to RakPeerInterface::Send(). This is less
   /// than or equal to USER_MESSAGE_BYTES_PUSHED. A message would be pushed, but not yet sent, due
   /// to congestion control
   USER_MESSAGE_BYTES_SENT,

   /// How many user message bytes were resent. A message is resent if it is marked as reliable, and
   /// either the message didn't arrive or the message ack didn't arrive.
   ...
   /// How many user message bytes were received, but ignored due to data format errors. This will
   /// usually be 0.
   USER_MESSAGE_BYTES_RECEIVED_IGNORED,

   /// How many actual bytes were sent, including per-message and per-datagram overhead, and
   /// reliable
   /// message acks
   ACTUAL_BYTES_SENT,
...
struct RAK_DLL_EXPORT RakNetStatistics {
   ...
   /// For each type in RNSPerSecondMetrics, what is the total value over the lifetime of the
   /// connection?
   ...
   /// RakPeer::SetPerConnectionOutgoingBandwidthLimit()?
   ...
   /// How many messages are waiting in the resend buffer? This includes messages waiting for an
   /// ack, so should normally be a small value If the value is rising over time, you are exceeding
   /// the bandwidth capacity. See BPSLimitByCongestionControl
   ...
   /// Over the last second, what was our packetloss? This number will range from 0.0 (for none)
   /// to 1.0 (for 100%)
