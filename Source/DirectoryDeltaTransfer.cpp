...
class DDTCallback : public FileListTransferCBInterface {
  public:
   ...
   virtual bool OnFile(OnFileStruct *onFileStruct) {
      ...
      if (/* JE: aways true onFileStruct->fileName && */ onFileStruct->fileData &&
   ...
   virtual void OnFileProgress(FileProgressStruct *fps) {
      ...
      if (/* JE: Always true fps->onFileStruct->fileName && */ subdirLen < strlen(fps->onFileStruct->fileName)) {
         strcpy(fullPathToDir, outputSubdir);
...
void DirectoryDeltaTransfer::AddUploadsFromSubdirectory(const char *subdir) {
   availableUploads->AddFilesFromDirectory(applicationDirectory, subdir, true, false, true,
                                           FileListNodeContext(0, 0, 0, 0));
}

unsigned short DirectoryDeltaTransfer::DownloadFromSubdirectory(FileList &localFiles, const char *subdir,
...
unsigned short DirectoryDeltaTransfer::DownloadFromSubdirectory(const char *subdir, const char *outputSubdir,
                                                                bool prependAppDirToOutputSubdir, SystemAddress host,
                                                                FileListTransferCBInterface *onFileCallback,
                                                                PacketPriority _priority, char _orderingChannel,
                                                                FileListProgress *cb) {
   ...
   localFiles.AddFilesFromDirectory(prependAppDirToOutputSubdir ? applicationDirectory : 0, outputSubdir, true, false,
                                    true, FileListNodeContext(0, 0, 0, 0));
   return DownloadFromSubdirectory(localFiles, subdir, outputSubdir, prependAppDirToOutputSubdir, host, onFileCallback,
                                   _priority, _orderingChannel, cb);
}

void DirectoryDeltaTransfer::GenerateHashes(FileList &localFiles, const char *outputSubdir,
...
void DirectoryDeltaTransfer::OnDownloadRequest(Packet *packet) {
   ...
   // This will call the ddtCallback interface that was passed to FileListTransfer::SetupReceive on
   // the remote system
   fileListTransfer->Send(&delta, rakPeerInterface, packet->systemAddress, setId, priority, orderingChannel,
                          incrementalReadInterface, chunkSize);
}
