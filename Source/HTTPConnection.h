...
/// Instantiate a new instance of HTTPConnection, and associate TCPInterface with the class in the
/// constructor. Use Post() to send commands to the web server, and ProcessDataPacket() to update
/// the connection with packets returned from TCPInterface that have the system address of the web
/// server This class will handle connecting and reconnecting as necessary.
///
  ...
  public:
   ...
   struct BadResponse {
     public:
      ...
      BadResponse(const unsigned char *_data, int _code) : data((const char *)_data), code(_code) {}

      BadResponse(const char *_data, int _code) : data(_data), code(_code) {}
