...
class RakNetSocket2 {
  public:
   ...
   // In order for the handler to trigger, some platforms must call PollRecvFrom, some platforms
   // this create an internal thread.
...
class RNS2_WindowsStore8 : public RakNetSocket2 {
  public:
   ...
   static int WinRTSetSockOpt(Windows::Networking::Sockets::DatagramSocket ^ s, int level, int optname,
                              const char *optval, socklen_t optlen);

   static int WinRTIOCTLSocket(Windows::Networking::Sockets::DatagramSocket ^ s, long cmd, unsigned long *argp);

   static int WinRTGetSockName(Windows::Networking::Sockets::DatagramSocket ^ s, struct sockaddr *name,
                               socklen_t *namelen);
...
class RNS2_NativeClient : public RakNetSocket2 {
  public:
   ...
   // RNS2_NativeClient doesn't automatically call recvfrom in a thread - user must call Update()
   // from the main thread This causes buffered sends to send, until send is asynch pending It
   // causes recvfrom events to trigger the callback, and push a message to the event handler
   //
   // Example:
   //
...
class RAK_DLL_EXPORT SocketLayerOverride {
  public:
   ...
   /// dataOut
   // Return -1 to use RakNet's normal recvfrom, 0 to abort RakNet's normal recvfrom, and positive
   // to return data
