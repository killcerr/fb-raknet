...
bool RakNet::CanConnect(NATTypeDetectionResult type1, NATTypeDetectionResult type2) {
   ...
   bool connectionGraph[NAT_TYPE_COUNT][NAT_TYPE_COUNT] = {
       ...
       // Unknown,	InProgress,	Supports_UPNP
...
RakNetSocket2 *RakNet::CreateNonblockingBoundSocket(const char *bindAddr
   ...
   /*
   #ifdef __native_client__
   RakNetSocket2 *s = SocketLayer::CreateBoundSocket( 0, 0, false, bindAddr, true, 0, AF_INET,
   chromeInstance ); #else RakNetSocket2 *s = SocketLayer::CreateBoundSocket( 0, 0, false, bindAddr,
   true, 0, AF_INET, 0 ); #endif

   #ifdef _WIN32
           unsigned long nonblocking = 1;
           s->IOCTLSocket( FIONBIO, &nonblocking );
   #elif defined(_PS3) || defined(__PS3__) || defined(SN_TARGET_PS3) || defined(_PS4) ||
   defined(SN_TARGET_PSP2) int sock_opt=1; s->SetSockOpt(SOL_SOCKET, SO_NBIO, ( char * ) & sock_opt,
   sizeof ( sock_opt ) ); #elif defined(__native_client__)
           // Nop
   #else
...
/*
int RakNet::NatTypeRecvFrom(char *data, RakNetSocket2* socket, SystemAddress &sender,
RNS2EventHandler *eventHandler)
{
#if defined(__native_client__)
        RakAssert("TODO" && 0);
