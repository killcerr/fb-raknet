...
RNS2_NativeClient::~RNS2_NativeClient() {
   ...
   while (bufferedSends.Size()) RakNet::OP_DELETE(bufferedSends.Pop(), _FILE_AND_LINE_);
...
void RNS2_NativeClient::ProcessBufferedSend(void) {
   ...
   if (bindState != BS_BOUND) return;
   // Fast non-threadsafe check
   if (bufferedSends.IsEmpty() == true) return;
...
RNS2_SendParameters_NativeClient *RNS2_NativeClient::CloneSP(RNS2_SendParameters *sp, RNS2_NativeClient *socket2,
...
void RNS2_NativeClient::onSendTo(void *pData, int32_t dataSize) {
   if (dataSize <= 0) RAKNET_DEBUG_PRINTF("onSendTo: send failed with error %d\n", dataSize);
...
RNS2SendResult RNS2_NativeClient::Send(RNS2_SendParameters *sendParameters, const char *file, unsigned int line) {
   ...
   // This is called from multiple threads. Always buffer the send, until native client is
   // threadsafe
...
void RNS2_NativeClient::BufferSend(RNS2_SendParameters *sendParameters, const char *file, unsigned int line) {
   ...
   // Do not check to send immediately, because this was probably invoked from a thread and native
...
RNS2BindResult RNS2_Berkley::BindShared(RNS2_BerkleyBindParameters *bindParameters, const char *file,
                                        unsigned int line) {
   RNS2BindResult br;
   ...
   if (sr < 0) return BR_FAILED_SEND_TEST;
   ...
   _cfSocket = CFSocketCreateWithNative(NULL, rns2Socket, kCFSocketReadCallBack, SocketReadCallback,
 &context); #endif
   */
...
RNS2_Berkley::RNS2_Berkley() { rns2Socket = (RNS2Socket)INVALID_SOCKET; }
...
void RNS2_Berkley::SignalStopRecvPollingThread(void) { endThreads = true; }
...
void RNS2_Windows::GetMyIP(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
...
void RNS2_Linux::GetMyIP(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
   return GetMyIP_Windows_Linux(addresses);
}
#endif  // Linux
