...
class RAK_DLL_EXPORT Queue {
  public:
   ...
   queue_type &operator[](
       unsigned int position) const;           // Not a normal thing you do with a queue but can be used for efficiency
   void RemoveAtIndex(unsigned int position);  // Not a normal thing you do with a queue but can be
                                               // used for efficiency
...
template <class queue_type>
Queue<queue_type>::~Queue() {
   if (allocation_size > 0) RakNet::OP_DELETE_ARRAY(array, _FILE_AND_LINE_);
}
...
inline queue_type Queue<queue_type>::Pop(void) {
   ...
   if (head == 0) return (queue_type)array[allocation_size - 1];
