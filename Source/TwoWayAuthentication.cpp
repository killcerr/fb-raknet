...
TwoWayAuthentication::NonceGenerator::~NonceGenerator() { Clear(); }

void TwoWayAuthentication::NonceGenerator::GetNonce(char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH],
                                                    unsigned short *requestId, RakNet::AddressOrGUID remoteSystem) {
   TwoWayAuthentication::NonceAndRemoteSystemRequest *narsr =
       RakNet::OP_NEW<TwoWayAuthentication::NonceAndRemoteSystemRequest>(_FILE_AND_LINE_);
...
bool TwoWayAuthentication::NonceGenerator::GetNonceById(char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH],
                                                        unsigned short requestId, RakNet::AddressOrGUID remoteSystem,
                                                        bool popIfFound) {
   unsigned int i;
...
void TwoWayAuthentication::NonceGenerator::Clear(void) {
   unsigned int i;
   for (i = 0; i < generatedNonces.Size(); i++) RakNet::OP_DELETE(generatedNonces[i], _FILE_AND_LINE_);
...
TwoWayAuthentication::~TwoWayAuthentication() { Clear(); }
...
bool TwoWayAuthentication::AddPassword(RakNet::RakString identifier, RakNet::RakString password) {
   ...
   if (password == identifier) return false;  // Insecure
...
bool TwoWayAuthentication::Challenge(RakNet::RakString identifier, AddressOrGUID remoteSystem) {
   ...
   if (skhi.IsInvalid()) return false;
...
void TwoWayAuthentication::Update(void) {
   ...
   if (GreaterThan(curTime - CHALLENGE_MINIMUM_TIMEOUT, whenLastTimeoutCheck)) {
      while (outgoingChallenges.Size() &&
             GreaterThan(curTime - CHALLENGE_MINIMUM_TIMEOUT, outgoingChallenges.Peek().time)) {
...
void TwoWayAuthentication::OnRakPeerShutdown(void) { Clear(); }

void TwoWayAuthentication::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
   ...
   while (i < outgoingChallenges.Size()) {
      if ((rakNetGUID != UNASSIGNED_RAKNET_GUID && outgoingChallenges[i].remoteSystem.rakNetGuid == rakNetGUID) ||
          (systemAddress != UNASSIGNED_SYSTEM_ADDRESS &&
           outgoingChallenges[i].remoteSystem.systemAddress == systemAddress)) {
...
void TwoWayAuthentication::PushToUser(MessageID messageId, RakNet::RakString password,
                                      RakNet::AddressOrGUID remoteSystem) {
   RakNet::BitStream output;
   output.Write(messageId);
   if (password.IsEmpty() == false) output.Write(password);
   ...
   for (i = 0; i < outgoingChallenges.Size(); i++) {
      if (outgoingChallenges[i].remoteSystem == aog && outgoingChallenges[i].sentHash == false) {
         ...
         if (skhi.IsInvalid() == false) {
            ...
            bsOut.Write(requestId);
            bsOut.Write(
                outgoingChallenges[i].identifier);  // Identifier helps the other system lookup the password quickly.
...
void TwoWayAuthentication::OnPasswordResult(Packet *packet) {
   ...
   if (skhi.IsInvalid() == false) {
      ...
      if (memcmp(testHash, hashedNonceAndPw, HASHED_NONCE_AND_PW_LENGTH) == 0) {
         ...
         AddressOrGUID aog(packet);
         for (i = 0; i < outgoingChallenges.Size(); i++) {
            if (outgoingChallenges[i].identifier == passwordIdentifier && outgoingChallenges[i].remoteSystem == aog &&
                outgoingChallenges[i].sentHash == true) {
               outgoingChallenges.RemoveAtIndex(i);
...
void TwoWayAuthentication::Hash(char thierNonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH], RakNet::RakString password,
