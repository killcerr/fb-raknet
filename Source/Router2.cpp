...
1. Sender calls ConnectInternal(). A ConnnectRequest structure is allocated and stored in the
connectionRequests list, containing a list of every system we are connected to.
ID_ROUTER_2_QUERY_FORWARDING is sent to all connected systems.

2. Upon the router getting ID_ROUTER_2_QUERY_FORWARDING, ID_ROUTER_2_REPLY_FORWARDING is sent to the
sender indicating if that router is connected to the endpoint, along with the ping from the router
to the endpoint.

3. Upon the sender getting ID_ROUTER_2_REPLY_FORWARDING, the connection request structure is looked
up in Router2::UpdateForwarding. The ping is stored in that structure. Once all systems have
replied, the system continues to the next state. If every system in step 1 has been exhausted, and
routing has occured at least once, then ID_CONNECTION_LOST is returned. If every system in step 1
has been exhausted and routing has never occured, then ID_ROUTER_2_FORWARDING_NO_PATH is returned.
Otherwise, the router with the lowest ping is chosen, and RequestForwarding() is called with that
system, which sends ID_ROUTER_2_REQUEST_FORWARDING to the router.

4. When the router gets ID_ROUTER_2_REQUEST_FORWARDING, a MiniPunchRequest structure is allocated
and stored in the miniPunchesInProgress list. The function SendOOBMessages() sends
ID_ROUTER_2_REPLY_TO_SENDER_PORT from the routing sockets to both the sender and endpoint. It also
sends ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT through the regular RakNet connection.

5. The sender and endpoint should get ID_ROUTER_2_REPLY_TO_SENDER_PORT and/or
ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT depending on what type of router they have. If
ID_ROUTER_2_REPLY_TO_SENDER_PORT arrives, then this will reply back to the routing socket directly.
If ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT arrives, then the reply port is modified to be the port
specified by the router system. In both cases, ID_ROUTER_2_MINI_PUNCH_REPLY is sent. As the router
has already setup the forwarding, ID_ROUTER_2_MINI_PUNCH_REPLY will actually arrive to the endpoint
from the sender, and from the sender to the endpoint.

6. When ID_ROUTER_2_MINI_PUNCH_REPLY arrives, ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE will be sent to
the router. This is to tell the router that the forwarding has succeeded.

7. When ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE arrives on the router, the router will find the two
systems in the miniPunchesInProgress list, which was added in step 4 (See OnMiniPunchReplyBounce()).
gotReplyFromSource or gotReplyFromEndpoint will be set to true, depending on the sender. When both
gotReplyFromSource and gotReplyFromEndpoint have replied, then ID_ROUTER_2_REROUTE is sent to the
endpoint, and ID_ROUTER_2_FORWARDING_ESTABLISHED is sent to the sender.

8. When the endpoint gets ID_ROUTER_2_REROUTE, the system address is updated for the guid of the
sender using RakPeer::ChangeSystemAddress(). This happens in OnRerouted().

9. When the sender gets ID_ROUTER_2_FORWARDING_ESTABLISHED, then in OnForwardingSuccess() the
endpoint is removed from the connectionRequest list and moved to the forwardedConnectionList.

10. In OnClosedConnection(), for the sender, if the closed connection is the endpoint, then the
endpoint is removed from the forwardedConnectionList (this is a graceful disconnect). If the
connection was instead lost to the router, then ConnectInternal() gets called, which goes back to
step 1. If instead this was a connection requset in progress, then UpdateForwarding() gets called,
which goes back to step 3.

11. When the user connects the endpoint and sender, then the sender will get
ID_CONNECTION_REQUEST_ACCEPTED. The sender will look up the endpoint in the forwardedConnectionList,
and send ID_ROUTER_2_INCREASE_TIMEOUT to the endpoint. This message will call SetTimeoutTime() on
the endpoint, so that if the router disconnects, enough time is available for the reroute to
complete.
*/

#define MIN_MINIPUNCH_TIMEOUT 5000

void Router2DebugInterface::ShowFailure(const char *message) { printf("%s", message); }

void Router2DebugInterface::ShowDiagnostic(const char *message) { printf("%s", message); }
...
bool Router2::ConnectInternal(RakNetGUID endpointGuid, bool returnConnectionLostOnFailure) {
   ...
   if (largestPing < 0) {
      char buff[512];
      if (debugInterface)
         debugInterface->ShowFailure(FormatStringTS(buff, "Router2: ConnectInternal(%lld) failed at %s:%i\n",
   ...
   // StoreRequest(endpointGuid, Largest(ping*2), systemsSentTo). Set state
   // REQUEST_STATE_QUERY_FORWARDING
   ...
   for (unsigned int i = 0; i < guids.Size(); i++) {
      ...
      if (guids[i] != endpointGuid) {
         ...
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(
                FormatStringTS(buff, "Router2::ConnectInternal: at %s:%i, pack_id = %d", __FILE__, __LINE__, pack_id));
         }

      } else {
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                          "Router2::ConnectInternal: at %s:%i [else ..].: %lld==%lld",
                                                          __FILE__, __LINE__, guids[i].g, endpointGuid.g));
         }
      }
   }
   connectionRequestsMutex.Lock();
   ...
   if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          buff, "Broadcasting ID_ROUTER_2_QUERY_FORWARDING to %lld at %s:%i\n", endpointGuid.g, __FILE__, __LINE__));
   }

   return true;
}

void Router2::SetSocketFamily(unsigned short _socketFamily) { socketFamily = _socketFamily; }
...
void Router2::EstablishRouting(RakNetGUID endpointGuid) {
   ...
   if (cs != IS_DISCONNECTED && cs != IS_NOT_CONNECTED) {
      char buff[512];
      if (debugInterface)
         debugInterface->ShowFailure(FormatStringTS(buff,
                                                    "Router2 failed at %s:%i "
                                                    "(already connected to the %lld)\n",
...
PluginReceiveResult Router2::OnReceive(Packet *packet) {
   ...
   if (packet->data[0] == ID_ROUTER_2_INTERNAL) {
      switch (packet->data[1]) {
         ...
         case ID_ROUTER_2_REQUEST_FORWARDING: {
            if (debugInterface) {
               ...
               debugInterface->ShowDiagnostic(FormatStringTS(
                   buff, "Got ID_ROUTER_2_REQUEST_FORWARDING on ip %s from %lld, ", buff2, packet->guid.g));
            }
         ...
         case ID_ROUTER_2_INCREASE_TIMEOUT: {
            /// The routed system wants more time to stay alive on no communication, in case the
            /// router drops or crashes
            rakPeerInterface->SetTimeoutTime(rakPeerInterface->GetTimeoutTime(packet->systemAddress) + 10000,
      ...
      switch (packet->data[1]) {
         case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
            ...
            if (debugInterface) {
               ...
               debugInterface->ShowDiagnostic(
                   FormatStringTS(buff,
                                  "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on address %s, replying with "
                                  "ID_ROUTER_2_MINI_PUNCH_REPLY at %s:%i\n",
                                  sa.GetPort(), buff2, _FILE_AND_LINE_));

               //						packet->systemAddress.ToString(true,buff2);
               //						debugInterface->ShowDiagnostic(FormatStringTS(buff,"Got
               // ID_ROUTER_2_REPLY_TO_SENDER_PORT on address %s (%lld), "
               //                                       "replying with ID_ROUTER_2_MINI_PUNCH_REPLY
               //                                       at %s:%i\n", buff2,packet->guid.g, __FILE__,
               //                                       __LINE__));
            }
         ...
         case ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT: {
            ...
            if (debugInterface) {
               ...
               debugInterface->ShowDiagnostic(
                   FormatStringTS(buff,
                                  "Got ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT %i on address %s, "
      ...
      if (OnForwardingSuccess(packet) == false) return RR_STOP_PROCESSING_AND_DEALLOCATE;
      ...
      if (forwardingIndex < forwardedConnectionList.Size()) {
         ...
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                          "Got ID_CONNECTION_REQUEST_ACCEPTED, "
                                                          "sending ID_ROUTER_2_INCREASE_TIMEOUT to the %lld at %s:%i\n",
      ...
      if (packet->wasGeneratedLocally == false) return RR_STOP_PROCESSING_AND_DEALLOCATE;
   }
...
void Router2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
   ...
   while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
      if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid == rakNetGUID) {
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(
                FormatStringTS(buff, "Closed connection to the %lld, removing forwarding from list at %s:%i\n",
         ...
         forwardedConnectionList.RemoveAtIndexFast(forwardedConnectionIndex);
                 ...
                 forwardedConnectionList[forwardedConnectionIndex].weInitiatedForwarding) {
         // Lost connection to intermediary. Restart process to connect to endpoint. If failed, push
         // ID_CONNECTION_LOST. Also remove connection request if it already is in the list, to
         // restart it
         connectionRequestsMutex.Lock();
         ...
         if ((unsigned int)-1 != pos) {
            RakNet::OP_DELETE(connectionRequests[pos], __FILE__, __LINE__);
         ...
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(FormatStringTS(
                buff, "Closed connection %lld, restarting forwarding at %s:%i\n", rakNetGUID.g, __FILE__, __LINE__));
         }

         // This should not be removed - the connection is still forwarded, but perhaps through
         // another system
         //			forwardedConnectionList.RemoveAtIndexFast(forwardedConnectionIndex);
      } else
         forwardedConnectionIndex++;
      ...
      if (connectionRequestGuidIndex != (unsigned int)-1) {
         ...
         if (UpdateForwarding(cr) == false)  // If returns false, no connection request systems left
         {
            if (debugInterface) {
               char buff[512];
               debugInterface->ShowDiagnostic(
                   FormatStringTS(buff, "Aborted connection to the %lld, aborted forwarding at %s:%i\n", rakNetGUID.g,
                                  __FILE__, __LINE__));
            }

            RemoveConnectionRequest(connectionRequestIndex);
         } else  // Else a system in the connection request list dropped. If
                 // cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we are still waiting
                 // for other systems to respond.
         {
            if (debugInterface) {
               char buff[512];
               debugInterface->ShowDiagnostic(
                   FormatStringTS(buff, "Aborted connection attempt to %lld, restarting forwarding to %lld at %s:%i\n",
                                  rakNetGUID.g, cr->endpointGuid.g, __FILE__, __LINE__));
            }
            //                 if(volatile bool is_my_fix_a_truth = true) { // A system in the list
            //                 of potential systems to try routing to dropped. There is no need to
            ...
            //
            //                     if(false ==
            //                     ConnectInternal(cr->endpointGuid,cr->returnConnectionLostOnFailure))
            //                         if (debugInterface)
            //                         {
            //                             char buff[512];
            //                             debugInterface->ShowDiagnostic(FormatStringTS(buff,"ConnectInternal(cr->endpointGuid,cr->returnConnectionLostOnFailure)
            //                             is false. at %s:%i\n", __FILE__, __LINE__));
            //                         }
            //                 }
   ...
   while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
      if (forwardedConnectionList[forwardedConnectionIndex].intermediaryAddress == packet->systemAddress) {
         if (debugInterface) {
            char buff[512];
            debugInterface->ShowDiagnostic(
                FormatStringTS(buff, "Failed connection attempt to forwarded system (%lld : %s) at %s:%i\n",
                               forwardedConnectionList[forwardedConnectionIndex].endpointGuid.g,
                               packet->systemAddress.ToString(true), __FILE__, __LINE__));
         }
...
void Router2::OnRakPeerShutdown(void) { ClearAll(); }
// connectionRequestsMutex should already be locked
bool Router2::UpdateForwarding(ConnnectRequest *connectionRequest) {
   connectionRequest->connectionRequestSystemsMutex.Lock();

   //   RAKNET_DEBUG_PRINTF(__FUNCTION__": connectionRequest->connectionRequestSystems.Size = %d",
   //   connectionRequest->connectionRequestSystems.Size());
   ...
   if (connectionRequest->connectionRequestSystems.Size() == 0) {
      ...
      if (connectionRequest->returnConnectionLostOnFailure) {
         ReturnToUser(ID_CONNECTION_LOST, connectionRequest->endpointGuid, UNASSIGNED_SYSTEM_ADDRESS,
                      true);  // This is a connection which was previously established. Rerouting is
                              // not possible.
                              //             bool sendDisconnectionNotification = false;
         //             rakPeerInterface->CloseConnection(rakPeerInterface->GetSystemAddressFromGuid(connectionRequest->endpointGuid),
         //             sendDisconnectionNotification); RAKNET_DEBUG_PRINTF(__FUNCTION__": call
         //             rakPeerInterface->CloseConnection(%lld)" ,
         //             connectionRequest->endpointGuid.g);
      } else
      ...
      if (debugInterface) {
         char buff[512];
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff, "Forwarding failed, no remaining systems at %s:%i\n", _FILE_AND_LINE_));
      }

      forwardedConnectionListMutex.Lock();

      for (unsigned int forwardedConnectionIndex = 0; forwardedConnectionIndex < forwardedConnectionList.Size();
           forwardedConnectionIndex++) {
...
int ConnectionRequestSystemComp(const Router2::ConnectionRequestSystem &key,
   ...
   if (key.pingToEndpoint * (key.usedForwardingEntries + 1) == data.pingToEndpoint * (data.usedForwardingEntries + 1))
      return 1;
   if (key.guid < data.guid) return -1;
   if (key.guid > data.guid) return -1;
   return 0;
}
// connectionRequestsMutex should already be locked
void Router2::RequestForwarding(ConnnectRequest *connectionRequest) {
   ...
   DataStructures::OrderedList<ConnectionRequestSystem, ConnectionRequestSystem, ConnectionRequestSystemComp>
       commandList;
   unsigned int connectionRequestGuidIndex;
   connectionRequest->connectionRequestSystemsMutex.Lock();
   for (connectionRequestGuidIndex = 0; connectionRequestGuidIndex < connectionRequest->connectionRequestSystems.Size();
        connectionRequestGuidIndex++) {
      ...
      commandList.Insert(connectionRequest->connectionRequestSystems[connectionRequestGuidIndex],
                         connectionRequest->connectionRequestSystems[connectionRequestGuidIndex], true,
                         _FILE_AND_LINE_);
   }
   connectionRequest->connectionRequestSystemsMutex.Unlock();
   ...
   rakPeerInterface->Send(&bsOut, MEDIUM_PRIORITY, RELIABLE_ORDERED, 0,
                          connectionRequest->lastRequestedForwardingSystem, false);

   if (debugInterface) {
      char buff[512];
      debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                    "Sending ID_ROUTER_2_REQUEST_FORWARDING "
                                                    "(connectionRequest->lastRequestedForwardingSystem = %lld, "
                                                    "connectionRequest->endpointGuid = %lld) at %s:%i\n",
                                                    connectionRequest->lastRequestedForwardingSystem.g,
                                                    connectionRequest->endpointGuid.g, __FILE__, __LINE__));
   }
}
...
int Router2::ReturnFailureOnCannotForward(RakNetGUID sourceGuid, RakNetGUID endpointGuid) {
   // If the number of systems we are currently forwarding>=maxForwarding, return
   // ID_ROUTER_2_REPLY_FORWARDING,endpointGuid,false
   if (udpForwarder == 0 || udpForwarder->GetUsedForwardEntries() / 2 > maximumForwardingRequests) {
      char buff[512];
      if (debugInterface)
         debugInterface->ShowFailure(FormatStringTS(buff, "Router2 failed (%lld -> %lld) at %s:%i\n", sourceGuid.g,
                                                    endpointGuid.g, __FILE__, __LINE__));
   ...
   for (unsigned int i = 0; i < forwardedConnectionList.Size(); i++) {
      if ((forwardedConnectionList[i].endpointGuid == endpointGuid) ||
          (forwardedConnectionList[i].endpointGuid ==
           sourceGuid))  // sourceGuid is here so you do not respond to routing requests from
                         // systems you are already routing through.
      {
         forwardedConnectionListMutex.Unlock();
...
void Router2::OnQueryForwarding(Packet *packet) {
   ...
   if (pingToEndpoint == -1) {
      ...
      if (debugInterface)
         debugInterface->ShowFailure(
             FormatStringTS(buff, "Router2 failed (%lld) at %s:%i\n", packet->guid.g, __FILE__, __LINE__));
      return;
   }

   // If we are connected to endpointGuid, reply
   // ID_ROUTER_2_REPLY_FORWARDING,endpointGuid,true,ping,numCurrentlyForwarding
   RakNet::BitStream bsOut;
   bsOut.Write((MessageID)ID_ROUTER_2_INTERNAL);
   bsOut.Write((unsigned char)ID_ROUTER_2_REPLY_FORWARDING);
   bsOut.Write(endpointGuid);
   ...
   if (debugInterface) {
      char buff[512];
      debugInterface->ShowDiagnostic(FormatStringTS(buff, "Sending ID_ROUTER_2_REPLY_FORWARDING to the %lld at %s:%i\n",
                                                    packet->guid.g, __FILE__, __LINE__));
   }
}
...
void Router2::OnQueryForwardingReply(Packet *packet) {
   ...
   if (debugInterface) {
      ...
      packet->systemAddress.ToString(true, buff2);
      debugInterface->ShowDiagnostic(
          FormatStringTS(buff, "Got ID_ROUTER_2_REPLY_FORWARDING on address %s(%lld -> %lld) canForward=%i at %s:%i\n",
   ...
   if (canForward) {
      ...
      bs.Read(usedEntries);
      connectionRequests[connectionRequestIndex]
          ->connectionRequestSystems[connectionRequestGuidIndex]
          .usedForwardingEntries = usedEntries;
      connectionRequests[connectionRequestIndex]->connectionRequestSystems[connectionRequestGuidIndex].pingToEndpoint =
          rakPeerInterface->GetAveragePing(packet->guid) + pingToEndpoint;
   } else {
...
void Router2::SendForwardingSuccess(MessageID messageId, RakNetGUID sourceGuid, RakNetGUID endpointGuid,
   ...
   if (debugInterface) {
      ...
      if (messageId == ID_ROUTER_2_FORWARDING_ESTABLISHED)
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff, "Sending ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n", _FILE_AND_LINE_));
      else
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff, "Sending ID_ROUTER_2_REROUTED at %s:%i\n", _FILE_AND_LINE_));
   }
}
...
void Router2::SendOOBFromRakNetPort(OutOfBandIdentifiers oob, BitStream *extraData, SystemAddress sa) {
   ...
   rakPeerInterface->SendOutOfBand((const char *)ipAddressString, sa.GetPort(), (const char *)oobBs.GetData(),
                                   oobBs.GetNumberOfBytesUsed());
}
...
void Router2::SendOOBFromSpecifiedSocket(OutOfBandIdentifiers oob, SystemAddress sa, __UDPSOCKET__ socket) {
   ...
   // SocketLayer::SendTo_PC( socket, (const char*) bs.GetData(), bs.GetNumberOfBytesUsed(), sa,
   // __FILE__, __LINE__  );

   if (sa.address.addr4.sin_family == AF_INET) {
      sendto__(socket, (const char *)bs.GetData(), bs.GetNumberOfBytesUsed(), 0, (const sockaddr *)&sa.address.addr4,
               sizeof(sockaddr_in));
   } else {
#if RAKNET_SUPPORT_IPV6 == 1
      sendto__(socket, (const char *)bs.GetData(), bs.GetNumberOfBytesUsed(), 0, (const sockaddr *)&sa.address.addr6,
               sizeof(sockaddr_in6));
#endif
   }
}
...
void Router2::SendOOBMessages(Router2::MiniPunchRequest *mpr) {
   // Mini NAT punch
   // Send from srcToDestPort to packet->systemAddress (source). If the message arrives, the remote
   // system should reply.
   SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT, mpr->sourceAddress, mpr->forwardingSocket);

   // Send from destToSourcePort to endpointSystemAddress (destination). If the message arrives, the
   // remote system should reply.
...
void Router2::OnRequestForwarding(Packet *packet) {
   ...
   if (pingToEndpoint == -1) {
      ...
      if (debugInterface)
         debugInterface->ShowFailure(
             FormatStringTS(buff, "Router2 failed (packet->guid =%lld, endpointGuid = %lld) at %s:%i\n", packet->guid.g,
                            endpointGuid.g, __FILE__, __LINE__));
      return;
   }
   ...
   UDPForwarderResult result = udpForwarder->StartForwarding(packet->systemAddress, endpointSystemAddress, 30000, 0,
                                                             socketFamily, &forwardingPort, &forwardingSocket);
   ...
   if (result == UDPFORWARDER_FORWARDING_ALREADY_EXISTS) {
      if (debugInterface) {
         ...
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff,
                            "Got ID_ROUTER_2_REQUEST_FORWARDING, result=UDPFORWARDER_FORWARDING_ALREADY_EXISTS "
                            "(packet->guid =%lld, endpointGuid = %lld) at %s:%i\n",
      ...
      if (debugInterface)
         debugInterface->ShowFailure(
             FormatStringTS(buff,
                            "Router2 failed at %s:%i with UDPFORWARDER_NO_SOCKETS, packet->systemAddress=%s, "
                            "endpointSystemAddress=%s, forwardingPort=%i, forwardingSocket=%i\n",
      ...
      if (debugInterface)
         debugInterface->ShowFailure(
             FormatStringTS(buff,
                            "Router2 failed at %s:%i with UDPFORWARDER_INVALID_PARAMETERS, "
                            "packet->systemAddress=%s, "
                            "endpointSystemAddress=%s, forwardingPort=%i, forwardingSocket=%i\n",
                            __FILE__, __LINE__, buff2, buff3, forwardingPort, forwardingSocket));
      SendFailureOnCannotForward(packet->guid, endpointGuid);
      ...
      if (debugInterface)
         debugInterface->ShowFailure(
             FormatStringTS(buff,
                            "Router2 failed at %s:%i with UDPFORWARDER_BIND_FAILED, packet->systemAddress=%s, "
                            "endpointSystemAddress=%s, forwardingPort=%i, forwardingSocket=%i\n",
                            __FILE__, __LINE__, buff2, buff3, forwardingPort, forwardingSocket));
      SendFailureOnCannotForward(packet->guid, endpointGuid);
   } else {
      if (debugInterface) {
         ...
         char buff[512];
         debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                       "Got ID_ROUTER_2_REQUEST_FORWARDING.\n"
                                                       "endpointAddress=%s\nsourceAddress=%s\nforwardingPort=%i\n "
...
void Router2::OnMiniPunchReplyBounce(Packet *packet) {
   ...
   if (debugInterface) {
      char buff[512];
      debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                    "Got ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE from "
                                                    "guid=%lld (miniPunchesInProgress.Size() = %d)",
   ...
   while (i < miniPunchesInProgress.Size()) {
      if (miniPunchesInProgress[i].sourceGuid == packet->guid ||
         ...
         if (miniPunchesInProgress[i].sourceGuid == packet->guid) miniPunchesInProgress[i].gotReplyFromSource = true;
         ...
         if (debugInterface) {
            ...
            debugInterface->ShowDiagnostic(
                FormatStringTS(buff,
                               "Processing ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, gotReplyFromSource=%i "
                               "gotReplyFromEndpoint=%i at %s:%i\n",
                               miniPunchesInProgress[i].gotReplyFromSource,
                               miniPunchesInProgress[i].gotReplyFromEndpoint, __FILE__, __LINE__));
         }

         if (miniPunchesInProgress[i].gotReplyFromEndpoint == true &&
             miniPunchesInProgress[i].gotReplyFromSource == true) {
            SendForwardingSuccess(ID_ROUTER_2_REROUTED, miniPunchesInProgress[i].endpointGuid,
                                  miniPunchesInProgress[i].sourceGuid, miniPunchesInProgress[i].forwardingPort);
            SendForwardingSuccess(ID_ROUTER_2_FORWARDING_ESTABLISHED, miniPunchesInProgress[i].sourceGuid,
                                  miniPunchesInProgress[i].endpointGuid, miniPunchesInProgress[i].forwardingPort);
            miniPunchesInProgress.RemoveAtIndexFast(i);
         } else {
            i++;
         }
...
void Router2::OnMiniPunchReply(Packet *packet) {
   ...
   bs.Read(routerGuid);
   SendOOBFromRakNetPort(ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, 0,
...
void Router2::OnRerouted(Packet *packet) {
   ...
   forwardedConnectionListMutex.Lock();
   for (forwardingIndex = 0; forwardingIndex < forwardedConnectionList.Size(); forwardingIndex++) {
      if (forwardedConnectionList[forwardingIndex].endpointGuid == endpointGuid) break;
   }
   ...
   if (forwardingIndex < forwardedConnectionList.Size()) {
      ...
      if (debugInterface) {
         char buff[512];
         debugInterface->ShowDiagnostic(FormatStringTS(buff,
                                                       "FIX: Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED,"
                                                       " Calling RakPeer::ChangeSystemAddress(%lld, %s) at %s:%i\n",
                                                       endpointGuid.g, intermediaryAddress.ToString(true), __FILE__,
                                                       __LINE__));
      }
   } else {
      ...
      // add to forwarding list. This is only here to avoid reporting direct connections in
      // Router2::ReturnFailureOnCannotForward
      ...
      if (debugInterface) {
         ...
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff,
                            "Got ID_ROUTER_2_REROUTE, returning ID_ROUTER_2_REROUTED, Calling "
                            "RakPeer::ChangeSystemAddress at %s:%i\n",
                            __FILE__, __LINE__));
      }
   }
}
...
bool Router2::OnForwardingSuccess(Packet *packet) {
   ...
   if (forwardingIndex < forwardedConnectionList.Size()) {
      ...
      if (debugInterface) {
         ...
         debugInterface->ShowDiagnostic(
             FormatStringTS(buff,
                            "Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning ID_ROUTER_2_REROUTED, "
                            "Calling RakPeer::ChangeSystemAddress at %s:%i\n",
                            _FILE_AND_LINE_));
      }
      ...
      if (debugInterface) {
         char buff[512];
         debugInterface->ShowDiagnostic(FormatStringTS(
             buff, "Got and returning to user ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n", _FILE_AND_LINE_));
      }
   }
   return true;  // Return packet to user
}
...
int Router2::GetLargestPingAmongConnectedSystems(void) const {
   ...
   for (index = 0; index < rakPeerInterface->GetMaximumNumberOfPeers(); index++) {
      ...
      if (g != UNASSIGNED_RAKNET_GUID) {
         ...
         if (avePing > largestPing) largestPing = avePing;
      }
   }
   return largestPing;
}
...
unsigned int Router2::GetConnectionRequestIndex(RakNetGUID endpointGuid) {
   ...
   for (i = 0; i < connectionRequests.Size(); i++) {
      if (connectionRequests[i]->endpointGuid == endpointGuid) return i;
   }
...
unsigned int Router2::ConnnectRequest::GetGuidIndex(RakNetGUID guid) {
   ...
   for (i = 0; i < connectionRequestSystems.Size(); i++) {
      if (connectionRequestSystems[i].guid == guid) return i;
   }
   return (unsigned int)-1;
}

void Router2::ReturnToUser(MessageID messageId, RakNetGUID endpointGuid, const SystemAddress &systemAddress,
...
void Router2::SetDebugInterface(Router2DebugInterface *_debugInterface) { debugInterface = _debugInterface; }

Router2DebugInterface *Router2::GetDebugInterface(void) const { return debugInterface; }
