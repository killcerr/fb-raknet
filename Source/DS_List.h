...
/// \details Usually the Queue class is used instead, since it has all the same functionality and is
/// only worse at random access.
///
...
class RAK_DLL_EXPORT List {
  public:
   ...
   void Replace(const list_type &input, const list_type filler, const unsigned int position, const char *file,
                unsigned int line);
...
void List<list_type>::Insert(const list_type &input, const unsigned int position, const char *file, unsigned int line) {
   ...
   if (list_size == allocation_size) {
      ...
      // copy old array over
      for (unsigned int counter = 0; counter < list_size; ++counter) new_array[counter] = listArray[counter];
...
inline void List<list_type>::Replace(const list_type &input, const list_type filler, const unsigned int position,
   ...
   if ((list_size > 0) && (position < list_size)) {
      ...
      while (list_size < position) listArray[list_size++] = filler;
...
inline void List<list_type>::Replace(const list_type &input) {
   if (list_size > 0) listArray[list_size - 1] = input;
}
...
void List<list_type>::Preallocate(unsigned countNeeded, const char *file, unsigned int line) {
   ...
   if (allocation_size == 0) amountToAllocate = 16;
   while (amountToAllocate < countNeeded) amountToAllocate <<= 1;
