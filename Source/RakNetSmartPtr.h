...
class RAK_DLL_EXPORT RakNetSmartPtr {
  ...
  public:
   RakNetSmartPtr() : ptr(0), reference(0) {
      // Do not allocate by default, wasteful if we just have a list of preallocated and unassigend
      // smart pointers
   }
   ...
   // Allow you to change the values of the internal contents of the pointer, without changing what
   // is pointed to by other instances of the smart pointer
   void Clone(bool copyContents) {
      if (IsUnique() == false) {
         ...
         if (copyContents) *ptr = *oldPtr;
      }
   }

   int GetRefCount(void) const { return reference->GetRefCount(); }
   ...
   bool operator==(const RakNetSmartPtr<T> &sp) { return ptr == sp.ptr; }
