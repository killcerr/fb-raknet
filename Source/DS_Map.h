...
// http://www.cs.auckland.ac.nz/software/AlgAnim/red_black.html This makes insertions and deletions
// faster.  But then traversals are slow, while they are currently fast.
...
/// \note IMPORTANT! If you use defaultMapKeyComparison then call IMPLEMENT_DEFAULT_COMPARISON or
/// you will get an unresolved external linker error.
...
class RAK_DLL_EXPORT Map {
  public:
   ...
   // Has to be a static because the comparison callback for DataStructures::OrderedList is a C
   // function
...
template <class key_type, class data_type, int (*key_comparison_func)(const key_type &, const key_type &)>
Map<key_type, data_type, key_comparison_func> &Map<key_type, data_type, key_comparison_func>::operator=(
...
data_type &Map<key_type, data_type, key_comparison_func>::Get(const key_type &key) const {
   if (HasSavedSearchResult(key)) return mapNodeList[lastSearchIndex].mapNodeData;
...
unsigned Map<key_type, data_type, key_comparison_func>::GetIndexAtKey(const key_type &key) {
   if (HasSavedSearchResult(key)) return lastSearchIndex;
...
bool Map<key_type, data_type, key_comparison_func>::Has(const key_type &key) const {
   if (HasSavedSearchResult(key)) return true;

   bool objectExists;
   unsigned index;
   index = mapNodeList.GetIndexFromKey(key, &objectExists);
   if (objectExists) SaveLastSearch(key, index);
   return objectExists;
}
