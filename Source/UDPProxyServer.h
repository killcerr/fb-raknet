...
/// \file
/// \brief A RakNet plugin performing networking to communicate with UDPProxyServer. It allows
...
struct UDPProxyServerResultHandler {
   ...
   /// UDPProxyCoordinator::SetRemoteLoginPassword() than what we passed \param[out] usedPassword
   /// The password we passed to UDPProxyServer::LoginToCoordinator() \param[out] proxyServer The
   /// plugin calling this callback
...
/// \details When NAT Punchthrough fails, it is possible to use a non-NAT system to forward messages
/// from us to the recipient, and vice-versa.<BR> The class to forward messages is UDPForwarder, and
/// it is triggered over the network via the UDPProxyServer plugin.<BR> The UDPProxyServer connects
/// to UDPProxyServer to get a list of servers running UDPProxyServer, and the coordinator will
/// relay our forwarding request. \ingroup UDP_PROXY_GROUP
class RAK_DLL_EXPORT UDPProxyServer : public PluginInterface2 {
  public:
   ...
   /// \brief The server IP reported to the client is the IP address from the server to the
   /// coordinator. If the server and coordinator are on the same LAN, you need to call
   /// SetServerPublicIP() to tell the client what address to connect to \param[in] ip IP address to
   /// report in UDPProxyClientResultHandler::OnForwardingSuccess() and
   /// UDPProxyClientResultHandler::OnForwardingNotification() as proxyIPAddress
   void SetServerPublicIP(RakString ip);

   /// Operative class that performs the forwarding
   /// Exposed so you can call UDPForwarder::SetMaxForwardEntries() if you want to change away from
   /// the default UDPForwarder::Startup(), UDPForwarder::Shutdown(), and UDPForwarder::Update() are
