...
/// \file
/// \brief \b RakNet's plugin functionality system, version 2.  You can derive from this to create
/// your own plugins.
///
...
/// For each message that arrives on an instance of RakPeer, the plugins get an opportunity to
/// process them first. This enumeration represents what to do with the message \ingroup
/// PLUGIN_INTERFACE_GROUP
enum PluginReceiveResult {
   ...
   /// The plugin is going to hold on to this message.  Do not deallocate it but do not pass it to
...
class RAK_DLL_EXPORT PluginInterface2 {
  public:
   ...
   /// Called when a connection is dropped because the user called RakPeer::CloseConnection() for a
   /// particular system \param[in] systemAddress The system whose connection was closed \param[in]
   /// rakNetGuid The guid of the specified system \param[in] lostConnectionReason How the
   /// connection was closed: manually, connection lost, or notification of disconnection
   ...
   /// OnReliabilityLayerNotification(), OnInternalPacket(), and OnAck() If true, then you cannot
   /// call RakPeer::AttachPlugin() or RakPeer::DetachPlugin() for this plugin, while RakPeer is
   /// active
   ...
   /// Called on a receive from the socket, per datagram, that does not go through the reliability
   /// layer \pre To be called, UsesReliabilityLayer() must return true \param[in] data The data
   /// being sent \param[in] bitsUsed How many bits long \a data is \param[in] remoteSystemAddress
   /// Which system this message is being sent to
   ...
   /// \param[in] frameNumber The number of frames sent or received so far for this player depending
   /// on \a isSend .  Indicates the frame of this user message. \param[in] remoteSystemAddress The
   /// player we sent or got this packet from \param[in] time The current time as returned by
   /// RakNet::GetTimeMS() \param[in] isSend Is this callback representing a send event or receive
   /// event?
