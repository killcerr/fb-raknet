...
namespace RakNet {
/// Class to write a series of variables, copy the contents to memory, and return if the newly
/// written value is different than what was last written Can also encode the reads, writes, and
/// results directly to/from a bitstream
class VariableListDeltaTracker {
  public:
   ...
   /// Records the passed value of the variable to memory, and returns true if the value is
   /// different from the write before that (or if it is the first write) \pre Call StartWrite()
   /// before doing the first of a series of calls to WriteVar or other functions that call WriteVar
   ...
   bool WriteVar(const VarType &varData) {
      ...
      if (temp.GetNumberOfBytesUsed() != variableList[nextWriteIndex].byteLength) {
         variableList[nextWriteIndex].lastData =
             (char *)rakRealloc_Ex(variableList[nextWriteIndex].lastData, temp.GetNumberOfBytesUsed(), _FILE_AND_LINE_);
      ...
      if (variableList[nextWriteIndex].isDirty == false &&
          memcmp(temp.GetData(), variableList[nextWriteIndex].lastData, variableList[nextWriteIndex].byteLength) == 0) {
         nextWriteIndex++;
   ...
   /// Calls WriteVar. If the variable has changed, writes true, and writes the variable. Otherwise
   /// writes false.
   template <class VarType>
   ...
   /// Calls WriteVarToBitstream(). Additionally, adds the boolean result of WriteVar() to boolean
   /// bit array
   template <class VarType>
   bool WriteVarToBitstream(const VarType &varData, RakNet::BitStream *bitStream, unsigned char *bArray,
      ...
      if (WriteVarToBitstream(varData, bitStream) == true) {
         ...
         if ((writeOffset & 7) == 0) bArray[writeOffset >> 3] = 0;

         return false;
      }
   }

   /// Paired with a call to WriteVarToBitstream(), will read a variable if it had changed.
   /// Otherwise the values remains the same.
   template <class VarType>
   static bool ReadVarFromBitstream(VarType &varData, RakNet::BitStream *bitStream) {
      bool wasWritten;
      if (bitStream->Read(wasWritten) == false) return false;
      if (wasWritten) {
         if (bitStream->Read(varData) == false) return false;
      }
      return wasWritten;
   }

   /// Variables flagged dirty will cause WriteVar() to return true, even if the variable had not
   /// otherwise changed This updates all the variables in the list, where in each index \a
   /// varsWritten is true, so will the variable at the corresponding index be flagged dirty
