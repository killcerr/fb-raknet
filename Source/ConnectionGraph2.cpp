...
int RakNet::ConnectionGraph2::SystemAddressAndGuidComp(const SystemAddressAndGuid &key,
...
ConnectionGraph2::ConnectionGraph2() { autoProcessNewConnections = true; }

ConnectionGraph2::~ConnectionGraph2() {}

bool ConnectionGraph2::GetConnectionListForRemoteSystem(RakNetGUID remoteSystemGuid, SystemAddress *saOut,
                                                        RakNetGUID *guidOut, unsigned int *outLength) {
   ...
   for (idx2 = 0; idx2 < *outLength; idx2++) {
      if (guidOut) guidOut[idx2] = remoteSystems[idx]->remoteConnections[idx2].guid;
      if (saOut) saOut[idx2] = remoteSystems[idx]->remoteConnections[idx2].systemAddress;
   }
   return true;
}
...
uint16_t ConnectionGraph2::GetPingBetweenSystems(RakNetGUID g1, RakNetGUID g2) const {
   ...
   if (g1 == rakPeerInterface->GetMyGUID()) return (uint16_t)rakPeerInterface->GetAveragePing(g2);
   if (g2 == rakPeerInterface->GetMyGUID()) return (uint16_t)rakPeerInterface->GetAveragePing(g1);
...
/// Returns the system with the lowest total ping among all its connections. This can be used as the
/// 'best host' for a peer to peer session
RakNetGUID ConnectionGraph2::GetLowestAveragePingSystem(void) const {
   ...
   if (lowestPingIdx == (unsigned int)-1) return rakPeerInterface->GetMyGUID();
   return remoteSystems[lowestPingIdx]->guid;
}

void ConnectionGraph2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void ConnectionGraph2::SetAutoProcessNewConnections(bool b) { autoProcessNewConnections = b; }

bool ConnectionGraph2::GetAutoProcessNewConnections(void) const { return autoProcessNewConnections; }
...
void ConnectionGraph2::AddParticipant(const SystemAddress &systemAddress, RakNetGUID rakNetGUID) {
   ...
   for (i = 0; i < addresses.Size(); i++) {
      if (addresses[i] == systemAddress) continue;
...
void ConnectionGraph2::GetParticipantList(DataStructures::OrderedList<RakNetGUID, RakNetGUID> &participantList) {
   ...
   unsigned int i;
   for (i = 0; i < remoteSystems.Size(); i++) participantList.InsertAtEnd(remoteSystems[i]->guid, _FILE_AND_LINE_);
}

void ConnectionGraph2::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming) {
   (void)isIncoming;
   if (autoProcessNewConnections) AddParticipant(systemAddress, rakNetGUID);
}
...
PluginReceiveResult ConnectionGraph2::OnReceive(Packet *packet) {
   if (packet->data[0] == ID_REMOTE_CONNECTION_LOST || packet->data[0] == ID_REMOTE_DISCONNECTION_NOTIFICATION) {
      ...
      if (objectExists) {
         ...
         if (objectExists) remoteSystems[idx]->remoteConnections.RemoveAtIndex(idx2);
      }
      ...
      if (objectExists) {
         ...
         for (unsigned int idx2 = 0; idx2 < numAddresses; idx2++) {
            ...
            if (objectExists == false) remoteSystems[idx]->remoteConnections.InsertAtIndex(saag, ii, _FILE_AND_LINE_);
         }
      }
   }
