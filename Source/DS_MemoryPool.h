...
/// Very fast memory pool for allocating and deallocating structures that don't have constructors or
/// destructors. Contains a list of pages, each of which has an array of the user structures
...
class RAK_DLL_EXPORT MemoryPool {
  ...
  protected:
   ...
   // availablePages contains pages which have room to give the user new blocks.  We return these
   // blocks from the head of the list unavailablePages are pages which are totally full, and from
   // which we do not return new blocks. Pages move from the head of unavailablePages to the tail of
   // availablePages, and from the head of availablePages to the tail of unavailablePages
...
MemoryBlockType *MemoryPool<MemoryBlockType>::Allocate(const char *file, unsigned int line) {
   ...
   if (InitPage(availablePages, availablePages, file, line) == false) return 0;
...
void MemoryPool<MemoryBlockType>::Release(MemoryBlockType *m, const char *file, unsigned int line) {
   ...
   if (curPage->availableStackSize == 0) {
      ...
      if (unavailablePagesSize > 0 && curPage == unavailablePages) unavailablePages = unavailablePages->next;
      ...
      if (curPage->availableStackSize == BlocksPerPage() && availablePagesSize >= DS_MEMORY_POOL_MAX_FREE_PAGES) {
