...
TelnetTransport::~TelnetTransport() {
   Stop();
   if (sendSuffix) rakFree_Ex(sendSuffix, _FILE_AND_LINE_);
   if (sendPrefix) rakFree_Ex(sendPrefix, _FILE_AND_LINE_);
}
...
void TelnetTransport::Stop(void) {
   ...
   tcpInterface->Stop();
   unsigned i;
   for (i = 0; i < remoteClients.Size(); i++) RakNet::OP_DELETE(remoteClients[i], _FILE_AND_LINE_);
...
void TelnetTransport::CloseConnection(SystemAddress systemAddress) { tcpInterface->CloseConnection(systemAddress); }
...
Packet *TelnetTransport::Receive(void) {
   ...
   */
   ...
   for (i = 0; i < remoteClients.Size(); i++) {
      if (remoteClients[i]->systemAddress == p->systemAddress) remoteClient = remoteClients[i];
   }
   ...
   if (p->length == 3 && p->data[0] == 27 && p->data[1] == 91 && p->data[2] == 65) {
      if (remoteClient->lastSentTextInput[0]) {
         ...
         for (int i = 0; remoteClient->textInput[i]; i++) remoteClient->textInput[i] = 8;
         strcat(remoteClient->textInput, remoteClient->lastSentTextInput);
         tcpInterface->Send((const char *)remoteClient->textInput, (unsigned int)strlen(remoteClient->textInput),
   ...
   // Hack - I don't know what the hell this is about but cursor keys send 3 characters at a time. I
   // can block these
   // Up=27,91,65
   // Down=27,91,66
   // Right=27,91,67
...
SystemAddress TelnetTransport::HasNewIncomingConnection(void) {
   ...
   // 03/16/06 Can't force the stupid windows telnet to use line mode or local echo so now I have to
   // track all the remote players and their input buffer
...
void TelnetTransport::AutoAllocate(void) {
   if (tcpInterface == 0) tcpInterface = new TCPInterface;
}
