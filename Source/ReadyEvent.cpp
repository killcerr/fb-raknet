...
ReadyEvent::ReadyEvent() { channel = 0; }

ReadyEvent::~ReadyEvent() { Clear(); }
...
bool ReadyEvent::IsEventCompletionProcessing(int eventId) const {
   ...
   if (objectExists) {
      ...
      ReadyEventNode *ren = readyEventNodeList[eventIndex];
      if (ren->eventStatus == ID_READY_EVENT_FORCE_ALL_SET) return false;
...
bool ReadyEvent::HasEvent(int eventId) { return readyEventNodeList.HasData(eventId); }

unsigned ReadyEvent::GetEventListSize(void) const { return readyEventNodeList.Size(); }

int ReadyEvent::GetEventAtIndex(unsigned index) const { return readyEventNodeList[index]->eventId; }
...
bool ReadyEvent::AddToWaitList(int eventId, RakNetGUID guid) {
   ...
   if (eventExists == false) eventIndex = CreateNewEvent(eventId, false);

   // Don't do this, otherwise if we are trying to start a 3 player game, it will not allow the 3rd
   ...
   if (numAdded > 0) UpdateReadyStatus(eventIndex);
   return numAdded > 0;
}
...
ReadyEventSystemStatus ReadyEvent::GetReadyStatus(int eventId, RakNetGUID guid) {
   ...
   if (objectExists) {
      ...
      if (objectExists == false) return RES_NOT_WAITING;
      if (ren->systemList[systemIndex].lastReceivedStatus == ID_READY_EVENT_SET) return RES_READY;
      if (ren->systemList[systemIndex].lastReceivedStatus == ID_READY_EVENT_UNSET) return RES_WAITING;
      if (ren->systemList[systemIndex].lastReceivedStatus == ID_READY_EVENT_ALL_SET) return RES_ALL_READY;
   }

   return RES_UNKNOWN_EVENT;
}

void ReadyEvent::SetSendChannel(unsigned char newChannel) { channel = newChannel; }
...
void ReadyEvent::OnReadyEventPacketUpdate(Packet *packet) {
   ...
   if (objectExists) {
      ...
      if (systemExists) {
         ...
         if (ren->systemList[systemIndex].lastReceivedStatus == packet->data[0]) return;
         ...
         if (ren->eventStatus == ID_READY_EVENT_FORCE_ALL_SET) return;
         UpdateReadyStatus(readyIndex);
         if (wasCompleted == false && IsEventCompletedByIndex(readyIndex)) PushCompletionPacket(readyIndex);
      }
   }
}
...
void ReadyEvent::OnReadyEventQuery(Packet *packet) {
   ...
   if (objectExists) {
      ...
      // Force the non-default send, because our initial send may have arrived at a system that
      // didn't yet create the ready event
      if (objectExists) SendReadyUpdate(readyIndex, systemIndex, true);
   }
}

void ReadyEvent::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void ReadyEvent::OnRakPeerShutdown(void) { Clear(); }
...
bool ReadyEvent::SetEventByIndex(int eventIndex, bool isReady) {
   ...
   if (ren->eventStatus == ID_READY_EVENT_UNSET && isReady == false) return false;  // Success - no change
   if (ren->eventStatus == ID_READY_EVENT_FORCE_ALL_SET) return false;              // Can't change
...
bool ReadyEvent::IsEventCompletedByIndex(unsigned eventIndex) const {
   ReadyEventNode *ren = readyEventNodeList[eventIndex];
   unsigned i;
   if (ren->eventStatus == ID_READY_EVENT_FORCE_ALL_SET) return true;
   if (ren->eventStatus != ID_READY_EVENT_ALL_SET) return false;
   for (i = 0; i < ren->systemList.Size(); i++)
      if (ren->systemList[i].lastReceivedStatus != ID_READY_EVENT_ALL_SET) return false;
   return true;
}
...
void ReadyEvent::UpdateReadyStatus(unsigned eventIndex) {
   ...
   if (ren->eventStatus == ID_READY_EVENT_SET) {
      // If you are set, and no other systems are ID_READY_EVENT_UNSET, then change your status to
      ...
      // If you are all set, and any systems are ID_READY_EVENT_UNSET, then change your status to
      // ID_READY_EVENT_SET
      anyUnset = false;
      for (i = 0; i < ren->systemList.Size(); i++) {
         if (ren->systemList[i].lastReceivedStatus == ID_READY_EVENT_UNSET) {
...
void ReadyEvent::SendReadyUpdate(unsigned eventIndex, unsigned systemIndex, bool forceIfNotDefault) {
   ReadyEventNode *ren = readyEventNodeList[eventIndex];
   RakNet::BitStream bs;
   // I do this rather than write true or false, so users that do not use BitStreams can still read
   // the data
...
void ReadyEvent::RemoveFromAllLists(RakNetGUID guid) {
   ...
   for (eventIndex = 0; eventIndex < readyEventNodeList.Size(); eventIndex++) {
      ...
      if (systemExists) readyEventNodeList[eventIndex]->systemList.RemoveAtIndex(systemIndex);
...
void ReadyEvent::PushCompletionPacket(unsigned eventId) {
   ...
   // Not necessary
   /*
   // Pass a packet to the user that we are now completed, as setting ourselves to signaled was the
   last thing being waited on Packet *p = AllocatePacketUnified(sizeof(MessageID)+sizeof(int));
