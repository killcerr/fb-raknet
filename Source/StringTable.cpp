...
StringTable::~StringTable() {
   ...
   for (i = 0; i < orderedStringList.Size(); i++) {
      if (orderedStringList[i].b) rakFree_Ex(orderedStringList[i].str, _FILE_AND_LINE_);
   }
}
...
StringTable *StringTable::Instance(void) { return instance; }
...
bool StringTable::DecodeString(char *output, int maxCharsToWrite, RakNet::BitStream *input) {
   ...
   if (!input->Read(hasIndex)) return false;
   if (hasIndex == false) {
...
#ifdef _DEBUG
         // Critical error - got a string index out of range, which means AddString was called more
         // times on the remote system than on this system. All systems must call AddString the same
         // number of types, with the same strings in the same order.
         RakAssert(0);
#endif
         return false;
      }
