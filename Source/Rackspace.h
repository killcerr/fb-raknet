...
/// \pre Packets returned from TCPInterface::HasLostConnection() must be passed to
/// Rackspace::OnClosedConnection()
class RAK_DLL_EXPORT Rackspace {
  public:
   ...
   /// \details All requests to authenticate and operate against Cloud Servers are performed using
   /// SSL over HTTP (HTTPS) on TCP port 443. Times out after 24 hours - if you get
   /// RET_Authenticate_Unauthorized in the RackspaceEventCallback callback, call again \sa
   /// RackspaceEventCallback::OnAuthenticationResult() \param[in] _tcpInterface An instance of
   /// TCPInterface, build with OPEN_SSL_CLIENT_SUPPORT 1 and already started \param[in]
   /// _authenticationURL See
   /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf . US-based accounts
   /// authenticate through auth.api.rackspacecloud.com. UK-based accounts authenticate through
   /// lon.auth.api.rackspacecloud.com \param[in] _rackspaceCloudUsername Username you registered
   /// with Rackspace on their website \param[in] _apiAccessKey Obtain your API access key from the
   /// Rackspace Cloud Control Panel in the Your Account API Access section. \return The address of
   /// the authentication server, or UNASSIGNED_SYSTEM_ADDRESS if the connection attempt failed
   SystemAddress Authenticate(TCPInterface *_tcpInterface, const char *_authenticationURL,
                              const char *_rackspaceCloudUsername, const char *_apiAccessKey);
   ...
   /// \details Create a server with a given image (harddrive contents) and flavor (hardware
   /// configuration) Get the available images with ListImages() Get the available flavors with
   /// ListFlavors() It is possible to configure the server in more detail. See the XML schema at
   /// http://docs.rackspacecloud.com/servers/api/v1.0 You can execute such a custom command by
   /// calling AddOperation() manually. See the implementation of CreateServer for how to do so. The
   /// server takes a while to build. Call GetServerDetails() to get the current build status.
   /// Server id to pass to GetServerDetails() is returned in the field <server ... id="1234"> \sa
   /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf \sa
   /// RackspaceEventCallback::OnCreateServerResult() \param[in] name Name of the server. Only
   /// alphanumeric characters, periods, and hyphens are valid. Server Name cannot start or end with
   /// a period or hyphen. \param[in] imageId Which image (harddrive contents, including OS) to use
   /// \param[in] flavorId Which flavor (hardware config) to use, primarily how much memory is
   /// available.
   ...
   /// \param[in] serverId Which server to get details on. You can call ListServers() to get the
   ...
   /// \param[in] serverId Which server to get details on. You can call ListServers() to get the
   /// list of active servers. \param[in] newName The new server name. Leave blank to leave
   /// unchanged. Only alphanumeric characters, periods, and hyphens are valid. Server Name cannot
   /// start or end with a period or hyphen. \param[in] newPassword The new server password. Leave
   /// blank to leave unchanged.
   void UpdateServerNameOrPassword(RakNet::RakString serverId, RakNet::RakString newName,
   ...
   /// \param[in] serverId Which server to operate on. You can call ListServers() to get the list of
   /// active servers.
   ...
   /// \param[in] serverId Which server to operate on. You can call ListServers() to get the list of
   /// active servers. \param[in] ipAddress Which IP address. You can call ListServerAddresses() to
   /// get the list of addresses for the specified server
   ...
   /// \param[in] serverId Which server to operate on. You can call ListServers() to get the list of
   /// active servers. \param[in] rebootType Should be either "HARD" or "SOFT"
   ...
   /// \param[in] serverId Which server to operate on. You can call ListServers() to get the list of
   /// active servers. \param[in] imageId Which image (harddrive contents, including OS) to use
   void RebuildServer(RakNet::RakString serverId, RakNet::RakString imageId);

   /// \brief Changes the hardware configuration of a server. This does not take effect until you
   /// call ConfirmResizedServer() \sa
   /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf \sa
   /// RackspaceEventCallback::OnResizeServerResult() \sa RevertResizedServer() \param[in] serverId
   /// Which server to operate on. You can call ListServers() to get the list of active servers.
   /// \param[in] flavorId Which flavor (hardware config) to use, primarily how much memory is
   /// available.
   ...
   /// \brief List all images (software configs, including operating systems), which includes images
   /// you create yourself \sa
   /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf \sa
   /// RackspaceEventCallback::OnListImagesResult() \sa CreateImage()
   void ListImages(void);

   /// \brief Images a running server. This essentially copies the harddrive, and lets you start a
   /// server with the same harddrive contents later \sa
   /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf \sa
   /// RackspaceEventCallback::OnCreateImageResult() \sa ListImages() \param[in] serverId Which
   /// server to operate on. You can call ListServers() to get the list of active servers.
   ...
   /// finish executing The callbacks are called in the order they are added
   void AddEventCallback(Rackspace2EventCallback *callback);
   /// \brief Removes a callback from the list of callbacks to be called when any of the above
   /// functions finish executing The callbacks are called in the order they are added
   ...
   /// Call this when TCPInterface returns something other than UNASSIGNED_SYSTEM_ADDRESS from
   /// HasLostConnection()
   ...
   /// \brief Mostly for internal use, but you can use it to execute an operation with more complex
   /// xml if desired See the Rackspace.cpp on how to use it
   void AddOperation(RackspaceOperationType type, RakNet::RakString httpCommand, RakNet::RakString operation,
