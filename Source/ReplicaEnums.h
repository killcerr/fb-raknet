...
/// \file
/// \brief Contains enumerations used by the ReplicaManager system.  This file is a lightweight
/// header, so you can include it without worrying about linking in lots of other crap
///
...
enum ReplicaReturnResult {
   ...
   /// This means cancel the processing - don't send any network messages and don't change the
   /// current state.
   ...
   /// Only returned from the Replica::SendConstruction interface, means act as if the other system
   /// had this object but don't actually Send a construction packet.  This way you will still send
   /// scope and serialize packets to that system
