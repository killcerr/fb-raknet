...
/// \file
/// \brief A simple TCP based server allowing sends and receives.  Can be connected by any TCP
/// client, including telnet.
///
...
class RAK_DLL_EXPORT TCPInterface {
  public:
   ...
   /// \param[in] threadPriority Passed to the thread creation routine. Use THREAD_PRIORITY_NORMAL
   /// for Windows. For Linux based systems, you MUST pass something reasonable based on the thread
   /// priorities for your application. \param[in] socketFamily IP version: For IPV4, use AF_INET
   /// (default). For IPV6, use AF_INET6. To autoselect, use AF_UNSPEC.
   bool Start(unsigned short port, unsigned short maxIncomingConnections, unsigned short maxConnections = 0,
              int _threadPriority = -99999, unsigned short socketFamily = AF_INET, const char *bindAddress = 0);
   ...
   SystemAddress Connect(const char *host, unsigned short remotePort, bool block = true,
                         unsigned short socketFamily = AF_INET, const char *bindAddress = 0);
  ...
  protected:
   ...
   bool CreateListenSocket_WinStore8(unsigned short port, unsigned short maxIncomingConnections,
                                     unsigned short socketFamily, const char *hostAddress);
#else
