...
/// \file
/// \brief If _USE_RAK_MEMORY_OVERRIDE is defined, memory allocations go through rakMalloc,
/// rakRealloc, and rakFree
///
...
RAK_DLL_EXPORT Type *OP_NEW_4(const char *file, unsigned int line, const P1 &p1, const P2 &p2, const P3 &p3,
                              const P4 &p4) {
#if _USE_RAK_MEMORY_OVERRIDE == 1
   char *buffer = (char *)(GetMalloc_Ex())(sizeof(Type), file, line);
...
// Call to make RakNet allocate a large block of memory, and do all subsequent allocations in that
// memory block Initial and reallocations will be done through whatever function is pointed to by
// yourMMapFunction, and yourDirectMMapFunction (default is malloc) Allocations will be freed
// through whatever function is pointed to by yourMUnmapFunction (default free)
void UseRaknetFixedHeap(size_t initialCapacity, void *(*yourMMapFunction)(size_t size) = RakNet::_DLMallocMMap,
