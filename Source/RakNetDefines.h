...
// If you want to change these defines, put them in RakNetDefinesOverrides so your changes are not
// lost when updating RakNet The user should not edit this file
#include "RakNetDefinesOverrides.h"

/// Define __GET_TIME_64BIT to have RakNet::TimeMS use a 64, rather than 32 bit value.  A 32 bit
/// value will overflow after about 5 weeks. However, this doubles the bandwidth use for sending
/// times, so don't do it unless you have a reason to. Comment out if you are using the iPod Touch
/// TG. See http://www.jenkinssoftware.com/forum/index.php?topic=2717.0 This must be the same on all
/// systems, or they won't connect
#ifndef __GET_TIME_64BIT
#define __GET_TIME_64BIT 1
#endif

// Define _FILE_AND_LINE_ to "",0 if you want to strip out file and line info for memory tracking
// from the EXE
#ifndef _FILE_AND_LINE_
#define _FILE_AND_LINE_ __FILE__, __LINE__
#endif

/// Define __BITSTREAM_NATIVE_END to NOT support endian swapping in the BitStream class.  This is
/// faster and is what you should use unless you actually plan to have different endianness systems
/// connect to each other Enabled by default.
...
/// Note that OpenSSL carries its own license restrictions that you should be aware of. If you don't
/// agree, don't enable this define This also requires that you enable header search paths to
/// DependentExtensions\openssl-1.0.0d
// #define OPEN_SSL_CLIENT_SUPPORT
#ifndef OPEN_SSL_CLIENT_SUPPORT
#define OPEN_SSL_CLIENT_SUPPORT 0
#endif

/// Threshold at which to do a malloc / free rather than pushing data onto a fixed stack for the
/// bitstream class Arbitrary size, just picking something likely to be larger than most packets
...
/// This many datagrams are tracked by datagramNumber. If more than this many datagrams are sent,
/// then an ack for an older datagram would be ignored This results in an unnecessary resend in that
/// case
...
/// This will be evident because RakNetStatistics::messagesInSend buffer will increase over time,
/// yet at the same time the outgoing bandwidth per second is less than your connection supports
...
/// If two calls between RakNet::GetTime() happen farther apart than this time in microseconds, this
/// delta will be returned instead Note: This will cause ID_TIMESTAMP to be temporarily inaccurate
/// if you set a breakpoint that pauses the UpdateNetworkLoop() thread in RakPeer Define in
/// RakNetDefinesOverrides.h to enable (non-zero) or disable (0)
...
// This results in large messages not taking up time to reassembly with memcpy, but is vulnerable to
// attackers causing the host to run out of memory
...
// Controls how many allocations occur at once for the memory pool of incoming datagrams waiting to
// be transferred between the recvfrom thread and the main update thread Has large effect on memory
// usage, per instance of RakPeer. Approximately MAXIMUM_MTU_SIZE*BUFFERED_PACKETS_PAGE_SIZE bytes,
// once after calling RakPeer::Startup()
...
// datagrams. Has small effect on memory usage per connection. Uses about 256
// bytes*INTERNAL_PACKET_PAGE_SIZE per connection
#ifndef INTERNAL_PACKET_PAGE_SIZE
#define INTERNAL_PACKET_PAGE_SIZE 8
#endif

// If defined to 1, the user is responsible for calling RakPeer::RunUpdateCycle and
// RakPeer::RunRecvfrom
