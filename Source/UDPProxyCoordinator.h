...
/// \file
/// \brief Essentially maintains a list of servers running UDPProxyServer, and some state management
/// for UDPProxyClient to find a free server to forward datagrams
///
...
namespace RakNet {
/// When NAT Punchthrough fails, it is possible to use a non-NAT system to forward messages from us
/// to the recipient, and vice-versa The class to forward messages is UDPForwarder, and it is
/// triggered over the network via the UDPProxyServer plugin. The UDPProxyClient connects to
/// UDPProxyCoordinator to get a list of servers running UDPProxyServer, and the coordinator will
/// relay our forwarding request \brief Middleman between UDPProxyServer and UDPProxyClient,
/// maintaining a list of UDPProxyServer, and managing state for clients to find an available
/// forwarding server. \ingroup NAT_PUNCHTHROUGH_GROUP
class RAK_DLL_EXPORT UDPProxyCoordinator : public PluginInterface2 {
  public:
   ...
   /// For UDPProxyServers logging in remotely, they must pass a password to
   /// UDPProxyServer::LoginToCoordinator(). It must match the password set here. If no password is
   /// set, they cannot login remotely. By default, no password is set
   ...
   struct ForwardingRequest {
      ...
      SystemAddress requestingAddress;  // Which system originally sent the network message to start
                                        // forwarding
  ...
  protected:
   ...
   void SendAllBusy(SystemAddress senderClientAddress, SystemAddress targetClientAddress, RakNetGUID targetClientGuid,
                    SystemAddress requestingAddress);
   void Clear(void);

   void SendForwardingRequest(SystemAddress sourceAddress, SystemAddress targetAddress, SystemAddress serverAddress,
                              RakNet::TimeMS timeoutOnNoDataMS);
   ...
   // DataStructures::Multilist<ML_ORDERED_LIST, ForwardingRequest*, SenderAndTargetAddress>
