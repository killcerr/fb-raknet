...
HuffmanEncodingTree::~HuffmanEncodingTree() { FreeMemory(); }
...
void HuffmanEncodingTree::FreeMemory(void) {
   ...
   while (nodeQueue.Size() > 0) {
      ...
      if (node->left) nodeQueue.Push(node->left, _FILE_AND_LINE_);

      if (node->right) nodeQueue.Push(node->right, _FILE_AND_LINE_);

      RakNet::OP_DELETE(node, _FILE_AND_LINE_);
   }

   // Delete the encoding table
   for (int i = 0; i < 256; i++) rakFree_Ex(encodingTable[i].encoding, _FILE_AND_LINE_);
...
void HuffmanEncodingTree::GenerateFromFrequencyTable(unsigned int frequencyTable[256]) {
   ...
   for (counter = 0; counter < 256; counter++) {
      ...
      if (node->weight == 0) node->weight = 1;  // 0 weights are illegal

      leafList[counter] = node;  // Used later to generate the encryption table

      InsertNodeIntoSortedList(node,
                               &huffmanEncodingTreeNodeList);  // Insert and maintain sort order.
   }

   // 2.  While there is more than one tree, take the two smallest trees and merge them so that the
   // two trees are the left and right children of a new node, where the new node has the weight the
   // sum of the weight of the left and right child nodes.
#ifdef _MSC_VER
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
   ...
   while (1) {
      ...
      // Put the new node back into the list at the correct spot to maintain the sort.  Linear
      // search time
   ...
   // Generate the encryption table. From before, we have an array of pointers to all the leaves
   // which contain pointers to their parents. This can be done more efficiently but this isn't bad
   // and it's way easier to program and debug
   ...
   for (counter = 0; counter < 256; counter++) {
      ...
      do {
         if (currentNode->parent->left == currentNode)  // We're storing the paths in reverse order.since we are going
      ...
      // Write to the bitstream in the reverse order that we stored the path, which gives us the
      // correct order from the root to the leaf
      while (tempPathLength-- > 0) {
         if (tempPath[tempPathLength])  // Write 1's and 0's because writing a bool will write the
                                        // BitStream TYPE_CHECKING validation bits if that is
                                        // defined along with the actual data bit, which is not what
                                        // we want
            bitStream.Write1();
         else
            bitStream.Write0();
...
void HuffmanEncodingTree::EncodeArray(unsigned char *input, size_t sizeInBytes, RakNet::BitStream *output) {
   ...
   // For each input byte, Write out the corresponding series of 1's and 0's that give the encoded
   // representation
   for (counter = 0; counter < sizeInBytes; counter++) {
      output->WriteBits(encodingTable[input[counter]].encoding, encodingTable[input[counter]].bitLength,
   ...
   if (output->GetNumberOfBitsUsed() % 8 != 0) {
      // Find an input that is longer than the remaining bits.  Write out part of it to pad the
      // output to be byte aligned.
...
#ifdef _DEBUG
      RakAssert(counter != 256);  // Given 256 elements, we should always be able to find an input
                                  // that would be >= 7 bits

#endif
   }
}

unsigned HuffmanEncodingTree::DecodeArray(RakNet::BitStream *input, BitSize_t sizeInBits, size_t maxCharsToWrite,
   ...
   // For each bit, go left if it is a 0 and right if it is a 1.  When we reach a leaf, that gives
   // us the desired value and we restart from the root
   ...
   for (unsigned counter = 0; counter < sizeInBits; counter++) {
      ...
      {
         if (outputWriteIndex < maxCharsToWrite) output[outputWriteIndex] = currentNode->value;
   ...
   for (unsigned counter = 0; counter < sizeInBits; counter++) {
      ...
      if (currentNode->left == 0 && currentNode->right == 0)  // Leaf
      {
         output->WriteBits(&(currentNode->value), sizeof(char) * 8,
                           true);  // Use WriteBits instead of Write(char) because we want to avoid TYPE_CHECKING
         currentNode = root;
      }
   }
}
