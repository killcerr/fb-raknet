 ...
 *
 * Use the AVL balanced tree if you want the tree to be balanced after every deletion and addition.
 * This avoids the potential worst case scenario where ordered input to a binary search tree gives
 * linear search time results.  It's not needed if input will be evenly distributed, in which case
 * the search time is O (log n).  The search time for the AVL balanced binary tree is O (log n)
 * irregardless of input.
 *
 * Has the following member functions
 * unsigned int Height(<index>) - Returns the height of the tree at the optional specified starting
 * index.  Default is the root add(element) - adds an element to the BinarySearchTree bool
 * del(element) - deletes the node containing element if the element is in the tree as defined by a
 * comparison with the == operator.  Returns true on success, false if the element is not found bool
 * IsInelement) - returns true if element is in the tree as defined by a comparison with the ==
 * operator.  Otherwise returns false DisplayInorder(array) - Fills an array with an inorder search
 * of the elements in the tree.  USER IS REPONSIBLE FOR ALLOCATING THE ARRAY!.
 * DisplayPreorder(array) - Fills an array with an preorder search of the elements in the tree. USER
 * IS REPONSIBLE FOR ALLOCATING THE ARRAY!. DisplayPostorder(array) - Fills an array with an
 * postorder search of the elements in the tree. USER IS REPONSIBLE FOR ALLOCATING THE ARRAY!.
 * DisplayBreadthFirstSearch(array) - Fills an array with a breadth first search of the elements in
 * the tree.  USER IS REPONSIBLE FOR ALLOCATING THE ARRAY!. clear - Destroys the tree.  Same as
 * calling the destructor unsigned int Height() - Returns the height of the tree unsigned int size()
 * - returns the size of the BinarySearchTree GetPointerToNode(element) - returns a pointer to the
 * comparision element in the tree, allowing for direct modification when necessary with complex
 * data types. Be warned, it is possible to corrupt the tree if the element used for comparisons is
 * modified.  Returns NULL if the item is not found
 *
 *
 * EXAMPLE
 * @code
 ...
 * array[2]; // returns 15
 * @endcode
 * compress - reallocates memory to fit the number of elements.  Best used when the number of
 * elements decreases
 *
...
template <class BinarySearchTreeType>
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::BalanceTree(
    typename BinarySearchTree<BinarySearchTreeType>::node *current, bool rotateOnce) {
...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::Add(const BinarySearchTreeType &input) {
   typename BinarySearchTree<BinarySearchTreeType>::node *current =
       BinarySearchTree<BinarySearchTreeType>::Add(input, _FILE_AND_LINE_);
...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::Del(const BinarySearchTreeType &input) {
   typename BinarySearchTree<BinarySearchTreeType>::node *current =
       BinarySearchTree<BinarySearchTreeType>::Del(input, _FILE_AND_LINE_);
   BalanceTree(current, false);
}

template <class BinarySearchTreeType>
bool AVLBalancedBinarySearchTree<BinarySearchTreeType>::RightHigher(
...
template <class BinarySearchTreeType>
bool AVLBalancedBinarySearchTree<BinarySearchTreeType>::LeftHigher(
    typename BinarySearchTree<BinarySearchTreeType>::node *A) {
   if (A == 0) return false;
...
template <class BinarySearchTreeType>
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::RotateRight(
    typename BinarySearchTree<BinarySearchTreeType>::node *C) {
...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::DoubleRotateRight(
    typename BinarySearchTree<BinarySearchTreeType>::node *A) {
   // The left side of the left child must be higher for the tree to balance with a right rotation.
   // If it isn't, rotate it left before the normal rotation so it is.
   RotateLeft(A->left->right);
   RotateRight(A->left);
}

template <class BinarySearchTreeType>
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::RotateLeft(
    typename BinarySearchTree<BinarySearchTreeType>::node *C) {
   typename BinarySearchTree<BinarySearchTreeType>::node *A, *B, *D;
   /*
     RIGHT ROTATION
...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::DoubleRotateLeft(
    typename BinarySearchTree<BinarySearchTreeType>::node *A) {
   // The left side of the right child must be higher for the tree to balance with a left rotation.
   // If it isn't, rotate it right before the normal rotation so it is.
...
unsigned int BinarySearchTree<BinarySearchTreeType>::HeightRecursive(typename BinarySearchTree::node *current) {
   ...
   if ((current->left == 0) && (current->right == 0)) return 1;  // Leaf

   if (current->left != 0) left_height = 1 + HeightRecursive(current->left);

   if (current->right != 0) right_height = 1 + HeightRecursive(current->right);
...
BinarySearchTreeType *&BinarySearchTree<BinarySearchTreeType>::GetPointerToNode(const BinarySearchTreeType &element) {
   ...
   if (this->direction == this->NOT_FOUND) return dummyptr;

   return tempnode->item;
}

template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node *&BinarySearchTree<BinarySearchTreeType>::Find(
    const BinarySearchTreeType &element, typename BinarySearchTree<BinarySearchTreeType>::node **parent) {
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node *&BinarySearchTree<BinarySearchTreeType>::FindParent(
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node *BinarySearchTree<BinarySearchTreeType>::Del(
   ...
   if (BinarySearchTree_size == 0) return 0;
   ...
   if ((current->right) == 0 && (current->left) == 0)  // Leaf node, just remove it
   {
      ...
      return parent;
   } else if ((current->right) != 0 && (current->left) == 0)  // Node has only one child, delete it and cause the parent
                                                              // to point to that child
   {
      if (parent) {
         if (direction == RIGHT)
   ...
   } else  // Go right, then as left as far as you can
   {
      parent = current;
      direction = RIGHT;
      current = current->right;  // Must have a right branch because the if statements above
                                 // indicated that it has 2 branches
      ...
      // Replace the value held by the node to RakNet::OP_DELETE(with the value pointed to by
      // current, _FILE_AND_LINE_);
...
template <class BinarySearchTreeType>
typename BinarySearchTree<BinarySearchTreeType>::node *BinarySearchTree<BinarySearchTreeType>::Add(
    const BinarySearchTreeType &input, const char *file, unsigned int line) {
...
#pragma warning(disable : 4127)  // warning C4127: conditional expression is constant
      ...
      while (true)  // This loop traverses the tree to find a spot for insertion
      {
         if (input < *(current->item)) {
            ...
            return 0;  // ((input == current->item) == true) which is not allowed since the tree
                       // only takes discrete values.  Do nothing
      }
   }
}
...
void BinarySearchTree<BinarySearchTreeType>::DisplayInorder(BinarySearchTreeType *return_array) {
   ...
   else if (BinarySearchTree_size == 1) {
   ...
   while (index != BinarySearchTree_size) {
      // direction is set by the find function and holds the direction of the parent to the last
      // node visited.  It is used to prevent revisiting nodes
...
template <class BinarySearchTreeType>
void BinarySearchTree<BinarySearchTreeType>::DisplayPostorderRecursive(typename BinarySearchTree::node *current,
                                                                       BinarySearchTreeType *return_array,
                                                                       unsigned int &index) {
   if (current->left != 0) DisplayPostorderRecursive(current->left, return_array, index);

   if (current->right != 0) DisplayPostorderRecursive(current->right, return_array, index);
...
BinarySearchTree<BinarySearchTreeType>::BinarySearchTree(const BinarySearchTree &original_copy) {
   ...
   // This is a copy of the constructor.  A bug in Visual C++ made it so if I just put the
   // constructor call here the variable assignments were ignored.
   BinarySearchTree_size = 0;
   root = 0;
...
template <class BinarySearchTreeType>
BinarySearchTree<BinarySearchTreeType> &BinarySearchTree<BinarySearchTreeType>::operator=(
