...
#elif defined(_WIN32)
// IP_DONTFRAGMENT is different between winsock 1 and winsock 2.  Therefore, Winsock2.h must be
// linked againt Ws2_32.lib winsock.h must be linked against WSock32.lib.  If these two are mixed up
// the flag won't work correctly
#include <winsock2.h>
...
void CommandParserInterface::ParseConsoleString(char *str, const char delineator, unsigned char delineatorToggle,
   ...
   for (strIndex = 0; strIndex < strLen; strIndex++) {
      if (str[strIndex] == delineator && replaceDelineator) str[strIndex] = 0;

      if (str[strIndex] == '\n' || str[strIndex] == '\r') str[strIndex] = 0;
   ...
   for (strIndex = 0, parameterListIndex = 0; strIndex < strLen;) {
      if (str[strIndex] != 0) {
         ...
         if (parameterListIndex >= parameterListLength) break;

         strIndex++;
         while (str[strIndex] != 0 && strIndex < strLen) strIndex++;
      } else
         strIndex++;
   }
...
void CommandParserInterface::SendCommandList(TransportInterface *transport, const SystemAddress &systemAddress) {
   ...
   if (commandList.Size()) {
      for (i = 0; i < commandList.Size(); i++) {
         ...
         if (i < commandList.Size() - 1) transport->Send(systemAddress, ", ");
      }
...
void CommandParserInterface::RegisterCommand(unsigned char parameterCount, const char *command,
...
bool CommandParserInterface::GetRegisteredCommand(const char *command, RegisteredCommand *rc) {
   ...
   if (objectExists) *rc = commandList[index];
   return objectExists;
}

void CommandParserInterface::OnTransportChange(TransportInterface *transport) { (void)transport; }

void CommandParserInterface::OnNewIncomingConnection(const SystemAddress &systemAddress,
...
void CommandParserInterface::ReturnResult(bool res, const char *command, TransportInterface *transport,
...
void CommandParserInterface::ReturnResult(int res, const char *command, TransportInterface *transport,
                                          const SystemAddress &systemAddress) {
   transport->Send(systemAddress, "%s returned %i.\r\n", command, res);
}

void CommandParserInterface::ReturnResult(const char *command, TransportInterface *transport,
                                          const SystemAddress &systemAddress) {
   transport->Send(systemAddress, "Successfully called %s.\r\n", command);
}

void CommandParserInterface::ReturnResult(char *res, const char *command, TransportInterface *transport,
                                          const SystemAddress &systemAddress) {
   transport->Send(systemAddress, "%s returned %s.\r\n", command, res);
}

void CommandParserInterface::ReturnResult(SystemAddress res, const char *command, TransportInterface *transport,
                                          const SystemAddress &systemAddress) {
   char addr[128];
