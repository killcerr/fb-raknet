...
/// CC_RAKNET_UDT_PACKET_HISTORY_LENGTH should be a power of 2 for the writeIndex variables to wrap
/// properly
...
/// <LI>Each datagram containing a user message has a sequence number which is set after calling
/// OnSendBytes(). Set it by calling GetAndIncrementNextDatagramSequenceNumber() <LI>System is
/// designed to be used from a single thread. <LI>Each packet should have a timeout time based on
/// GetSenderRTOForACK(). If this time elapses, add the packet to the head of the send list for
/// retransmission.
/// </OL>
///
/// Recommended:
/// <OL>
/// <LI>Call sendto in its own thread. This takes a significant amount of time in high speed
/// networks.
/// </OL>
///
/// Algorithm:
...
/// <LI>On a periodic interval (SYN time is the best) call Update(). Also call ShouldSendACKs(), and
/// send buffered ACKS if it returns true. <LI>Call OnSendAck() when sending acks. <LI>When you want
/// to send or resend data, call GetNumberOfBytesToSend(). It will return you enough bytes to keep
/// you busy for \a estimatedTimeToNextTick. You can send more than this to fill out a datagram, or
/// to send packet pairs <LI>Call OnSendBytes() when sending datagrams. <LI>When data arrives,
/// record the sequence number and buffer an ACK for it, to be sent from Update() if
/// ShouldSendACKs() returns true <LI>Every 16 packets that you send, send two of them back to back
/// (a packet pair) as long as both packets are the same size. If you don't have two packets the
/// same size, it is fine to defer this until you do. <LI>When you get a packet, call OnGotPacket().
/// If the packet is also either of a packet pair, call OnGotPacketPair() <LI>If you get a packet,
/// and the sequence number is not 1 + the last sequence number, send a NAK. On the remote system,
/// call OnNAK() and resend that message. <LI>If you get an ACK, remove that message from
/// retransmission. Call OnNonDuplicateAck(). <LI>If a message is not ACKed for
/// GetRTOForRetransmission(), resend it.
/// </OL>
class CCRakNetUDT {
  public:
   CCRakNetUDT();
   ...
   int GetRetransmissionBandwidth(CCTimeType curTime, CCTimeType timeSinceLastTick, uint32_t unacknowledgedBytes,
                                  bool isContinuousSend);
   int GetTransmissionBandwidth(CCTimeType curTime, CCTimeType timeSinceLastTick, uint32_t unacknowledgedBytes,
                                bool isContinuousSend);

   /// Acks do not have to be sent immediately. Instead, they can be buffered up such that groups of
   /// acks are sent at a time This reduces overall bandwidth usage How long they can be buffered
   /// depends on the retransmit time of the sender Should call once per update tick, and send if
   /// needed
   ...
   bool OnGotPacket(DatagramSequenceNumberType datagramSequenceNumber, bool isContinuousSend, CCTimeType curTime,
   ...
   void OnAck(CCTimeType curTime, CCTimeType rtt, bool hasBAndAS, BytesPerMicrosecond _B, BytesPerMicrosecond _AS,
              double totalUserDataBytesAcked, bool isContinuousSend, DatagramSequenceNumberType sequenceNumber);
   ...
   /// Be sure to call OnSendAckGetBAndAS() before calling OnSendAck(), since whether you write it
   /// or not affects \a numBytes
   ...
   /// If the time difference between when a message was last transmitted, and the current time is
   /// greater than RTO then packet is eligible for retransmission, pending congestion control RTO =
   /// (RTT + 4 * RTTVar) + SYN If we have been continuously sending for the last RTO, and no ACK or
   /// NAK at all, SND*=2; This is per message, which is different from UDT, but RakNet supports
   /// packetloss with continuing data where UDT is only RELIABLE_ORDERED Minimum value is 100
   /// milliseconds
  ...
  protected:
   ...
   /// When we do an update process on the SYN interval, nextSYNUpdate is set to the next time we
   /// should update Normally this is nextSYNUpdate+=SYN, in order to update on a consistent
   /// schedule However, if this would result in an immediate update yet again, it is set to SYN
   /// microseconds past the current time (in case the thread did not update for a long time)
   ...
   /// Sent to the sender by the receiver from packetPairRecieptHistory whenever a back to back
   /// packet arrives on the receiver Updated by B = B * .875 + incomingB * .125
   ...
   /// Used to calculate packet arrival rate (in UDT) but data arrival rate (in RakNet, where not
   /// all datagrams are the same size) Filter is used to cull lowest half of values for
   /// bytesPerMicrosecond, to discount spikes and inactivity Referred to in the documentation as
   /// AS, data arrival rate AS is sent to the sender and calculated every 10th ack Each node
   /// represents (curTime-lastPacketArrivalTime)/bytes Used with
   ...
   /// Tracks the time the last packet that arrived, so BytesPerMicrosecond can be calculated for
   /// packetArrivalHistory when a new packet arrives
   ...
   /// When the receiver last calculated and send B and AS, from packetArrivalHistory and
   /// packetPairRecieptHistory Used to prevent it from being calculated and send too frequently, as
   ...
   /// How many times we have decremented SND this congestion period. Used to limit the number of
   /// decrements to 5
   uint32_t DecCount;
   ...
   /// If a packet is marked as a packet pair, lastPacketPairPacketArrivalTime is set to the time it
   /// arrives This is used so when the 2nd packet of the pair arrives, we can calculate the time
   /// interval between the two
   CCTimeType lastPacketPairPacketArrivalTime;

   /// If a packet is marked as a packet pair, lastPacketPairSequenceNumber is checked to see if the
   /// last packet we got was the packet immediately before the one that arrived If so, we can use
   /// lastPacketPairPacketArrivalTime to get the time between the two packets, and thus estimate
   /// the link capacity Initialized to -1, so the first packet of a packet pair won't be treated as
   /// the second
   ...
   // CC_RAKNET_UDT_PACKET_HISTORY_LENGTH times Otherwise, the default values in the array generate
   // inaccuracy
   ...
   // --------------------------- PROTECTED METHODS ---------------------------
   /// Update nextSYNUpdate by SYN, or the same amount past the current time if no updates have
   ...
   static BytesPerMicrosecond CalculateListMedianRecursive(
       const BytesPerMicrosecond inputList[CC_RAKNET_UDT_PACKET_HISTORY_LENGTH], int inputListLength, int lessThanSum,
       int greaterThanSum);
   ...
   /// shouldn't be delayed for this reason
   ...
   void UpdateWindowSizeAndAckOnAckPerSyn(CCTimeType curTime, CCTimeType rtt, bool isContinuousSend,
                                          DatagramSequenceNumberType sequenceNumber);

   /// Sets halveSNDOnNoDataTime to the future, and also resets ExpCount, which is used to multiple
   /// the RTO on no data arriving at all
