...
/// \details The table data structure is useful if you want to store a set of structures and perform
/// queries on those structures.<BR> This is a relatively simple and fast implementation of the
/// types of tables commonly used in databases.<BR> See TableSerializer to serialize data members of
/// the table.<BR> See LightweightDatabaseClient and LightweightDatabaseServer to transmit the table
/// over the network.
class RAK_DLL_EXPORT Table {
  public:
   ...
   /// \details New rows are added with empty values for all cells.  However, if you specify
   /// initialCelLValues you can specify initial values It's up to you to ensure that the values in
   /// the specific cells match the type of data used by that row rowId can be considered the
   /// primary key for the row.  It is much faster to lookup a row by its rowId than by searching
   /// keys. rowId must be unique Rows are stored in sorted order in the table, using rowId as the
   /// sort key \param[in] rowId The UNIQUE primary key for the row.  This can never be changed.
   ...
   /// \note If you are going to update many cells of a particular row, it is more efficient to call
   /// GetRow and perform the operations on the row directly. \note Row pointers do not change, so
   /// you can also write directly to the rows for more efficiency.
   /// \param[in] rowId The ID of the row
   ...
   /// \brief Note this is much less efficient to call than GetRow, then working with the cells
   /// directly. Numeric, string, binary
   ...
   /// \brief Gets a row.  More efficient to do this and access Row::cells than to repeatedly call
   /// GetCell. You can also update cells in rows from this function. \param[in] rowId The ID of the
   /// row \return The desired row, or 0 if no such row.
   ...
   /// \param[in] columnSubset An array of column indices.  Only columns in this array are returned.
   /// Pass 0 for all columns \param[in] numColumnSubset The number of elements in \a columnSubset
   /// \param[in] inclusionFilters An array of FilterQuery.  All filters must pass for the row to be
   /// returned. \param[in] numInclusionFilters The number of elements in \a inclusionFilters
   /// \param[in] rowIds An arrow of row IDs.  Only these rows with these IDs are returned.  Pass 0
   /// for all rows. \param[in] numRowIDs The number of elements in \a rowIds \param[out] result The
   /// result of the query.  If no rows are returned, the table will only have columns.
   ...
   /// If a row cell on column n has the same value as a a different row on column n, then the row
   /// will be compared on column n+1 \param[in] sortQueries A list of SortQuery structures,
   /// defining the sorts to perform on the table \param[in] numColumnSubset The number of elements
   /// in \a numSortQueries \param[out] out The address of an array of Rows, which will receive the
   /// sorted output.  The array must be long enough to contain all returned rows, up to
   /// GetRowCount()
   ...
   /// \param[in] outLength The size of the \a out array
   /// \param[in] columnDelineator What character to print to delineate columns
   /// \param[in] printDelineatorForBinary Binary output is not printed.  True to still print the
   /// delineator. \param[in] inputRow The row to print
   void PrintRow(char *out, int outLength, char columnDelineator, bool printDelineatorForBinary,
  ...
  protected:
   ...
   // 16 is arbitrary and is the order of the BPlus tree.  Higher orders are better for searching
   // while lower orders are better for Insertions and deletions.
