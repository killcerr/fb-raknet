...
class RAK_DLL_EXPORT RakString {
  public:
   ...
   // Lets you modify the string. Do not make the string longer - however, you can make it shorter,
   // or change the contents. Pointer is only valid in the scope of RakString itself
   ...
   /// Searches the string for the content specified in stringToFind and returns the position of the
   /// first occurrence in the string. Search only includes characters on or after position pos,
   /// ignoring any possible occurrences in previous locations. \param[in] stringToFind The string
   /// to find inside of this object's string \param[in] pos The position in the string to start the
   /// search \return Returns the position of the first occurrence in the string.
   ...
   /// Does the given IP address match the IP address encoded into this string, accounting for
   /// wildcards?
   ...
   /// https://servers.api.rackspacecloud.com/v1.0 to https://,  servers.api.rackspacecloud.com,
   /// /v1.0
   ...
   static RakNet::RakString FormatForPOST(const char *uri, const char *contentType, const char *body,
                                          const char *extraHeaders = "");
   static RakNet::RakString FormatForPUT(const char *uri, const char *contentType, const char *body,
                                         const char *extraHeaders = "");
   ...
   /// masterserver2.raknet.com/testServer?__gameId=comprehensivePCGame&__rowId=1 \return Formatted
   /// string
   ...
   /// \param[in] writeLanguageId encode the languageId variable in the stream. If false, 0 is
   /// assumed, and DeserializeCompressed will not look for this variable in the stream (saves
   /// bandwidth) \pre StringCompressor::AddReference must have been called to instantiate the class
   /// (Happens automatically from RakPeer::Startup())
   ...
   static void SerializeCompressed(const char *str, BitStream *bs, uint8_t languageId = 0,
   ...
   /// \param[in] bs Bitstream to serialize from
   /// \param[in] readLanguageId If true, looks for the variable langaugeId in the data stream. Must
   /// match what was passed to SerializeCompressed \return true if the deserialization was
   /// successful \pre StringCompressor::AddReference must have been called to instantiate the class
   /// (Happens automatically from RakPeer::Startup())
  ...
  protected:
   static RakNet::RakString FormatForPUTOrPost(const char *type, const char *uri, const char *contentType,
...
inline void ReplaceStdString(std::string &subject, const std::string &search, const std::string &replace) {
   size_t pos = 0;
