...
/// \details Given two systems, each of whom known a common password, verify the password without
/// transmitting it This can be used to determine what permissions are should be allowed to the
/// other system
///
...
// How often to check for ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT, and the minimum
// timeout time. Maximum is double this value.
...
/// \brief Implements two way authentication
/// \details Given two systems, each of whom known a common password / identifier pair, verify the
/// password without transmitting it This can be used to determine what permissions are should be
/// allowed to the other system If the other system should not send any data until authentication
/// passes, you can use the MessageFilter plugin for this. Call MessageFilter::SetAllowMessageID()
/// including ID_TWO_WAY_AUTHENTICATION_NEGOTIATION when doing so. Also attach MessageFilter first
/// in the list of plugins \note If other systems challenges us, and fails, you will get
/// ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILED. \ingroup PLUGINS_GROUP
class RAK_DLL_EXPORT TwoWayAuthentication : public PluginInterface2 {
  public:
   ...
   /// \a identifier is transmitted in plaintext with the request. It is only needed because the
   /// system supports multiple password. It is used to only hash against once password on the
   /// remote system, rather than having to hash against every known password. \param[in] identifier
   /// A unique identifier representing this password. This is transmitted in plaintext and should
   /// be considered insecure \param[in] password The password to add \return True on success, false
   /// on identifier==password, either identifier or password is blank, or identifier is already in
   /// use
   ...
   /// ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT, or
   /// ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILED ID_TWO_WAY_AUTHENTICATION_SUCCESS will be
   /// returned if and only if the other system has called AddPassword() with the same
   /// identifier\password pair as this system. \param[in] identifier A unique identifier
   /// representing this password. This is transmitted in plaintext and should be considered
   /// insecure \return True on success, false on remote system not connected, or identifier not
   /// previously added with AddPassword()
   ...
   struct RAK_DLL_EXPORT NonceGenerator {
      NonceGenerator();
      ~NonceGenerator();
      void GetNonce(char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH], unsigned short *requestId,
                    RakNet::AddressOrGUID remoteSystem);
      void GenerateNonce(char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH]);
      bool GetNonceById(char nonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH], unsigned short requestId,
  ...
  protected:
   ...
   void Hash(char thierNonce[TWO_WAY_AUTHENTICATION_NONCE_LENGTH], RakNet::RakString password,
