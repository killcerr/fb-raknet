...
int FCM2ParticipantComp(FullyConnectedMesh2::FCM2Participant *const &key,
                        FullyConnectedMesh2::FCM2Participant *const &data) {
   if (key->fcm2Guid < data->fcm2Guid) return -1;
   if (key->fcm2Guid > data->fcm2Guid) return 1;
   return 0;
}
...
void FullyConnectedMesh2::SetAutoparticipateConnections(bool b) { autoParticipateConnections = b; }
...
void FullyConnectedMesh2::ResetHostCalculation(void) {
   ...
   totalConnectionCount = 0;
   ourFCMGuid = 0;
   for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) SendFCMGuidRequest(fcm2ParticipantList[i]->rakNetGuid);
}
...
bool FullyConnectedMesh2::AddParticipantInternal(RakNetGUID rakNetGuid, FCM2Guid theirFCMGuid) {
   for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) {
      if (fcm2ParticipantList[i]->rakNetGuid == rakNetGuid) {
         if (theirFCMGuid != 0) fcm2ParticipantList[i]->fcm2Guid = theirFCMGuid;
         /*
...
bool FullyConnectedMesh2::HasParticipant(RakNetGUID participantGuid) {
   unsigned int i;
   for (i = 0; i < fcm2ParticipantList.Size(); i++) {
      if (fcm2ParticipantList[i]->rakNetGuid == participantGuid) return true;
   }
   return false;
}
/*
...
PluginReceiveResult FullyConnectedMesh2::OnReceive(Packet *packet) {
   switch (packet->data[0]) {
      case ID_REMOTE_NEW_INCOMING_CONNECTION: {
         if (connectOnNewRemoteConnections) ConnectToRemoteNewIncomingConnections(packet);
      } break;
      ...
      case ID_FCM2_VERIFIED_JOIN_ACCEPTED:
         if (packet->wasGeneratedLocally == false) OnVerifiedJoinAccepted(packet);
         return RR_CONTINUE_PROCESSING;
...
void FullyConnectedMesh2::OnAttach(void) {
   ...
   if (rakPeerInterface->IsActive()) startupTime = RakNet::GetTimeUS();
}
...
void FullyConnectedMesh2::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void FullyConnectedMesh2::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID, bool isIncoming) {
   ...
   if (autoParticipateConnections) AddParticipant(rakNetGUID);
}

void FullyConnectedMesh2::OnFailedConnectionAttempt(Packet *packet,
...
void FullyConnectedMesh2::SendFCMGuidRequest(RakNetGUID rakNetGuid) {
   if (rakNetGuid == rakPeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS)) return;
...
void FullyConnectedMesh2::OnInformFCMGuid(Packet *packet) {
   ...
   if (AddParticipantInternal(packet->guid, theirFCMGuid)) {
      // 1/19/2010 - Relay increased total connection count in case new participant only connects to
      // part of the mesh
      unsigned int idx;
      RakNet::BitStream bsOut;
      ...
      for (idx = 0; idx < fcm2ParticipantList.Size(); idx++) {
         if (packet->guid != fcm2ParticipantList[idx]->rakNetGuid)
            rakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, fcm2ParticipantList[idx]->rakNetGuid,
                                   false);
      }
   }
...
unsigned int FullyConnectedMesh2::GetParticipantCount(void) const { return fcm2ParticipantList.Size(); }
...
bool FullyConnectedMesh2::ParticipantListComplete(void) {
   for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++) {
      if (fcm2ParticipantList[i]->fcm2Guid == 0) return false;
   }
   return true;
}
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(Packet *packet) {
   ...
   for (unsigned int i = 0; i < count; i++) {
      ...
      rakPeerInterface->Connect(str, remoteAddress.GetPort(), connectionPassword.C_String(),
                                (int)connectionPassword.GetLength());
   }
}

unsigned int FullyConnectedMesh2::GetTotalConnectionCount(void) const { return totalConnectionCount; }

void FullyConnectedMesh2::StartVerifiedJoin(RakNetGUID client) {
   // Assert is because there is no point calling StartVerifiedJoin() if this client is already a
   // participant
   ...
   for (i = 0; i < fcm2ParticipantList.Size(); i++) {
      ...
      // &fcm2ParticipantList[i]->userContext );
      WriteVJSUserData(&vjsOut, fcm2ParticipantList[i]->rakNetGuid);
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet *packet, bool accept, BitStream *additionalData) {
   ...
   CategorizeVJIP(&vjip, participatingMembersOnClientSucceeded, participatingMembersOnClientFailed,
                  participatingMembersNotOnClient, clientMembersNotParticipatingSucceeded,
      ...
      for (i = 0; i < participatingMembersNotOnClient.Size(); i++) {
         ...
         for (unsigned int j = 0; j < fcm2ParticipantList.Size(); j++) {
            if (fcm2ParticipantList[j]->rakNetGuid == participatingMembersNotOnClient[i]) {
               ...
               // &fcm2ParticipantList[j]->userContext );
               WriteVJSUserData(&vjsOut, fcm2ParticipantList[j]->rakNetGuid);
               bsOut.Write(vjsOut.GetNumberOfBitsUsed());
               bsOut.Write(&vjsOut);
               bsOut.AlignWriteToByteBoundary();
   ...
   if (accept) {
      ...
      if (additionalData) bsOut.Write(additionalData);

      for (unsigned int i = 0; i < fcm2ParticipantList.Size(); i++)
         SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, fcm2ParticipantList[i]->rakNetGuid, false);

      // Process immediately
      // This is so if another ID_FCM2_VERIFIED_JOIN_CAPABLE is buffered, it responds with
...
void FullyConnectedMesh2::GetVerifiedJoinRequiredProcessingList(RakNetGUID host,
...
void FullyConnectedMesh2::GetVerifiedJoinAcceptedAdditionalData(Packet *packet, bool *thisSystemAccepted,
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet *packet) {
   ...
   if (curIndex != (unsigned int)-1) {
      ...
      for (unsigned int i = 0; i < vjip->vjipMembers.Size(); i++) {
         if (vjip->vjipMembers[i].workingFlag == false) vjip->vjipMembers[i].joinInProgressState = JIPS_UNNECESSARY;
      }
   ...
   // 11/13/2013 - ReadVerifiedJoinInProgressMember may set JIPS_CONNECTED, so this may already be
   // done http://www.jenkinssoftware.com/forum/index.php?topic=5211.0
   if (ProcessVerifiedJoinInProgressIfCompleted(vjip)) {
      // Completed
      return RR_STOP_PROCESSING_AND_DEALLOCATE;
   }
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinCapable(Packet *packet) {
   ...
   // If this assert hits, AddParticipant() was called on this system, or another system, which it
...
void FullyConnectedMesh2::OnVerifiedJoinFailed(RakNetGUID hostGuid, bool callCloseConnection) {
   ...
   if (curIndex == (unsigned int)-1) return;
   ...
   if (callCloseConnection) {
      ...
      for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
         ...
         if (vjip->vjipMembers[j].userData != 0) RakNet::OP_DELETE(vjip->vjipMembers[j].userData, _FILE_AND_LINE_);
      }
   }
...
void FullyConnectedMesh2::OnVerifiedJoinAccepted(Packet *packet) {
   ...
   if (systemToAddGuid == rakPeerInterface->GetMyGUID()) {
      ...
      if (cs == IS_CONNECTED) AddParticipant(systemToAddGuid);
   }
}

void FullyConnectedMesh2::OnVerifiedJoinRejected(Packet *packet) { OnVerifiedJoinFailed(packet->guid, true); }

unsigned int FullyConnectedMesh2::GetJoinsInProgressIndex(RakNetGUID requester) const {
   for (unsigned int i = 0; i < joinsInProgress.Size(); i++) {
      if (joinsInProgress[i]->requester == requester) return i;
   }
   return (unsigned int)-1;
}

void FullyConnectedMesh2::UpdateVerifiedJoinInProgressMember(const AddressOrGUID systemIdentifier,
                                                             RakNetGUID guidToAssign,
                                                             FullyConnectedMesh2::JoinInProgressState newState) {
...
bool FullyConnectedMesh2::ProcessVerifiedJoinInProgressIfCompleted(VerifiedJoinInProgress *vjip) {
   ...
   if (anyProcessing == true) return false;
...
void FullyConnectedMesh2::ReadVerifiedJoinInProgressMember(RakNet::BitStream *bsIn,
                                                           VerifiedJoinInProgressMember *vjipm) {
...
unsigned int FullyConnectedMesh2::GetVerifiedJoinInProgressMemberIndex(const AddressOrGUID systemIdentifier,
                                                                       VerifiedJoinInProgress *vjip) {
   for (unsigned int j = 0; j < vjip->vjipMembers.Size(); j++) {
      if ((systemIdentifier.rakNetGuid != UNASSIGNED_RAKNET_GUID &&
           vjip->vjipMembers[j].guid == systemIdentifier.rakNetGuid) ||
          (systemIdentifier.systemAddress != UNASSIGNED_SYSTEM_ADDRESS &&
           vjip->vjipMembers[j].systemAddress == systemIdentifier.systemAddress)) {
         return j;
      }
   }
   return (unsigned int)-1;
...
void FullyConnectedMesh2::CategorizeVJIP(VerifiedJoinInProgress *vjip,
                                         ...
                                         DataStructures::List<RakNetGUID> &clientMembersNotParticipatingSucceeded,
                                         DataStructures::List<RakNetGUID> &clientMembersNotParticipatingFailed) {
   for (unsigned int i = 0; i < vjip->vjipMembers.Size(); i++) vjip->vjipMembers[i].workingFlag = false;
