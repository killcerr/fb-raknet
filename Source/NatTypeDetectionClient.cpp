...
NatTypeDetectionClient::NatTypeDetectionClient() { c2 = 0; }
...
void NatTypeDetectionClient::DetectNATType(SystemAddress _serverAddress) {
   ...
   if (c2->IsBerkleySocket()) ((RNS2_Berkley *)c2)->CreateRecvPollingThread(0);
#endif
...
void NatTypeDetectionClient::OnCompletion(NATTypeDetectionResult result) {
   ...
   // Symmetric and port restricted are determined by server, so no need to notify server we are
   // done
...
bool NatTypeDetectionClient::IsInProgress(void) const { return serverAddress != UNASSIGNED_SYSTEM_ADDRESS; }
...
void NatTypeDetectionClient::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
                                                PI2_LostConnectionReason lostConnectionReason) {
   (void)lostConnectionReason;
   (void)rakNetGUID;

   if (IsInProgress() && systemAddress == serverAddress) Shutdown();
}

void NatTypeDetectionClient::OnRakPeerShutdown(void) { Shutdown(); }

void NatTypeDetectionClient::OnDetach(void) { Shutdown(); }
...
void NatTypeDetectionClient::Shutdown(void) {
   ...
   if (c2 != 0) {
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
      if (c2->IsBerkleySocket()) ((RNS2_Berkley *)c2)->BlockOnStopRecvPollingThread();
#endif
