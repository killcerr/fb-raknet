...
/// \internal
/// \brief Ordered Channel Heap .  This is a heap where you add to it on multiple ordered channels,
/// with each channel having a different weight.
///
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
void OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::Push(
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
void OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::PushAtHead(
    const unsigned index, const channel_key_type &channelID, const heap_data_type &data) {
   ...
   if (queueAndWeight->randResultQueue.Size() == 0) {
      // Set maxRange to the greatest random number waiting to be returned, rather than 1.0
      // necessarily This is so weights are scaled similarly among channels.  For example, if the
      // head weight for a used channel was .25 and then we added another channel, the new channel
      // would need to choose between .25 and 0 If we chose between 1.0 and 0, it would be 1/.25
      // (4x) more likely to be at the head of the heap than it should be
      ...
      if (index == 0) {
         maxRange = GreatestRandResult();
         if (maxRange == queueAndWeight->randResultQueue[0]) maxRange = 1.0;
      } else if (index >= queueAndWeight->randResultQueue.Size())
         maxRange = queueAndWeight->randResultQueue[queueAndWeight->randResultQueue.Size() - 1] * .99999999;
      else
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
heap_data_type OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::Pop(
   ...
   if (startingIndex != 0) {
      // Ugly - have to count in the heap how many nodes have the same channel, so we know where to
      // delete from in the queue
      ...
      // TODO - ordered channel heap uses progressively lower values as items are inserted.  But
      // this won't give relative ordering among channels.  I have to renormalize after every pop.
      queueAndWeight->randResultQueue.Pop();
   }

   // Try to remove the channel after every pop, because doing so is not valid while there are
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
heap_data_type OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::Peek(
    const unsigned startingIndex) const {
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
void OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::AddChannel(
    const channel_key_type &channelID, const double weight) {
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
void OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::RemoveChannel(
    channel_key_type channelID) {
   if (map.Has(channelID)) {
      ...
      if (map[i]->randResultQueue.Size() == 0) {
         ...
         // Signal this channel for deletion later, because the heap has nodes with this channel
         // right now
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
heap_data_type &OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::operator[](
...
template <class channel_key_type, class heap_data_type,
          int (*channel_key_comparison_func)(const channel_key_type &, const channel_key_type &)>
unsigned OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::ChannelSize(
...
void OrderedChannelHeap<channel_key_type, heap_data_type, channel_key_comparison_func>::Clear(void) {
   unsigned i;
   for (i = 0; i < map.Size(); i++) RakNet::OP_DELETE(map[i], _FILE_AND_LINE_);
