...
ListenerContext::~ListenerContext() {
   ...
   //  - explicit: by using delete operator (the listener is closed even if there are outstanding
   //  references to it).
...
void ListenerContext::OnMessage(Windows::Networking::Sockets::DatagramSocket ^ socket,
   ...
   for (unsigned int i = 0; i < uselessBuffer->Length; i++) recvFromStruct->data[i] = managedBytes[i];
...
void RNS2_WindowsStore8::GetMyIP(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS]) {
...
RNS2SendResult RNS2_WindowsStore8::Send(RNS2_SendParameters *sendParameters, const char *file, unsigned int line) {
   ...
   task<unsigned int>(outputStreamAndDataWriter->dataWriter->StoreAsync()).then([&](task<unsigned int> writeTask) {
      try {
         ...
         //   rootPage->NotifyUser("Send failed with error: " + exception->Message,
...
void RNS2_WindowsStore8::DomainNameToIP(const char *domainName, char ip[65]) {
   ...
   IAsyncOperation<Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^> ^ op =
       DatagramSocket::GetEndpointPairsAsync(hostName, L"0");
   ...
   op->Completed = ref new AsyncOperationCompletedHandler<
       ...
       [&](IAsyncOperation<Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^> ^
               result,
           AsyncStatus status) {
          Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair ^> ^ result2 =
              result->GetResults();
          if (result2->Size > 0) {
   ...
   while (!completed) RakSleep(0);
   /*
   op->Completed = ref new AsyncOperationCompletedHandler<
   Windows::Foundation::Collections::IVectorView<EndpointPair^>^ >(
           [](IAsyncOperation< Windows::Foundation::Collections::IVectorView<EndpointPair^>^ >^
   operation)
        {
...
int RNS2_WindowsStore8::WinRTInet_Addr(const char *str) {
   ...
   for (partsIndex = 0; partsIndex < 4; partsIndex++) parts[partsIndex] = 0;
...
int RNS2_WindowsStore8::WinRTSetSockOpt(Windows::Networking::Sockets::DatagramSocket ^ s, int level, int optname,
                                        const char *optval, socklen_t optlen) {
   RakAssert("WinRTSetSockOpt Unsupported" && 0);
   return 0;
}

int RNS2_WindowsStore8::WinRTIOCTLSocket(Windows::Networking::Sockets::DatagramSocket ^ s, long cmd,
                                         unsigned long *argp) {
   RakAssert("WinRTIOCTLSocket Unsupported" && 0);
   return 0;
}

int RNS2_WindowsStore8::WinRTGetSockName(Windows::Networking::Sockets::DatagramSocket ^ s, struct sockaddr *name,
                                         socklen_t *namelen) {
   RakAssert("WinRTGetSockName Unsupported" && 0);
   return 0;
}
