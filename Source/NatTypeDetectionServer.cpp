...
NatTypeDetectionServer::NatTypeDetectionServer() { s1p2 = s2p3 = s3p4 = s4p5 = 0; }
...
void NatTypeDetectionServer::Startup(const char *nonRakNetIP2, const char *nonRakNetIP3, const char *nonRakNetIP4
   ...
   if (s3p4->IsBerkleySocket()) ((RNS2_Berkley *)s3p4)->CreateRecvPollingThread(0);
#endif
}
...
void NatTypeDetectionServer::Shutdown() {
   ...
   if (s3p4 != 0) {
#if !defined(__native_client__) && !defined(WINDOWS_STORE_RT)
      if (s3p4->IsBerkleySocket()) ((RNS2_Berkley *)s3p4)->BlockOnStopRecvPollingThread();
#endif
   ...
   while (bufferedPackets.Size()) RakNet::OP_DELETE(bufferedPackets.Pop(), _FILE_AND_LINE_);
...
void NatTypeDetectionServer::Update(void) {
   ...
   while (recvStruct) {
      ...
      if (data[0] == NAT_TYPE_PORT_RESTRICTED && recvStruct->socket == s3p4) {
         ...
         if (readSuccess) {
            ...
            //	RakAssert("Didn't read GUID in Update in NatTypeDetectionServer.cpp. Message format
            // error" && 0);
         }
      }
   ...
   // Only socket that receives messages is s3p4, to see if the external address is different than
   that of the connection to rakPeerInterface char data[ MAXIMUM_MTU_SIZE ]; int len; SystemAddress
   senderAddr; len=NatTypeRecvFrom(data, s3p4, senderAddr);
   // Client is asking us if this is port restricted. Only client requests of this type come in on
   s3p4 while (len>0 && data[0]==NAT_TYPE_PORT_RESTRICTED)
   {
      ...
      if (time > natDetectionAttempts[i].nextStateTime) {
         ...
         switch (natDetectionAttempts[i].detectionState) {
...
#endif
               // S4P5 sends to C2. If arrived, no NAT. Done. (Else S4P5 potentially banned, do not
               // use again).
            ...
            case STATE_TESTING_FULL_CONE_2:
               ...
               // S2P3 sends to C1 (Different address, different port, to previously used port on
               // client). If received, Full-cone nat. Done.  (Else S2P3 potentially banned, do not
               // use again).
               saOut = natDetectionAttempts[i].systemAddress;
            ...
            case STATE_TESTING_ADDRESS_RESTRICTED_2:
               ...
               // S1P2 sends to C1 (Same address, different port, to previously used port on
               // client). If received, address-restricted cone nat. Done.
               saOut = natDetectionAttempts[i].systemAddress;
               saOut.SetPortHostOrder(natDetectionAttempts[i].systemAddress.GetPort());
            ...
            case STATE_TESTING_PORT_RESTRICTED_2:
               // C1 sends to S3P4. If address of C1 as seen by S3P4 is the same as the address of
               // C1 as seen by S1P1, then port-restricted cone nat. Done
...
#ifdef NTDS_VERBOSE
               printf(
                   "Warning, exceeded final check STATE_TESTING_PORT_RESTRICTED_2.\nExpected that "
                   "client "
                   "would have sent NAT_TYPE_PORT_RESTRICTED on s3p4.\nDefaulting to Symmetric\n");
#endif
               bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_RESULT);
               bs.Write((unsigned char)NAT_TYPE_SYMMETRIC);
               rakPeerInterface->Send(&bs, HIGH_PRIORITY, RELIABLE, 0, natDetectionAttempts[i].systemAddress, false);
...
void NatTypeDetectionServer::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void NatTypeDetectionServer::OnDetectionRequest(Packet *packet) {
   ...
   bsIn.Read(isRequest);
   if (isRequest) {
      if (i != (unsigned int)-1) return;  // Already in progress
      ...
      if (i == (unsigned int)-1) return;  // Unknown
      // They are done
      natDetectionAttempts.RemoveAtIndexFast(i);
   }
}
...
unsigned int NatTypeDetectionServer::GetDetectionAttemptIndex(const SystemAddress &sa) {
   for (unsigned int i = 0; i < natDetectionAttempts.Size(); i++) {
      if (natDetectionAttempts[i].systemAddress == sa) return i;
   }
...
unsigned int NatTypeDetectionServer::GetDetectionAttemptIndex(RakNetGUID guid) {
   for (unsigned int i = 0; i < natDetectionAttempts.Size(); i++) {
      if (natDetectionAttempts[i].guid == guid) return i;
   }
   return (unsigned int)-1;
}
