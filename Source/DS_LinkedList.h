...
// Prototype to prevent error in CircularLinkedList class when a reference is made to a LinkedList
// class
...
class CircularLinkedList {
  public:
   ...
   // {CircularLinkedList(original_copy);}  // Converts linked list to circular type
...
inline void CircularLinkedList<CircularLinkedListType>::End(void) {
   if (this->root) this->position = this->root->previous;
}
...
bool LinkedList<LinkedListType>::operator=(const LinkedList<LinkedListType> &original_copy) {
   ...
   if ((&original_copy) != this) {
      ...
      if (original_copy.list_size == 0) {
         ...
         // Complete the circle.  Set the next pointer of the newest node to the root and the
         // previous pointer of the root to the newest node
...
LinkedList<LinkedListType>::LinkedList(const LinkedList &original_copy) {
   ...
   if (original_copy.list_size == 0) {
      ...
      // Complete the circle.  Set the next pointer of the newest node to the root and the previous
      // pointer of the root to the newest node
      this->position->next = this->root;
...
void CircularLinkedList<CircularLinkedListType>::Insert(const CircularLinkedListType &input) {
   ...
   if (list_size == 0) {
      ...
      this->root = this->position;  // Since we're inserting into a 1 element list the old root is
                                    // now the second item
      this->list_size = 2;
   }
      ...
      // Since the root pointer is bound to a node rather than an index this moves it back if you
      // insert an element at the root
...
template <class CircularLinkedListType>
typename CircularLinkedList<CircularLinkedListType>::node *CircularLinkedList<CircularLinkedListType>::FindPointer(
    const CircularLinkedListType &input) {
   node *current;

   if (this->list_size == 0) return 0;

   current = this->root;

   // Search for the item starting from the root node and incrementing the pointer after every check
   // If you wind up pointing at the root again you looped around the list so didn't find the item,
...
template <class CircularLinkedListType>
inline void CircularLinkedList<CircularLinkedListType>::Concatenate(
    const CircularLinkedList<CircularLinkedListType> &L) {
   ...
   for (counter = 0; counter < L.list_size; counter++) {
      ...
      // Update pointers.  Moving ptr keeps the current pointer at the end of the list since the add
      // function does not move the pointer
      ptr = ptr->next;
...
template <class CircularLinkedListType>
CircularLinkedList<CircularLinkedListType> CircularLinkedList<CircularLinkedListType>::Mergesort(
...
template <class CircularLinkedListType>
CircularLinkedList<CircularLinkedListType> CircularLinkedList<CircularLinkedListType>::Merge(CircularLinkedList L1,
...
LinkedList<LinkedListType> &LinkedList<LinkedListType>::operator++() {
   if ((this->list_size != 0) && (this->position->next != this->root)) this->position = this->position->next;
...
template <class LinkedListType>
LinkedList<LinkedListType> &LinkedList<LinkedListType>::operator--() {
   if ((this->list_size != 0) && (this->position != this->root)) this->position = this->position->previous;
