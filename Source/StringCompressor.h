...
/// \file
/// \brief \b Compresses/Decompresses ASCII strings and writes/reads them to BitStream class
/// instances.  You can use this to easily serialize and deserialize your own strings.
///
...
/// You can call GenerateTreeFromStrings to compress and decompress other languages efficiently as
/// well.
class RAK_DLL_EXPORT StringCompressor {
  public:
   ...
   /// \param[in] maxCharsToWrite The max number of bytes to write of \a input.  Use 0 to mean no
   /// limit. \param[out] output The bitstream to write the compressed string to \param[in]
   /// languageID Which language to use
   ...
   /// be truncated. \param[in] input The bitstream containing the compressed string \param[in]
   /// languageID Which language to use
   ...
   void EncodeString(const RakNet::RakString *input, int maxCharsToWrite, RakNet::BitStream *output,
