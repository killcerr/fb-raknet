...
int StatisticsHistory::TrackedObjectComp(const uint64_t &key, StatisticsHistory::TrackedObject *const &data) {
   if (key < data->trackedObjectData.objectId) return -1;
   if (key == data->trackedObjectData.objectId) return 0;
   return 1;
}

int TimeAndValueQueueCompAsc(StatisticsHistory::TimeAndValueQueue *const &key,
                             StatisticsHistory::TimeAndValueQueue *const &data) {
   if (key->sortValue < data->sortValue) return -1;
   if (key->sortValue > data->sortValue) return 1;
   if (key->key < data->key) return -1;
   if (key->key > data->key) return 1;
   return 0;
}
...
StatisticsHistory::~StatisticsHistory() { Clear(); }
...
bool StatisticsHistory::RemoveObject(uint64_t objectId, void **userData) {
   ...
   if (userData) *userData = objects[idx]->trackedObjectData.userData;
...
StatisticsHistory::TrackedObjectData *StatisticsHistory::GetObjectAtIndex(unsigned int index) const {
   return &objects[index]->trackedObjectData;
}
...
void StatisticsHistory::AddValueByIndex(unsigned int index, RakString key, SHValueType val, Time curTime,
   ...
   queue->longTermSum += tav.val;
   queue->longTermCount = queue->longTermCount + 1;
   if (queue->longTermLowest > tav.val) queue->longTermLowest = tav.val;
   if (queue->longTermHighest < tav.val) queue->longTermHighest = tav.val;
}

StatisticsHistory::SHErrorCode StatisticsHistory::GetHistoryForKey(uint64_t objectId, RakString key,
                                                                   StatisticsHistory::TimeAndValueQueue **values,
                                                                   Time curTime) const {
   if (values == 0) return SH_INVALID_PARAMETER;

   unsigned int idx = GetObjectIndex(objectId);
   if (idx == (unsigned int)-1) return SH_UKNOWN_OBJECT;
   TrackedObject *to = objects[idx];
   DataStructures::HashIndex hi = to->dataQueues.GetIndexOf(key);
   if (hi.IsInvalid()) return SH_UKNOWN_KEY;
   *values = to->dataQueues.ItemAtIndex(hi);
   (*values)->CullExpiredValues(curTime);
   return SH_OK;
}

bool StatisticsHistory::GetHistorySorted(uint64_t objectId, SHSortOperation sortType,
   ...
   for (unsigned int i = 0; i < itemList.Size(); i++) {
      ...
      else if (sortType == SH_SORT_BY_RECENT_SUM_OF_SQUARES_ASCENDING ||
      ...
      else if (sortType == SH_SORT_BY_LONG_TERM_HIGHEST_ASCENDING ||
      ...
      if (sortType == SH_SORT_BY_RECENT_SUM_ASCENDING || sortType == SH_SORT_BY_LONG_TERM_SUM_ASCENDING ||
          sortType == SH_SORT_BY_RECENT_SUM_OF_SQUARES_ASCENDING || sortType == SH_SORT_BY_RECENT_AVERAGE_ASCENDING ||
          sortType == SH_SORT_BY_LONG_TERM_AVERAGE_ASCENDING || sortType == SH_SORT_BY_RECENT_HIGHEST_ASCENDING ||
          sortType == SH_SORT_BY_RECENT_LOWEST_ASCENDING || sortType == SH_SORT_BY_LONG_TERM_HIGHEST_ASCENDING ||
   ...
   for (unsigned int i = 0; i < sortedQueues.Size(); i++) values.Push(sortedQueues[i], _FILE_AND_LINE_);
   return true;
}

void StatisticsHistory::MergeAllObjectsOnKey(RakString key, TimeAndValueQueue *tavqOutput,
...
void StatisticsHistory::GetUniqueKeyList(DataStructures::List<RakString> &keys) {
   ...
   for (unsigned int idx = 0; idx < objects.Size(); idx++) {
      ...
      for (unsigned int k = 0; k < keyList.Size(); k++) {
         ...
         if (hasKey == false) keys.Push(keyList[k], _FILE_AND_LINE_);
      }
   }
}

StatisticsHistory::TimeAndValueQueue::TimeAndValueQueue() { Clear(); }

StatisticsHistory::TimeAndValueQueue::~TimeAndValueQueue() {}

void StatisticsHistory::TimeAndValueQueue::SetTimeToTrackValues(Time t) { timeToTrackValues = t; }
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetRecentLowest(void) const {
   ...
   for (unsigned int idx = 0; idx < values.Size(); idx++) {
      if (values[idx].val < out) out = values[idx].val;
   }
   return out;
}
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetSumSinceTime(Time t) const {
   ...
   for (int i = values.Size(); i > 0; --i) {
      if (values[i - 1].time >= t) sum += values[i - 1].val;
   }
   return sum;
}

void StatisticsHistory::TimeAndValueQueue::MergeSets(const TimeAndValueQueue *lhs, SHDataCategory lhsDataCategory,
   ...
   if (lhsDataCategory == StatisticsHistory::DC_DISCRETE && rhsDataCategory == StatisticsHistory::DC_DISCRETE) {
      ...
      while (rhsIndex < rhs->values.Size() && lhsIndex < lhs->values.Size()) {
         if (rhs->values[rhsIndex].time < lhs->values[lhsIndex].time) {
            ...
            newTimeAndValue.val =
                rhs->values[rhsIndex].val + lastTimeAndValueLhs.val + lastSlopeLhs * timeSinceOppositeValue;
            newTimeAndValue.time = rhs->values[rhsIndex].time;
            lastTimeAndValueRhs = rhs->values[rhsIndex];
            if (rhsIndex > 0 && rhs->values[rhsIndex].time != rhs->values[rhsIndex - 1].time &&
                rhsDataCategory == StatisticsHistory::DC_CONTINUOUS)
               lastSlopeRhs = (rhs->values[rhsIndex].val - rhs->values[rhsIndex - 1].val) /
                              (SHValueType)(rhs->values[rhsIndex].time - rhs->values[rhsIndex - 1].time);
            rhsIndex++;
            ...
            newTimeAndValue.val =
                lhs->values[lhsIndex].val + lastTimeAndValueRhs.val + lastSlopeRhs * timeSinceOppositeValue;
            newTimeAndValue.time = lhs->values[lhsIndex].time;
            lastTimeAndValueLhs = lhs->values[lhsIndex];
            if (lhsIndex > 0 && lhs->values[lhsIndex].time != lhs->values[lhsIndex - 1].time &&
                lhsDataCategory == StatisticsHistory::DC_CONTINUOUS)
               lastSlopeLhs = (lhs->values[lhsIndex].val - lhs->values[lhsIndex - 1].val) /
                              (SHValueType)(lhs->values[lhsIndex].time - lhs->values[lhsIndex - 1].time);
            lhsIndex++;
         } else {
...
void StatisticsHistory::TimeAndValueQueue::ResizeSampleSet(
    int maxSamples, DataStructures::Queue<StatisticsHistory::TimeAndValue> &histogram, SHDataCategory dataCategory,
    Time timeClipStart, Time timeClipEnd) {
   ...
   while (timeBoundary <= endTime) {
      ...
      if (dataCategory == DC_CONTINUOUS) {
         if (dataIndex > 0 && dataIndex < values.Size() && values[dataIndex - 1].time < timeBoundary &&
...
SHValueType StatisticsHistory::TimeAndValueQueue::Interpolate(StatisticsHistory::TimeAndValue t1,
                                                              StatisticsHistory::TimeAndValue t2, Time time) {
   if (t2.time == t1.time) return (t1.val + t2.val) / 2;
   //	if (t2.time > t1.time)
   //	{
...
StatisticsHistory::TrackedObject::~TrackedObject() {
   ...
   DataStructures::List<RakString> keyList;
   for (unsigned int idx = 0; idx < itemList.Size(); idx++) RakNet::OP_DELETE(itemList[idx], _FILE_AND_LINE_);
}
...
void StatisticsHistoryPlugin::SetTrackConnections(bool _addNewConnections, int _newConnectionsObjectType,
...
void StatisticsHistoryPlugin::Update(void) {
   ...
   for (unsigned int idx = 0; idx < guids.Size(); idx++) {
      ...
      if (objectIndex != (unsigned int)-1) {
         statistics.AddValueByIndex(objectIndex, "RN_ACTUAL_BYTES_SENT",
                                    (SHValueType)stats[idx].valueOverLastSecond[ACTUAL_BYTES_SENT], curTime, false);

         statistics.AddValueByIndex(objectIndex, "RN_USER_MESSAGE_BYTES_RESENT",
                                    (SHValueType)stats[idx].valueOverLastSecond[USER_MESSAGE_BYTES_RESENT], curTime,
                                    false);

         statistics.AddValueByIndex(objectIndex, "RN_ACTUAL_BYTES_RECEIVED",
                                    (SHValueType)stats[idx].valueOverLastSecond[ACTUAL_BYTES_RECEIVED], curTime, false);

         statistics.AddValueByIndex(objectIndex, "RN_USER_MESSAGE_BYTES_PUSHED",
                                    (SHValueType)stats[idx].valueOverLastSecond[USER_MESSAGE_BYTES_PUSHED], curTime,
                                    false);

         statistics.AddValueByIndex(objectIndex, "RN_USER_MESSAGE_BYTES_RECEIVED_PROCESSED",
                                    (SHValueType)stats[idx].valueOverLastSecond[USER_MESSAGE_BYTES_RECEIVED_PROCESSED],
                                    curTime, false);

         statistics.AddValueByIndex(objectIndex, "RN_lastPing", (SHValueType)rakPeerInterface->GetLastPing(guids[idx]),
                                    curTime, false);

         statistics.AddValueByIndex(objectIndex, "RN_bytesInResendBuffer", (SHValueType)stats[idx].bytesInResendBuffer,
                                    curTime, false);

         statistics.AddValueByIndex(objectIndex, "RN_packetlossLastSecond",
                                    (SHValueType)stats[idx].packetlossLastSecond, curTime, false);
      }
   }
...
/*
void StatisticsHistoryPlugin::OnDirectSocketSend(const char *data, const BitSize_t bitsUsed,
SystemAddress remoteSystemAddress)
{
        // Would have to use GetGuidFromSystemAddress for every send
}

void StatisticsHistoryPlugin::OnDirectSocketReceive(const char *data, const BitSize_t bitsUsed,
SystemAddress remoteSystemAddress)
{
}
*/
void StatisticsHistoryPlugin::OnClosedConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
...
void StatisticsHistoryPlugin::OnNewConnection(const SystemAddress &systemAddress, RakNetGUID rakNetGUID,
                                              bool isIncoming) {
   (void)systemAddress;
   (void)isIncoming;
