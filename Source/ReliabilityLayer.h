...
/// \file
/// \brief \b [Internal] Datagram reliable, ordered, unordered and sequenced sends.  Flow control.
...
class SortedSplittedPackets {
  ...
  public:
   ...
   unsigned int AllocSize() { return allocation_size; }
   unsigned int AddedPacketsCount() { return addedPacketsCount; }
...
#else
   // This is here for progress notifications, since progress notifications return the first packet
   // data, if available
...
/// Datagram reliable, ordered, unordered and sequenced sends.  Flow control.  Message splitting,
...
class ReliabilityLayer  //<ReliabilityLayer>
{
  public:
   ...
   /// deliver a reliable packet Default time is 10,000 or 10 seconds in release and 30,000 or 30
   /// seconds in debug. \param[in] time Time, in MS
   ...
   /// Packets are read directly from the socket layer and skip the reliability layer because
   /// unconnected players do not use the reliability layer This function takes packet data after a
   /// player has been confirmed as connected. \param[in] buffer The socket data \param[in] length
   /// The length of the socket data \param[in] systemAddress The player that this data is from
   ...
   bool HandleSocketReceiveFromConnectedPlayer(const char *buffer, unsigned int length, SystemAddress &systemAddress,
                                               DataStructures::List<PluginInterface2 *> &messageHandlerList,
                                               int MTUSize, RakNetSocket2 *s, RakNetRandom *rnr, CCTimeType timeRead,
   ...
   /// \param[in] orderingChannel 0 to 31.  Specifies what channel to use, for relational ordering
   /// and sequencing of packets. \param[in] makeDataCopy If true \a data will be copied. Otherwise,
   /// only a pointer will be stored. \param[in] MTUSize maximum datagram size \param[in]
   /// currentTime Current time, as per RakNet::GetTimeMS() \param[in] receipt This number will be
   /// returned back with ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS and is only returned with the
   /// reliability types that contain RECEIPT in the name \return True or false for success or
   /// failure.
   bool Send(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability,
             unsigned char orderingChannel, bool makeDataCopy, int MTUSize, CCTimeType currentTime, uint32_t receipt);
   ...
   /// \param[in] maxBitsPerSecond if non-zero, enforces that outgoing bandwidth does not exceed
   /// this amount \param[in] messageHandlerList A list of registered plugins
  ...
  private:
   ...
   void SendBitStream(RakNetSocket2 *s, SystemAddress &systemAddress, RakNet::BitStream *bitStream, RakNetRandom *rnr,
   ...
   /// Parse an internalPacket and figure out how many header bits would be written.  Returns that
   /// number
   ...
   /// Search the specified list for sequenced packets on the specified ordering channel, optionally
   /// skipping those with splitPacketId, and delete them
   ...
   /// Split the passed packet into chunks under MTU_SIZE bytes (including headers) and save those
   /// new chunks
   ...
   /// Take all split chunks with the specified splitPacketId and try to reconstruct a packet. If we
   /// can, allocate and return it.  Otherwise return 0
   InternalPacket *BuildPacketFromSplitPacketList(SplitPacketIdType splitPacketId, CCTimeType time, RakNetSocket2 *s,
   ...
   /// Creates a copy of the specified internal packet with data copied from the original starting
   /// at dataByteOffset for dataByteLength bytes. Does not copy any split data parameters as that
   ...
   // DataStructures::LinkedList<InternalPacket*> *GetOrderingListAtOrderingStream( unsigned char
   // orderingChannel );
   ...
   void InsertPacketIntoResendList(InternalPacket *internalPacket, CCTimeType time, bool firstResend,
   ...
   struct DatagramHistoryNode {
      DatagramHistoryNode() {}
      DatagramHistoryNode(MessageNumberNode *_head, CCTimeType ts) : head(_head), timeSent(ts) {}
   ...
   // datagramHistory holds a linked list of MessageNumberNode. Each MessageNumberNode refers to one
   // element in resendList which can be cleared on an ack.
   ...
   struct UnreliableWithAckReceiptNode {
      ...
      UnreliableWithAckReceiptNode(DatagramSequenceNumberType _datagramNumber, uint32_t _sendReceiptSerial,
   ...
   MessageNumberNode *AddFirstToDatagramHistory(DatagramSequenceNumberType datagramNumber,
                                                DatagramSequenceNumberType messageNumber, CCTimeType timeSent);
   MessageNumberNode *AddSubsequentToDatagramHistory(MessageNumberNode *messageNumberNode,
                                                     DatagramSequenceNumberType messageNumber);
   DatagramSequenceNumberType datagramHistoryPopCount;

   DataStructures::MemoryPool<InternalPacket> internalPacketPool;
   // DataStructures::BPlusTree<DatagramSequenceNumberType, InternalPacket*, RESEND_TREE_ORDER>
   // resendTree;
   ...
   // If we backoff due to packetloss, don't remeasure until all waiting resends have gone out or
   // else we overcount
   ...
   // 1. Each ordered message transmits OrderingIndexType orderedWriteIndex. There are
   // NUMBER_OF_ORDERED_STREAMS independent values of these. The value
   //    starts at 0. Every time an ordered message is sent, the value increments by 1
   // 2. Each sequenced message contains the current value of orderedWriteIndex for that channel,
   // and additionally OrderingIndexType sequencedWriteIndex.
   //    sequencedWriteIndex resets to 0 every time orderedWriteIndex increments. It increments by 1
   //    every time a sequenced message is sent.
   // 3. The receiver maintains the next expected value for the orderedWriteIndex, stored in
   // orderedReadIndex.
   // 4. As messages arrive:
   //    If a message has the current ordering index, and is sequenced, and is < the current highest
   //    sequence value, discard If a message has the current ordering index, and is sequenced, and
   //    is >= the current highest sequence value, return immediately If a message has a greater
   //    ordering index, and is sequenced or ordered, buffer it If a message has the current
   //    ordering index, and is ordered, buffer, then push off messages from buffer
   // 5. Pushing off messages from buffer:
   //    Messages in buffer are put in a minheap. The value of each node is calculated such that
   //    messages are returned: A. (lowest ordering index, lowest sequence index) B. (lowest
   //    ordering index, no sequence index) Messages are pushed off until the heap is empty, or the
   //    next message to be returned does not preserve the ordered index For an empty heap, the heap
   //    weight should start at the lowest value based on the next expected ordering index, to avoid
   //    variable overflow

   // Sender increments this by 1 for every ordered message sent
   OrderingIndexType orderedWriteIndex[NUMBER_OF_ORDERED_STREAMS];
   // Sender increments by 1 for every sequenced message sent. Resets to 0 when an ordered message
   // is sent
   ...
   // Highest value received for sequencedWriteIndex for the current value of orderedReadIndex on
   // the same channel.
   ...
   /// receivedPacketsBaseIndex + the index into the queue If 0, we got got that packet.  Otherwise,
   /// the time to give up waiting for that packet. If we get a packet number where
   /// (receivedPacketsBaseIndex-packetNumber) is less than half the range of
   /// receivedPacketsBaseIndex then it is a duplicate Otherwise, it is a duplicate packet (and
   /// ignore it).
   ...
   // Limit changes in throughput to once per ping - otherwise even if lag starts we don't know
   // about it In the meantime the connection is flooded and overrun.
   CCTimeType nextAllowedThroughputSample;
   bool bandwidthExceededStatistic;

   // If Update::maxBitsPerSecond > 0, then throughputCapCountdown is used as a timer to prevent
   // sends for some amount of time after each send, depending on the amount of data sent
   ...
   void SortSplitPacketList(DataStructures::List<InternalPacket *> &data, unsigned int leftEdge,
                            unsigned int rightEdge) const;
   void SendACKs(RakNetSocket2 *s, SystemAddress &systemAddress, CCTimeType time, RakNetRandom *rnr,
                 BitStream &updateBitStream);
   ...
   // Every 16 datagrams, we make sure the 17th datagram goes out the same update tick, and is the
   // same size as the 16th
   ...
   // ourOffset refers to a section within externallyAllocatedPtr. Do not deallocate
   // externallyAllocatedPtr until all references are lost
   void AllocInternalPacketData(InternalPacket *internalPacket, InternalPacketRefCountedData **refCounter,
