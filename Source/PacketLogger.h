...
/// \file
/// \brief This will write all incoming and outgoing network messages to the local console screen.
/// See derived functions for other outputs
///
...
/// This will write all incoming and outgoing messages to the console window, or to a file if you
/// override it and give it this functionality. \ingroup PACKETLOGGER_GROUP
class RAK_DLL_EXPORT PacketLogger : public PluginInterface2 {
  public:
   ...
   // Translate the supplied parameters into an output line - overloaded version that takes a
   // MessageIdentifier and translates it into a string (numeric or textual representation based on
   // printId); this calls the second version which takes a const char* argument for the
   // messageIdentifier
   virtual void FormatLine(char *into, const char *dir, const char *type, unsigned int reliableMessageNumber,
                           unsigned int frame, unsigned char messageIdentifier, const BitSize_t bitLen,
   ...
   virtual void FormatLine(char *into, const char *dir, const char *type, unsigned int reliableMessageNumber,
                           unsigned int frame, const char *idToPrint, const BitSize_t bitLen, unsigned long long time,
   ...
   /// Events on low level sends and receives.  These functions may be called from different threads
   /// at the same time.
