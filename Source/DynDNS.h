...
enum DynDnsResultCode {
   ...
   RC_UNKNOWN_RESULT,                    // DynDNS returned a result code that was not documented as of 12/4/2010 on
                                         // http://www.dyndns.com/developers/specs/flow.pdf
...
class RAK_DLL_EXPORT DynDNS {
  public:
   ...
   bool WasResultSuccessful(void) const {
      return result == RC_SUCCESS || result == RC_DNS_ALREADY_SET || result == RC_NO_CHANGE;
   }
  ...
  protected:
   void Stop(void);
   void SetCompleted(RakNet::DynDnsResultCode _result, const char *_resultDescription) {
      Stop();
      result = _result;
