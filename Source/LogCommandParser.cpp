...
LogCommandParser::LogCommandParser() {
   RegisterCommand(CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS, "Subscribe",
                   "[<ChannelName>] - Subscribes to a named channel, or all channels");
   RegisterCommand(CommandParserInterface::VARIABLE_NUMBER_OF_PARAMETERS, "Unsubscribe",
...
bool LogCommandParser::OnCommand(const char *command, unsigned numParameters, char **parameterList,
...
void LogCommandParser::AddChannel(const char *channelName) {
   ...
   // No more available channels - max 32 with this implementation where I save subscribed channels
   // with bit operations
   RakAssert(0);
}

void LogCommandParser::WriteLog(const char *channelName, const char *format, ...) {
   if (channelName == 0 || format == 0) return;
...
void LogCommandParser::PrintChannels(const SystemAddress &systemAddress, TransportInterface *transport) const {
   ...
   if (anyChannels == false) transport->Send(systemAddress, "None.\r\n");
}
...
unsigned LogCommandParser::Subscribe(const SystemAddress &systemAddress, const char *channelName) {
   ...
   if (channelName) {
      channelIndex = GetChannelIndexFromName(channelName);
      if (channelIndex == (unsigned)-1) return channelIndex;
   }
...
unsigned LogCommandParser::GetChannelIndexFromName(const char *channelName) {
   unsigned i;
   for (i = 0; i < 32; i++) {
      if (channelNames[i] == 0) return (unsigned)-1;

      if (_stricmp(channelNames[i], channelName) == 0) return i;
   }
   return (unsigned)-1;
}
