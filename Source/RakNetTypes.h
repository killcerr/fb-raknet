...
enum ConnectionState {
   ...
   /// No longer connected
   IS_DISCONNECTED,
   /// Was never connected, or else was disconnected long enough ago that the entry has been
   /// discarded
   IS_NOT_CONNECTED
};
...
/// This is not necessarily a unique identifier. For example, if a system has both LAN and internet
/// connections, the system may be identified by either one, depending on who is communicating<BR>
/// Therefore, you should not transmit the SystemAddress over the network and expect it to identify
/// a system, or use it to connect to that system, except in the case where that system is not
/// behind a NAT (such as with a dedciated server) Use RakNetGUID for a unique per-instance of
...
struct RAK_DLL_EXPORT SystemAddress {
   ...
   /// This is not used internally, but holds a copy of the port held in the address union, so for
   /// debugging it's easier to check what port is being held
   ...
   /// "2001:db8:63b3:1::3490" You can write the port as well, using the portDelineator, for example
   /// "192.0.2.1|1234" \param[in] str A printable IP string, for example "192.0.2.1" or
   /// "2001:db8:63b3:1::3490". Pass 0 for \a str to set to UNASSIGNED_SYSTEM_ADDRESS \param[in]
   /// portDelineator if \a str contains a port, delineate the port with this character.
   /// portDelineator should not be '.', ':', '%', '-', '/', a number, or a-f \param[in] ipVersion
   /// Only used if str is a pre-defined address in the wrong format, such as 127.0.0.1 but you want
   /// ip version 6, so you can pass 6 here to do the conversion \note The current port is unchanged
   /// if a port is not specified in \a str \return True on success, false on ipVersion does not
   /// match type of passed string
   ...
   /// \internal sockaddr_in6 requires extra data beyond just the IP and port. Copy that extra data
   /// from an existing SystemAddress that already has it
   ...
   /// \internal Used internally for fast lookup. Optional (use -1 to do regular lookup). Don't
   /// transmit this.
