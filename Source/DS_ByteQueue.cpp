...
bool ByteQueue::ReadBytes(char *out, unsigned maxLengthToRead, bool peek) {
   ...
   if (peek == false) IncrementReadOffset(bytesToRead);
...
void ByteQueue::Clear(const char *file, unsigned int line) {
   if (lengthAllocated) rakFree_Ex(data, file, line);
   readOffset = writeOffset = lengthAllocated = 0;
   data = 0;
}
...
void ByteQueue::IncrementReadOffset(unsigned length) { readOffset = (readOffset + length) % lengthAllocated; }
...
void ByteQueue::Print(void) {
   unsigned i;
   for (i = readOffset; i != writeOffset; i++) RAKNET_DEBUG_PRINTF("%i ", data[i]);
   RAKNET_DEBUG_PRINTF("\n");
}
